...
将数据点 1 的 x 值代入方程 $$ y = 2x $$ 得到的 y 值为 30，而不是目标值 100。这个差值+20 被称为误差值。为了得到一个可工作的分类器，必须最小化这个误差值。增加分类器线的斜率是唯一减小误差的方法。让我们用符号∆来表示斜率的变化，€表示误差，y [ t ]表示期望的目标值。因此，误差变为

$$ {\textsf{C}\hspace{-1.7ex}{=}} ={y}_t-y $$
...
将η设置为 0.5 是一个合理的起点，如果只应用一半的更新。对于初始数据点，新的 `Δ = 0.5 * 1.333 = 0.667`。因此，新的分类器线是 `y = 2.667x`。我不会展示这个变化的散点图线，但可以说，它略逊于原始版本。没关系，因为下一个版本应该会好得多。
