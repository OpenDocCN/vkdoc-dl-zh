...
```python
# 1. 导入库
# 导入tensorflow库，特别是keras.models和keras.layers，用于设计包含密集层和展平层的顺序模型。我们需要从tensorflow.keras.optimizers导入Adam优化器。
...
# 2. 加载数据集
# 从tensorflow.keras.datasets加载MNIST数据集，并使用load_data()函数获取训练和测试数据。由于图像是灰度图，像素的最大值为255。如果我们将每个像素除以255，就实现了最小-最大归一化。
mnist = tf.keras.datasets.mnist
(X_train, y_train), (X_test, y_test) = mnist.load_data()
X_train, X_test = X_train / 255.0, X_test / 255.0

# 3. 编译和训练模型
# 为了编译模型，我们使用compile函数并设置优化器、损失和指标等参数。由于这是一个多分类问题，使用稀疏分类交叉熵作为损失函数。
...
# 4. 绘制训练历史
# 注意，编译模型后，输出被保存在一个名为history的变量中。这是一个字典，我们从中绘制训练和验证准确率。
...
# 14. 对最佳模型进行不同学习率的经验分析，并绘制准确率和损失曲线。
...
下表（表 5-5）显示了用于分类MNIST数据集的六个不同架构的平均验证准确率。

![图像](img/611710_1_En_5_Fig11_HTML.jpg)

**图 5-11**
最佳架构（单个隐藏层和512个神经元）在不同学习率下的准确率和损失曲线

![图像](img/611710_1_En_5_Fig10_HTML.jpg)

**图 5-10**
具有两个隐藏层和128个、16个神经元的架构的准确率和损失曲线

![图像](img/611710_1_En_5_Fig9_HTML.jpg)

**图 5-9**
具有两个隐藏层和128个、32个神经元的架构的准确率和损失曲线
...
此讨论将在以下章节中继续，因为CNN、序列模型和自动编码器也需要超参数调整。读者预计在继续前进之前尝试练习以掌握该概念。
...
1.  CIFAR数据集([`https://www.cs.toronto.edu/~kriz/cifar.html`](https://www.cs.toronto.edu/~kriz/cifar.html))包含60,000张属于10个类别的图片。每个类别有6000张图片。数据集分为两部分，训练集和测试集，分别有50,000张和10,000张图片。
    ...
    1.  使用dropout层来减少模型的方差。
```


1. 找出学习率变化对模型性能的影响。
2. 使用正则化来查看模型在测试数据上是否给出更好的结果。
3. 每层改变激活函数是否会影响损失曲线的平滑度？
4. 现在探索 STL 数据集（[`https://cs.stanford.edu/~acoates/stl10/#:~:text=The%20STL%2D10%20dataset%20is,dataset%20but%20with%20some%20modifications`](https://cs.stanford.edu/~acoates/stl10/#:~:text=The%20STL%2D10%20dataset%20is,dataset%20but%20with%20some%20modifications)），并再次执行上述问题中所述的任务。
