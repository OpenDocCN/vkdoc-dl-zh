...
![图 9-1](img/611710_1_En_9_Fig1_HTML.jpg)
...
![图 9-2](img/611710_1_En_9_Fig2_HTML.jpg)
...
然后找到损失相对于权重的导数，根据链式法则变为 `y^(<t>) = g2) + b[y])` （从方程 (1) 到方程 (2) 替换 `h^(<t>)` 的值）：
...
![图 9-5](img/611710_1_En_9_Fig5_HTML.jpg)
...
![图 9-6](img/611710_1_En_9_Fig6_HTML.jpg)
...
![图 9-7](img/611710_1_En_9_Fig7_HTML.jpg)
...
![图 9-8](img/611710_1_En_9_Fig8_HTML.jpg)
...
图 9-17 损失和准确率曲线：模型 4

![图片](img/611710_1_En_9_Fig16_HTML.jpg)

图 9-16 损失和准确率曲线：模型 3

![图片](img/611710_1_En_9_Fig15_HTML.jpg)

图 9-15 损失和准确率曲线：模型 2

![图片](img/611710_1_En_9_Fig14_HTML.jpg)

图 9-14 损失和准确率曲线：模型 1

上述实验的结果总结在表 9-2 中。

表 9-2 四种不同的 RNN 模型在 Treebank 数据集上的平均验证准确率
...
2. 使用自动编码器创建嵌入（第十一章）。
3. 使用单层 RNN，每层有 64 个单元（如果你想，可以更改单元数）。
4. 使用两层 RNN，每层有 64 和 32 个单元。
5. 使用 dropout 并分析引入此层对测试集模型性能的影响。
...
2. 使用自动编码器（第十一章）创建（a）中获得的图像的嵌入。
3. 使用具有 32 个单元的单层 RNN（如果你想，可以更改单元数量）。
4. 使用具有 32 和 16 个单元的两层 RNN。
5. 使用 dropout 并分析引入此层对测试集模型性能的影响。
...
2. 在神经网络中，输入和输出是如何相关的？
    1. 它们彼此线性相关。
    2. 它们彼此独立。
    3. 它们是顺序处理的。
    4. 它们是递归处理的。

3. RNN 与一般神经网络相比，是如何处理信息的？
    1. 独立地
    2. 随机
    3. 并行
    4. 顺序
