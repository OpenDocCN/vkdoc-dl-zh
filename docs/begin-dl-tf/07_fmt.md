...
函数 `f(x)` 的导数可以写成`f'`(x)或者![$$ \frac{dy}{dx} $$](img/515226_1_En_7_Chapter_TeX_IEq1.png)。从几何的角度来说，一元函数的导数就是这里函数的切线的斜率，也就是函数值沿着 `x` 方向的变化率。考虑一个物理学上的例子，比如自由落体运动的位移函数的表达式![$$ y=\frac{1}{2}g{t}² $$](img/515226_1_En_7_Chapter_TeX_IEq2.png)。对时间的导数是![$$ \frac{dy}{dt}=\frac{d\frac{1}{2}g{t}²}{dt}= gt $$](img/515226_1_En_7_Chapter_TeX_IEq3.png)。考虑到速度 `v` 定义为位移的变化率，那么位移对时间的导数就是速度，即 `v = gt` 。

事实上，导数是一个非常宽泛的概念。因为我们之前遇到的函数大多是一元函数，所以自变量只有两个方向: `x+` 和`x-`。当函数的自变量个数大于 1 时，函数导数的概念推广到函数值在任意方向的变化率。导数本身是标量，没有方向，但是导数表征了函数值在某个方向上的变化率。在这些任意方向中，沿着坐标轴的几个方向比较特殊，也叫偏导数。对于一元函数，导数写成![$$ \frac{dy}{dx} $$](img/515226_1_En_7_Chapter_TeX_IEq4.png)。对于多元函数的偏导数，记为![$$ \frac{\partial y}{\partial {x}_1},\frac{\partial y}{\partial {x}_2},\cdots $$](img/515226_1_En_7_Chapter_TeX_IEq5.png)。偏导数是导数的特例，没有方向。

考虑一个本质上是多元函数的神经网络模型，比如 shape `[784，256]`的一个权重矩阵 `W` ，其中包含 `784 × 256` 的连接权重，我们需要求 `784 × 256` 的偏导数。需要注意的是，在数学表达习惯中，要讨论的自变量一般记为 `x` ，但在神经网络中，一般用来表示输入，如图片、文本、语音数据等。网络的自变量是网络参数集 `θ = { w1 、 b1 、 w2 、 b2 、⋯}`。当使用梯度下降算法优化网络时，需要请求网络的所有偏导数。所以我们也关注误差函数 `L` 沿自变量 `θi` 方向输出的导数，即![$$ \frac{\partial L}{\partial {w}_1},\frac{\partial L}{\partial {b}_1},\cdots $$](img/515226_1_En_7_Chapter_TeX_IEq6.png)。用向量形式写出函数的所有偏导数:
...
`η`是学习率。梯度下降算法一般是求函数 `L` 的最小值，有时也希望求函数的最大值，这就需要按如下方式更新梯度:
...
*   线性函数 `y = ax + c` 的导数为 `a` 。比如 `y = 2x + 1` 的导数就是![$$ \frac{dy}{dx}=2 $$](img/515226_1_En_7_Chapter_TeX_IEq9.png)。

*   函数`x^a`的导数为`ax^(a-1)`。比如`y=x^2`的导数就是![$$ \frac{dy}{dx}=2x $$](img/515226_1_En_7_Chapter_TeX_IEq10.png)。

*   指数函数的导数`a^x`是`a^x * ln a`。比如`y=e^x`的导数就是![$ \frac{dy}{dx}={e}^x lnln\ e={e}^x $](img/515226_1_En_7_Chapter_TeX_IEq11.png)</sup>

*   对数函数 `x` 的导数为![$$ \frac{1}{xln\ a} $$](img/515226_1_En_7_Chapter_TeX_IEq12.png)。比如 `y = ln x` 的导数就是![$$ \frac{dy}{dx}=\frac{1}{xln\ e}=\frac{1}{x} $$](img/515226_1_En_7_Chapter_TeX_IEq13.png)
...
考虑到目标函数`L = y - (xw + b)^2 = [(xw + b) - y]^2`，设`g = xw + b`

考虑目标函数`L = a ln(xw + b)`，设`g = xw + b`，导数为:
...
*   *i* ≠ *j*. Extend the Softmax function:
    ...
    ![$$ =\frac{-{e}^{z_j}}{\sum_{k=1}^K{e}^{z_k}}\times \frac{e^{z_i}}{\sum_{k=1}^K{e}^{z_k}} $$](img/515226_1_En_7_Chapter_TeX_Equaq.png)
...
这里我们直接推导出最终损耗值 *L* 对网络输出的 logits 变量*z*<sub>I</sub>的偏导数，展开为:
...
变量的上标代表层数。例如，`o^{(1)}` 代表第一层的输出，`x` 为网络的输入。我们以权重参数 `w_{j1}` 的偏导数 `$$ \frac{\partial L}{\partial {w}_{j1}} $$` 为例。为了便于演示，我们绘制了如图 7-7 所示的神经元模型。图中未示出 Bias `b`，输入节点数为 j，从第 `j` 个节点的输入到输出 `o^{(1)}` 的权重连接记为 `$$ {w}_{j1}^{(1)} $$`，其中上标表示权重参数所属的层数，下标表示当前连接的起始节点数和结束节点数。例如，下标 `j1` 表示上一层的第 `j` 个节点到当前层的第一个节点。激活函数 `σ` 之前的变量称为 `$$ {z}_1^{(1)} $$`，激活函数 `σ` 之后的变量称为 `$$ {o}_1^{(1)} $$`。因为只有一个输出节点，所以 `$$ {o}_1^{(1)}={o}^{(1)}=o $$`。误差值 `L` 由输出和实际标签之间的误差函数计算。
...
其中，`t` 为真实标签值。加入 `$$ \frac{1}{2} $$` 不影响梯度的方向，计算更简单。我们以 `J`th(`J`∈【1，`J`)节点的权重变量 `w_{j1}` 为例，考虑损失函数 `L`:
...
考虑到 `o_1=σ(z_1)` 以及 Sigmoid 函数的导数为 `σ'=σ(1-σ)`，我们有:
...
从上式可以看出，误差对权重的偏导数 `w_{j1}` 只与输出值 `o_1`，真值 `t`，以及连接到当前权重的输入 `x_j` 有关。
...
多输出全连接网络层模型与单神经元模型的区别在于它有更多的输出节点 `$$ {o}_1^{(1)},{o}_2^{(1)},{o}_3^{(1)},\cdots, {o}_K^{(1)} $$`，每个输出节点对应一个真实的标签 `t1`、`t2`、…、`t_K`。`w_{JK}` 是第 `j` 个输入节点和第 `k` 个输出节点的连接权重。均方差可以表示为:
...
可以看出 `w_{JK}` 的偏导数只与当前连接的输出节点 `$$ {o}_k^{(1)} $$` 相关，对应 true 的标签 `$$ {t}_k^{(1)} $$`，对应输入节点 `x_j`。

让 `δ_k = (o_k - t_k) * o_k * (1 - o_k)`。

变量 `δ_k` 表征了连线末端节点误差梯度传播的某种特性。使用表示法 `δ_k` 后，偏导数 `$$ \frac{\partial L}{\partial {w}_{jk}} $$` 只与当前连接的起始节点 `x_j` 和结束节点 `δ_k` 相关。后面我们会看到 `δ_k` 在循环推导梯度中的作用。
...
事实上，在推导梯度的过程中，或多或少地使用了链式法则。考虑到复合函数 `y = f(u)`，`u = g(x)`，我们可以从 `$$ \frac{dy}{du} $$` 和 `$$ \frac{du}{dx} $$`:

![$$ \frac{dy}{dx}=\frac{dy}{du}\bullet \frac{du}{dx}={f}^{\prime}\left(g(x)\right)\bullet {g}^{\prime }(x) $$](img/515226_1_En_7_Chapter_TeX_Equbs.png)

推导出 `$$ \frac{dy}{dx} $$`

考虑两个变量的复合函数 `z = f(x, y)`，其中 `x=g(t)`，`y=h(t)`，那么导数 `$$ \frac{dz}{dt} $$` 可以由 `$$ \frac{\partial z}{\partial x} $$` 和 `$$ \frac{\partial z}{\partial y} $$` 推导。

比如 `$$ z={\left(2t+1\right)}²+{e}^{t²} $$`，设 `x = 2t + 1`，`y = t^2`，那么 `z=x^2+e^y`。利用前面的公式，我们有:
...
在前面的代码中，我们通过 Tensorflow 中的自动梯度计算和链式法则计算了`$$ \frac{\partial {y}_2}{\partial {y}_1} $$` `,` `$$ \frac{\partial {y}_1}{\partial {w}_1} $$` `,`和`$$ \frac{\partial {y}_2}{\partial {w}_1} $$`，我们知道`$$ \frac{\partial {y}_2}{\partial {y}_1}\bullet \frac{\partial {y}_1}{\partial {w}_1} $$`和`$$ \frac{\partial {y}_2}{\partial {w}_1} $$`应该等于`.`，它们的结果如下:
...
因为 *L* 通过各个输出节点 *o* <sub>*k*</sub> 与*w*<sub>ij</sub>关联，这里不能去掉求和符号，可以用链式法则分解均方误差函数:

`$$ \frac{\partial L}{\partial {w}_{ij}}={\sum}_k\left({o}_k-{t}_k\right)\frac{\partial }{\partial {w}_{ij}}{o}_k $$`

代入*o*<sub>k=*σ*(*z*<sub>*k*</sub>):</sub>
...
把*σ*(*z*<sub>k</sub>写成 *o* <sub>*k*</sub> ，再考虑链式法则，我们有:
...
因为`$$ \frac{\partial {o}_j}{\partial {w}_{ij}} $$`与 *k* 没有关联，所以我们有:
 ...
 *其中`$$ \frac{\partial {z}_j}{\partial {w}_{ij}} $$`是 *o* <sub>* i *</sub> ，所以:
...
此时，`$$ \frac{\partial L}{\partial {w}_{ij}} $$`可以写成当前连接的起始节点的输出值 *o* <sub>* i *</sub> 和结束节点的梯度变量信息`$$ {\delta}_j^{(J)} $$`
