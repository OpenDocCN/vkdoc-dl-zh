...
*   指数函数的导数*a*<sup>x是*a*<sup>*x*</sup>*ln ln a*。比如*y*=*e*<sup>*x*</sup>的导数就是![$ \frac{dy}{dx}={e}^x lnln\ e={e}^x $](img/515226_1_En_7_Chapter_TeX_IEq11.png)</sup>
...
考虑到*o*<sub>1</sub>=*σ*(*z*<sub>1</sub>)以及 Sigmoid 函数的导数为*σ*<sup>′</sup>=*σ*(1-σ)，我们有:
...
从上式可以看出，误差对权重的偏导数*w*<sub>j1</sub>只与输出值 *o* <sub>1</sub> ，真值 *t* ，以及连接到当前权重的输入 *x* <sub>*j*</sub> 有关。
...
多输出全连接网络层模型与单神经元模型的区别在于它有更多的输出节点![$$ {o}_1^{(1)},{o}_2^{(1)},{o}_3^{(1)},\cdots, {o}_K^{(1)} $$](img/515226_1_En_7_Chapter_TeX_IEq36.png)，每个输出节点对应一个真实的标签*t*1、*t*2、…、 *t* <sub>* K *</sub> 。*w*<sub>*JK*</sub>是第 *j* 个输入节点和第 *k* 个输出节点的连接权重。均方差可以表示为:
...
可以看出*w*<sub>JK的偏导数只与当前连接的输出节点![$ {o}_k^{(1)} $](img/515226_1_En_7_Chapter_TeX_IEq40.png)相关，对应 true 的标签![$ {t}_k^{(1)} $](img/515226_1_En_7_Chapter_TeX_IEq41.png)，对应输入节点 *x* <sub>* j *</sub> 。</sub>
