...
以 Softmax 层为例，它可以使用 tf.nn.softmax 函数来完成正向传播中的 softmax 操作，也可以通过层来构建 Softmax 网络层。`Softmax(axis=-1)` 类，其中轴参数指定 Softmax 操作的尺寸。首先，导入相关的子模块，如下所示:
...
对于需要创建自定义逻辑的网络层，可以通过自定义类来实现。创建自定义的网络层类时，需要从层中继承。创建自定义网络类时，需要从 `keras` 继承。模型基类，所以用这种方式创建的自定义类可以很容易地使用层/模型基类。该类提供的参数管理和其他功能也可以与其他标准网络层类交互使用。
...
通过修改为 `self.kernel = self.add_variable('W', [inp_dim, outp_dim], trainable=False)`，我们可以设置张量 `*W*` 不可训练，然后观察张量的管理状态:
...
如你所见，张量此时不由 `trainable_variables` 管理。另外，创建为 tf 的类成员变量。类初始化中变量也自动包含在张量管理中，例如:
