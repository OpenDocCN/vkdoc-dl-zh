...
清单 3-5
ChatGPT 解释为什么 JavaScript 如此出色
```

因此，当我们简要查看清单 3-5 时，我们看到响应（称为 `ChatCompletion`）是我们在本章开头向 ChatGPT 提出的问题的答案。然而，我们的响应周围有很多元数据，让我们更详细地分析 `ChatCompletion` 对象。

### 处理响应（`ChatCompletion`）

**表 3-6**  
`ChatCompletion` 对象响应的结构
