...
模型完成训练后，我们希望继续操作，并通过可视化方式在验证数据集上对其进行测试。我们通过以下代码实现这一点：

![预测输出示例](img/502181_1_En_6_Fig7_HTML.jpg)

**图 6-7** 预测输出示例

图 6-7 展示了模型输出的可视化样本。注意模型如何仅预测单个类别；这是有意为之，因为模型每次只训练输出一个类别。
...
这个模型训练速度很快，所以可以返回去尝试不同的类别，看看这对预测输出有什么影响。
...
![SRGAN 的样本训练输出](img/502181_1_En_6_Fig8_HTML.jpg)
