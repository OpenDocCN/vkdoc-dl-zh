...
它为任何给定的层提供了许多定制。我们可以指定单元的数量(即该层的神经元)、激活类型、内核和偏差的类型初始化以及其他约束。大多数情况下，我们只是使用像 units 和 activation 这样的参数。为简单起见，其余部分可以保留默认值。当我们在专门的用例中工作时，这些额外的参数变得很重要，在这些用例中，为给定的层使用特定类型的约束和初始化器是非常重要的。
...
*   分类准确性-`keras.metrics.categorical_accuracy`

*   稀疏分类准确性-`keras.metrics.sparse_categorical_accuracy`
