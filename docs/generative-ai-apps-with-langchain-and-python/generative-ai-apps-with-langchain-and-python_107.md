...
`recursive_text_splitter = RecursiveCharacterTextSplitter(separators=[". ", "! ", "? "], chunk_size=30, chunk_overlap=5)` `recursive_documents = recursive_text_splitter.create_documents([text])` `print("RecursiveCharacterTextSplitter:")` `for doc in recursive_documents:` `print(doc.page_content)` `print("---")`
