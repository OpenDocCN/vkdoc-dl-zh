...
- 我们使用 `os.environ["OPENAI_API_KEY"]` 将 **OpenAI API 密钥** 设置为环境变量。API 密钥的值被赋值给该变量。
- 通过使用 `os.getenv("OPENAI_API_KEY")` 将其赋值给 `openai.api_key`，确认 **API 密钥** 设置正确。
...
- 我们创建一个 **`OpenAI` 类的实例** 并将其赋值给变量 `llm`。这表示将用于生成响应的 OpenAI 语言模型。
