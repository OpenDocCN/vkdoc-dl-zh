...
S. Ruder 撰写了一篇名为“梯度下降优化算法概述”的论文，对更复杂的优化器进行了研究（见 `goo.gl/KgKVgG`）。这篇论文并不适合初学者，需要较强的数学背景，但它提供了对更复杂算法的概述，包括 `Adagrad`、`Adadelta` 和 `Nadam`。此外，它还回顾了适用于 `Hogwild!`、`Downpour SGD` 等分布式环境的权重更新方案。这绝对是一篇值得花时间阅读的文章。
...
为了理解这意味着什么，让我们绘制 `θ[n]`，`b[n]` 和 `v[n]`。为了做到这一点，让我们假设 `θ[n]` 具有高斯形状（确切的形式并不重要，只是为了说明目的），并且让我们使用 `β = 0.9`（见 `图 5-1`）。
...
让我们简要讨论一下 `图 5-1`。高斯曲线 (`θ[n]`) 将与 `b[n]` 卷积以获得 `v[n]`。结果可以在右侧图表中看到。所有这些项 (1 − `β`)`β`^(`n` − `i`)`θ[i]` 对于 `i` = 1, …, 50（在右侧图表中绘制）将被累加以获得 `v[50]`。请注意 `v[n]` 是 `n` = 1, …, 50 时所有 `θ[n]` 的平均值。每个项都乘以一个项 (`b[n]`)；即对于 `n` = 50 为 1，然后随着 `n` 向 1 减少而迅速减少。基本上，这是一个加权平均，具有指数递减的权重。远离 `n` = 50 的项越来越不重要，而接近 `n` = 50 的项获得更多权重。这同样是一个移动平均。对于每个 `n`，所有前面的项都被加起来并乘以一个权重 (`b[n]`)。
...
随着你选择的 `β` 值越来越小，具有显著不同于零的权重的 `θ[n]` 点的数量会减少，如 `图 5-2` 所示，我们在图中绘制了不同 `β` 值的 `b[n]` 系列。
...
我们最后要讨论的算法被称为 `Adam`（自适应矩估计）。它将 `RMSProp` 和 `动量`的思想结合成一个优化器。像 `动量`一样，它使用过去导数的指数加权平均，像 `RMSProp`一样，它使用过去平方导数的指数加权平均。

你需要计算与 `动量` 和 `RMSProp` 相同的量，然后计算以下量
...
其中我们用 `β[1]` 表示 `动量` 中使用的超参数，用 `β[2]` 表示我们在 `RMSProp` 中使用的超参数。然后，类似于我们在 `RMSProp` 中所做的，我们使用以下方程更新我们的权重
...
优化器将尝试最小化与 `w[0]` 和 `w[1]` 相关的 MSE。你将在书籍的在线版本中找到进行此比较的完整代码，网址为 `https://adl.toelt.ai`。“。在在线代码中，你可以看到从头开始实现的梯度下降。在 `图 5-3` 中，你可以看到 GD 算法在 200 次迭代中达到 MSE 函数最小值的参数空间路径。
...
图 5-4 中一个显著的不同之处在于，`GD` 路径相当直接，而其他路径则不那么直接，`Adam` 在最小值周围形成循环，`RMSProp` 在接近最小值时振荡。从图中很难看出哪个更快，最好的检查方法是绘制 $${w}_0^{\left[i\right]}$$ 和 $${w}_1^{\left[i\right]}$$ 与 `i` 的图像，其中 `i` 表示迭代次数。在图 5-5 中，你可以看到不同算法如何快速地将 $${w}_0^{\left[i\right]}$$ 收敛到 2.0 的预期值。在这种情况下，`Adam` 和 `GD` 表现相当，而 `RMSProp` 似乎要慢一些。

## 图 5-5

![](img/463356_2_En_5_Fig5_HTML.jpg)

$${w}_0^{\left[i\right]}$$ 与不同优化器的迭代次数的图像。

当考虑 $${w}_1^{\left[i\right]}$$ 时，会出现不同的图像。图 5-6 展示了不同算法收敛的速度有多快。

## 图 5-6

![](img/463356_2_En_5_Fig6_HTML.jpg)

$${w}_1^{\left[i\right]}$$ 与不同优化器的迭代次数的图像。
...
图 5-7 的放大图。$${w}_1^{\left[i\right]}$$ 与不同优化器的迭代次数的图像。
