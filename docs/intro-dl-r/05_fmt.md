...
其中 `w^k` 是权重，`b_k` 是偏差，`x` 是特定像素的值，`tanh` 是数据中的非线性。下标 `I`、`j` 指的是代表特征/激活图的矩阵的条目。权重 `w^k` 最终将特征图中的像素连接到前一层。最终，卷积图层是由前面描述的操作生成的特征地图的堆叠。然后，我们将特征地图放入池层。我们计算输出体积的空间大小为
...
# AlexNet、VGGNet 和 ResNet 架构

*   **AlexNet**：由 Alex Krizhevsky、Ilya Sutskever 和 Geoffrey Hinton 开发，赢得了 2012 年的 ILSVRC。其架构与 LeNet 相似，使用了“非饱和神经元”并高效地实现了 GPU 用于卷积层。全连接层中的神经元与前一层的所有神经元相连，响应归一化层位于第一和第二卷积层之后，第二、四、五层的卷积核仅与前一层中位于同一 GPU 上的卷积核图相连。其架构如下（如图 5-10 所示）：
...
*   **VGGNet**：在 ILSVRC 2014 竞赛中获得第二名（仅次于 AlexNet）。VGGNet 由牛津大学的 Karen Simonyan 和 Andrew Zisserman 开发。感受野为 3×3，有 1×1 过滤器，步幅为 2，最大池化大小为 2×2。该架构如下：输入通过若干卷积层馈送到三个全连接层（第一层和第二层有 4096 个通道，最后一层是执行 1000 路分类的 softmax 层）。

*   **ResNet**：2015 年 ILSVRC 的第一名，拥有 152 层——远超前述网络。由微软研究院的何恺明、张祥雨、任少卿和孙剑开发。该架构旨在形成一个学习参考层输入的残差函数的网络，而非学习未参考函数的网络。最终结果是，网络更易于学习和优化，并通过增加深度来提高精度，而非降低精度。

## 正规化

当多层感知器具有不止一层时，它们被认为具有逼近给定目标的能力，这可能导致过拟合。为防止过拟合，通常建议对输入数据进行正则化。然而，在 CNN 的情况下，这是一个稍有不同的过程，我们可以使用：

1.  **DropOut**：这是从人脑观察到的现象中获得的灵感。它是一个给定的隐藏层不被通过的概率，该概率我们设置为一个超参数。
2.  **随机池**：其中随机选择激活。可以说，随机池不需要超参数，可以作为一种启发式方法，与其他正则化技术一起使用。
3.  **DropConnect**：它是 dropout 的推广，其中每个连接都可以以 `1–p` 的概率被丢弃。该层中的每个单元都从前一层中的随机单元输入数据，这些数据在每次迭代时都会发生变化。这有助于确保权重不会过重。
4.  **权重衰减**：其功能类似于 L1/L2 正则化，其中我们严重惩罚大的权重向量。

在这些方法中，CNN 对使用 DropOut 有相当大的热情，因为它被证明是一种有效和强大的技术。除了防止过拟合之外，已经观察到丢弃提高了具有大量参数的网络的计算效率，因为这种形式的正则化实际上导致网络在给定迭代期间变得更小。在所有这些迭代之后，较小网络的性能可以被平均为完整网络性能的一般预测。第二，可以观察到，丢弃层在网络中引入了随机化的性能，这使得数据内的噪声被平均，使得其对数据内的信号的屏蔽被减小。
