...
*   `Decision`: 由性别隔离的个人做出的决定，关于他们是否愿意进行第二次约会。1 = 是（您想再次看到日期），0 = 否（您不想再次看到日期）。
*   `Like`: 总的来说，你有多喜欢这个人？（1 = 完全不喜欢，10 = 喜欢很多）。
*   `PartnerYes`: 你认为这个人会答应你的可能性有多大？（1 = 不太可能，10 = 极有可能）。
*   `Age`: 年龄。
*   `Race`: 白种人、亚洲人、黑人、拉丁美洲人或其他人种。
*   `Attractive`: 用 1-10 的标准给伴侣的吸引力打分（1 = 糟糕，10 = 很棒）。
*   `Sincere`: 给合作伙伴的销售诚意评分，1-10（1 = 糟糕，10 = 很好）。
*   `Fun`: 用 1-10 分的标准评价伴侣的有趣程度（1 = 糟糕，10 = 很棒）。
*   `Ambitious`: 用 1-10 的标准给伴侣的雄心壮志打分（1 = 糟糕，10 = 伟大）。
*   `SharedInterests`: 用 1-10 分（1 = 很糟糕，10 = 很棒）来评价你和伴侣共同兴趣/爱好的程度。
...
我们从改变阈值开始，该阈值决定了我们是基于 `lambda` 参数将观察分类为 1 还是 0。我们迭代该算法，并将基于该参数的 `AUC` 分数附加到 `AUC` 向量。在这个迭代循环之后，我们应该使用一个图来直观地评估性能。当绘制 `λ` 值的 `AUC` 得分向量时，我们编写以下代码，并观察图 10-11 中所示的输出：

![A435493_1_En_10_Fig11_HTML.jpg](img/A435493_1_En_10_Fig11_HTML.jpg)

图 10-11。`AUC` 随 `lambda` 值的变化
```
...
我们看到，当`λ`值为 0.15 时，AUC 得分最高，因此我们将使用该`λ`值。这是我建议你如何调整机器学习算法参数的一个例子。每个参数都应该单独调整，以便实现给定的目标，无论是最小化 MSE 还是最大化 AUC。在逻辑回归中，对数优势阈值实际上是我们需要调整的唯一参数。我们可以在测试集上通过多次迭代来查看优化模型的性能:
...
图 10-13。Logistic regression AUC histogram over 100 iterations

![A435493_1_En_10_Fig12_HTML.jpg](img/A435493_1_En_10_Fig12_HTML.jpg)

图 10-12。Logistic regression AUC over 100 iterations
...
图 10-14。KNN classifier AUC over 100 iterations
...
图 10-16。KNN AUC over 100 iterations on test set histogram

![A435493_1_En_10_Fig15_HTML.jpg](img/A435493_1_En_10_Fig15_HTML.jpg)

图 10-15。KNN AUC over 100 iterations on test set
...
图 10-18。Bayes classifier AUC histogram over 100 iterations

![A435493_1_En_10_Fig17_HTML.jpg](img/A435493_1_En_10_Fig17_HTML.jpg)

图 10-17。Bayes classifier AUC performance over 100 iterations
