...
设输入向量`x=[x_1, x_2, ..., x_n]^T`和权重向量`w=[w_1, w_2, ..., w_n]`。
...
`ŷ = f(x, w^T) = σ(Σ_i^n x_i w_i)`

其中

`σ = 1 / (1 + e^(-x))`

`ŷ = { 1 if y ≥ π*, 0 elsewhere }`

其中`π*`是对数概率的阈值，如第三章中逻辑回归所述。

### WH 算法

该算法由 Bernard Widrow 和 Macron Hoff 在 20 世纪 50 年代末开发，用于训练 SLP 模型。虽然类似于用于训练神经网络的梯度方法(如前所述)，但 WH 算法使用所谓的瞬时算法，由

`w_i(k+1) = w_i(k) - η (∂E/∂w_i) 1`

`∂E/∂w_i = 1/2 Σ_{m=1}^M 1(h_{θ_x} - y(k)) (-∂y(k)/∂w_i)`

`= Σ_{m=1}^M (h_{θ_x} - y(k)) (-x_i(k))`
...
导致后来神经网络模型发展的 SLP 模型的主要限制是，感知器模型只有在处理明显线性可分的数据时才是准确的。这显然在数据更加密集和复杂的情况下变得困难，并且有效地消除了这种技术在实际环境中遇到的分类问题中的有用性。这方面的一个例子是异或问题。假设我们有两个输入，`x_1` 和 `x_2`，对于这两个输入，给定了响应 `y`，使得以下为真:
...
`E(k) = 1/2 Σ_{i=1}^M (h_{θ_i}(k) - y_i(k))²`

我们用下面的公式表示输入到隐藏层的单个神经元的加权和值:

`s_{h, j}(k) = Σ_{i=1}^M w_{h, j, i} x_i(k)`

同样，我们把从隐藏层到输出层的输出表示如下:

`s_{o, j}(k) = Σ_{i=1}^H w_{o, j, i} o_{h, i}(k)`

用下面的权重表示:

`w_{ij}(k+1) = w_{ij}(k) - η ∂E(k)/∂w_{ij}`
...
该方法的操作部分是学习速率`η`的变换。根据定义，当函数相对于学习率的方向导数等于零时，`η`最小化该函数。根据链式法则:
