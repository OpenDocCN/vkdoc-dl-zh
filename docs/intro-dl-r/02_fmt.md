...
![$$ {\sigma}²=\frac{\varSigma {\left( X - \mu \right)}²}{N} $$](img/A435493_1_En_2_Chapter_Equn.gif)
...
![$$ {R}²=1-{\displaystyle \sum_i^n}\frac{{\left({\widehat{y}}_i- y\right)}²}{{\left({\widehat{y}}_i-\overline{y}\right)}²} $$](img/A435493_1_En_2_Chapter_Equv.gif)
...
此外，`A` 的特征向量放在矩阵 `U` 中。`U` 的每一列是 `A` 的特征向量。特征值存储在对角矩阵 `Λ` 中，其中矩阵的迹或对角线给出特征值。由此我们相应地改写第一个方程:

![$$ AU= U \Lambda $$](img/A435493_1_En_2_Chapter_Equdk.gif)

![$$ A= U\Lambda {U}^{-1} $$](img/A435493_1_En_2_Chapter_Equdl.gif)
...
当我们固定![$$ {\mathrm{\mathbb{R}}}^n $$](img/A435493_1_En_2_Chapter_IEq19.gif) 和![$$ {\mathrm{\mathbb{R}}}^m $$](img/A435493_1_En_2_Chapter_IEq19.gif) 的基时，线性变换![$$ \mathrm{\mathcal{L}} $$](img/A435493_1_En_2_Chapter_IEq19.gif)可以用矩阵 `A` 来表示。具体地，存在![$$ A\in {\mathrm{\mathbb{R}}}^{mxn} $$](img/A435493_1_En_2_Chapter_IEq20.gif)使得下面的表示成立。假设![$$ x\in {\mathrm{\mathbb{R}}}^n $$](img/A435493_1_En_2_Chapter_IEq21.gif)是给定的向量，`x'` 是 `x` 相对于![$$ {\mathrm{\mathbb{R}}}^m $$](img/A435493_1_En_2_Chapter_IEq19.gif) 的给定基的代表。如果![$$ y=\mathrm{\mathcal{L}}(x) $$](img/A435493_1_En_2_Chapter_IEq22.gif)和 `y'` 是 `y` 相对于![$$ {\mathrm{\mathbb{R}}}^m $$](img/A435493_1_En_2_Chapter_IEq19.gif) 给定基的代表，则

![$$ {y}^{\prime } = Ax\hbox{'} $$](img/A435493_1_En_2_Chapter_Equdq.gif)

我们称 `A` 为![$$ \mathrm{\mathcal{L}} $$](img/A435493_1_En_2_Chapter_IEq19.gif) 关于![$$ {\mathrm{\mathbb{R}}}^n $$](img/A435493_1_En_2_Chapter_IEq19.gif) 和![$$ {\mathrm{\mathbb{R}}}^m $$](img/A435493_1_En_2_Chapter_IEq19.gif) 的给定基的矩阵表示。
...
矩阵 `Q` 的子矩阵是通过连续地从 `Q` 中移除行和列而获得的矩阵的行列式。主要子矩阵是 `det(Q)` 本身以及通过移除第 `i` 行和第 `i` 列而获得的矩阵的行列式。
...
投影是从向量空间到其自身的线性变换 `P`，使得![$$ {P}²= P $$](img/A435493_1_En_2_Chapter_IEq31.gif)直观地，这意味着每当 `P` 被应用于任何值两次时，它给出与它被应用一次时相同的结果。它的图像是不变的，而且这个定义推广了图形投影的概念。![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)是![$$ {\mathrm{\mathbb{R}}}^n $$](img/A435493_1_En_2_Chapter_IEq19.gif)的子空间，这个子空间的维数也等于![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)中线性无关向量的最大数目如果![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)是![$$ {\mathrm{\mathbb{R}}}^n $$](img/A435493_1_En_2_Chapter_IEq19.gif)的子空间，则![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)降级的![$$ {\mathcal{V}}^{\perp } $$](img/A435493_1_En_2_Chapter_IEq37.gif)的正交补由与![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)中的每个向量正交的所有向量组成，因此，以下为真:

![$$ {\mathcal{V}}^{\perp }=\left\{ x:{v}^T x=0\ for\ all\ v\in\ \mathcal{V}\right\} $$](img/A435493_1_En_2_Chapter_Equdt.gif)

![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)的正交补也是子空间。总之，![$$ \mathcal{V}\ \mathrm{and}\ {\mathcal{V}}^{\perp } $$](img/A435493_1_En_2_Chapter_IEq40.gif)跨越![$$ {\mathrm{\mathbb{R}}}^n $$](img/A435493_1_En_2_Chapter_IEq19.gif)，在这个意义上，每个向量![$$ x\in\ {\mathrm{\mathbb{R}}}^n $$](img/A435493_1_En_2_Chapter_IEq41.gif)都可以表示为

![$$ x={x}_1+{x}_2 $$](img/A435493_1_En_2_Chapter_Equdu.gif)

其中![$$ {x}_1\in \mathcal{V}\ \mathrm{and}\ {x}_2\in {\mathcal{V}}^{\perp }. $$](img/A435493_1_En_2_Chapter_IEq42.gif)我们称上面的表示为 `x` 关于![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)的正交分解，我们说 `x_1` 和 `x_2` 分别是 `x` 到子空间![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)和![$$ {\mathcal{V}}^{\perp } $$](img/A435493_1_En_2_Chapter_IEq37.gif)的正交投影。我们写![$$ {\mathrm{\mathbb{R}}}^n = \mathcal{V} \otimes {\mathcal{V}}^{\perp } $$](img/A435493_1_En_2_Chapter_IEq46.gif)并且说![$$ {\mathrm{\mathbb{R}}}^n $$](img/A435493_1_En_2_Chapter_IEq19.gif)是![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)和![$$ {\mathcal{V}}^{\perp }. $$](img/A435493_1_En_2_Chapter_IEq37.gif)的直和，我们说 `P` 的线性变换是到![$$ \mathcal{V} $$](img/A435493_1_En_2_Chapter_IEq32.gif)的正交投影，对于所有的![$$ x\in {\mathrm{\mathbb{R}}}^n $$](img/A435493_1_En_2_Chapter_IEq50.gif)我们有![$$ P x\in \mathcal{V}\ \mathrm{and}\ x- P x\in {\mathcal{V}}^{\perp } $$](img/A435493_1_En_2_Chapter_IEq51.gif)
...
设`u=[u_1, u_2, ..., u_n]`，`u ∈ ℝ^n`，其中`u_i`中至少有一个是非零的。满足线性方程

`u_1x_1 + u_2x_2 + ... + u_nx_n = v`

的所有点`x = [x_1, x_2, ..., x_n]^T`的集合

称为空间`ℝ^n`的超平面。我们可以用下面的等式来描述超平面：

`{x ∈ ℝ^n : u^T x = v}`

超平面不一定是`ℝ^n`的子空间，因为一般来说，它不包含原点。对于`n = 2`，超平面的方程具有形式`u_1x_1 + u_2x_2 = v`，它是一条直线的方程。因此，直线是`ℝ^2`中的超平面。在`ℝ^3`中，超平面就是普通平面。超平面`h`将`ℝ^n`分成两个半空间，表示如下：

`H_+ = {x ∈ ℝ^n : u^T x ≥ 0}`

`H_- = {x ∈ ℝ^n : u^T x ≤ 0}`

这里`H_+`是正半空间，`H_-`是负半空间。超平面`H`本身由`<u, x - a> = 0`的点组成，其中`a = [a_1, a_2, ..., a_n]^T`是超平面的任意点。简单地说，超平面`H`是向量`u`和`x–a`彼此正交的所有点`x`。

#### 顺序

实数序列是一个函数，其定义域是自然数 1，2，…，k 的集合，值域包含在`ℝ`内。因此，一个实数序列可以看作一组数`{x_1, x_2, ..., x_k}`，通常也表示为`{x_k}`。
...
此外，如果每一项都大于或等于前一项，则称一个序列是单调递增的。例如，序列`a_n(n) = 1`是单调递增的，如果且仅如果对于所有的`a_{n+1} ≥ a_n`。术语非递减和非递增经常用于代替递增和递减，以避免分别与严格递增和严格递减的任何可能的混淆。

如果实数序列的所有项都小于某个实数，则称该序列从上有界。这意味着存在`M`使得对于所有`n`，`a_n ≤ M`。任何这样的`M`称为一个上界。同样，如果对于某个实数`m`，`a_n ≥ m`对于所有大于某个`N`的`n`，那么序列从下有界，任何这样的`m`称为下界。

#### 限制

极限是当输入或索引接近某个值时，函数或序列接近的值。一个数`x^* ∈ ℝ`称为数列的极限，如果对于任何正`ϵ`都有一个数`K`，使得对于所有的`k > K`，`|x_k - x^*| < ϵ`：
...
如果存在一个线性函数`ℒ: ℝ^n → ℝ^m`和一个向量`y ∈ ℝ^m`，使得

`A(x) = ℒ(x) + y`

，则认为该函数是仿射的。

对于每一个`x ∈ ℝ^n`，考虑一个函数`f: ℝ^n → ℝ^m`和一个点`x_0 ∈ ℝ^n`。我们想要找到一个仿射函数`A`，它在点`x_0`附近逼近`f`。第一，很自然的强加这个条件：

`A(x_0) = f(x_0)`

我们通过`ℒ`，

`ℒ + y = ℒ(x) - ℒ(x_0) + f(x_0) = ℒ(x - x_0) + f(x_0)`

`A(x) = ℒ(x - x_0) + f(x_0)`

的线性度得到`y = f(x_0) - ℒ(x_0)`。

我们还要求`A(x)`接近`f(x)`的速度要比`x`接近`x_0`的速度快。
...
`∇f(x) = grad f(x) = df(x)/dx`

#### 海森矩阵

函数可以不止一次可微，这就引出了海森矩阵的概念。Hessian 是一个标量值函数的二阶偏导数的方阵，或者标量场：

`H = [[∂²f/∂x₁², ∂²f/∂x₁∂x₂, ..., ∂²f/∂x₁∂xₙ], ..., [∂²f/∂xₙ∂x₁, ∂²f/∂xₙ∂x₂, ..., ∂²f/∂xₙ²]]`
...
`J = df/dx = [∂f/∂x₁, ..., ∂f/∂xₙ] = [[∂f₁/∂x₁, ..., ∂f₁/∂xₙ], ..., [∂fₘ/∂x₁, ..., ∂fₘ/∂xₙ]]`
