...
自编码器的网络包括一个编码器网络，输入层有 6 个输入数据点，通过权重 `Wij` 映射到隐藏层的 4 个节点，并通过权重 `Wjk` 映射到隐藏层的 4 个节点，这些节点映射到解码器网络的输出层的 6 个节点。
...
一段代码，在 Jupyter Notebook 中导入所有必要的包，包含从 `tensorflow.dot.keras` 导入 8 个库，从 `sklearn` 导入 5 个库，从 `seaborn` 导入 `sns`，从 `pandas` 导入 `pd`，从 `numpy` 导入 `np`，从 `matplotlib` 导入 2 个库，以及 `sys`，并打印它们的版本，共有 31 行。
...
一段包含 5 行的代码。描述了使用标准缩放方法对金额的 `df` 进行缩放，以及将值重塑为 `(-1, 1)`，`df0` 有 20000 个样本，`df1` 有 400 个样本，`df = pd.concat([df0, df1])`。
...
代码包括日志文件名 = simple autoencoder，编码维度 = 12，输入维度 = `x_train.shape[1]`，输入数组 = 输入维度的输入，使用密集方法编码和解码，自动编码器 = 模型（输入数组，解码），以及 `autoencoder.summary()`。
...
+   **ReLU**：在深度学习模型中，Rectified Linear Unit 是最常用的激活函数。如果它接收到任何负输入，则函数返回 0，但对于任何正值 `x`，它返回该值。因此，它可以写成
...
代码读取，`autoencoder.compile(optimizer=RMSprop, loss='mean_squared_error', metrics=[mae, accuracy])`。
...
代码包括 `batch_size = 32`、`epochs = 20` 和 `history = autoencoder.fit(x_train, x_train, batch_size=batch_size, epochs=epochs, verbose=1, shuffle=True, validation_data=(x_test, x_test), callbacks=[tensorboard])`。
...
包含 `mae`、准确率、验证损失、验证 `mae` 和验证准确率的输出，对于 20 个 epoch 中的 1 到 20，包括第 20 个 epoch 的验证准确率为 0.2414，第 15 个 epoch 的验证准确率为 0.4615，以及第 20 个 epoch 的验证准确率为 0.4608。
...
一个主图网络，包含节点、链接和辅助节点。主图中的节点包括 `div_no_nan_1`、`identity_1` 到 `identity_5`、`div_no_nan_2`、`identity`、`RMSprop`、`gradient_tape` 和 `mean_squared_error`。
...
评估模型损失和准确性的代码包括 `score = autoencoder.evaluate(x_test, x_test, verbose=1)`、`print('Test loss:', score[0])` 和 `print('Test accuracy:', score[1])`。
...
一段代码包括 `threshold = 10.00`、`y_pred = autoencoder.predict(x_test)`、`y_label` 和 `error = []`，`if is_anomaly: y_label.append(1), else y_label.append(0)`，和 `error.append(y_dist)`。
...
一段代码读取 `print(classification_report(y_test, y_label))`。包括一个具有精确度、召回率、f1 分数和支持的列输出，包括 0、1、准确度、宏平均和加权平均。
...
代码读取为：`print(classification_report(y_test, y_label))`。包括具有精确度、召回率、f1 分数和支持的输出，包括 0、1、准确度、宏平均和加权平均。
...
两个代码行的代码。第 1 行读取为：`roc_auc_score(y_test, y_label)`。第 2 行读取为：`0.806310023706077`。
...
错误与数据点的散点图。正常图大多聚集在水平阈值大约 10 以下，一些异常图在上方。散点图上方的一段代码读取为：`viz.draw_anomaly(y_test, error, threshold)`。
...
神经网络代码包括双引号中的日志文件名，`encoding_dim = 12`，`input_dim = x_train.shape[1]`，`autoencoder = InputArrayModel`，`decoded`，以及 `autoencoder.summary()`。
...
主图网络包含节点和链接以及辅助节点。主图中的节点包括 `div_no_nan_1`，`identity_1` 到 `identity_5`，`div_no_nan_2`，`identity`，`RMSprop`，`gradient_tape`，和 `mean_squared_error`。
...
![图 6-26](img/483137_2_En_6_Fig29_HTML.png)

代码包括日志文件名=`"deep autoencoder"`，编码维度=`16`，输入维度=`x_train.shape[1]`，输入数组、编码和解码的语句，`autoencoder = Model(inputs, decoded)` 和 `autoencoder.summary()`。
...
![图 6-27](img/483137_2_En_6_Fig30_HTML.jpg)
...
![图 6-28](img/483137_2_En_6_Fig31_HTML.png)

在 Jupyter Notebook 中导入包的代码包括：`import tensorflow.keras as keras`, `import sklearn`, `import seaborn as sns`, `import pandas as pd`, `import numpy as np`, `import matplotlib`, `import matplotlib.pyplot as plt`, 和 `import matplotlib.gridspec as gridspec`。
...
![图 6-29](img/483137_2_En_6_Fig32_HTML.png)

加载 MNIST 数据的代码包括：`from keras.datasets import mnist`, `import numpy as np`, 和 `(x_train, x_test) = mnist.load_data()`。
...
![图 6-30](img/483137_2_En_6_Fig33_HTML.png)

重塑 MNIST 数据的代码包括：`from keras.datasets import mnist`, `import numpy as np`, 和 `x_train = np.reshape(x_train, (len(x_train), 28, 28, 1))`, 以及 `x_test` 的语句。
...
![图 6-31](img/483137_2_En_6_Fig34_HTML.png)

包含以下内容的代码：`from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D`, `from keras.models import Model`, `from keras import backend as K`, `log_filename = "cnn_autoencoder_2"`, 语句用于 `input_img`, `x`, `encoded`, 和 `decoded`, 以及 `autoencoder`, 和 `summary()` 的调用。
...
![图 6-32](img/483137_2_En_6_Fig35_HTML.png)
...
![图 6-33](img/483137_2_En_6_Fig36_HTML.png)

代码包括 `batch_size = 32`, `epochs = 20` 和 `history = autoencoder.fit(x_train, x_train, batch_size=batch_size, epochs=epochs, verbose=1, shuffle=True, validation_data=(x_test, x_test), callbacks=...)`。
...
![图 6-34](img/483137_2_En_6_Fig37_HTML.png)

评估模型损失和准确率的代码包括 `score = autoencoder.evaluate(x_test, x_test, verbose=1)`, `print('Test loss:', score[0])`, 和 `print('Test accuracy:', score[1])`。
...
![图 6-35a](img/483137_2_En_6_Fig38_HTML.png)

基于模型进行预测的代码包括 `decoded_imgs = autoencoder.predict(x_test)`, `n = 10`, `plt.figure(figsize=(20, 4))`, `for i in range(1, n):`, 显示原始图像和重建图像的语句。
...
![图 6-35b](img/483137_2_En_6_Fig39_HTML.jpg)
...
![图 6-36a](img/483137_2_En_6_Fig40_HTML.png)

一段代码包括 `encoder = Model(input_img, encoded)`, `encoded_imgs = encoder.predict(x_test)`, `n = 10`, `plt.figure(figsize=(20, 8))`, `for i in range(1, n):`, 和 `plt.gray()`。

图 6-36a

显示编码图像的代码

![图 6-36b](img/483137_2_En_6_Fig41_HTML.jpg)
...
![图 6-37](img/483137_2_En_6_Fig42_HTML.jpg)

一个主图网络，包含节点和链接以及辅助节点。主图中的节点包括 `div_no_nan_1`，`identity_1` 到 `identity_5`，`div_no_nan_2`，`identity`，`RMSprop`，`gradient_tape`，和 `mean_squared_error`。
...
![图 6-38](img/483137_2_En_6_Fig43_HTML.jpg)
...
![图 6-39](img/483137_2_En_6_Fig44_HTML.jpg)
...
![图 6-40](img/483137_2_En_6_Fig45_HTML.jpg)
...
![图 6-41](img/483137_2_En_6_Fig46_HTML.png)
...
![图 6-42](img/483137_2_En_6_Fig47_HTML.jpg)
...
![图 6-43](img/483137_2_En_6_Fig48_HTML.png)

导入所有包的代码包括 `import tensorflow.keras as keras`，`import sklearn`，`import seaborn as sns`，`import pandas as pd`，`import numpy as np`，`import matplotlib`，`import matplotlib.pyplot as plt`，以及 `import matplotlib.gridspec as gridspec`。
...
![图 6-44](img/483137_2_En_6_Fig49_HTML.png)

加载 MNIST 图像的代码包括 `from keras.datasets import mnist`，`import numpy as np`，以及 `(x_train, y_train), (x_test, y_test) = mnist.load_data()`。
...
![图 6-45](img/483137_2_En_6_Fig50_HTML.png)

重新塑造 MNIST 数据的代码包括 `from keras.datasets import mnist`，`import numpy as np`，`noise_factor = 0.3`，`print(x_train_noisy.shape)`，`print(x_test_noisy.shape)`，和 `print(y_test.shape)`。
...
![图 6-46a](img/483137_2_En_6_Fig51_HTML.png)

显示图像的代码包括 `n = 11`，`plt.figure(figsize=(20, 2))`，`for i in range(1, n):`，`ax = plt.subplot(1, n, i)`，和 `plt.imshow(x_test_noisy[i].reshape(28, 28))`。

图 6-46a

显示图像的代码

![图 6-46b](img/483137_2_En_6_Fig52_HTML.jpg)
...
编译模型的代码包括 `autoencoder.compile(optimizer='RMSprop', loss='mean_squared_error', metrics=['accuracy'])`。
...
开始训练模型的代码包括批量大小 = 32，周期 = 20，和 `history = autoencoder.fit(x_train_noisy, x_train, batch_size=batch_size, epochs=epochs, verbose=1, shuffle=True, validation_data=(x_test_noisy, x_test), callbacks=callbacks)`。
...
代码读取 `score = autoencoder.evaluate(x_test, x_test, verbose=1)`。接下来的行包括 `print('Test loss:', score[0])` 和 `print('Test accuracy:', score[1])`。
...
代码包括 `decoded_imgs = autoencoder.predict(x_test_noisy, n=10)`, `plt.figure(figsize=(20, 4))`, `for i in range(1, n)` 用于显示原始图像和重建图像，以及 `plt.show()`。
...
代码读取 `encoder = Model(input_img, encoded)`, `encoded_imgs = encoder.predict(x_test_noisy, n=10)`, `plt.figure(figsize=(20, 8))`, 和 `for i in range(1, n)` 用于显示编码图像的语句。
...
一个主要图网络，包含如 `include_div_no_nan`, `div_no_nan_1`, `identity_1`, `identity_2`, `identity_3`, `cast_2`, `cast_3`, `cast_4`, `greater`, `sum_1`, `sum_2`, `RMSprop`, `gradient_tape`, 和 `mean_squared_error` 等节点，通过不同粗细的箭头与模型连接。
...
在我们深入探讨变分自动编码器的机制之前，让我们快速回顾一下本章中已经介绍过的正常自动编码器。自动编码器基本上使用一个编码器层和一个解码器层，至少是这样。编码器层将输入数据特征降低到潜在表示，解码器层将潜在表示扩展以生成输出，目标是训练模型以足够好的方式来重现输入作为输出。输入和输出之间的任何差异都可能表明某种异常行为或偏离正常情况，这通常被称为异常检测。从某种意义上说，输入被压缩成一个更小的表示，其维度少于输入，称为瓶颈，然后从瓶颈中重建输入。
...
导入所有必要包的代码包括 `import tensorflow.keras as keras`, `import sklearn`, `import seaborn as sns`, `import pandas as pd`, `import numpy as np`, `import matplotlib`, `import matplotlib.pyplot as plt`, 和 `import matplotlib.gridspec as gridspec`。
...
读取包含信用卡数据的文件路径的代码，包括 `df = pd.read_csv(file_path_or_buffer=file_path, header=0, sep=',')`, `print(df.shape[0])`, 和 `df.head()`。
...
使用标准缩放方法对数据帧中的金额列进行标准化的代码。包括 `df0 = df.query("class == 0").sample(20000)`, `df1 = df.query("class == 1").sample(400)`, 和 `df = pd.concat([df0, df1])`。
...
样本分布的代码包括重新参数化技巧，通过从各向同性单位高斯分布中采样进行重新参数化，参数包括，返回值，张量的 `z` 为采样潜在向量，默认情况下随机正态分布具有均值 = 0 和 `std = 1.0`。
...
代码包括日志文件名 = `variational autoencoder`, `VAE` 模型 = `encoder` + `decoder`，构建编码器模型，使用重新参数化技巧将采样输出作为输入，实例化编码器模型，构建和实例化解码器模型，实例化 `VAE` 模型，并计算 `VAE` 损失。
...
一段代码读取`vae.compile(optimizer='adam', loss='mean_squared_error', metrics=['accuracy'])`和`vae.summary()`。
...
训练模型的代码包括`history = vae.fit(x_train, x_train, batch_size=batch_size, epochs=epochs, verbose=1, shuffle=True, validation_data=(x_test, x_test))`，以及一个回调语句。
...
代码读取`score = vae.evaluate(x_test, x_test, verbose=1)`，`print('Test loss:', score[0])`，和`print('Test accuracy:', score[1])`。
...
代码读取`threshold = 10`，`y_pred = vae.predict(x_test)`，`if is_anomaly: y_label.append(1) else: y_label.append(0)`，和`error.append(y_dist)`。
...
2 行代码。第 1 行读取`roc_auc_score(y_test, y_label)`。第 2 行读取`0.9159364709499422`。
...
错误与数据对比的散点图。正常点聚集在约 10 的水平阈值下方，一些异常点位于上方。散点图上方的代码为`viz.draw_anomaly(y_test, error, threshold)`。
...
包含节点和链接的主图网络以及辅助节点。主图中的节点包括`div_no_nan`、`div_no_nan_1`、`identity_1`、`identity_2`、`identity_3`、`assign_add_variance`、`cast_3`、`cast_4`、`Adam`、`gradient_tape`、`model`和`mean_squared_error`。
...
包含节点和链接的模型图包括`tf.math.reduce_`、`AddN`、`AddN_1`、`tf.operators_`、`tf.math.multiply_1`、`tf.math.reduce_S`、`decoder`、`dense`、`encoder`、`gradient_tape`、`Adam`、`cast`和`iterator_Get_Next`。
