...
![img/483137_2_En_2_Fig1_HTML.jpg](img/483137_2_En_2_Fig1_HTML.jpg)
...
![img/483137_2_En_2_Fig2_HTML.jpg](img/483137_2_En_2_Fig2_HTML.jpg)
...
三行 Python 代码导入库 `pandas` 作为 `pd`，`NumPy` 作为 `np`，以及 `matplotlib.pyplot` 作为 `plt`。
...
一行 Python 代码使用 `pd.read_csv` 函数返回 Pandas Dataframe 对象。
...
![图片](img/483137_2_En_2_Fig9_HTML.png)
...
![图片](img/483137_2_En_2_Fig10_HTML.png)

四行 Python 代码创建了一个包含两行和三列的 dataframe。列标题是 `Model1`、`Model2` 和 `Model3`。
...
![图片](img/483137_2_En_2_Fig11_HTML.png)

四行 Python 代码创建了一个包含三个行和两个列的 dataframe，用于 `Model1`、`Model2` 和 `Model3`。数据来自 `metrics` 字典。键是 `Model1`、`Model2` 和 `Model3`。值是 0.9 和 0.8、0.2 和 0.3、以及 0.3 和 0.2。
...
使用索引函数的 3 行 Python 输出，列头为 `PassengerId`、`Survived`、`Pclass`、`Name`、`Sex`、`Age`、`SibSp`、`Parch`、`Ticket`、`Fare`、`Cabin` 和 `Embarked`。数据类型是 object。
...
一行 Python 代码读取 `df`，名称用单引号和方括号括起来，点号在 2 的位置。
...
一行 Python 代码读取 `df`，`Name` 在单引号和方括号中，点 2 的头部。
...
一行 Python 代码读取 `df`，开方括号，`PassengerId`，`Name`，`Survived`，每个都在单引号中，闭方括号，点 2 的头部。
...
两行 Python 代码使用 `df.iloc` 函数选择并切片前五行，然后是前四列。读取 `df.iloc[:5, :4]`。
...
一行 Python 代码读取 `view.loc['b':'d', 'Name':'Ticket']`。
...
一行 Python 代码读取 `df[df['Age'] > 50].head()`。
...
一行 Python 代码使用 `df` 函数和 `&` 运算符来过滤数据，如果年龄超过 50 岁且性别为女性。
...
一行 Python 代码使用 `df` 函数选择包含 Missis 头衔的行的数据。
...
一行 Python 代码使用 `df` 函数和否定运算符来过滤"Embarked"列中不是 S 或 C 的值。
...
一段 4 行的 Python 代码使用 `df.copy()` 和 `loc` 函数将"male"的值替换为 0，将"female"的值替换为 1。
...
一行 Python 代码读取 `df.sort_values("Fare", ascending=True)`。
...
一行 Python 代码读取 `df.sort_values(['Pclass', 'Age'], ascending=[True, False])`。
...
一段 4 行的 Python 代码定义了一个名为 `custom_func` 的函数，参数为 `x`，并返回 `2 * x + x`。使用此函数在 `df2` 的票价列上应用 `apply` 方法，并使用 `head` 方法返回前 2 行。
...
一行 Python 代码读取 `pd.concat([df_male, df_female])`。
...
一行两行的 Python 代码使用 lambda 函数和 apply 方法对小于 18 岁的儿童标签进行操作，否则标签为成人，并将结果分配给变量 `age_class`，并使用 head 方法调用前两行。
...
一行 Python 代码使用 drop 方法在 `df2` 中删除 `axis=1` 的年龄状态列。
...
一行 Python 代码读取 `df['Embarked'].value_counts(dropna=False)`。
...
一行 Python 代码读取 `array(['S', 'C', 'Q'], dtype=object)`。
...
一行 Python 代码读取 `df[['Fare', 'Age']].describe()`。
...
以下是一行 Python 代码：`df2 = df[["Survived", "Pclass", "Age", "SibSp", "Parch", "Fare"]]`。`df2.corr()`。
...
Python 代码读取 `noise = np.random.normal(size=100)`。`plt.plot(noise)`。
...
一段 3 行的 Python 代码读取 `x = np.random.random(size=100)`。`y = np.random.random(size=100)`。`plt.scatter(x, y)`。
...
一行 Python 代码读取 `plt.hist(df["Fare"])`。
...
一行 Python 代码读取 `plt.bar(df["Sex"].unique(), df["Sex"].value_counts())`。
...
一行 Python 代码读取 `df["Age"].isna().sum(), len(df["Age"])`。
...
一行 Python 代码 `df.dropna(subset=["Age"])`。
...
使用 5 行 Python 代码在 `df` 的年龄列上使用 `mean` 函数，将其分配给 `df2` 的副本，并使用 `fillna` 函数和 `isna` 函数检查年龄是否因性别而填写错误。
...
一段 3 行的 Python 代码用于从 `sklearn.preprocessing` 导入 `LabelEncoder`。读取 `df2 = df.copy()`。`df2["Embarked"].unique()`。
...
一段 2 行的 Python 代码用于将 `df2` 中的“Embarked”列的值进行转换。
...
一行代码使用逆转换函数对 `df2` 中“Embarked”列的值进行操作，并使用 `head` 方法调用前两行。
...
一段 3 行的 Python 代码使用选择列表索引和 `corr` 函数来选择列。
...
一行 Python 代码使用了 `drop` 方法，在 `axis = 1` 的乘客 ID、姓名、票号和客舱列上，并选择了 `df2` 中的 `survived` 列用于 `corr` 函数。

图 2-87

删除不必要的列，并在结果的相关矩阵中仅选择`'Survived'`列

执行图 2-87 中的代码后，你的输出应该看起来类似于表 2-36。再次提醒，完整的表格将会非常长。

表 2-36

更新后的相关矩阵截断到`Survived`列
...
看起来，通过进行独热编码，我们发现了与`'Survived'`比其他特征更相关的特征。例如，包括 `Title_11`、`Title_12` 和 `Title_8`。如果我们像图 2-88 中所示，使用标签编码器对这些特征进行逆变换，我们就会立即明白原因。

![](img/483137_2_En_2_Fig89_HTML.png)

一行 Python 代码读取 `title_label_encoder.inverse_transform([8, 11, 12])`。
