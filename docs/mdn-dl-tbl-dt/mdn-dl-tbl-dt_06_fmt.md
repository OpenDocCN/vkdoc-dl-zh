...
列表 4-8
以编程方式生成一个架构（如列表 4-5 中所示），用于输入形状为 (200, 200, 1) 的样本数据集
...
示例卷积神经网络的编译和训练。假设 MNIST 数据集的训练集和验证集已加载到 `X_train`、`y_train`、`X_val` 和 `y_val` 中
...
2.  对称卷积模块/模块 A 的三次重复
3.  减少模块
4.  非对称卷积模块/模块 B 的四次重复
5.  减少模块
6.  扩展滤波器库模块/模块 C 的两次重复
...
列表 4-49
定义一个新的多模态架构，使用 `EfficientNetB1` 模型架构来处理图像组件
...
列表 4-50
定义一个新的多模态架构，使用 `EfficientNetB3` 模型架构来处理图像组件
...
**图 4-75** 初始软排序 1D CNN 模型在 Forest Cover 数据集上的性能历史
...
**Listing 4-58** 设计一个更强大/复杂的软排序 1D CNN

此模型使用了 487,879 个参数（图 4-76）。

![图片](img/525591_1_En_4_Fig76_HTML.png)

输入层输入 1 的输入和输出中的 None comma 值流程图，包括 `dense`、`reshape`、`conv1d`、`add`、`averagepooling1d` 和 `flatten`。

**图 4-76** Keras 对我们更新的软排序 1D CNN 架构的可视化
...
**图 4-77** 我们更新的软排序 1D CNN 设计在 Forest Cover 数据集上的性能历史（准确率和损失）
...
**图 4-78** 使用“mapper”从无序 1D 表示到有序 2D 表示应用标准二维 CNN 于表格数据的蓝图
...
**Listing 4-59** 使用标准软排序用于二维卷积组件的样板代码
...
**图 4-79** 使用 DeepInsight 方法将表格数据集中的特征映射到有序 2D 表示（模板矩阵）中的像素坐标的可视化。来自 Sharma 等人。
...
**图 4-80** DeepInsight 管道：将向量映射到像素坐标。来自 DeepInsight 论文（见脚注#2）。来自 Sharma 等人。
...
**图 4-81** DeepInsight 映射的图像模式可视化。癌症、文本和元音数据集样本之间的差异显示在中间列的小块中。这些差异通过卷积滤波器提取，以实现比另一种方法更有效的分类。来自 Sharma 等人。

**表 4-4** DeepInsight 与其他常见结构化数据方法的性能对比，使用各种数据集。来自 Sharma 等人。
