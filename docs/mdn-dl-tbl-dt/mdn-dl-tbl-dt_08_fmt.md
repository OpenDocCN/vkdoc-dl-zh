...
生成一个合成数据集，其中目标向量是倒数第二个和倒数第四个元素的和
...
![图 6-48](img/525591_1_En_6_Fig48_HTML.png)
...
![图 6-49](img/525591_1_En_6_Fig49_HTML.png)
...
![图 6-50](img/525591_1_En_6_Fig50_HTML.png)
...
![图 6-52](img/525591_1_En_6_Fig52_HTML.png)

2 个表格描述了替换标记检测。表 1 有 4 行和 5 列。表 2 有 4 行和 2 列。表 1 引导到表 2。

替换标记检测的可视化

![图 6-51](img/525591_1_En_6_Fig51_HTML.png)
...
![图 6-53](img/525591_1_En_6_Fig53_HTML.png)
...
![图 6-54](img/525591_1_En_6_Fig54_HTML.png)
...
![图 6-56](img/525591_1_En_6_Fig56_HTML.png)
...
![图 6-55](img/525591_1_En_6_Fig55_HTML.png)
...
在每个步骤 `i` 中，通过学习掩码进行特征选择后，选定的特征会通过特征转换器 `F[i]`（图 6-62）传递。我们将特征转换器的输出分割，以收集决策步骤输出 `d[i] ∈ R^(B × N_d)` 对某些维度 `N[d]` 的信息以及为下一步提供信息 `a[i] ∈ R^(B × N_a)` 对某些维度 `N[a]`。（这两个维度都是预设的。）
...
`d_out = Σ(i=1 to N_steps) ReLU(d[i])`
...
`η_q[i] = Σ(c=1 to N_d) ReLU(d_q,c[i])`
