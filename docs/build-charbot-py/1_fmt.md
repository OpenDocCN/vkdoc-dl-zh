...
构建聊天机器人时最困难的部分是跟踪 `if...else` 代码块。要做的决策越多，`if...else` 会出现在代码中。但同时需要这些块来编码复杂的对话流。如果问题很复杂，并且在现实生活中需要很多 `if…else`，那么这将需要代码以同样的方式进行调整。
...
*   [`https://core.rasa.ai`](https://core.rasa.ai)
...
例如，当用户对聊天机器人说:“预订电影票”时，我们作为人类可以理解用户想要预订电影票。这是机器人的意图。可以命名为`book_movie_intent`。

另一个例子是当用户说，“我想点餐”，或者“你能帮我点餐吗？”这些可以被命名为`order_food`意图。同样，您可以定义任意多的意图。
