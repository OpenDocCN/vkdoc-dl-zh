...
*   `order_ID`:bot 需要理解用户的订单 ID 以便跟踪。
...
*   **默认欢迎意图:** 我们可以扩展这个欢迎意图用于我们自己的聊天机器人。您应该添加一些您自己的用户表达式和默认响应。
...
如果你看一下图 3-6 ，你会发现我们有一个名为 `food_items_entity` 的实体，但是当我们选择单词时，我们将参数命名为 `food_items_entity1` 和 `food_items_entity2`；食物数量也是如此，我们将第一个和第二个参数分别命名为 `quantity1` 和 `quantity2`。
...
现在，回到我们对**物品描述**的意图，你应该会看到类似图 3-6 的东西。
...
添加“*完成。您的最终金额为 XYZ，作为回应，您的订单为 `$quantity1 $food_items_entity1` 和 `$quantity2 $food_items_entity2`"* 。
...
4.  点击 `Create App ID`，如下图 3-15 所示。
...
6.  在 `Token Generation` 部分，让我们选择我们希望机器人连接到的 Facebook 页面（见图 3-16）。
...
3.  勾选 `订阅字段` 下的 `消息` 和 `消息 _ 回发` 选项。您完全可以选择您的用例所需的任何一个。
...
5.  点击 `Confirm` 按钮，如下图 3-19 所示，公开你的 facebook 应用。
...
在这里找到代码：`flask_online_eats_demo.zip`
