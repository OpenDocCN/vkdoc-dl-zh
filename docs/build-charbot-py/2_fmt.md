...
**命名实体识别**(**NER**)，又名**实体识别**或**实体提取**，是在给定文本中寻找[命名实体](https://en.wikipedia.org/wiki/Named_entity)并将其分类到预定义类别的过程。
...
如果您在上面的输出中看到，在标记化过程之后，`U.K` .作为一个单词出现，这是有意义的，因为 `U.K` .是一个国家名称，拆分它是错误的。即使在这之后，如果您对 spaCy 的标记化不满意，那么在完全依赖 spaCy 的标记化方法之前，您可以使用它的 `add_special_case` 方法来添加您自己的规则。
