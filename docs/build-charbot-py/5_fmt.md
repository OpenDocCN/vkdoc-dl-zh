...
让我们在我们的项目目录`horoscope_bot`文件夹中创建一个名为`credentials.yml`的文件，并在那里添加我们的脸书凭证。如果你不知道如何得到它，那么现在就创建这个文件，在本章的下一节你可以找到得到脸书证书的步骤。
...
1.  前往 [`https://developers.facebook.com/`](https://developers.facebook.com/) 并创建一个应用（如果您还没有的话）。我们为 OnlineEatsBot 创建了一个；现在我们将为 HoroscopeBot 创建一个。输入详细信息并点击 Create App ID。查看图 5-2 了解如何输入机器人的显示名称和您的联系电子邮件。
...
2.  一旦您的应用创建完成，转到 Settings 下的 Basic，并点击 App Secret 下的 Show 按钮。这是您的 `fb_secret`。参考图 5-3 了解在哪里可以获取您的 `fb_secret` 密钥。
...
3.  转到该应用的仪表板，向下滚动到“Add a Product”。点击 Add Product，然后添加 Messenger（点击 SetUp）。查看图 5-4。
...
4.  在 Messenger 的设置下，当您向下滚动到 Token Generation 部分时，您将获得一个为您的应用创建新页面的链接。如果您还没有页面，请创建一个或从“Select a page”下拉菜单中选择一个页面。“Page Access Token”就是您的 `fb_access_token`。参考图 5-5。
...
5.  就在 Token Generation 部分之后，在 Webhooks 下，点击“Setup Webhooks”。参考图 5-6。
...
6.  接下来，选择一个验证令牌，我们稍后需要使用它。验证令牌可以是任何随机字符串。这将是您的 `fb_verify`。查看图 5-7 了解在 facebook 应用中添加验证令牌的位置。现在，将回调 URL 部分保持空白。不要关闭浏览器；只需保留它——我们将再次回到这里。
...
7.  随身携带 `fb_verify`、`fb_secret` 和 `fb_access_token` 将你的机器人连接到脸书。
...
1.  创建一个名为 `actions_app` 的新文件夹，并进入目录:
...
3.  用以下内容创建一个 `requirements.txt` 文件。这会告诉 Heroku 应用程序安装软件包及其版本。
...
在这一步中，我们将遵循一些类似于我们刚才所做的步骤和命令，但这是一个我们将创建的新应用程序，它将成为我们用于对话管理的主要应用程序。所以，我们开始吧。首先回到主项目目录(即，在 `horoscope_bot` 中)并创建一个文件名 `Procfile` 并向其中添加以下内容:
...
一旦成功完成部署，您将看到 Heroku 给出的一个 url，上面写着应用程序已部署到 `<url>`。
...
图 5-10 Facebook Messenger webhooks 配置
...
1.  转到此 url [`https://api.slack.com/slack-apps`](https://api.slack.com/slack-apps) 并单击按钮“Create App”。参考图 5-13。

    ![img/461879_1_En_5_Fig15_HTML.jpg](img/461879_1_En_5_Fig15_HTML.jpg)

    图 5-13 在 Slack 中创建应用程序

2.  下一步是创建一个 Bot User。要创建一个 bot user，请单击“Add features and functionality”下的 **Bots**。在新页面中，您将获得一个“Add a Bot User”的选项。查看图 5-14 以了解如何添加详细信息并添加 bot user。

    ![img/461879_1_En_5_Fig16_HTML.jpg](img/461879_1_En_5_Fig16_HTML.jpg)

    图 5-14 在 Slack 上给你的机器人命名
...
4.  返回“Basic Information”选项卡。单击“Install your app to your workspace”。该应用将要求确认身份。请像对待任何其他应用一样授权它。查看图 5-15，了解授权的样子。

    ![img/461879_1_En_5_Fig17_HTML.jpg](img/461879_1_En_5_Fig17_HTML.jpg)

    图 5-15 授权您的 Slack 应用程序

你会在“添加特性和功能”下找到带有绿色勾号的“机器人和权限”选项卡，这意味着我们的机器人和应用程序集成良好。这是一个迹象，表明我们目前做得很好。

5.  转到 OAuth & Permissions 部分，复制 **Bot user OAuth access token**。

6.  将复制的令牌粘贴到我们的 Python 脚本 `slack.py` 中。给一个你喜欢的频道名。如果你想让你的机器人发布到一个频道，那么你可以给一个频道名称。我已经给了 `@slackbot`。如果不设置 `slack_channel` 关键字参数，消息将被传递回发送它们的用户。
...
订阅机器人事件只不过是声明机器人必须回复的事件。这里我们只演示两种情况:第一，当有人提到机器人的名字时(即`app_reference`)，第二，当有人直接向机器人发送消息时(即`message.im`)。
...
到目前为止，您一定非常熟悉为 Heroku 部署创建`Procfile`。我们将再次使用现有的`Procfile`，并在那里进行修改，以将基于 API 的 chatbot 部署到 web 上。在创建现有`Procfile`的备份后，可以随意创建新的 proc file。
