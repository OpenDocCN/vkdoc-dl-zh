...
接下来，在第 5 章[中，我们看了函数近似的各种方法。我们将瓦片编码视为实现线性函数近似的一种方式。然后，我们讨论了 DQN，即使用神经网络作为函数近似器的批量 Q 学习。我们经历了长时间推导，以得出如等式(](05.html) [5 中给出的权重(具有神经网络参数)更新等式。25](05.html#Equ24) 。此处转载如下:
...
![$$ L=\frac{1}{N}{\sum}_{i=1}^N{\left[{r}_i+\left(\left(1- don{e}_i\right).\upgamma .\underset{a_i^{\prime }}{\max}\hat{q}\left({s}_i^{\prime },{a}_i^{\prime };{w}_t^{-}\right)\right)\hbox{--} \hat{q}\left({s}_i,{a}_i;{w}_t\right)\right]}^2 $$](img/502835_1_En_6_Chapter_TeX_Equ3.png)
...
![$$ L=\frac{1}{N}{\sum}_{i=1}^N{\left[\left({r}_i+\left(\left(1- don{e}_i\right).\upgamma .\underset{a_i^{\prime }}{\max}\hat{q}\left({s}_i^{\prime },{a}_i^{\prime };{w}_t^{-}\right)\right)\hbox{--} \hat{q}\left({s}_i,{a}_i;{w}_t\right)\right).{w}_i\right]}^2 $$](img/502835_1_En_6_Chapter_TeX_Equ11.png)
...
![$$ L=\frac{1}{N}{\sum}_{i=1}^N{\left[{r}_i+\left(\left(1- don{e}_i\right).\upgamma .\hat{q}\left({s}_i^{\prime }, argma{x}_{a^{\prime }}\hat{q}\left({s}_i^{\prime },{a}^{\prime };{w}_t\right);{w}_t^{-}\right)\right)\hbox{--} \hat{q}\left({s}_i,{a}_i;{w}_t\right)\right]}^2 $$](img/502835_1_En_6_Chapter_TeX_Equ12.png)
...
然后，在关于函数近似的第 [5](05.html) 章中，我们看到当我们切换到将状态/动作值表示为参数化函数时，这些方程随着参数 *w* 的引入而发生了一些变化。
...
在上式中，权重 *w* <sub>1</sub> 对应网络的初始公共部分， *w* <sub>2</sub> 对应网络预测状态值![$$ \hat{V} $$](img/502835_1_En_6_Chapter_TeX_IEq7.png)的部分，最后 *w* <sub>3</sub> 对应网络预测优势![$$ \hat{A} $$](img/502835_1_En_6_Chapter_TeX_IEq8.png)的部分。图 [6-7](#Fig7) 显示了一个典型的网络架构。
...
到目前为止，我们一直在输出输入状态 *s* 的 *Q* ( *s* ， *a* )值。输出中的单元数量大小为`n_action`。在某种程度上，输出值是预期的 *Q* ( *s* ， *a* )，使用蒙特卡洛技术对多个样本进行平均，以形成对实际预期值*E*[*Q*(*s*， *a* )的估计![$$ \hat{Q}\left(s,a\right) $$](img/502835_1_En_6_Chapter_TeX_IEq14.png)。
...
![$$ Q\left(s,a\right)={\sum}_i{z}_i{p}_i\left(s,a\right) $$](img/502835_1_En_6_Chapter_TeX_Equl.png)
