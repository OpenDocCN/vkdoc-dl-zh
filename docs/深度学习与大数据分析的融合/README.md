...
- Stan Zdonik, 美国罗得岛普罗维登斯布朗大学
- Shashi Shekhar, 美国明尼苏达大学明尼阿波利斯
- Xindong Wu, 美国佛蒙特大学伯灵顿
- Lakhmi C. Jain, 澳大利亚南澳大学阿德莱德
- David Padua, 美国伊利诺伊大学厄巴纳-香槟
- Xuemin Sherman Shen, 加拿大滑铁卢大学滑铁卢
- Borko Furht, 美国佛罗里达大西洋大学博卡拉顿
- V. S. Subrahmanian, 美国马里兰大学计算机科学系 College Park, MD, USA
- Martial Hebert, 美国宾夕法尼亚州匹兹堡卡内基梅隆大学
- Katsushi Ikeuchi, 日本东京都目黑区东京大学
- Bruno Siciliano, 意大利那不勒斯费德里科二世大学电气与信息工程系
- Sushil Jajodia, 美国弗吉尼亚州乔治梅森大学
- Newton Lee, 美国洛杉矶教育研究和奖学金研究所 CA, USA

斯普林格计算机科学简报提供了关于前沿研究和实际应用的简明摘要，涵盖了广泛的领域。该系列以50至125页的紧凑卷册为特色，内容涵盖了从专业到学术的各种内容。

典型的主题可能包括：

- 关于最新分析技术的及时报告
- 作为期刊文章中新研究成果和背景文献综述之间的桥梁
- 关于热门或新兴主题的快照
- 深入的案例研究或临床示例
- 介绍学生必须理解的核心概念，以便做出独立贡献

简报允许作者以最少的时间投入来展示他们的想法，并让读者吸收这些想法。简报将作为斯普林格的电子书系列的一部分出版，拥有全球数百万用户。此外，简报也可供个人购买纸质版和电子版。简报的特点是快速、全球范围的电子传播、标准出版合同、易于使用的稿件准备和格式指南，以及加快的出版进度。我们的目标是在接受后8-12周内出版。本系列考虑出版征稿和非征稿的稿件。
...
ISSN 2191-5776（电子版）
斯普林格计算机科学简报
ISBN 978-981-13-3458-0
ISBN 978-981-13-3459-7（电子书）
[https://doi.org/10.1007/978-981-13-3459-7](https://doi.org/10.1007/978-981-13-3459-7)
国会图书馆控制号码：2018962781

©作者（们），在Springer Nature Singapore Pte Ltd.的独家许可下，2019年
本作品受版权保护。出版商保留所有权利，无论是全部还是部分材料，特别是翻译、再版、插图重用、朗诵、广播、微缩胶片复制或以任何其他物理方式复制，以及传输或信息存储和检索、电子适应、计算机软件，或通过类似或不同的已知或今后开发的方法。
在本出版物中使用一般描述性名称、注册名称、商标、服务标志等并不意味着，即使在没有具体声明的情况下，这些名称不受相关保护法律和法规的约束，因此可以自由使用。

出版商、作者和编辑可以安全地假设本书中的建议和信息在出版日期时是真实准确的。出版商、作者或编辑对本书中所包含的材料不提供任何明示或暗示的保证，也不对可能存在的任何错误或遗漏负责。出版商在已发表的地图和机构附属关系方面保持中立。

这本斯普林格印记由注册公司斯普林格自然新加坡私人有限公司出版。注册公司地址是：1 52 Beach Road, #21-01/04 Gateway East, Singapore 189721, Singapore

## 关于本书

本书介绍了深度学习技术、概念和算法，用于对大数据进行分类和分析。使用传统技术进行大数据分析存在诸多挑战和问题，如高速分析、准确性和实时处理大数据的高效性。此外，物联网在智慧城市、智能家居和电子健康等领域逐渐增长。这么多连接设备每天产生大量的数据，因此我们需要复杂的算法来处理、组织和分类这么多的数据，以减少处理时间和空间。同样，现有的大数据深度学习技术和算法由于深度学习的两个主要分支，即卷积和深度置信网络，具有几个优势。本书介绍了基于这两种深度学习类型的所有技术和应用。此外，本书帮助学生、研究人员和新手理解基于深度学习方法的大数据分析。此外，本书还介绍了用于实时分析大数据的新编程语言和工具，如Hadoop、Spark和GraphX。讨论了各种机器学习技术与深度学习范式的结合，以支持高端数据处理、数据分类和实时数据处理问题。

这本书的分类和呈现都保持得非常简单，以帮助读者和学生理解各种深度学习范式和框架的基本概念。这本书主要关注理论，而不是深度学习概念的数学背景，因为这些技术背后的理论仍需要进一步改进和完善，以使学生和从业人员能够理解和发展他们的数学模型。

这本书由五章组成，首先介绍了大数据和深度学习技术，然后介绍了大数据和深度学习技术的整合，最后介绍了使用深度学习技术进行大数据分析的未来方向。

## 本书的目标和范围

深度学习技术在计算机科学和工程的许多领域中被广泛采用进行数据分析。大数据分析的引入面临着诸多挑战，例如高效处理大量数据并在较短时间内完成。然而，传统的方法和数据库管理系统无法在有限的时间内提供处理大数据的功能。因此，研究人员尝试从不同领域尝试各种技术和算法，以尽快处理数据的系统。Hadoop生态系统是为解决这类问题而开发的，采用了映射和归约的概念。Hadoop生态系统可以处理大量的数据；然而，它无法提供实时处理数据的功能。因此，需要先进的技术来处理实时生成的大量数据，这些数据具有高速度、大容量和多样性（即3V概念）。此外，大数据总是来自异构的源头；因此，使用传统的SQL查询技术进行学习功能总是需要高端节点。然而，在每个地方安装高端节点是一项困难且不可能的任务。因此，使用深度学习技术可以实现不安装高端节点的集成学习技术。本书的目标是将学术界、研究界和工业界汇集在一个平台上，为大数据分析设计和提出深度学习技术。本书的分类进一步分为四个主要类别：（1）用于跨层和跨域服务的新颖和复杂的实时深度学习分析，（2）来自异构和跨领域大数据分析的最新理论和实践，（3）来自世界顶级研究社区和实验室的调查和教程，以及（4）使用深度学习技术分析大数据的基准和测试平台的构建。
...
- 通过深度学习技术进行模式识别
- 创新的科学方法，用于实时大数据处理
- 用于城市规划、管理和可持续性的大数据处理
- 智能家居、智能城市、电子健康保健业务模式的可持续性
- 从物联网生成的大数据的分析和处理
- 使用大数据分析保护信息隐私和防止数据泄漏
- 在大数据上进行新颖而高效的机器和数据挖掘
- 基于深度学习技术的大数据集和基准生成和处理，用于大数据分析。

## 目录

- 1. 引言
   Bilal Jan, Haleem Farman 和 Murad Khan
   - 1.1 机器学习
     - 1.1.1 监督学习
     - 1.1.2 无监督学习
   - 1.2 深度学习
   ...
   - 参考文献
- 2. 大数据分析
   Bhagya Nathali Silva, Muhammad Diyan 和 Kijun Han
   - 2.1 概述
   - 2.2 大数据的特征
   - 2.3 大数据处理
   - 2.4 大数据中的数据分析问题
   ...
   - 参考文献
- 3. 深度学习方法和应用
   Jamil Ahmad, Haleem Farman 和 Zahoor Jan
   - 3.1 背景
   - 3.2 深度学习网络的分类
   - 3.3 用于监督学习的深度网络
   - 3.4 用于无监督学习的深度网络
   ...
   - 参考文献
- 4. 大数据和深度学习的整合
   Muhammad Talha, Shaukat Ali, Sajid Shah, Fiaz Gul Khan 和 Javed Iqbal
   - 4.1 大数据分析中的机器学习
     - 4.1.1 机器学习和大数据应用
   ...
   - 4.4 深度学习和迁移学习在大数据中的应用
     - 4.4.1 医疗保健
     - 4.4.2 金融
- 5. 大数据和深度学习在无线身体区域网络中的未来
   Fasee Ullah, Ihtesham Ul Islam, Abdul Hanan Abdullah 和 Atif Khan
   - 5.1 引言
   - 5.2 前馈网络模型
     - 5.2.1 深度学习框架
   - 5.3 深度学习的未来
   ...
   - 5.5 无线体域网的应用
     - 5.5.1 无线体域网的未来应用
     - 5.5.2 生物医学传感器在无线体域网中的应用
   - 5.6 无线体域网中的现有挑战
     - 5.6.1 路由协议
   - 5.7 MAC协议
     - 5.7.1 IEEE 802.15.4的超帧结构
     - 5.7.2 IEEE 802.15.6的超帧结构
   - 5.8 大数据介绍
   - 5.9 大数据在WBAN中的应用
     - 5.9.1 生命体征监测和分析
     - 5.9.2 患者异常状况的早期检测
     - 5.9.3 每日基础活动监测病人使用BMSs
   - 5.10 WBAN的未解决问题
     - 5.10.1 BMS的资源约束架构
     - 5.10.2 热点路径
     - 5.10.3 WBAN中的QoS
