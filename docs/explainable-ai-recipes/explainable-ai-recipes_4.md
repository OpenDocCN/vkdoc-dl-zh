...
| Pressure (from Chievres weather station) | | 毫米汞柱 |
| aRH_out | 外部湿度（来自 Chievres 气象站） | % |
| Wind speed (from Chievres weather station) | | 米/秒 |
| Visibility (from Chievres weather station) | | 千米 |
| Tdewpoint (from Chievres weather station) | | 摄氏度 |
| rv1 | 随机变量 1 | 无量纲 |
| rv2 | 随机变量 2 | 无量纲 |
...
![](img/540435_1_En_4_Fig11_HTML.jpg)

一张 SHAP 蜂群图。渐变刻度表示从高到低的特征值深浅。`lights` 特征的最大正 SHAP 值接近 200，其高特征值对应的最大负 SHAP 值为 -50。

**图 4-11** SHAP 值对模型预测的影响

```python
shap.dependence_plot("lights", shap_values, X)
```

![](img/540435_1_En_4_Fig12_HTML.jpg)

一张关于 `lights` 和 `T8` 相对于 `lights` 的 SHAP 散点图。渐变刻度表示 `T8` 值在 19 到 25 之间的深浅。具有低 `T8` 和最大 SHAP 值的点大约位于 (20, 200) 和 (30, 200)。高 `T8` 和最小 SHAP 值的点几乎位于 (0, 0)。

**图 4-12** `lights` 的 SHAP 值相对于 `T8` 和 `lights` 的绘图
...
![](img/540435_1_En_4_Fig13_HTML.jpg)

一张 SHAP 部分依赖图，展示了 SHAP 值与 `lights` 的关系。实线从 (0, 50) 开始，水平移动到 (5, 50)，然后急剧上升至 (6, 133)，最后在 (70, 134) 结束。最高的柱状图位于 (0, 125)。两条虚线 `E[lights]` 和 `E[f(x)]` 在 (5, 98) 处相交。所有数值均为估计值。

**图 4-13** `lights` 的部分依赖图
...
SHAP 偏依赖图展示了 SHAP 值与马力（power b h p）的关系。曲线在 (0, 5) 和 (300, 44) 之间上升，伴有多个尖锐峰值，最终在 (500, 40) 处结束。图中标出了一个点 (200, 17.5)。最高柱状图位于 (75, 10)。两条虚线（E power b h p 和 E f of x）在 (110, 8) 处相交。所有数值均为估计值。
...
SHAP 偏依赖图展示了 SHAP 值与马力（power b h p）的关系。曲线在 (0, 5) 和 (300, 45) 之间上升，伴有多个尖锐峰值，最终在 (600, 40) 处结束。最高柱状图位于 (75, 17)。两条虚线（E power b h p 和 E f of x）在 (110, 9) 处相交。所有数值均为估计值。
...
SHAP 偏依赖图展示了 SHAP 值与地点班加罗尔（location Bangalore）的关系。一条水平直线位于 (-0.1, 9.49) 和 (1.0, 9.49) 之间。两个柱状图分别位于 (0.0, 9.9) 和 (1.0, 9.04)。两条虚线（E location Bangalore 和 E f of x）在 (0.8, 9.49) 处相交。所有数值均为估计值。

**图 4-18** 虚拟变量班加罗尔与 SHAP 值的关系
...
SHAP 偏依赖图展示了 SHAP 值与地点加尔各答（location Kolkata）的关系。一条水平直线位于 (-0.1, 9.49) 和 (1.0, 9.49) 之间。两个柱状图分别位于 (0.0, 9.9) 和 (1.0, 9.05)。两条虚线（E location Kolkata 和 E f of x）在 (0.8, 9.49) 处相交。所有数值均为估计值。
...
一个散点图和直方图，展示了 SHAP 值与马力（power B H P）的关系。点簇大致在 (50, -5) 和 (200, 14) 之间呈曲线上升。直方图的最高柱状图大约在 (100, 15) 处。
...
一个 SHAP 瀑布图，展示了特征与 SHAP 值的关系。马力（power b h p）具有最高的 SHAP 值 9.84，而科钦（Kochi）地点具有最低的 SHAP 值 -0.54。f(x) 和 E[f(x)] 的线条分别位于 21.319 和 8.383。所有数值均为估计值。
...
一个 SHAP 蜂群图。马力（power B H P）和车龄的高特征值分别具有最大和最小的 SHAP 值 30 和 -10。所有数值均为估计值。
...
一个包含所有特征的 SHAP 蜂群图。马力（power B H P）和车龄的高特征值分别具有最大和最小的 SHAP 值 30 和 -10。其余特征的值大多为 0。所有数值均为估计值。
...
LIME 模型的 SHAP 局部条形图和表格。预测值为 16.50。一个包含特征和值两列的表格有 14 行。图中条形的最小值和最大值分别为 transmission（-2.91）和 power b h p（19.96）。
...
结果表中有一个名为 `BIAS` 的特征。这可以解释为线性回归模型的截距项。其他特征根据其权重大小以降序排列。`show_weights` 函数提供模型的全局解释，而 `show_prediction` 函数则通过考虑训练集中的一条记录来提供局部解释。
