...
打开一个新的笔记本并编写推理部分的执行代码。此代码在本地编写，并从 Google Colab 获取文件。代码包含三个部分：给定文本的预处理（替换适当的词和正则表达式）、将文本转换为所需的数组格式，以及模型推理。一旦拟合了模型，你就一次运行一个词的推理代码。起始词被初始化为词的起始标记。请注意，在代码块末尾，解码器状态会为下一个词重新初始化。