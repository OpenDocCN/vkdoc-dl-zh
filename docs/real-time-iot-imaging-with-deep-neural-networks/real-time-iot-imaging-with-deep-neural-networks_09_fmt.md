...
图 5-2 MQTT Explorer，I am a MQTT message
...
要访问控制台，请转到 `http://192.168.1.104:12101/`，其中 `192.168.1.104` 是树莓派的 IP 地址。
...
如果您已经安装了 `eSpeak` (`http://espeak.sourceforge.net/`)（一个文本转语音的命令行工具），您也可以生成一个 WAV 文件并将其输入到 Rhasspy。
...
我在配置中启用 Snowboy 工具包 (`https://snowboy.kitt.ai/`) 时取得了一些成功，如图 5-28 所示。
