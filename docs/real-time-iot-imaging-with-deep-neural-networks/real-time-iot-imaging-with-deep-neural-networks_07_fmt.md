...
加载图片使用的是 `Imgcodecs` 类中的 `imread` 函数。用路径调用 `imread` 会返回一个 `Mat` 对象，与你之前一直在处理的对象类型相同。将两个 `Mat`（马塞尔 `Mat` 和海滩 `Mat`）相加，只需像之前一样使用 `Core.add` 函数即可，如代码清单 2-4 所示。
...
图 2-9 `addWeighted`。马塞尔终于可以在海滩上放松了
