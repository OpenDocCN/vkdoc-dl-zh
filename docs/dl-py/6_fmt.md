...
一个全连通层可以描述为`y=f(xw)`，其中 `x` 为输入向量， `y` 为输出向量， `w` 为一组权重， `f` 为激活函数。相应地，一个卷积层可以描述为`y=f(s(xw)`，其中 `s` 表示输入和权值之间的卷积运算。
...
现在让我们来看看池化运算，它几乎总是与卷积一起用于 CNN。池化操作背后的思想是，如果事实上已经发现了特征的确切位置，那么它就不是问题。它只是提供了平移不变性。例如，假设任务是学习识别照片中的人脸。还假设照片中的人脸是倾斜的(通常如此)，我们有一个卷积层来检测眼睛。我们想从照片中眼睛的方向提取出它们的位置。池化操作实现了这一点，并且是 CNN 的重要组成部分。

图 6-7 说明了二维输入的池化操作。应注意以下几点:
...
*   与输入相比，池化产生的输出在维度上要小得多。

## 卷积-检测器-池构建模块

现在让我们来看看卷积-检测器-池模块，它可以被看作是 CNN 的一个构建模块，并看看我们前面介绍的所有操作是如何协同工作的。参见图 6-8 和图 6-9 。需要注意以下几点。
...
*   卷积、检测器和池化操作按顺序应用，以将输入转换为输出。输出被称为*特征图*。

*   输出通常会传递到其他层(卷积层或全连接层)。

*   多个卷积-检测器-池模块可以并行应用，消耗相同的输入并产生多个输出或特征图。
...
在介绍了 CNN 的所有组成元素之后，我们现在可以完整地看一个 CNN 的例子(见图 6-11 )。CNN 由两级卷积-检测器-池模块组成，每级都有多个滤波器/内核产生多个特征图。在这两个阶段之后，我们有一个产生输出的全连接层。一般而言，CNN 可能具有多级卷积-检测器-池模块(采用多个滤波器)，通常后跟一个全连接层。
...
*批量标准化*是一种技术，通过标准化每个小批量的层输入来帮助训练非常深的神经网络。标准化输入有助于稳定学习过程，从而大大减少训练深度网络所需的训练次数。批量标准化层被添加在卷积层之后，并且通常是卷积运算的标准块的一部分。也就是说，卷积层、批量标准化层、激活和最大池化操作在同一序列中的组合被定义为一个卷积单元。我们通常在 CNN 中添加几个这样的单元。
...
到目前为止，我们已经讨论了 CNN 背后的关键组成概念:卷积运算和池化运算，以及它们如何结合使用。现在让我们后退一步，用这些构件来内化 CNN 背后的思想。

*   首先要考虑的是 CNN 的容量。用卷积运算代替神经网络的至少一个全连接层的 CNN 比全连接的网络具有更小的容量。也就是说，存在全连接网络能够模拟 CNN 不能模拟的数据集。因此，要注意的第一点是，CNN 通过限制容量并因此使训练有效来实现更多。

*   要考虑的第二个想法是，学习驱动卷积运算的滤波器在某种意义上是表示学习。例如，已学习的过滤器可以学习检测边缘、形状等。这里要考虑的要点是，我们不是手动描述要从输入数据中提取的特征；相反，我们描述的是一个学会设计特性/表现的架构。

*   要考虑的第三个想法是池化操作引入的位置不变性。池化操作将特征的位置与其被检测到的事实分开。检测直线的过滤器可能会在图像的任何部分检测到该特征，但池化操作会选择检测到该特征的事实(最大池化)。

*   第四个想法是等级。一个 CNN 可以有多个卷积层和池化层堆叠在一起，然后是一个全连接的网络。这允许 CNN 建立一个概念层次，其中更抽象的概念基于更简单的概念(参见第一章)。

*   最后一个想法是在一系列卷积层和池化层的末端存在一个全连接层。一系列卷积层和池化层生成特征，标准神经网络学习最终的分类/回归函数。将 CNN 的这一方面与传统的机器学习区分开来是很重要的。在传统的机器学习中，专家会手工设计特征，并将其输入神经网络。在 CNN 中，这些特征/表示是从数据中学习的。
...
**Listing 6-2** Loading the Dataset into Memory
...
**Listing 6-3** Normalizing the Data and Preparing the Training/Validation Datasets
...
**Listing 6-5** Creating a Model Instance and Defining the Loss Function and Optimizer
...
**Listing 6-6** Training a CNN Model
...
Listing 6-7 Making Predictions
...
Listing 6-8 Importing the Packages for This Exercise
...
Listing 6-9 Enabling the GPU (If Available) in the Kernel
...
Listing 6-10 Plotting Sample Images from the Training Dataset
...
Listing 6-11 Transforming the Data and Creating the Training and Validation Sets
...
Listing 6-13 Defining the Evaluation Function
...
**清单 6-14** 定义损失函数和优化器，创建模型实例，并训练指定数量的纪元。
