...
如前所述，神经网络可以看作是一个函数 *f*<sub>*θ*</sub>:*x*→*y*，它以 *x*∈*R*<sup>*n*</sup> 作为输入，产生 *y*∈*R*<sup>*m*</sup>。我们现在可以更精确地了解 *θ*。它只是网络中所有单元的所有权重的集合。
...
这里，我们将输入 *x*∈*R*<sup>n</sup> 视为维数为 1 × *n*，将权重矩阵 *w*<sub>1</sub> 视为维数为 *n*×*p*<sub>1</sub>，将偏差项视为维数为 1×*p*<sub>1</sub> 的向量。那么请注意，*x**w*<sub>1</sub>+*b* 产生一个维数为 1 × *p*<sub>1</sub> 的向量，函数 *f* 简单变换向量的每个元素产生 ![$ {o}_1\in {R}^{p_1} $](img/478491_2_En_3_Chapter_TeX_IEq4.png)。
...
让我们假设我们已经生成了一个模型，在给定 *x* 的情况下预测 *y* 的概率。我们用 *f* ( *x* ， *θ* ) 来表示这个模型，其中 *θ* 表示模型的参数。最大似然背后的思想是找到一个最大化 *P*(*D*|*θ*) 的 *θ*。假设一个伯努利分布，并给定每个例子 {( *x*<sub>1</sub> ， *y*<sub>1</sub> )，( *x*<sub>2</sub> ， *y*<sub>2</sub> )，…(*x*<sub>*n*</sub>， *y*<sub>*n*</sub>)}，我们可以对两边进行对数运算得出如下:
...
术语 *log n* ！还有 *log n*<sub>1</sub> ！*n*2！⋯*n*??*k*！不被 *θ* 参数化，并且可以被安全地忽略，因为我们试图找到最大化 *P*(*D*|*θ*) 的 *θ*。由此，我们有了以下:
...
现在让我们讨论使用最大似然法推导回归中使用的平方误差。让我们假设 `y` ∈ `R`。与前面的情况不同，我们假设我们有一个预测概率的模型，我们将假设我们有一个预测 `y` 值的模型。为了应用最大似然思想，我们假设实际的 `y` 和预测的 `$$ \hat{y} $$` 之间的差具有零均值和方差为 `σ`2 的高斯分布。然后，可以显示最小化
...
更新过程可以使用以下公式来简化。首先，我们计算过去梯度的指数加权平均值为 `ν` <sub>`t`</sub> ，其中 `ν`<sub>`t`</sub>=`γν`<sub>`t`—1</sub>+`η`∇<sub>`θ`</sub>`j`(`θ`)和 `θ`=`θ`-
...
其中 `η`–是定义初始学习率的超参数， `g`<sub>`t`</sub> 是 `θ` 中参数/权重 `w` 在时间 `t` 的梯度。我们将 `∈` 加到分母上，以避免被零除的情况。
...
**Listing 3-3: Defining a Feed Forward Neural Network**
...
**Listing 3-4: Defining the Loss, Optimizer, and Training Function for the Neural Network**
...
Listing 3-5 Training Model with Various Optimizers
