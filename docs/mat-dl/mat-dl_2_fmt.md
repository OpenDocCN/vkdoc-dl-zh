...
图中的圆圈和箭头分别表示节点和信号流。`x[1]`、`x[2]` 和 `x[3]` 是输入信号。`w[1]`、`w[2]` 和 `w[3]` 是对应信号的权重。最后，`b` 是偏置，它是与信息存储相关联的另一个因素。换句话说，神经网络的信息以权重和偏置的形式存储。
...
学习率 `α` 决定了每次权重变化的大小。如果这个值太高，输出会在解周围徘徊并无法收敛。相反，如果这个值太低，计算达到解的速度会太慢。
...
它与上一节的 delta 规则相同，只是将 `e[i]` 替换为 `δ[i]`。在这个方程中，`δ[i]` 定义为：
...
+   `φ′` = 输出节点 `i` 的激活函数 `φ` 的导数

回想一下，我们在示例中使用了线性激活函数 `φ(x) = x`。这个函数的导数是 `φ'(x) = 1`。将这个值代入方程 2.4，得到 `δ[i]` 如下：
