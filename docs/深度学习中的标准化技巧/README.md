...
加州大学洛杉矶分校 Gerard Medioni 加拿大多伦多大学计算机科学系 Sven Dickinson

该系列出版与计算机视觉和模式识别相关的主题。范围涵盖了顶级计算机科学会议的范围，包括场景重建、事件检测、视频跟踪、物体识别、3D姿态估计、学习、索引、运动估计和图像恢复的科学。作为一门科学学科，计算机视觉关注从图像中提取信息的人工系统背后的理论。图像数据可以采用多种形式，例如视频序列、多摄像头视图或来自医学扫描仪的多维数据。作为一门技术学科，计算机视觉致力于将其理论和模型应用于计算机视觉系统的构建，例如自动驾驶汽车/导航系统、医学图像分析和工业机器人。
...
ISSN 2153-1064 (电子版)
计算机视觉综合讲座
ISBN 978-3-031-14594-0
ISBN 978-3-031-14595-7 (电子书)
[https://doi.org/10.1007/978-3-031-14595-7](https://doi.org/10.1007/978-3-031-14595-7)

© 版权所有的编辑（如适用）和作者，独家许可给 Springer Nature Switzerland AG 2022
本作品受版权保护。出版商独家授权，无论全部还是部分材料，包括翻译、重印、插图的再利用、朗读、广播、微缩胶片复制或以任何其他实体方式复制、传输或信息存储和检索、电子适应、计算机软件，或通过现在已知或今后开发的类似或不同的方法。

在本出版物中使用一般描述性名称、注册名称、商标、服务标志等，并不意味着即使在没有具体声明的情况下，这些名称也不受相关保护法律和法规的约束，因此可以自由使用。

出版商、作者和编辑可以安全地假设本书中的建议和信息在出版日期时被认为是真实和准确的。出版商、作者或编辑对本文中所包含的材料不提供明示或暗示的保证，也不对可能存在的任何错误或遗漏负责。出版商在已发表的地图和机构关系中保持中立。
