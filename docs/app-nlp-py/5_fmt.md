...
数据是制表符分隔的，但也是`.txt`格式。在我们开始训练 BRNN 之前，这需要一些数据清洗。
...
`input_chars` 和 `output_chars` 的内容如下：

```
input_chars: [u' ', u'"', u'#', u'%', u'&', u"'", u'(', u')', u',', u'-', u'.', u'/', u'0', u'1', u'2', u'3', u'4', u'5', u'6', u'7', u'8', u'9', u':', u';', u'>', u'?', u'_', u'a', u'b', u'c', u'd', u'e', u'f', u'g', u'h', u'i', u'j', u'k', u'l', u'm', u'n', u'o', u'p', u'q', u'r', u's', u't', u'u', u'v', u'w', u'x', u'y', u'z']
output_chars: [u' ', u'!', u'"', u'$', u'%', u'&', u"'", u'(', u')', u'+', u',', u'-', u'.', u'/', u'0', u'1', u'2', u'3', u'4', u'5', u'6', u'7', u'8', u'9', u':', u';', u'?', u'[', u']', u'a', u'b', u'c', u'd', u'e', u'f', u'g', u'h', u'i', u'j', u'k', u'l', u'm', u'n', u'o', u'p', u'q', u'r', u's', u't', u'u', u'v', u'w', u'x', u'y', u'z']
```
...
我们首先实例化两个输入向量和一个输出向量，用 `x_encoder`、`x_decoder` 和 `y_decoder` 表示。接下来，这表示数据通过神经网络并对照目标标签进行验证的顺序。虽然我们在这里选择创建的一次性编码是相似的，但是我们通过创建一个三维数组来评估每个问题和答案，做了一点小小的改变。每行代表一个问题，每个时间步长代表一个字符，每列代表我们的字符集中的字符类型。我们对每个问答对重复这个过程，直到我们有一个包含整个数据集的数组，它产生 4980 个数据观察值。
...
模型预测：`saint bernadette soubiroust`
实际输出：`saint bernadette soubirous`
模型预测：`a copper statue of christ`
实际输出：`a copper statue of christ`
模型预测：`the main building`
实际输出：`the main building`
模型预测：`a marian place of prayer and reflection`
实际输出：`a marian place of prayer and reflection`
模型预测：`a golden statue of the virgin mary`
实际输出：`a golden statue of the virgin mary`
模型预测：`september 18760`
实际输出：`september 1876`
模型预测：`twice`
实际输出：`twice`
模型预测：`the observer`
...
如您所见，这个模型表现相当好，只有三个 epoch。尽管由于添加了额外的字符，在拼写上有一些问题，但在大多数情况下，消息本身是正确的。您可以继续尝试这个问题，特别是通过改变模型架构来看看是否有一个能产生更好的准确性。
