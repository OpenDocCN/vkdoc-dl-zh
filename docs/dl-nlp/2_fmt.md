...
> **Note**
>
> 我们在这里的实现中使用了关于 skip-gram 的论文中给出的概率得分函数。对于训练集中的每个单词 `w_i` ，我们将按照
>
> ![$$ P\left({w}_i\right)=1-\left(\sqrt{\frac{t}{f\left({w}_i\right)}}\right) $$](img/A461351_1_En_2_Chapter_Equd.gif)
>
> 给出的概率将其丢弃，其中 `t` 是阈值参数，`f(w_i)`是单词 `w_i` 在总数据集中的出现频率。
...
图 2-14 使用 CBOW 模型训练 Wikipedia 语料库后获得的词向量的二维表示
