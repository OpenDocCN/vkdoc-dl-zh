...
1.  在右上角菜单中点击 *Runtime*。
2.  从下拉菜单中选择 *Change runtime type*。
3.  从 *Hardware accelerator* 下拉菜单中选择 *GPU*。
4.  点击 *SAVE*。
...
```py
print ('length of window:', len(x_train[0]), '\n')
...
```py
print ('target from the 1st window:', '   ')
print (np.round(y_train[0], 8))
print ('last obs from the
...
train_mv = train_mv.cache().shuffle(BUFFER_SIZE).batch(BATCH_SIZE).repeat()
