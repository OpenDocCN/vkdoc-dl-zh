...
让我们创建两个简单的 7 `×` 7 过滤器。我们希望我们的第一个过滤器中间有一条垂直白线，而第二个过滤器中间有一条水平白线。*过滤器* 用于在卷积
...
Predict on the *test_cf* data:

```python
predictions = np.argmax(model.predict(test_cf), axis=-1)
```

Wrap the *predict* and *argmax* methods to directly obtain predicted labels instead of generating a probability array.
...
As shown in List 7-12, get the first five actual labels.
...
Get the first batch of images. Since we set the batch size to 128, we get the first 128 images. Slice the first five images from the batch. Convert to label names.

Compare *pred_labels* with *actual_labels* to understand the prediction performance.

### Building Prediction Plots

First, extract 20 samples from the test set, as shown in List 7-13.
...
Build a function to display results, as shown in List 7-14.
...
Red titles indicate incorrect classification.

### Building a CNN with Keras Data

Although recommended to load data in TFDS format, Keras is very popular in the industry. So let's build a model from *keras.datasets*.
...
### Creating Variables to Store Training and Test Samples

Create variables as shown in List 7-15 to store training and test data.
...
Always displaying some images is a good idea. In this case, we displayed 30 images from the training data set. Visualization allows us to verify that images and labels correspond. That is, the frog image is labeled as frog, etc.

List 7-16 shows sample images from the training set.
...
Build the input pipeline by scaling images and cutting them into TensorFlow consumable parts. Continue with (where appropriate) shuffling, batching, and prefetching.

List 7-17 creates the input pipeline.
...
List 7-19 compiles and trains the model.
...
List 7-20 visualizes training performance results.
...
In the past few years, many improvements to the basic CNN architecture have been developed, which have greatly improved prediction performance. Although we have not covered these advancements in this lesson, we believe we have provided a basic foundation for CNNs to help you easily use these latest advancements, even many advancements that may appear in the future.
