...
章节的笔记本位于以下 URL：[TensorFlow](https://github.com/paperd/tensorflow)。
...
免费 GPU 有使用限制。请参考以下 URL 了解更多信息：[Colab GPU 限制](https://research.google.com/colaboratory/faq.html#resource-limits)*.*
...
`load_digits` 数据集经过大量预处理，所以我们不必担心清理或整理。它由 1,797 个 8 × 8 像素图像组成。每个图像是一个 64 像素的矩阵，代表从 0 到 9 的手写数字。我们通过 8 乘以 8 得到 64 像素。**像素**是一个介于 0 到 255 之间的整数值，用于表示图像数据。`load_digits` 数据集通常用于训练机器学习系统通过算法分类识别手写数字图像。
...
The image name is *Figure0201.png*. We import *Image* from the *PIL* library. When accessing the Pillow module, Python expects PIL. We also import *matplotlib.pyplot*, which is a module of the Python plotting library *matplotlib*. We open the image and display it in the Colab notebook.
