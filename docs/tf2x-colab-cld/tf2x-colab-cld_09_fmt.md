...
[情感分析](https://monkeylearn.com/sentiment-analysis/%2523:%253F:text%2520analysis%2520is%2520the%2520interpretation,or%2520services%2520in%2520online%2520feedback)
...
```python
readable(train_reviews[longest_index])
```
...
Since we already know the index of the longest comment, we can easily retrieve it from `train_reviews`. Display a part of it, as the comment is quite long. We can also easily retrieve the label from `train_labels`. Make the label readable and display it. Finally, display the length of the longest review.

Let's see what the shortest comment looks like. But we can't do this directly.

We must first find the minimum number of words in the comments:
...
Since we don't know which comment is the shortest, we use the `amin` method to return the minimum value.

Now we can get the index of the shortest comment in the training samples:
...
We use the `where` method to return the index we seek. Since this method returns all comments that meet the condition, we grab the first one and display its index.

Listing 9-14 shows the comment, its readable label, and its length.
...
Display the first label and its review as an integer-encoded comment, as shown in Listing 9-15.
...
Display the first comment in a readable form:
...
## Training Keras IMDb Data

Limit the vocabulary size to improve performance:
...
Truncate the text size to 80 words to improve performance:
...
Load data using a limited vocabulary:
...
Pad the samples to ensure that all sequences have the same length:
...
Prepare TensorFlow consumable training data:

```python
train_k = tf.data.Dataset.from_tensor_slices((x_train, y_train))
train_ks = train_k.shuffle(buffer_size).batch(batch_size).prefetch(1)
```

Prepare TensorFlow consumable test data:

```python
test_k = tf.data.Dataset.from_tensor_slices((x_test, y_test))
...
embed_size = 128
model = Sequential([
    Embedding(vocab_size, embed_size, mask_zero=True, input_shape=[None]),
    GRU(128, return_sequences=True),
    GRU(128),
    Dense(1, activation="sigmoid")
])
...
```python
epochs = 2
model.fit(train_ks, batch_size=BATCH_SIZE, epochs=epochs, validation_data=(test_ks))
```
