...
2. 定位文件：点击 *第八章*，点击 *数据*，然后点击 *two_cities.txt*。
3. 点击 *原始* 按钮。
4. 复制文本（*Ctrl+A+C*）。
5. 将其粘贴到 *记事本* 或其他基本文本编辑器中（*Ctrl+V*）。
6. 将其保存在您的计算机上为 [*two_cities.txt*](https://github.com/paperd/tensorflow)。
7. 将文件拖放到您的 Google Drive 的 *Colab Notebooks* 文件夹中。
...
5. 将“另存为类型”更改为 [*所有文件*](https://github.com/paperd/tensorflow)。
6. 将其保存在您的计算机上为 [*two_cities.txt*](https://github.com/paperd/tensorflow)。
7. 处理文本文件。
...
TensorFlow 数据设计用于处理无限序列。因此，它不会尝试在内存中打乱整个序列。相反，它维护一个缓冲区，其中打乱元素。我们将*BUFFER_SIZE*设置为
...
第二层是一个 GRU 层。可学习参数的数量基于公式 `3 × (n² × mn + 2n)`，其中 `m` 是输入维度，`n` 是输出维度。乘以 `3` 是因为 GRU 有三个操作集，需要这些大小的权重矩阵。将 `n` 乘以 `2` 是因为 RNN 的反馈循环。因此，我们得到 3,938,304 个可学习参数。以下是结果分解：
