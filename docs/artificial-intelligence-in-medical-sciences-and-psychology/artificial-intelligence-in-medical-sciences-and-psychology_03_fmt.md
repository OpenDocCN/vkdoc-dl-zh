...
![$$ P\left(,{S}_{i2},{S}_{i3}\dots, {S}_{ik-1}\right)=P\left({S}_{ij}|{S}_{ik-1}\right) $$](img/527603_1_En_3_Chapter_TeX_Equ1.png) `(公式 3-1)`

其中，`S[ij]` 表示隐藏状态的独立观测值，`S[i1]` 表示第一个隐藏状态，`S[i2]` 表示第二个隐藏状态，以此类推。

`图 3-1` 展示了转移概率（定义见`公式 3-2`）和初始概率（定义见`公式 3-3`）：

![$$ {\alpha}_{ij}=P\left({S}_i|{S}_j\right) $$](img/527603_1_En_3_Chapter_TeX_Equ2.png) `(公式 3-2)`

![](img/527603_1_En_3_Fig1_HTML.png) `图 3-1`

转移概率以及

![$$ {\pi}_i=P\left({S}_i\right). $$](img/527603_1_En_3_Chapter_TeX_Equ3.png) `(公式 3-3)`
...
**注意**：约翰霍普金斯大学每天更新数据。要重现本练习，请从本章的 GitHub 文件夹中下载数据集。

## 描述性分析

在执行高斯隐马尔可夫模型之前，首先应进行探索性描述分析。在本节中，你将通过构建直方图和箱线图来考察数据的分布情况。随后，你将通过集中趋势和离散程度来描述数据。`清单 3-1` 收集数据并识别空值（见`图 3-2`）。首先，在你的环境中安装 `pandas`：`pip install pandas`。此外，在你的环境中安装 `Matplotlib`：`pip install matplotlib`。同时，在你的环境中安装 `seaborn`：`pip install seaborn`。

![](img/527603_1_En_3_Fig2_HTML.png) `图 3-2`
...
`清单 3-2` 删除缺失值。此外，它还绘制了另一张热力图以验证空值是否已被删除（见`图 3-3`）。

![](img/527603_1_En_3_Fig3_HTML.png) `图 3-3`
...
`清单 3-3` 构建了美国确诊 COVID-19 病例的直方图（见`图 3-4`）。

![](img/527603_1_En_3_Fig4_HTML.png) `图 3-4`
...
`清单 3-4` 构建了美国确诊 COVID-19 病例的箱线图，以确认`图 3-3` 中发现的分布情况（见`图 3-5`）。

![](img/527603_1_En_3_Fig5_HTML.jpg) `图 3-5`
...
`清单 3-5` 使用均值替换异常值。然后，它构建了另一个箱线图以验证异常值是否已被移除（见`图 3-6`）。首先，在你的环境中安装 `NumPy`：`pip install numpy`。

![](img/527603_1_En_3_Fig6_HTML.jpg) `图 3-6`
...
![](img/527603_1_En_3_Fig7_HTML.png) `图 3-7`
...
`清单 3-7` 报告了均值、中位数和标准差等关键统计量（见`表 3-1`）。
...
`清单 3-10` 描述了高斯隐马尔可夫模型的隐藏状态（见`表 3-3`）。
...
![](img/527603_1_En_3_Fig8_HTML.jpg) `图 3-8` 高斯隐马尔可夫模型的隐藏状态
...
`清单 3-14` 描绘了美国新冠确诊病例的蒙特卡洛模拟结果（见`图 3-9`）。

![](img/527603_1_En_3_Fig9_HTML.jpg) `图 3-9` 美国新冠确诊病例的蒙特卡洛模拟结果
...
`清单 3-15` 概述了美国新冠确诊病例蒙特卡洛模拟结果的描述性统计（见`表 3-4`）。
...
`清单 3-16` 构建了美国新冠确诊病例蒙特卡洛模拟结果的描述性统计（见`图 3-10`）。

![](img/527603_1_En_3_Fig10_HTML.png) `图 3-10` 美国新冠确诊病例蒙特卡洛模拟结果的直方图
