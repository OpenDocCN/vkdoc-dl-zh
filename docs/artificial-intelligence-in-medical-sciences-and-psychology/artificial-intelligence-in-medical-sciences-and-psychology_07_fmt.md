...
![图 7-1 生存分析家族](img/527603_1_En_7_Fig1_HTML.png)
...
![图 7-2 临床试验设置](img/527603_1_En_7_Fig2_HTML.png)
...
![图 7-3 空值热力图](img/527603_1_En_7_Fig3_HTML.jpg)
...
![图 7-4 年龄计数图](img/527603_1_En_7_Fig4_HTML.jpg)
...
![图 7-5 `Year_of_operation` 计数图](img/527603_1_En_7_Fig5_HTML.png)
...
![图 7-6 生存数据配对图](img/527603_1_En_7_Fig6_HTML.png)
...
### 执行 Nelson-Aalen 加性模型

清单 7-9 执行了 `Nelson-Aalen` 加性模型，并概述了时间列（`Year_of_operation`）和事件列（`Event`）。

```python
aalen_additive_method = AalenAdditiveFitter().fit(survival_censored_data, "Year_of_operation", event_col = "Event")
```

**清单 7-9：执行 Nelson-Aalen 加性模型**

清单 7-10 概述了 `Nelson-Aalen` 加性模型的概况表（见表 7-6）。

**表 7-6 Nelson-Aalen 加性模型概况表**
...
**清单 7-10：概述 Nelson-Aalen 加性模型的概况表**

#### 概述 Nelson-Aalen 加性模型的置信区间

表 7-6 显示，`Age` 的系数为 0，`Axillary_lymph_nodes` 的系数为 0，截距的系数为 0.03。此外，一致性指数为 0.47。清单 7-11 概述了 `Nelson-Aalen` 加性模型的置信区间（表 7-7）。

**表 7-7 概述 Nelson-Aalen 加性模型的置信区间**
...
`清单 7-12` 描绘了 Nelson-Aalen 加法模型的生存风险（见 `图 7-7`）。
...
`清单 7-13` 描绘了 Nelson-Aalen 加法模型的累积生存风险（见 `图 7-8`）。
...
`清单 7-14` 描绘了 Nelson-Aalen 加法模型的基线生存风险（见 `图 7-9`）。
