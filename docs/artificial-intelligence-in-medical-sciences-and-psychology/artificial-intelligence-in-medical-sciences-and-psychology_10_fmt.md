...
首先在你的环境中安装 `pandas`：`pip install pandas`。
...
其中 `ρ_T` 代表信度检验，`k` 代表李克特量表中的项目数，`σ_ij` 代表 `X_i` 和 `X_j` 之间的关系，而 ![$$ {\sigma}_X² $$](img/527603_1_En_9_Chapter_TeX_IEq1.png) 代表项目间的方差和协方差。

**清单 9-3** 识别克朗巴赫系数 Alpha

首先在你的环境中安装 `pingouin`：`pip install pingouin`。
...
通过执行 `factor_analyzer` 库中的 `calculate_bartlett_sphericity` 方法（参见表 9-3，该方法返回 p 值）来汇总 p 值。首先在你的环境中安装 `factor_analyzer`：`pip install factor_analyzer`。
...
通过执行 `factor_analyzer` 库中的 `calculate_kmo()` 方法（参见表 9-4，该方法返回 p 值）来汇总 p 值。
...
构建了一个碎石图（参见图 9-3）。首先在你的环境中安装 `Matplotlib`：`pip install matplotlib`。
...
通过执行 `FactorAnalyzer()` 方法整合了因子载荷，然后指定了因子数量和旋转方法（见表 9-5）。
