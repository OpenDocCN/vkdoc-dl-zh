...
![图 5-1 待逼近的函数](img/477043_2_En_5_Fig1_HTML.jpg)
...
![图 5-2 网络架构](img/477043_2_En_5_Fig2_HTML.jpg)
...
单击桌面上的 `NetBeans` 图标以打开 `NetBeans IDE`。`IDE` 屏幕分为几个窗口。`导航窗口`是您查看项目的地方（`图 5-3`）。单击项目前面的 `+` 图标可显示项目的组件，例如源包、测试包和库。

![图 5-3 NetBeans IDE](img/477043_2_En_5_Fig3_HTML.jpg)

要创建新项目，请选择 `文件` ➤ `新建项目`。将出现如 `图 5-4` 所示的窗口。

![图 5-4 新建项目对话框](img/477043_2_En_5_Fig4_HTML.jpg)

单击 `下一步`。将出现如 `图 5-5` 所示的对话框。

![图 5-5 命名新项目](img/477043_2_En_5_Fig5_HTML.jpg)

输入项目名称 `Sample1_Norm`，然后单击 `完成` 按钮。`图 5-6` 显示了出现的下一个对话框。

![图 5-6 Sample1_Norm 项目](img/477043_2_En_5_Fig6_HTML.jpg)

创建的项目显示在 `导航窗口` 中（`图 5-7`）。

![图 5-7 已创建的项目](img/477043_2_En_5_Fig7_HTML.jpg)

现在，源代码显示在 `源代码窗口` 中，如 `图 5-8` 所示。

![图 5-8 新项目的源代码](img/477043_2_En_5_Fig8_HTML.jpg)
...
要创建新项目，请选择 `文件` ➤ `新建项目`。此时将显示如 `图 5-9` 所示的对话框。

![图 5-9 NetBeans IDE](img/477043_2_En_5_Fig9_HTML.jpg)

点击 `下一步`。在下一个对话框（如 `图 5-10` 所示）中，输入项目名称，然后点击 `完成` 按钮。

![图 5-10 新建 NetBeans 项目](img/477043_2_En_5_Fig10_HTML.jpg)

项目创建完成后，您应该在 `导航窗口` 中看到它（`图 5-11`）。

![图 5-11 项目 Sample1](img/477043_2_En_5_Fig11_HTML.jpg)

程序的源代码会显示在 `源代码窗口` 中（`图 5-12`）。

![图 5-12 程序 Sample1.java 的源代码](img/477043_2_En_5_Fig12_HTML.jpg)

同样，这仅仅是 Java 程序自动生成的骨架。现在让我们在此添加必要的逻辑。首先，包含所有必要的导入文件。这里有三组导入语句（`Java 导入`、`Encog 导入` 和 `XChart 导入`），其中两组（属于 `Encog` 的那组和属于 `XChart` 的那组）被标记为错误，因为 `NetBeans` 无法找到它们（`图 5-13`）。

![图 5-13 被标记为错误的导入语句](img/477043_2_En_5_Fig13_HTML.jpg)

以下是修复方法。右键点击项目并选择 `属性`。此时将出现“`项目属性`”对话框（`图 5-14`）。

![图 5-14 “项目属性”对话框](img/477043_2_En_5_Fig14_HTML.jpg)

在“`项目属性`”对话框的左侧列中，选择 `库`。点击 `属性` 对话框右侧的 `添加 JAR/文件夹` 按钮。点击“`Java 平台`”字段（位于屏幕顶部）的下拉箭头，然后导航到 `Encog` 包的安装位置（`图 5-15`）。

![图 5-15 Encog 的安装位置](img/477043_2_En_5_Fig15_HTML.jpg)

双击“`安装`”文件夹，将显示两个 `JAR` 文件（`图 5-16`）。

![图 5-16 Encog JAR 文件位置](img/477043_2_En_5_Fig16_HTML.jpg)

选中这两个 `JAR` 文件，然后点击 `打开` 按钮。它们将被包含在要添加到 `NetBeans` 的 `JAR` 文件列表中（`图 5-17`）。

![图 5-17](img/477043_2_En_5_Fig17_HTML.jpg)

要包含在 `NetBeans IDE` 中的 `Encog` JAR 文件列表

再次点击“添加 JAR/文件夹”按钮。再次点击“Java 属性”字段的下拉箭头，导航到 `XChart` 包的安装位置（图 5-18）。

![图 5-18 XChart 位置](img/477043_2_En_5_Fig18_HTML.jpg)

双击 `XChart-3.5.0` 文件夹，然后选择两个 `XChart` JAR 文件（图 5-19）。

![图 5-19 XChart 文件](img/477043_2_En_5_Fig19_HTML.jpg)

点击“打开”按钮。现在我们有了要包含在 `NetBeans IDE` 中的四个 JAR 文件（来自 `Encog` 和 `XChart`）的列表（图 5-20）。

![图 5-20 要包含在 NetBeans IDE 中的 JAR 文件列表](img/477043_2_En_5_Fig20_HTML.jpg)

最后，点击“确定”，源文件中的所有错误都将消失。

与其为每个新项目都执行此操作，更好的方法是设置一个新的全局库。从主菜单栏中，选择“工具” > “库”。将出现如图 5-21 所示的屏幕。

![图 5-21 创建全局库](img/477043_2_En_5_Fig21_HTML.jpg)
...
![图 5-22 设置断点](img/477043_2_En_5_Fig22_HTML.jpg)
...
![图 5-23 调试时用于推进执行的图标](img/477043_2_En_5_Fig23_HTML.jpg)
...
![图 5-24 训练结果图表](img/477043_2_En_5_Fig24_HTML.jpg)
...
![图 5-25 网络未训练点处的近似图表](img/477043_2_En_5_Fig25_HTML.jpg)
...
![图 5-26 原始函数与逼近函数](img/477043_2_En_5_Fig26_HTML.jpg)
