...
[Oracle JDK 和 NetBeans 下载页面](https://www.oracle.com/technetwork/java/javase/downloads/jdk-netbeans-jsp-3413139-esa.html)

屏幕上将显示如图 4-1 所示的界面。

![Java 下载页面](img/477043_2_En_4_Fig1_HTML.png)
图 4-1 Java 下载页面
...
![系统属性对话框](img/477043_2_En_4_Fig2_HTML.jpg)
图 4-2 系统属性对话框

点击“高级”选项卡上的“环境变量”按钮。然后点击“新建”以打开允许输入新环境变量的对话框（图 4-3）。

![新建系统变量对话框](img/477043_2_En_4_Fig3_HTML.jpg)
图 4-3 新建系统变量对话框

在“变量名”字段中输入 `JAVA_HOME`。在“变量值”字段中输入已安装 Java 环境的路径（图 4-4）。

![设置 JAVA_HOME 环境变量](img/477043_2_En_4_Fig4_HTML.jpg)
图 4-4 设置 `JAVA_HOME` 环境变量

点击“确定”。接下来，选择 `CLASSPATH` 环境变量并点击“编辑”。

将 Java JDK 的 `lib` 目录路径添加到 `CLASSPATH` 变量值字段中，并添加 Java 的 `jar` 文件（见图 4-5）。

![更新后的 CLASSPATH 系统变量](img/477043_2_En_4_Fig5_HTML.jpg)
图 4-5 更新后的 `CLASSPATH` 系统变量
...
要安装 `Encog`，请访问 [Encog 官方网站](https://www.heatonresearch.com/encog)。向下滚动到名为 Encog Java Links 的部分，然后点击 Encog Java Download/Release 链接。在下一个屏幕上，为 `Encog` 4.3 版本选择以下两个文件：
...
![XChart 主页](img/477043_2_En_4_Fig6_HTML.jpg)
图 4-6 XChart 主页
...
![CLASSPATH 环境变量](img/477043_2_En_4_Fig7_HTML.jpg)
图 4-7 `CLASSPATH` 环境变量

```
C:\Download\XChart\xchart-3.8.0\xchart-3.8.0.jar
