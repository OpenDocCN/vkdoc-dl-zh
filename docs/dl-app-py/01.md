...
本章涵盖深度学习框架 TensorFlow 的基础知识。深度学习在模式识别方面做得非常好，特别是在图像、声音、语音、语言和时序数据的背景下。在深度学习的帮助下，你可以分类、预测、聚类和提取特征。好在 2015 年 11 月，谷歌发布了 TensorFlow，已经在谷歌搜索、垃圾邮件检测、语音识别、谷歌助手、Google Now、Google Photos 等谷歌大部分产品中使用。本章的目的是解释TensorFlow的基本组成部分。
...
重要的是要认识到`init`是初始化所有全局变量的TensorFlow子图的句柄。在调用`sess.run`之前，变量是未初始化的。
