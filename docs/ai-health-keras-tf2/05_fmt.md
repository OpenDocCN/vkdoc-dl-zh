...
您还将学习如何在模板文档——遵循标准模板或实体集的文档——上使用不同的信息提取技术。您将构建从开箱即用的 OCR 到图形卷积网络（GCN）的信息提取用例。GCN 相对较新，属于图形神经网络类，这是一种正在积极研究和应用的思想。

## 数据

您将在此案例中使用的数据是 ICDAR 2019 年扫描收据 OCR 和信息提取数据集上的稳健阅读挑战。网站链接为 [`ICDAR 2019 数据集链接`](https://rrc.cvc.uab.es/?ch=13)。在网站注册后可以很容易地从下载部分获得。您可能会发现博客/文章提到原始数据中的数据问题，因为一些数据被错误地标注，但这已被团队纠正。
...
1.  OCR 输出：数据集中的每个图像都用文本边界框（`bbox`）和每个文本 `bbox` 的副本进行注释。位置被标注为具有四个顶点的矩形，从顶部开始按顺时针顺序排列。
    1.  你可以简化这种表示。你实际需要的是（`x_min`，`y_min`）和（`x_max`，`y_max`），分别是矩形的右上角和左下角。
...
1.  读取 OCR 输出，只需保持（`x_min`，`y_min`）和（`x_max`，`y_max`），即（`x_1`，`y_1`）和（`x_3`，`y_3`）。
...
为此，您将使用 `fuzzywuzzy` 包，这是一个非常有效的包，它提供了对各种类型的模糊匹配（如 Levenshtein 距离、音标匹配等）的访问，并可以以各种方式应用（如令牌级、字符级等）。
...
![图5-4](img/502837_1_En_5_Fig4_HTML.jpg)
...
def graph_modelling(key, save_graph=False):
    ...
    for word_ids in (right_connections.keys() | bottom_connections.keys()):
        if word_ids in right_connections:
            final_connections.setdefault(word_ids, []).append(right_connections[word_ids]['right'])
        if word_ids in bottom_connections:
            final_connections.setdefault(word_ids, []).append(bottom_connections[word_ids])
...
![图 5-5 不同账单的网络布局示例](img/502837_1_En_5_Fig5_HTML.jpg)
...
![图 5-6 多重图的并与训练过程。来源: `github.com/tkipf/gcn`](img/502837_1_En_5_Fig6_HTML.jpg)
...
![图 5-7 建模连接的数据，恰当的例子](img/502837_1_En_5_Fig7_HTML.jpg)
...
![图 5-8 作为多部图的神经网络](img/502837_1_En_5_Fig8_HTML.jpg)
...
![图 5-9 具有邻接矩阵的无向图](img/502837_1_En_5_Fig9_HTML.jpg)
...
![图 5-10 图像的卷积运算](img/502837_1_En_5_Fig10_HTML.jpg)
...
我将要谈论的网络是 `Kipf` 和 `Welling` 在 2017 年题为“使用图形卷积网络的半监督分类”的论文中提出的 `GCN` 网络。此网络不考虑边缘功能，但您不会将它们用于您的应用程序。大多数复杂网络都需要边要素。正如在分子化学中，双键比单键强得多，所以两者不能以同样的方式处理；它们必须以不同的方式使用。但是，在您的案例中，边代表发票中不同文本实体之间的连接，因此具有相同的含义，因此您可以取消可以模拟边要素的网络。

但对于好奇的人来说，这里有两篇论文是由 `Kipf` 和 `Welling` 分享的对 `GCN` 建筑的改进:

- `Gilmer` 等人于 2017 年在 `ICML` 创作的《`MPNN`》
- 2018 年 `ICLR` `Velikovic` 等人的“图形注意力网络”

`Zhou`、`Cui`、`Zhang` 等人在“图神经网络:方法与应用综述”中的论文也给出了很好的综述

### 了解 GCNs

图 5-11 解释了卷积如何在图数据上工作，给定一个无向图 `G = (V, e)` 具有节点 `v_i ∊ V`，边 `(v_i, v_j) ∊ E` 和一个大小为 `NXN` 的邻接矩阵(`a`), 其中 `n` 表示节点的数量，特征矩阵(`h`)的大小为 `NXK`，其中 `k` 是特征向量的维数。要从每个节点的邻居中找到特征值，您需要将矩阵 `A` 和 `h` 相乘。

![图 5-11 图的卷积运算](img/502837_1_En_5_Fig11_HTML.jpg)
...
Kipf 和 Welling 在他们提出的想法中指出，与高度连接的层相比，度数较低的节点将对其邻居施加更多的影响。基本上，向所有节点传递信息的节点不会提供任何关于节点的“独特”信息。为此，作者建议将 `D^{-1} AH` 的结式矩阵与 `D^{-1}` 相乘。因为你要规格化两次，所以你要确保除以 `sqrt(DD)`。这样，在计算第 I 个节点的聚合要素表示时，不仅要考虑第 I 个节点的度，还要考虑第 j 个节点的度。这也被称为光谱规则。
...
`{H}_{updated}= Relu(1/sqrt(DD) * AHW)`

`Relu` 或任何其他非线性激活可以应用于此。这里 `W` 是大小为 (`K`x`K'`) 的可训练权重矩阵，其中 `K'` 是下一层的特征向量的维数。这基本上有助于通过随深度减小尺寸来解决过拟合问题。
...
![图 5-12: GCN 中的图层](img/502837_1_En_5_Fig12_HTML.jpg)

## 训练

对于像您这样的节点分类问题，训练主要包括以下步骤。
...
你形成了一个非常基本的 Keras 模型。您添加了两个大小为 8 和 4 的 GCN 图层。这两层也暗示着你要去的是每个节点的二级邻居。对于每个激活（节点嵌入），使用 `SELU` 激活函数来防止梯度消失问题。

你还引入了一个 Dropout，以防止过拟合。
...
![图 5-13: GCN 模式总结](img/502837_1_En_5_Fig13_HTML.jpg)
...
![图 5-14: 训练和验证性能曲线](img/502837_1_En_5_Fig14_HTML.jpg)
