...
医生(也称为提供者)可以安排成像或实验室测试来更好地诊断问题，开处方，并记录患者的依从性和处方结果。所有这些都存储在病人的病历中。图 [3-1](#Fig1) 显示了流水线的流向。前三个步骤展示了如何创建病历。
...
图 [3-7](#Fig7) 显示了一个普通的自编码器。每个自编码器都包含一个瓶颈层，它限制了输入的潜在表示的维数。
...
最后，请看图 [3-18](#Fig18) 中的性能和损耗图。一些聚类学习得非常好，如聚类 1 和 3，而聚类 2 中有轻微的过拟合，因为聚类 2 的损失图显示验证损失通常高于训练损失。
