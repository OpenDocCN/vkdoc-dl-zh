...
[`/mit-lcp.github.io/mimic-schema-spy/relationships.html`](https://mit-lcp.github.io/mimic-schema-spy/relationships.html)
...
5.  根据 HIPAA 合规性对数据进行去标识。还记得第章 1 吗？
    1.  日期被随机偏移。但是音程是保留的。
...
[`github.com/tensorflow/tensorflow/blob/r1.2/tensorflow/python/framework/ops.py%2523L2993`](https://github.com/tensorflow/tensorflow/blob/r1.2/tensorflow/python/framework/ops.py%2523L2993)
...
3.  注意卢瑟作为激活函数的使用。SELU 或缩放指数线性单位是一种相对较新的激活函数，具有许多优点，如权重和偏差的内部归一化，它将权重的平均值集中为零，并保证不会发生消失和爆炸梯度问题，这在直观上是有意义的，因为权重遵循标准正态分布。激活功能如图 3-8 所示。该图像改编自 Z. Huang 等人的论文“SNDCNN:用于语音识别的具有缩放指数线性单元的自归一化深度 CNN”。
