...
with three days of shortness of breath thought by her primary care doctor to be a COPD flare. Two days prior to admission,
she was started on a prednisone taper and one day prior to admission she required oxygen at home in order to maintain
    oxygen saturation greater than 90%. She has also been on levofloxacin and nebulizers, and was not getting better, and
...
```py
    'This is an 81-year-old female with a history of emphysema (not on home O2), who presents
with three days of shortness of breath thought by her primary care doctor to be a COPD flare. Two days prior to admission,
she was started on a prednisone taper and one day prior to admission she required oxygen at home in order to maintain
oxygen saturation greater than 90%. She has also been on levofloxacin and nebulizers, and was not getting better, and
...
| 注意名称 | 评分函数 |
| --- | --- |
| 加权平均 | $ \sum_{i=1}^{N} w_i e_i $ |
| 加权点积 | $ \sum_{i=1}^{N} w_i e_i \cdot h_i $ |
| 加权余弦 | $ \sum_{i=1}^{N} w_i e_i \cdot \cos(\theta_i) $ |
...
# Let's imagine these 20 most important concepts. See Figure 4-11.

![img/502837_1_En_4_Fig11_HTML.jpg](img/502837_1_En_4_Fig11_HTML.jpg)

Figure 4-11

Distribution of biomedical concepts in the BERT vocabulary

Wow, these words actually contain various biomedical concepts, such as diseases, body parts, organic chemicals (compounds), and pharmacological substances (used to treat pathological disorders). It looks like you have a model suitable for your task. All these concepts are also common in EHR notes.

Next, let's look at the subword and actual marked token lengths observed in your dataset.
...
Distribution of token lengths in the vocabulary

If you want to read the different words in the vocabulary carefully, you can access the following link:
...
BERT can be used in various ways for fine-tuning:

*   **Fine-tuning:** You add another set of layers on top of the last pre-trained layers of the BERT model and then train the entire model with a dataset specific to the task, although during this process you must ensure that the weights of the pre-trained model are not destroyed, so you freeze them for some periods and then restore them to full backpropagation of the BERT layers. This is also called warming up.

*   **Extracting weights from the last group of layers:** The extracted context embeddings are used as input for downstream tasks. They are fixed vectors and therefore not trainable. Four different types of methods are discussed in the original paper (Table 7).

    *   Weighted sum of 12 layers. The weight can be empirical.

    *   Using the last hidden layer.

    *   Extracting the second-to-last hidden layer (second-to-last).

    *   Connecting the last four hidden layers.

*   **Word embeddings:** Get word embeddings from the encoder layers of BERT. Wrappers exist in the Hugging Face变形库.

Fine-tuning is considered the best method to better control model performance, so you will adopt this method.

Since you will train multi-label classification, let's prepare the final dataset for it. You are making a real decision not to retain short sentences with only three or fewer labels.
...
You will also use the `full_data` variable to create training and validation sets. Your target will be a one-hot matrix, with each sample having a label for its belonging ICD-9 code, and the rest of the labels as zero.
...
You are finally ready to load the Huggingface transformers library and obtain the BERT model from DMIS Laboratory.
...
bert = TFBertModel.from_pretrained("./dmis-lab/biobert-large-cased-v1.1", from_pt = True)

```

The model shared by the DMIS team is a pytorch model and cannot be used directly for your task. You will use the wrapper functions provided in the transformers library to convert the pytorch model to a TensorFlow BERT model.

You must ensure that the parameter `from_pt = True` is passed, which indicates that you are trying to create a TFBertModel from Python pre-trained files.

Next, decide the model parameters you will use.
...
Ideally, you decide `MAX_LEN`. You can draw a histogram of sentence lengths in the corpus, but since the text is usually very long, you have already taken the maximum length based on the number of tokens.

Currently, the learning rate remains constant without warming up. The design parameters used, such as the choice of activation function, batch size, etc., are just empirical design choices, so you can explore and experiment with different design choices.

Just like in Chapter 3, you will create a generator function that generates input data in batch dimensions.
...
BERT takes a dictionary as input:

*   **Input ids** represent the index of the tokenized word according to the BERT model vocabulary.

*   **Token type ID** is also called segment ID. Because you are training a sequence classification problem, all token type ids are zero.

*   **Attention mask** is a 1/0 vector that tells us which word to pay attention to. Generally, all words are considered important, but this can be easily changed according to design decisions.

Please note that you will also pad the sentences to the possible maximum token length.
```
