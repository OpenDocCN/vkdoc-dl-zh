...
![图 1-1 医疗供应链](img/502837_1_En_1_Fig1_HTML.jpg)
...
![图 1-2 付款人的不同职能](img/502837_1_En_1_Fig2_HTML.jpg)
...
![图 1-3 2018 年美国医疗保健支出按支付者分布](img/502837_1_En_1_Fig3_HTML.jpg)
...
| `CINs` | 一个由松散关联的提供者机构（拥有不同的所有者）组成的网络，这些机构在同一社区内合作，以实现医疗保健的三重目标。尽管不属于同一母公司，但允许 `CIN` 与付款方签订集体合同。 |
| `ACOs` | 与付款方（商业或政府）签订合同的医院、医生和其他医疗保健提供商的网络。它协调对特定人群的护理。 |
...
*   定义受保护的健康信息（`PHI`）：2003 年，`HIPAA` 隐私规则定义了哪些健康信息应该受到保护。这包括付款、病史和付款人信息。
...
![图 1-4 Crunchbase 基本数据访问表单](img/502837_1_En_1_Fig4_HTML.jpg)
...
2.  填写表单，它看起来有点像图 1-4。
3.  在 `Crunchbase` 结束尽职调查后，您将在您注册的电子邮件地址收到一封电子邮件。这将是您的用户密钥。
...
![图 1-5 Crunchbase API 的 JSON 输出中的分页属性](img/502837_1_En_1_Fig5_HTML.jpg)

所以现在你有必要的数据从中得出初步的见解。让我们开始编码吧。

从图 1-6 中可以看出，这些解决方案以患者和医院为目标，主要关注可及性和应对慢性病。

![图 1-6：来自目标公司简短描述的词云](img/502837_1_En_1_Fig6_HTML.jpg)
...
plt.show()
```
图 1-7 讲的是同一个故事。你还可以找到一些临床数据和医疗设备以及公司名称的提及。

![图 1-7：Wordcloud 通过单词的 TF-IDF 得分进行加权](img/502837_1_En_1_Fig7_HTML.jpg)

图 1-7：Wordcloud 通过单词的 TF-IDF 得分进行加权

你当然可以扩展分析，摆弄数据，但我的想法是与你分享一种方法，从一个懒惰的角度来看待不断扩张的行业格局。

## 结论

你已经走了很长的路。我希望你现在对人工智能和医疗保健感兴趣。医疗保健，像任何其他系统一样，有其自身的缺陷和差距。在接下来的七个案例研究中，你将填补这些空白。但在此之前，你将学习如何设置你的系统并获取案例研究所需的数据。你还将非常简要地了解 `TensorFlow 2.0` 中最新和最棒的内容。