...
![ $$ Model=E\left[ Model/ Data\right]=\underset{Model}{\int } Model\ P\left( Model/ Data\right)d(Model) $$ ](img/A448418_1_En_5_Chapter_Equj.gif)

为了激发似然性和后验概率的概念，以及它们如何被用来导出模型参数，让我们再次回到硬币问题。

假设我们掷硬币六次，其中正面出现五次。如果要估计人头的概率，估计值会是多少？

这里，我们的模型是估计投掷硬币时正面 `θ` 的概率。每次投掷硬币都可以被视为一次独立的伯努利试验，正面的概率为 `θ`。给定模型，数据的可能性由
...
![ $$ P\left( Data/\theta \right)=P\left({x}_1{x}_2{x}_3{x}_4{x}_5{x}_6/\theta \right)=\prod \limits_{i=1}⁶P\left({x}_i/\theta \right) $$ ](img/A448418_1_En_5_Chapter_Equ1.gif)

(5.1.1)

每次掷骰子遵循伯努利分布，因此正面的概率为 `θ`，反面的概率为![ $$ \left(1-\theta \right) $$ ](img/A448418_1_En_5_Chapter_IEq7.gif)，通常其概率质量函数由
...
由于组合(1)和(2)有 5 个头和 1 个尾，所以作为 `θ` 的函数的似然 `L` 可以表示如下:

![ $$ L\left(\theta \right)={\theta}⁵\left(1-\theta \right) $$ ](img/A448418_1_En_5_Chapter_Equ3.gif)
...
如果我们对(5.1.3)中计算的似然性求导并将其设置为零，我们将得到 `θ`:

![ $$ \frac{d L\left(\theta \right)}{d\theta}=5{\theta}⁴-6{\theta}⁵=0\kern0.5em =>\theta =\frac{5}{6} $$ ](img/A448418_1_En_5_Chapter_Equm.gif)

的似然估计。

一般来说，如果有人问我们对 `θ` 的估计，而我们没有做类似的最大似然估计，我们会立刻根据我们在高中学到的概率的基本定义回答概率为![ $$ \frac{5}{6} $$ ](img/A448418_1_En_5_Chapter_IEq11.gif);即
...
这取决于我们对硬币的任何先验信念，即概率会不同。如果我们假设一个公平的硬币，这通常是最明显的假设，因为我们没有关于硬币的信息，![ $$ \theta =\frac{1}{2} $$ ](img/A448418_1_En_5_Chapter_IEq12.gif)将是一个很好的估计。然而，当我们假设而不是对 `θ` 进行先验点估计时，最好在 `θ` 上有一个概率分布，最大概率在![ $$ \theta =\frac{1}{2} $$ ](img/A448418_1_En_5_Chapter_IEq13.gif)。先验概率分布是模型参数 `θ` 的分布。

在这种情况下，带有参数![ $$ \alpha =2,\kern0.5em \beta =2 $$ ](img/A448418_1_En_5_Chapter_IEq14.gif)的贝塔分布将是一个很好的先验分布，因为它在![ $$ \theta =\frac{1}{2} $$ ](img/A448418_1_En_5_Chapter_IEq15.gif)具有最大概率，并且在它周围是对称的。![ $$ P\left(\theta \right)= Beta\left(\alpha =2,\beta =2\right)=\frac{\theta^{\alpha -1}{\left(1-\theta \right)}^{\beta -1}}{B\left(\alpha, \beta \right)}=\frac{\theta \left(1-\theta \right)}{B\left(\alpha, \beta \right)} $$ ](img/A448418_1_En_5_Chapter_Equo.gif)

对于 `α` 和 `β` 的固定值，`B(α, β)` 是常数，并且是该概率分布的归一化或配分函数。可以计算如下:
...
![ $$ P\left(\theta /D\right)\propto {\theta}⁵\left(1-\theta \right)\frac{\theta\ \left(1-\theta \right)}{6}=\kern1em \frac{\theta⁶{\left(1-\theta \right)}²}{6} $$ ](img/A448418_1_En_5_Chapter_Equq.gif)

由于我们忽略了数据的概率，所以出现了正比例符号。其实我们也可以把 6 拿出来，把后验表示成:

![ $$ P\left(\theta /D\right)\propto {\theta}⁶{\left(1-\theta \right)}² $$ ](img/A448418_1_En_5_Chapter_Equr.gif)

现在，![ $$ 0\le \theta \le 1 $$ ](img/A448418_1_En_5_Chapter_IEq17.gif)既然 `θ` 是一个概率。在 0 到 1 的范围内对![ $$ {\theta}⁶{\left(1-\theta \right)}² $$ ](img/A448418_1_En_5_Chapter_IEq18.gif)进行积分并取倒数，将得到后验概率的归一化因子，结果为 252。于是，后验可以表述为:

![ $$ P\left(\theta /D\right)=\frac{\theta⁶{\left(1-\theta \right)}²}{252} $$ ](img/A448418_1_En_5_Chapter_Equ5.gif)

(5.1.5)

现在我们有了后验概率，有两种方法可以估计 `θ`。我们可以最大化后验，得到 `θ` 的 MAP 估计如下:
...
现在，让我们来看看第二种方法，即纯贝叶斯方法，并取后验分布的平均值来平均 `θ`：

![后验分布平均值公式](A448418_1_En_5_Chapter_Equu.gif)

的所有不确定性

![图 5-1c：后验概率分布](img/A448418_1_En_5_Fig1c_HTML.gif)
图 5-1c：后验概率分布

![图 5-1b：先验概率分布](img/A448418_1_En_5_Fig1b_HTML.gif)
图 5-1b：先验概率分布

![图 5-1a：似然函数图](img/A448418_1_En_5_Fig1a_HTML.gif)
图 5-1a：似然函数图
...
在这种情况下，可以使用马尔可夫链蒙特卡罗抽样方法对模型参数进行抽样，然后它们的平均值就是后验分布平均值的合理估计。如果我们采样 `n` 组模型参数 `M_i`，那么

![后验分布平均值的近似公式](A448418_1_En_5_Chapter_Equv.gif)

通常，取分布的平均值，因为它使所有 `c` 的平方误差最小。

![c 等于 y 的期望](A448418_1_En_5_Chapter_IEq22.gif) 时，![期望 (y-c)²](A448418_1_En_5_Chapter_IEq21.gif) 最小化。假设我们试图用单个代表来表示分布的概率，以使概率分布的平方误差最小化，则平均值是最佳候选值。
...
![圆周率面积计算公式](img/A448418_1_En_5_Chapter_Equw.gif)

![图 5-2：圆周率面积计算示意图](img/A448418_1_En_5_Fig2_HTML.gif)
图 5-2：圆周率面积计算示意图
...
清单 5-1：通过蒙特卡罗抽样计算圆周率

输出结果为：`('The computed value of Pi:', 3.142)`
...
![图 5-3：封闭容器中气体在稳态下的运动，仅有三个状态：A、B 和 C](img/A448418_1_En_5_Fig3_HTML.gif)
图 5-3：封闭容器中气体在稳态下的运动，仅有三个状态：A、B 和 C

为了简单起见，让我们假设气体分子只有三种状态（在这种情况下是气体分子的位置），如图 5-3 所示。让我们用 `A`、`B` 和 `C` 来表示这些状态，用 `P_A`、`P_B`、`P_C` 来表示它们相应的概率。
...
![P(A)P(B/A)](img/A448418_1_En_5_Chapter_Equ6.gif)

`(5.2.1)` 给出

同样，从 `B` 到 `A` 的概率质量由

![P(B)P(A/B)](img/A448418_1_En_5_Chapter_Equ7.gif)

`(5.2.2)` 给出

所以，从 `(5.2.1)` 和 `(5.2.2)` 的稳态中，我们有

![P(A)P(B/A) = P(B)P(A/B)](img/A448418_1_En_5_Chapter_Equ8.gif)

`(5.2.3)` 来维持概率分布的平稳性。这叫做 `详细平衡条件`，是一个概率分布平稳性的充分但非必要条件。气体分子可以以更复杂的方式处于平衡，但是由于当可能的状态空间是无限的时，这种形式的细节平衡在数学上是方便的，所以这种方法已经被广泛用于马尔可夫链蒙特卡罗方法中，以基于当前点对下一个点进行采样，并且具有高的接受概率。简而言之，马尔可夫链的运动预计会像稳态下的气体分子一样，在高概率区域比在低概率区域花费更多的时间，从而保持详细的平衡条件不变。
...
`Metropolis` 算法是一种马尔可夫链蒙特卡罗方法，它使用当前接受的状态来确定下一个状态。时间 `t+1` 处的样本有条件地依赖于时间 `t` 处的样本。时间 `t+1` 处的建议状态来自正态分布，其平均值等于具有指定方差的时间 `t` 处的当前样本。一旦抽取，检查在时间 `t+1` 和时间 `t` 的样本之间的概率比。如果 `P(X^(t+1))/P(X^(t))` 大于或等于 1，则选择概率为 1 的样本 `X^(t+1)`；如果小于 1，则随机选择样本。接下来提到的是详细的实现步骤。

*   从任意一个随机采样点 `X^(1)` 开始。

*   选择条件依赖于 `X^(1)` 的下一个点 `X^(2)`。可以从一个均值为 `X^(1)` 和一些有限方差的正态分布中选择 `X^(2)`，假设方差为 `S^2`。因此，`X^(2) ~ Normal(X^(1), S^2)`。良好采样的一个关键决定因素是非常明智地选择方差 `S^2`。方差不应该太大，因为在这种情况下，下一个样本 `X^(2)` 停留在当前样本 `X^(1)` 附近的机会较小，在这种情况下，高概率区域可能不会被探索太多，因为下一个样本在大多数时间被选择为远离当前样本。同时，方差不应该太小。在这种情况下，下一个样本将几乎总是停留在当前点附近，因此探索远离当前区域的不同高概率区域的概率将降低。

*   一旦 `X^(2)` 已经从前面的步骤中生成，则使用一些特殊的试探法来确定是否接受它。

    *   如果比率 `P(X^(2))/P(X^(1)) >= 1` 则接受 `X^(2)` 并将其作为有效样本点。接受的样本成为生成下一个样本的 `X^(1)`。

    *   如果比率大于从 0 和 1 之间的均匀分布中随机产生的数字，则比率 `P(X^(2))/P(X^(1)) < 1` 时，`X^(2)` 被接受；即 `U[0, 1]`。

如我们所见，如果我们转向高概率样本，那么我们接受新样本，如果我们转向低概率样本，我们有时接受有时拒绝新样本。如果比率 `P(X^(2))/P(X^(1))` 很小，则拒绝的概率增加。比方说 `P(X^(2))/P(X^(1))` 的比值 = 0.1。当我们从一个均匀分布中产生一个介于 0 和 1 之间的随机数 `r_u` 时，那么 `r_u > 0.1` 的概率是 0.9，这又意味着新样本被拒绝的概率是 0.9。一般情况下，

![P(r_u > r) = 1 - r](img/A448418_1_En_5_Chapter_Equx.gif)

其中 `r` 是新样本和旧样本的概率之比。

让我们试着凭直觉理解为什么这种试探法适用于马尔可夫链蒙特卡罗方法。根据明细余额，

![P(X^(1))P(X^(2)/X^(1)) = P(X^(2))P(X^(1)/X^(2))](img/A448418_1_En_5_Chapter_Equy.gif)

我们假设转移概率服从正态分布。我们不检查我们采用的转移概率框架是否足以以我们希望坚持的详细平衡的形式保持概率分布的平稳性。让我们考虑保持分布平稳的两个状态 `X_1` 和 `X_2` 之间的理想转移概率由 `P(X_1/X_2)` 和 `P(X_2/X_1)` 给出。因此，根据明细平衡，必须满足以下条件:

![P(X_1/X_2)P(X_1) = P(X_2/X_1)P(X_2)](img/A448418_1_En_5_Chapter_Equz.gif)

然而，发现这样一个理想的转移概率函数，通过强加一个详细的平衡条件来确保平稳性是困难的。我们从合适的转移概率函数开始，假设为 `T(x/y)`，其中 `y` 表示当前状态，`x` 表示基于 `y` 采样的下一个状态。对于两个状态 `X_1` 和 `X_2`，假设的转移概率因此由从状态 `X_2` 到 `X_1` 的移动的 `T(X_1/X_2)` 和 `T(X_2/X_1)` 给出。由于假设的转移概率不同于通过详细平衡保持平稳性所需的理想转移概率，我们有机会根据下一步行动的好坏接受或拒绝样本。为了掩盖这个机会，考虑状态转移的接受概率，使得对于从 `X_1` 到 `X_2` 的状态转移，其中 `A(X_2/X_1)` 是从 `X_1` 到 `X_2` 的移动的接受概率。

根据明细平衡，

![P(X_1/X_2)P(X_1) = P(X_2/X_1)P(X_2)](img/A448418_1_En_5_Chapter_Equab.gif)

将理想转移概率替换为假设转移概率和接受概率的乘积，我们得到

![T(X_2/X_1)A(X_2/X_1)P(X_1) = T(X_1/X_2)A(X_1/X_2)P(X_2)](img/A448418_1_En_5_Chapter_Equac.gif)

重新排列这个，我们得到的接受概率比是

![A(X_2/X_1) / A(X_1/X_2) = (T(X_1/X_2)P(X_2)) / (T(X_2/X_1)P(X_1))](img/A448418_1_En_5_Chapter_Equad.gif)

满足这一点的一个简单建议由 `Metropolis` 算法给出为

![A(X_2/X_1) = min(1, (T(X_1/X_2)P(X_2)) / (T(X_2/X_1)P(X_1)))](img/A448418_1_En_5_Chapter_Equae.gif)

在 `Metropolis 算法` 中，假设的转移概率一般假设为对称的正态分布，因此 `T(X_1/X_2) = T(X_2/X_1)`。这简化了从 `X_1` 到 `X_2` 移动为

$$ A\left({X}_2/{X}_1\right)=\mathit{\min}\left(1,\frac{P\left({X}_2\right)}{P\left({X}_1\right)}\right) $$

的接受概率。

如果接受概率为 1，那么我们接受概率为 1 的移动，而如果接受概率小于 1，比如说 `r`，那么我们接受概率为 `r` 的新样本，拒绝概率为 `(1-r)` 的样本。以概率 `(1-r)` 拒绝样本是通过将比率与从 0 和 1 之间的均匀分布中随机生成的样本 `r_u` 进行比较，并在 `r_u > r` 的情况下拒绝样本来实现的。这是因为对于均匀分布概率 `P(r_u > r) = 1 - r`，这确保了保持期望的拒绝概率。

在 `清单 5-2` 中，我们展示了通过 `Metropolis 算法` 从二元高斯分布中进行抽样。

![使用 Metropolis 算法从多元高斯分布中采样的点图](img/A448418_1_En_5_Fig4_HTML.gif)

`图 5-4`：使用 `Metropolis 算法` 从多元高斯分布中采样的点图。
...
for i in range(num_samples):
    ...
    if (acceptance_ratio >= 1) or ((acceptance_ratio < 1) and (acceptance_ratio >= np.random.uniform(0, 1))):
        x_list.append(x2)
        y_list.append(y2)
        x1 = x2
        y1 = y2
...
`清单 5-2`：通过 `Metropolis 算法` 实现二元高斯分布

我们从输出中看到，采样点的均值和协方差非常接近于我们从中采样的二元高斯分布的均值和协方差。此外，`图 5-4` 中的散点图非常类似于二元高斯分布。

现在我们知道了从概率分布中采样的马尔可夫链蒙特卡罗方法，我们将在检查 `受限玻尔兹曼机` 时了解另一种称为 `吉布斯采样` 的 `MCMC` 方法。
...
![受限玻尔兹曼机可见层和隐藏层架构](img/A448418_1_En_5_Fig5_HTML.gif)

`图 5-5`：受限玻尔兹曼机可见层和隐藏层架构。

让 RBM 的可见单元用向量 `v={\left[{v}_1{v}_2\dots {v}_m\right]}^T\in {\mathrm{\mathbb{R}}}^{m\times 1}` 表示，隐藏单元用 `h={\left[{h}_1{h}_2\dots {h}_n\right]}^T\in {\mathrm{\mathbb{R}}}^{n\times 1}` 表示。同样，让连接第 `i` 个可见单元和第 `j` 个隐藏单元的权重用 `{w}_{ij},\forall i\in \left\{1,2,..m\right\},\forall j\in \left\{1,2,..n\right\}` 来表示。设包含权重 `w_{ij}` 的矩阵用 `W\in {\mathrm{\mathbb{R}}}^{m\times n}` 表示。

具有隐藏状态 `h` 和可见状态 `v` 的联合概率分布的能量由

$$ P\left(v,h\right)=\frac{e^{-E\left(h,v\right)}}{Z} \tag{5.3.1} $$

给出，其中 `E(v, h)` 是联合配置 `(v, h)` 的能量，`Z` 是归一化因子，通常称为配分函数。这个概率基于玻尔兹曼分布，并假设玻尔兹曼常数和热温度为 1。

$$ Z=\sum \limits_v\sum \limits_h{e}^{-E\left(v,h\right)} \tag{5.3.2} $$

联合构型 `(v, h)` 的能量 `E(v, h)` 由

$$ E\left(v,h\right)=-{b}^Tv-{c}^Th-{v}^T Wh \tag{5.3.3} $$

$$ E\left(v,h\right)=-\sum \limits_{i=1}^m{b}_i{v}_i-\sum \limits_{j=1}^n{c}_j{h}_j-\sum \limits_{j=1}^n\sum \limits_{i=1}^m{v}_i{w}_{ij}{h}_j \tag{5.3.4} $$

给出。

向量 `b\in {\left[{b}_1{b}_2\kern0.75em ..{b}_m\right]}^T\in {\mathrm{\mathbb{R}}}^{m\times 1}` 和 `c\in {\left[{c}_1{c}_2\kern0.75em ..{c}_n\right]}^T\in {\mathrm{\mathbb{R}}}^{n\times 1}` 分别是可见单元和隐藏单元的偏差，我们将在后面看到。

在任何图形概率模型中，其思想是计算各种事件集合的联合概率分布。结合 `(5.3.1)` 和 `(5.3.3)`，我们得到

$$ P\left(v,h\right)=\frac{e^{b^Tv+{c}^Th+{v}^T Wh}}{Z} \tag{5.3.5} $$
...
$$ {\displaystyle \begin{array}{l}P\left(h/v\right)=\frac{P\left(v,h\right)}{P(v)}\\ {}\kern3.5em =\frac{P\left(v,h\right)}{\sum_hP\left(v,h\right)}\\ {}\kern3.5em =\frac{\frac{e^{b^Tv+{c}^Th+{v}^T Wh}}{Z}}{\sum_h\frac{e^{b^Tv+{c}^Th+{v}^T Wh}}{Z}}\\ {}\kern3.5em =\frac{e^{b^Tv+{c}^Th+{v}^T Wh}}{\sum_h{e}^{b^Tv+{c}^Th+{v}^T Wh}}\\ {}\kern3.5em =\frac{e^{b^Tv}{e}^{c^Th}{e}^{v^T Wh}}{e^{b^Tv}{\sum}_h{e}^{c^Th}{e}^{v^T Wh}}\\ {}\kern3.5em =\frac{e^{c^Th}{e}^{v^T Wh}}{\sum_h{e}^{c^Th}{e}^{v^T Wh}}\end{array}} $$

我们可以根据所涉及的不同向量的分量展开分子和分母，如下:

$$ {\displaystyle \begin{array}{l}{e}^{c^Th}{e}^{v^T Wh}={e}^{c^Th+{v}^T Wh}\\ {}\kern3.5em ={e}^{\sum \limits_{j=1}^n\left({c}_j{h}_j+{v}^TW\left[:,j\right]{h}_j\right)}\end{array}} $$

由于和的指数等于指数的乘积，前面的等式可以写成乘积形式如下:

$$ {e}^{c^Th}{e}^{v^T Wh}=\prod \limits_{j=1}^n{e}^{c_j{h}_j+{v}^TW\left[:,j\right]{h}_j} \tag{5.3.6} $$

现在，让我们看看分母，它看起来类似于分子，只是具有所有可能的隐藏状态 `h` 的总和。使用 `(5.3.6)` 中的表达式 `{e}^{c^Th}{e}^{v^T Wh}`，分母可以表示为

## RBM 的条件独立性与训练

矢量和是指其分量的所有组合的和。每个隐藏单元 `h_i` 可以有二进制状态 0 或 1，因此有 `h_j \in \{0, 1\} \forall j \in \{1, 2, 3, .., n\}`。因此，(5.3.7) 中对向量 `h` 的求和可以展开成对应于其每个分量的多个求和：

$$
\sum_h e^{c^T h} e^{v^T W h} = \sum_{h_1=0}^1 \sum_{h_2=0}^1 .. \sum_{h_n=0}^1 \prod_{j=1}^n e^{c_j h_j + v^T W[:, j] h_j}
$$

$$
= \sum_{h_1=0}^1 \sum_{h_2=0}^1 .. \sum_{h_n=0}^1 \left( e^{c_1 h_1 + v^T W[:, 1] h_1} \right) \left( e^{c_2 h_2 + v^T W[:, 2] h_2} \right) \dots \left( e^{c_n h_n + v^T W[:, n] h_n} \right)
$$
...
$$
\sum_{j=1}^2 \sum_{i=1}^3 a_i b_j = a_1 b_1 + a_2 b_1 + a_3 b_1 + a_1 b_2 + a_2 b_2 + a_3 b_2
$$

$$
= b_1 (a_1 + a_2 + a_3) + b_2 (a_1 + a_2 + a_3)
$$
...
$$
= \left( \sum_{i=1}^3 a_i \right) \left( \sum_{j=1}^2 b_j \right)
$$

所以，我们看到，当我们取独立指数的变量元素时，变量乘积的和可以表示为变量和的乘积。类似于这个例子，`h`（即 `h_i`）的元素一般独立地包含在乘积 `\left( e^{c_1 h_1 + v^T W[:, 1] h_1} \right) \left( e^{c_2 h_2 + v^T W[:, 2] h_2} \right) \dots \left( e^{c_n h_n + v^T W[:, n] h_n} \right)` 中，因此 (5.3.8) 中的表达式可以简化为：

$$
\sum_h e^{c^T h} e^{v^T W h} = \sum_{h_1=0}^1 \left( e^{c_1 h_1 + v^T W[:, 1] h_1} \right) \sum_{h_2=0}^1 \left( e^{c_2 h_2 + v^T W[:, 2] h_2} \right) .. \sum_{h_n=0}^1 \left( e^{c_n h_n + v^T W[:, n] h_n} \right)
$$

$$
\sum_h e^{c^T h} e^{v^T W h} = \prod_{j=1}^n \sum_{h_j=0}^1 \left( e^{c_j h_j + v^T W[:, j] h_j} \right)
$$
...
$$
P(h/v) = \frac{\prod_{j=1}^n e^{c_j h_j + v^T W[:, j] h_j}}{\prod_{j=1}^n \sum_{h_j=0}^1 e^{c_j h_j + v^T W[:, j] h_j}}
$$

简化这两边 `h` 的分量，我们得到

$$
P(h_1 h_2 .. h_n / v) = \prod_{j=1}^n \left( \frac{e^{c_j h_j + v^T W[:, j] h_j}}{\sum_{h_j=0}^1 e^{c_j h_j + v^T W[:, j] h_j}} \right)
$$

$$
= \left( \frac{e^{c_1 h_1 + v^T W[:, 1] h_1}}{\sum_{h_1=0}^1 e^{c_1 h_1 + v^T W[:, 1] h_1}} \right) \left( \frac{e^{c_2 h_2 + v^T W[:, 2] h_2}}{\sum_{h_2=0}^1 e^{c_2 h_2 + v^T W[:, 2] h_2}} \right) \dots \left( \frac{e^{c_n h_n + v^T W[:, n] h_n}}{\sum_{h_n=0}^1 e^{c_n h_n + v^T W[:, n] h_n}} \right)
$$

(5.3.10)

以 `v` 为条件的 `h` 的元素的联合概率分布已经被分解成以 `v` 为条件的彼此独立的表达式的乘积。这导致这样的事实：给定 `v`，`h` 的分量（即，`h_i \forall i \in \{1, 2, \dots, n\}`）是彼此有条件独立的。这给出了我们

$$
P(h_1 h_2 .. h_n / v) = P(h_1 / v) P(h_2 / v) .. P(h_n / v)
$$
...
替换 (5.3.12) 中的 `h_j = 1` 和 `h_j = 0`，我们得到
...
(5.3.13) 和 (5.3.14) 的表达式说明了隐藏单元 `h_i \forall i \in \{1, 2, \dots, n\}` 是独立的 sigmoid 单元：

$$
P(h_j = 1 / v) = \sigma \left( c_j + v^T W[:, j] \right)
$$
...
其中 `\sigma(\cdot)` 表示 sigmoid 函数，使得
...
$$
P(v_1 v_2 .. v_m / h) = P(v_1 / h) P(v_2 / h) .. P(v_m / h)
$$
...
$$
L(\theta) = P(v^{(1)} v^{(2)} .. v^{(m)} / \theta)
$$

给出。由于输入的数据点是独立给定的模型，

$$
L(\theta) = P(v^{(1)} / \theta) P(v^{(2)} / \theta) .. P(v^{(m)} / \theta) = \prod_{t=1}^m P(v^{(t)} / \theta)
$$
...
现在，让我们取成本函数相对于组合参数 `\theta` 的梯度。我们可以认为 `C` 由两部分组成，`\rho^+` 和 `\rho^-`，其中

$$
\rho^+ = \sum_{t=1}^m \sum_h e^{-E(v^{(t)}, h)}
$$

$$
\rho^- = m \log \sum_v \sum_h e^{-E(v, h)}
$$

取 `\rho^+` 相对于 `\theta` 的梯度，我们得到
...
现在我们把
$$
\frac{\sum_h{e}^{-E\left({v}^{(t)},h\right)}{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)}{\sum_h{e}^{-E\left({v}^{(t)},h\right)}}
$$
简化为分子和分母都除以 $Z$:
$$
{\nabla}_{\theta}\left({\rho}^{+}\right)=\sum \limits_{t=1}^m\frac{\sum_h\frac{e^{-E\left({v}^{(t)},h\right)}}{Z}{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)}{\frac{\sum_h{e}^{-E\left({v}^{(t)},h\right)}}{Z}} \tag{5.3.23}
$$
$$
\frac{e^{-E\left({v}^{(t)},h\right)}}{Z}=P\left({v}^{(t)},h/\theta \right)
$$
$$
\frac{\sum_h{e}^{-E\left({v}^{(t)},h\right)}}{Z}=P\left({v}^{(t)}/\theta \right)
$$
。使用(5.3.23)中概率的这些表达式，我们得到
$$
{\displaystyle \begin{array}{l}{\nabla}_{\theta}\left({\rho}^{+}\right)=\sum \limits_{t=1}^m\frac{\sum_hP\left({v}^{(t)},h/\theta \right){\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)}{P\left({v}^{(t)}/\theta \right)}\\ {}\kern5.5em =\sum \limits_{t=1}^m\sum \limits_h\frac{P\left({v}^{(t)},h/\theta \right)}{P\left({v}^{(t)}/\theta \right)}{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)\\ {}\kern5.5em =\sum \limits_{t=1}^m\sum \limits_hP\left(h/{v}^{(t)},\theta \right){\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)\ \end{array}} \tag{5.3.24}
$$
如果愿意，可以从概率符号中删除 $\theta$，例如 $P(v^{(t)}, h/\theta)$、$P(v^{(t)}, h/\theta)$ 等等，以便于符号化，但是最好保留它们，因为它使得推导更加完整，这允许整个训练过程的更好的可解释性。

让我们看看函数的期望，它以一种更有意义的形式给出了(5.3.24)式中的表达式，这种形式对于训练是理想的。$f(x)$ 的期望，给定 $x$，遵循 `probability mass function` $P(x)$ 并由
$$
E\left[f(x)\right]=\sum \limits_xP(x)f(x)
$$
给出

如果 $x={\left[{x}_1{x}_2..{x}_n\right]}^T\in {\mathrm{\mathbb{R}}}^{n\times 1}$ 是多变量的，那么前面的表达式成立，并且
$$
E\left[f(x)\right]=\sum \limits_xP(x)f(x)=\sum \limits_{x_1}\sum \limits_{x_2}..\sum \limits_{x_n}P\left({x}_1,{x}_2,..,{x}_n\right)f\left({x}_1,{x}_2,..,{x}_n\right)
$$
类似地，如果 $f(x)$ 是函数的向量，例如 $f(x)={\left[{f}_1(x)\kern0.5em {f}_2(x)\right]}^T$，可以使用与期望相同的表达式。在这里，人们会得到一个期望向量，如下:
$$
E\left[f(x)\right]=\sum \limits_xP(x)f(x)=\left[\begin{array}{c}\sum \limits_{x_1}\sum \limits_{x_2}..\sum \limits_{x_n}P\left({x}_1,{x}_2,..,{x}_n\right){f}_1\left({x}_1,{x}_2,..,{x}_n\right)\\ {}\sum \limits_{x_1}\sum \limits_{x_2}..\sum \limits_{x_n}P\left({x}_1,{x}_2,..,{x}_n\right){f}_2\left({x}_1,{x}_2,..,{x}_n\right)\end{array}\right] \tag{5.3.25}
$$
为了在期望符号中明确提及概率分布，可以将变量 $x$ 遵循概率分布 $P(x)$ 的函数的期望或函数的向量的期望重写如下:
$$
{E}_{P(x)}\left[f(x)\right]=\sum \limits_xP(x)f(x)
$$
由于我们正在处理 `gradient`，`gradient` 是不同 `partial derivative` 的向量，并且对于给定的 $\theta$ 和 $v$ 值，每个 `partial derivative` 都是 $h$ 的函数，因此(5.3.24)中的表达式可以表示为关于概率分布 $P(h/v^{(t)}, \theta)$ 的 `gradient` 期望值 ${\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)$ 为
$$
{\nabla}_{\theta}\left({\rho}^{+}\right)=\sum \limits_{t=1}^m{E}_{P\left(h/{v}^{(t)},\theta \right)}\left[{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)\right] \tag{5.3.26}
$$
注意期望 ${E}_{P\left(h/{v}^{(t)},\theta \right)}\left[{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)\right]$ 是一个期望向量，如(5.3.25)中所示。

现在，让我们得到 ${\rho}^{-}=m\log \sum \limits_v\sum \limits_h{e}^{-E\left(v,h\right)}$ 相对于 $\theta$:
$$
{\displaystyle \begin{array}{l}{\nabla}_{\theta}\left({\rho}^{-}\right)=m\frac{\sum_v{\sum}_h{e}^{-E\left(v,h\right)}{\nabla}_{\theta}\left(-E\left(v,h\right)\right)}{\sum_v{\sum}_h{e}^{-E\left(v,h\right)}}\\ {}\kern5.5em =m\frac{\sum_v{\sum}_h{e}^{-E\left(v,h\right)}{\nabla}_{\theta}\left(-E\left(v,h\right)\right)}{Z}\\ {}\kern5.5em =m\sum \limits_v\sum \limits_h\frac{e^{-E\left(v,h\right)}}{Z}{\nabla}_{\theta}\left(-E\left(v,h\right)\right)\\ {}\kern5.5em =m\sum \limits_v\sum \limits_hP\left(v,h/\theta \right){\nabla}_{\theta}\left(-E\left(v,h\right)\right)\\ {}\kern5.5em =m{E}_{P\left(h,v/\theta \right)}\left[{\nabla}_{\theta}\left(-E\left(v,h\right)\right)\right]\end{array}} \tag{5.3.27}
$$
(5.3.27)中的期望在 $h$ 和 $v$ 的 `joint probability distribution` 上，而(5.3.26)中的期望在给定一个已知 $v$ 的 $h$ 上。结合(5.3.26)和(5.3.27)，我们得到
$$
{\nabla}_{\theta }(C)=\sum \limits_{t=1}^m{E}_{P\left(h/{v}^{(t)},\theta \right)}\left[{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)\right]-m{E}_{P\left(h,v/\theta \right)}\left[{\nabla}_{\theta}\left(-E\left(v,h\right)\right)\right] \tag{5.3.28}
$$
如果我们观察(5.3.28)式中所有参数的 `gradient`，它有两项。第一项取决于看到的数据 $v^{(t)}$，而第二项取决于来自模型的样本。第一项增加了给定观察数据的可能性，而第二项减少了来自模型的数据点的可能性。

现在，让我们对 $\theta$ 中每个参数集的 `gradient` 做一些简化；即 $b$、$c$、$w$。
$$
{\nabla}_b\left(-E\left(v,h\right)\right)={\nabla}_b\left({b}^Tv+{c}^Th+{v}^T Wh\right)=v \tag{5.3.29}
$$
$$
{\nabla}_c\left(-E\left(v,h\right)\right)={\nabla}_c\left({b}^Tv+{c}^Th+{v}^T Wh\right)=h \tag{5.3.30}
$$
$$
{\nabla}_W\left(-E\left(v,h\right)\right)={\nabla}_W\left({b}^Tv+{c}^Th+{v}^T Wh\right)=v{h}^T \tag{5.3.31}
$$
使用(5.3.28)至(5.3.31)，每个参数集的 `gradient` 表达式由
$$
{\nabla}_b(C)=\sum \limits_{t=1}^m{E}_{P\left(h/{v}^{(t)},\theta \right)}\left[{v}^{(t)}\right]-m{E}_{P\left(h,v/\theta \right)}\left[v\right] \tag{5.3.32}
$$
给出

由于第一项的概率分布是以 $v^{(t)}$ 为条件的，$v^{(t)}$ 相对于 $P(h/v^{(t)}, \theta)$ 的期望值就是 $v^{(t)}$。
$$
{\nabla}_b(C)=\sum \limits_{t=1}^m{v}^{(t)}-m{E}_{P\left(h,v/\theta \right)}\left[v\right] \tag{5.3.33}
$$
$$
{\nabla}_c(C)=\sum \limits_{t=1}^m{E}_{P\left(h/{v}^{(t)},\theta \right)}\left[h\right]-m{E}_{P\left(h,v/\theta \right)}\left[h\right] \tag{5.3.34}
$$
由于给定 $v^{(t)}$ 时 $h$（即 $h_j$）的每个单位都是独立的，因此可以容易地计算出 $h$ 在概率分布 $P(h/v^{(t)}, \theta)$ 上的期望值。他们中的每一个都是具有两种可能结果的 `sigmoid unit`，他们的期望只是 `sigmoid unit` 的输出；即
$$
{E}_{P\left(h/{v}^{(t)},\theta \right)}\left[h\right]={\hat{h}}^{(t)}=\sigma \left(c+{W}^T{v}^{(t)}\right)
$$
如果我们用 $\hat{h}$ 代替期望，那么(5.3.34)中的表达式可以写成
$$
{\nabla}_c(C)=\sum \limits_{t=1}^m{\widehat{h}}^{(t)}-m{E}_{P\left(h,v/\theta \right)}\left[h\right] \tag{5.3.35}
$$
同样，
$$
\begin{array}{l}{\nabla}_W\;(C)={\displaystyle \sum_{t=1}^m{E}_{P\left(h/{v}^{(t)},\;\theta \right)}}\left[{v}^{(t)}{h}^T\right]-m{E}_{P\left(h,\;v/\theta \right)}\left[h\right]\\ {}\kern1.92em ={\displaystyle \sum_{t=1}^m{v}^{(t)}{\widehat{h}}^{(t)T}}-m{E}_{P\left(h,\;v/\theta \right)}\left[h\right]\end{array} \tag{5.3.36}
$$
因此，(5.3.33)、(5.3.35)和(5.3.36)中的表达式代表关于三个参数集的 `gradient`。为便于参考:
$$
\left\{\begin{array}{l}{\nabla}_b(C)=\sum \limits_{t=1}^m{v}^{(t)}-m{E}_{P\left(h,v/\theta \right)}\left[v\right]\\ {}{\nabla}_c(C)=\sum \limits_{t=1}^m{\widehat{h}}^{(t)}-m{E}_{P\left(h,v/\theta \right)}\left[h\right]\\ {}{\nabla}_W(C)=\sum \limits_{t=1}^m{v}^{(t)}{\widehat{h}}^{(t)^T}-m{E}_{P\left(h,v/\theta \right)}\left[h\right]\end{array}\right. \tag{5.3.37}
$$
基于这些 `gradient`，可以调用 `gradient descent` 技术来迭代地获得最大化 `likelihood function` 的参数值。然而，在 `gradient descent` 的每次迭代中，计算关于 `joint probability distribution` $P(h, v/\theta)$ 的期望值有点复杂。`joint probability distribution` 很难计算，因为在 $h$ 和 $v$ 是中等至大维度向量的情况下，它们的组合数量似乎很大。`Markov chain Monte Carlo` 抽样(`MCMC`)技术，尤其是 `Gibbs sampling`，可用于从 `joint probability distribution` 中抽样，并计算(5.3.37)中不同参数集的期望值。然而，`MCMC` 技术需要很长时间才能收敛到 `stationary distribution`，之后它们提供了良好的样本。因此，在 `gradient descent` 的每次迭代中调用 `MCMC` 采样将使学习非常缓慢和不切实际。

### `Gibbs sampling`

`Gibbs sampling` 是一种 `Markov chain Monte Carlo` 方法，可用于从多变量概率分布中对观测值进行抽样。假设我们想从一个多元 `joint probability distribution` $P(x)$ 中取样，其中 $x={\left[{x}_1{x}_2..{x}_n\right]}^T$。

`Gibbs sampling` 根据其他变量的所有当前值生成变量 $x_i$ 的下一个值。设抽取的第 $t$ 个样本用 ${x}^{(t)}={\left[{x_1}^{(t)}{x_2}^{(t)}..{x_n}^{(t)}\right]}^T$ 表示。要生成接下来看到的 $(t+1)$ 样本，请遵循以下逻辑:

## 吉布斯采样基本原理

通过从以其余变量为条件的概率分布中采样来绘制变量 `x_j^{(t+1)}`。换句话说，从以下分布中采样 `x_j^{(t+1)}`：
...
因此，基本上，对于在给定其余变量条件下采样 `x_j`，对于 `x_j` 之前的 `j-1` 个变量，考虑它们在 `(t+1)` 实例的值，因为它们已经被采样；而对于其余变量，考虑它们在实例 `t` 的值，因为它们尚未被采样。此步骤对所有变量重复进行。如果每个 `x_j` 是离散的，并且可以取，比如说，两个值 0 和 1，那么我们需要计算概率 `p_1 = P(x_j^{(t+1)}=1 | x_1^{(t+1)}x_2^{(t+1)}..x_{j-1}^{(t+1)}x_{j+1}^{(t)}..x_n^{(t)})`。然后我们可以从 0 和 1 之间的均匀概率分布（即 `U[0, 1]`）中抽取一个样本 `u`，如果 `p_1 >= u`，则设置 `x_j^{(t+1)}=1`，否则设置 `x_j^{(t+1)}=0`。这种随机启发式方法确保概率 `p_1` 越高，`x_j^{(t+1)}` 被选为 1 的可能性就越大。然而，它仍然为 0 被选中留下了很小的空间，即使 `p_1` 相对较大，从而确保马尔可夫链不会陷入局部区域，并且也可以探索其他潜在的高密度区域。这些与我们在 Metropolis 算法中看到的启发式方法相同。
...
例如，如果需要从具有均值 `[0 0]` 和协方差矩阵 `[[1, ρ], [ρ, 1]]` 的二元正态分布进行吉布斯采样，那么条件概率分布可以计算如下：
...
如果将边际分布 `P(x_1)` 和 `P(x_2)` 导出为 `∫ P(x_1, x_2) dx_2` 和 `∫ P(x_1, x_2) dx_1`，则

![ $$ {x}_2/{x}_1\sim Normal\left(\rho {x}_1,1-{p}²\right) $$ ](img/A448418_1_En_5_Chapter_Equba.gif)

![ $$ {x}_1/{x}_2\sim Normal\left(\rho {x}_2,1-{p}²\right) $$ ](img/A448418_1_En_5_Chapter_Equbb.gif)

## 块状吉布斯采样

吉布斯抽样有几种变体。块状吉布斯采样就是其中之一。在块吉布斯采样中，一个以上的变量被组合在一起，然后在其余变量的条件下对该组变量一起进行采样，这与单独对单个变量进行采样相反。例如，在受限玻尔兹曼机中，隐藏的单元状态变量 `h = [h_1, h_2, ..., h_n]^T ∈ R^{n×1}` 可以在可见单元状态 `v = [v_1, v_2, ..., v_m]^T ∈ R^{m×1}` 的条件下一起采样，反之亦然。因此，对于从 `P(v, h)` 上的联合概率分布采样，通过块吉布斯采样，可以通过条件分布 `P(h|v)` 采样给定可见单元状态的所有隐藏状态，并且通过条件分布 `P(v|h)` 采样给定隐藏单元状态的所有可见单元状态。吉布斯采样的 `(t+1)` 迭代的样本可以生成为

![ $$ {h}^{\left(t+1\right)}\sim P\left(h/{v}^{(t)}\right) $$ ](img/A448418_1_En_5_Chapter_Equbc.gif)

![ $$ {v}^{\left(t+1\right)}\sim P\left(v/{h}^{\left(t+1\right)}\right) $$ ](img/A448418_1_En_5_Chapter_Equbd.gif)

因此，`(v^{(t+1)}, h^{(t+1)})` 是迭代 `(t+1)` 时的组合样本。

如果一定要计算一个函数 `f(h, v)` 的期望，可以计算如下：

![ $$ E\left[f\left(h,v\right)\right]\approx \frac{1}{M}\sum \limits_{t=1}^Mf\left({h}^{(t)},{v}^{(t)}\right) $$ ](img/A448418_1_En_5_Chapter_Eqube.gif)
...
![ $$ \left\{\begin{array}{c}{\nabla}_b(C)=\sum \limits_{t=1}^m{v}^{(t)}-m{E}_{P\left(h,v/\theta \right)}\left[v\right]\\ {}{\nabla}_c(C)=\sum \limits_{t=1}^m\hat{h}-m{E}_{P\left(h,v/\theta \right)}\left[h\right]\\ {}\kern1.25em {\nabla}_W(C)=\sum \limits_{t=1}^m{v}^{(t)}{\hat{h}}^T-m{E}_{P\left(h,v/\theta \right)}\left[v{h}^T\right]\end{array}\right. $$ ](img/A448418_1_En_5_Chapter_Equbf.gif)

期望 `E_{P(h, v|θ)}[v]`，`E_{P(h, v|θ)}[h]`，`E_{P(h, v|θ)}[vh^T]` 都需要从联合概率分布 `P(v, h|θ)` 中抽样。通过块吉布斯采样，样本 `(v, h)` 可以基于它们的条件概率被绘制如下，其中 `t` 表示吉布斯采样的迭代次数：
...
让采样更容易的是，隐藏单元的 `h_j` 在给定可见单元状态的情况下是独立的，反之亦然：
...
这允许在给定可见单元状态的值的情况下，单独的隐藏单元 `h_j` 被独立地并行采样。参数 `θ` 已从前面的符号中删除，因为当我们执行吉布斯采样时，`θ` 对于梯度下降的步长将保持不变。

现在，每个隐藏单元输出状态 `h_j` 可以是 0 或 1，其呈现状态 1 的概率由 (5.3.16) 给出为

![ $$ P\left({h}_j=1/v\right)=\sigma \left({c}_j+\sum \limits_{i=1}^m{v}_i{w}_{ij}\right) $$ ](img/A448418_1_En_5_Chapter_Equbj.gif)

该概率可以基于当前值 `v = v^{(t)}` 和模型参数 `c, W ∈ θ` 来计算。将计算的概率 `P(h_j=1 | v^{(t)})` 与从均匀分布 `U[0, 1]` 生成的随机样本 `u` 进行比较。如果 `P(h_j=1 | v^{(t)}) > u`，则采样 `h_j = 1`，否则 `h_j = 0`。以这种方式采样的所有这种 `h_j` 形成组合的隐藏单元状态向量 `h^{(t+1)}`。
...
给定 `h^{(t+1)}`，可以对每个可见单元进行独立采样，以与隐藏单元相同的方式获得组合 `v^{(t+1)}`。在 `(t+1)` 迭代中生成的所需样本由 `(v^{(t+1)}, h^{(t+1)})` 给出。

所有的期望 `E_{P(h, v|θ)}[v]`，`E_{P(h, v|θ)}[h]`，`E_{P(h, v|θ)}[vh^T]` 都可以通过取 Gibbs 抽样产生的样本的平均值来计算。通过 Gibbs 采样，如果我们在考虑老化周期和自相关之后取 `N` 个样本，如前所述，所需的期望值可以计算如下：

![ $$ {E}_{P\left(h,v/\theta \right)}\left[v\right]\approx \frac{1}{N}\sum \limits_{i=1}^N{v}^{(i)} $$ ](img/A448418_1_En_5_Chapter_Equbl.gif)

![ $$ {E}_{P\left(h,v/\theta \right)}\left[h\right]\approx \frac{1}{N}\sum \limits_{i=1}^N{h}^{(i)} $$ ](img/A448418_1_En_5_Chapter_Equbm.gif)

![ $$ {E}_{P\left(h,v/\theta \right)}\left[v{h}^T\right]\approx \frac{1}{N}\sum \limits_{i=1}^N{v}^{(i)}{h^{(i)}}^T $$ ](img/A448418_1_En_5_Chapter_Equbn.gif)

然而，对联合分布进行吉布斯采样以在梯度下降的每次迭代中生成 `N` 个样本成为一项繁琐的任务，并且通常是不切实际的。还有一种接近这些期望的方法，叫做对比差异，我们将在下一节讨论。

## 对比分歧

在梯度下降的每一步对联合概率分布 `P(h, v/θ)` 执行吉布斯采样变得具有挑战性，因为诸如吉布斯采样的马尔可夫链蒙特卡罗方法需要很长时间来收敛，这是产生无偏样本所需要的。这些从联合概率分布中抽取的无偏样本用于计算期望项 `E_{P(h, v/θ)}[v]`、`E_{P(h, v/θ)}[h]` 和 `E_{P(h, v/θ)}[VH^T]`，它们只不过是(5.3.28)中推导出的梯度组合表达式中的项 ![ $$ {E}_{P\left(h,v/\theta \right)}\left[{\nabla}_{\theta}\left(-E\left(v,h\right)\right)\right] $$ ](A448418_1_En_5_Chapter_IEq90.gif) 的组成部分。
...
对比散度通过对候选样本 ![ $$ \left(\overline{v},\overline{h}\right) $$ ](img/A448418_1_En_5_Chapter_IEq92.gif) 进行点估计来近似总体期望 ![ $$ {E}_{P\left(h,v/\theta \right)}\left[{\nabla}_{\theta}\left(-E\left(v,h\right)\right)\right] $$ ](A448418_1_En_5_Chapter_IEq91.gif)，该候选样本通过仅执行几次迭代的吉布斯采样来获得。![ $$ {E}_{P\left(h,v/\theta \right)}\left[{\nabla}_{\theta}\left(-E\left(v,h\right)\right)\right]\kern0.5em \approx {\nabla}_{\theta}\left(-E\left(\overline{v},\overline{h}\right)\right) $$ ](A448418_1_En_5_Chapter_Equbq.gif)
...
下图 5-6 说明了如何对每个输入数据点 `v^{(t)}` 进行吉布斯采样，以通过点估计获得联合概率分布的期望近似值。吉布斯采样从 `v^{(t)}` 开始，基于条件概率分布 `P(h/v^{(t)})`，获得新的隐藏状态 `h'`。如前所述，每个隐藏单元 `h_j` 可以被独立采样，然后被组合以形成隐藏状态向量 `h'`。然后基于条件概率分布 `P(v/h')` 对 `v'` 进行采样。该迭代过程通常运行几次迭代，最终采样的 `v` 和 `h` 作为候选样本 ![ $$ \left(\overline{v},\overline{h}\right) $$ ](img/A448418_1_En_5_Chapter_IEq93.gif)。

![A448418_1_En_5_Fig6_HTML.gif](img/A448418_1_En_5_Fig6_HTML.gif)
图 5-6. Gibbs sampling for two iterations to get one sample for Contrastive Divergence
...
图 5-8. Simulated images given the hidden states

![A448418_1_En_5_Fig7_HTML.jpg](img/A448418_1_En_5_Fig7_HTML.jpg)
图 5-7. Actual test images
...
图 5-9. Schematic diagram of a matrix factorization method for collaborative filtering
...
![A448418_1_En_5_Fig11_HTML.gif](img/A448418_1_En_5_Fig11_HTML.gif) 图 5-11。Restricted Boltzmann view for User B

![A448418_1_En_5_Fig10_HTML.gif](img/A448418_1_En_5_Fig10_HTML.gif) 图 5-10。Restricted Boltzmann view for User A
...
![ $$ P\left({v_i}^{(k)}=1/h\right)=\frac{e^{\left({b_i}^{(k)}+{\sum}_{j=1}^n{h}_j{w_{ij}}^{(k)}\right)}}{\sum_{l=1}^K{e}^{\left({b_i}^{(l)}+{\sum}_{j=1}^n{h}_j{w_{ij}}^{(l)}\right)}} $$ ](img/A448418_1_En_5_Chapter_Equ46.gif) (5.4.1) 其中 `w_{ij}^{(k)}` 是连接可见单元 `i` 的第 `k` 个等级指数和第 `j` 个隐藏单元的权重，`b_i^{(k)}` 表示可见单元 `i` 的第 `k` 个等级的偏差。

一个联合构形的能量 `E(v, h)` 由

![ $$ E\left(v,h\right)=-\sum \limits_{k=1}^K\sum \limits_{i=1}^m{b_i}^{(k)}{v_i}^{(k)}-\sum \limits_{j=1}^n{c}_j{h}_j-\sum \limits_{k=1}^K\sum \limits_{j=1}^n\sum \limits_{i=1}^m{v_i}^{(k)}{w_{ij}}^{(k)}{h}_j $$ ](img/A448418_1_En_5_Chapter_Equ47.gif) (5.4.2) 给出

所以，

![ $$ P\left(v,h\right)\propto {e}^{-E\left(v,h\right)}={e}^{\sum \limits_{k=1}^K\sum \limits_{i=1}^m{b_i}^{(k)}{v_i}^{(k)}+\sum \limits_{j=1}^n{c}_j{h}_j+\sum \limits_{k=1}^K\sum \limits_{j=1}^n\sum \limits_{i=1}^m{v_i}^{(k)}{w_{ij}}^{(k)}{h}_j} $$ ](img/A448418_1_En_5_Chapter_Equ48.gif) (5.4.3)

给定输入 `v`，隐藏单元的概率是：

![ $$ P\left( hj=1/v\right)=\frac{e^{\left({c}_j+{\sum}_{i=1}^m{\sum}_{k=1}^K{v_i}^{(k)}{w_{ij}}^{(k)}\right)}}{1+{e}^{\left({c}_j+{\sum}_{i=1}^m{\sum}_{k=1}^K{v_i}^{(k)}{w_{ij}}^{(k)}\right)}} $$ ](img/A448418_1_En_5_Chapter_Equ49.gif) (5.4.4)

现在，一个显而易见的问题是：我们如何预测用户没有看过的电影的评级？事实证明，这方面的计算并不复杂，可以用线性时间来计算。做出该决定的最具信息性的方式是调节用户的概率，假设对电影 `q` 的评级 `r` 是以用户已经提供的电影评级为条件的。假设用户已经提供的电影评分用 `V` 表示，那么我们需要计算概率 `P(v_q^{(k)}/V)` 如下：

![ $$ P\left({v_q}^{(k)}/V\right)=\sum \limits_hP\left({v_q}^{(k)},h/V\right)=\frac{\sum_hP\left({v_q}^{(k)},h,V\right)}{P(V)} $$ ](img/A448418_1_En_5_Chapter_Equ50.gif) (5.4.5)

因为 `P(V)` 对于所有电影分级 `k` 是固定的，所以从（5.4.5）我们有

![ $$ {\displaystyle \begin{array}{l}P\left({v_q}^{(k)}/V\right)\propto \sum \limits_hP\left({v_q}^{(k)},h,V\right)\\ {}\kern5em \propto \sum \limits_h{e}^{-E\left({v_q}^{(k)},h,V\right)}\end{array}} $$ ](img/A448418_1_En_5_Chapter_Equ51.gif) (5.4.6)

这是一种三向能量配置，可以通过在（5.4.2）中添加 `v_q^{(k)}` 的贡献来轻松计算：

![ $$ E\left({v_q}^{(k)},V,h\right)=-\sum \limits_{k=1}^K\sum \limits_{i=1}^m{b_i}^{(k)}{v_i}^{(k)}-\sum \limits_{j=1}^n{c}_j{h}_j-\sum \limits_{k=1}^K\sum \limits_{j=1}^n\sum \limits_{i=1}^m{v_i}^{(k)}{w_{ij}}^{(k)}{h}_j-\sum \limits_{j=1}^n{v_q}^{(k)}{w_{qj}}^{(k)}{h}_j-{v_q}^{(k)}{b}^{(k)} $$ ](img/A448418_1_En_5_Chapter_Equ52.gif) (5.4.7)

将 `v_q^{(k)}=1` 代入（5.4.7）中，可以发现 `E(v_q^{(k)}=1,V,h)` 的值与

![ $$ P\left({v_q}^{(k)}=1,V,h\right) $$ ](img/A448418_1_En_5_Chapter_Equbu.gif)

成正比。

对于等级 `K` 的所有 `K` 值，需要计算前面的量 `E(v_q^{(k)}=1,V,h)`，然后归一化以形成概率。然后，可以取概率最大的 `k` 值，或者从导出的概率中计算 `k` 的期望值：

![ $$ \widehat{k}=\underset{k}{\underbrace{argmax}}P\left({v_q}^{(k)}=1/V\right) $$ ](img/A448418_1_En_5_Chapter_Equbv.gif)

![ $$ \hat{k}=\sum \limits_{k=1}⁵k\times P\left({v_q}^{(k)}=1/V\right) $$ ](img/A448418_1_En_5_Chapter_Equbw.gif)

与基于最大概率的评级硬分配相比，得出评级的期望方法提供了更好的预测。

此外，利用评级矩阵 `V` 来导出用户对特定未评级电影 `q` 的评级 `k` 的概率的一种简单方法是，在给定可见评级输入 `V` 的情况下，首先对隐藏状态 `h` 进行采样；即绘制 `h∼P(h/V)`。隐藏单元是所有电影共有的，因此携带了所有电影的模式信息。从采样的隐藏单元中，我们尝试采样 `v_q^{(k)}` 的值；即绘制 `v_q^{(k)}∼P(v_q^{(k)}/h)`。这种背靠背采样，首先从 `V→h` 开始，然后从 `h→v_q^{(k)}` 开始，相当于采样 `v_q^{(k)}∼P(v_q^{(k)}/V)`。我希望这有助于提供一个更容易的解释。

## 深度信念网络

深度信念网络基于受限玻尔兹曼机，但与 `RBM` 不同，`DBN` 有多个隐藏层。通过保持先前 `K-1` 层中的所有权重不变来训练每个隐藏层 `K` 中的权重。`K-1` 层中隐藏单元的活动作为第 `k` 层的输入。在训练期间的任何特定时间，两层都参与学习它们之间的权重联系。学习算法与受限玻尔兹曼机的算法相同。图 5-12 所示为深度信念网络的高层示意图。

![A448418_1_En_5_Fig12_HTML.gif](img/A448418_1_En_5_Fig12_HTML.gif) 图 5-12。Deep belief network using RBMs

像 `RBM` 一样，在 `DBN` 中，每一层都可以通过使用对比散度的梯度下降来训练。`DBN` 学习算法用于学习用于监督学习的深度网络的初始权重，以便网络从一组良好的初始权重开始。一旦深度信念网络的预训练完成，我们就可以基于手头的监督问题向 `DBN` 添加输出层。假设我们想要训练一个模型来对 `MNIST` 数据集执行分类。在这种情况下，我们将不得不追加一个十级 `SoftMax` 层。然后，我们可以通过误差反向传播对模型进行微调。由于该模型已经具有来自无监督 `DBN` 学习的初始权重集，当调用反向传播时，该模型将有更快收敛的好机会。
...
我们现在来看看 `DBN` 预训练权重的实现，然后通过将输出层附加到 `RBM` 的隐藏层来训练分类网络。在 `清单 5-3a` 中，我们实现了 `RBM`，其中我们学习了可见到隐藏连接的权重，假设所有的单元都是 `sigmoid`。对于该 `RBM`，我们将堆叠 `MNIST` 数据集的十个类的输出层，并使用从可见到隐藏单元学习的权重作为分类网络的初始权重来训练分类模型。当然，我们会有一组新的权重，对应于隐藏层到输出层的连接。具体实现见 `清单 5-3b`。
...
![图 5-13. 基本自编码器架构](img/A448418_1_En_5_Fig13_HTML.gif)

图 5-13 表示一个基本的自编码器，带有一个隐藏层和输入输出层。输入 $x={\left[{x}_1{x}_2\ {x}_3..{x}_6\right]}^T\in {\mathrm{\mathbb{R}}}^{6\times 1}$ 而隐藏层 $h={\left[{h}_1{h}_2\ {h}_3{h}_4\right]}^T\in {\mathrm{\mathbb{R}}}^{4\times 1}$。输出 `y` 被选择为等于 `x`，使得重构的输入 $\hat{y}$ 之间的误差可以被最小化，从而在隐藏层中获得输入的有意义的表示。出于通用性的目的，让我们取

$$ x={\left[{x}_1{x}_2\ {x}_3..{x}_n\right]}^T\in {\mathrm{\mathbb{R}}}^{n\times 1} $$

$$ h={\left[{h}_1{h}_2\ {h}_3..{h}_d\right]}^T\in {\mathrm{\mathbb{R}}}^{d\times 1} $$

假设从 `x` 到 `h` 的权重由权重矩阵
```
![ $$ W\in {\mathrm{\mathbb{R}}}^{d\times n} $$ ](img/A448418_1_En_5_Chapter_IEq108.gif)
```
表示，隐藏单元的偏差由
```
![ $$ b={\left[{b}_1{b}_2\ {b}_3..{h}_d\right]}^T\in {\mathrm{\mathbb{R}}}^{d\times 1} $$ ](A448418_1_En_5_Chapter_IEq109.gif)
```
表示。

类似地，假设从 `h` 到 `y` 的权重由权重矩阵
```
![ $$ {W}^{\hbox{'}}\in {\mathrm{\mathbb{R}}}^{n\times d} $$ ](img/A448418_1_En_5_Chapter_IEq110.gif)
```
表示，输出单元的偏差由
```
![ $$ {b}^{\prime }={\left[\ {b}_1{b}_2\ {b}_3..{b}_n\right]}^T\in {\mathrm{\mathbb{R}}}^{n\times 1} $$ ](A448418_1_En_5_Chapter_IEq111.gif)
```
表示。
...
```
，其中 `f_1` 是隐藏层的逐元素激活函数。根据其用途，激活函数可以是线性的、`ReLU` 的、`sigmoid` 的等等。
...
```
![ $$ C=\sum \limits_{k=1}^m{\widehat{y}}^{(k)}-{y^{(k)}}_2²=\sum \limits_{k=1}^m{\widehat{y}}^{(k)}-{x^{(k)}}_2² $$ ](img/A448418_1_En_5_Chapter_Equcc.gif)
```
其中
```
![ $$ {\hat{y}}^{(k)}-{x^{(k)}}_2² $$ ](img/A448418_1_En_5_Chapter_IEq112.gif)
```
是重构的输出向量和原始输入向量之间的欧几里德或 `l^2` 范数距离，`m` 是模型被训练的数据点的数量。

如果我们用向量
```
![ $$ \theta =\left[W;b;{W}^{\prime };{b}^{\prime}\right] $$ ](A448418_1_En_5_Chapter_IEq113.gif)
```
表示模型的所有参数，那么成本函数 `C` 可以相对于模型 `θ` 的所有参数最小化，以导出模型
```
![ $$ \widehat{\theta}=\underset{\theta }{\underbrace{Arg\ \mathit{\operatorname{Min}}}}\ C\left(\theta \right)=\underset{\theta }{\underbrace{Arg\ \mathit{\operatorname{Min}}}}\sum \limits_{k=1}^m{\widehat{y}}^{(k)}-{x^{(k)}}_2² $$ ](img/A448418_1_En_5_Chapter_Equcd.gif)
```
...
，其中 `ϵ` 为学习速率，`t` 代表迭代次数，
```
![ $$ {\nabla}_{\theta }C\left({\theta}^{(t)}\right) $$ ](img/A448418_1_En_5_Chapter_IEq114.gif)
```
为代价函数在
```
![ $$ \theta ={\theta}^{(t)} $$ ](img/A448418_1_En_5_Chapter_IEq115.gif)
```
处相对于 `θ` 的梯度。
...
*   当隐藏层的维度小于输入层的维度时，即
    ```
    ![ $$ \left(d<n\right) $$ ](img/A448418_1_En_5_Chapter_IEq116.gif)
    ```
    其中 `d` 是隐藏层，`n` 是输入层的维度，则自编码器作为数据压缩网络工作，将数据从高维空间投影到由隐藏层给出的低维空间。这是一种有损数据压缩技术。它还可以用于降低输入信号中的噪声。
*   当
    ```
    ![ $$ \left(d<n\right) $$ ](img/A448418_1_En_5_Chapter_IEq117.gif)
    ```
    和所有的激活函数都是线性的，那么网络学习进行线性 `PCA`(主成分分析)。
*   当
    ```
    ![ $$ \left(d\ge n\right) $$ ](img/A448418_1_En_5_Chapter_IEq118.gif)
    ```
    和激活函数是线性的时，网络可以学习一个身份函数，这可能没有任何用处。然而，如果成本函数被正则化以产生稀疏隐藏表示，则网络仍然可以学习数据的有趣表示。
*   输入数据的复杂非线性表示可以通过在网络中有更多的隐藏层和使激活函数非线性来学习。这种模型的示意图如图 5-14 所示。当采用多个隐藏层时，必须采用非线性激活函数来学习数据的非线性表示，因为几层线性激活等效于单个线性激活层。

## 用于监督学习的通过自编码器的特征学习

![A448418_1_En_5_Fig14_HTML.gif](img/A448418_1_En_5_Fig14_HTML.gif)

图 5-14. Autoencoder with multiple hidden layers

当我们处理多个隐藏层时，如图 5-14 所示，并且在神经单元中具有非线性激活函数，那么隐藏层学习输入数据的变量之间的非线性关系。如果我们正在处理两个类别的分类相关问题，其中输入数据由
```
![ $$ x\in {\mathrm{\mathbb{R}}}^{6\times 1} $$ ](img/A448418_1_En_5_Chapter_IEq119.gif)
```
表示，我们可以通过如图 5-14 所示训练自编码器，然后使用第二个隐藏层向量
```
![ $$ {h}^{(2)}\in {\mathrm{\mathbb{R}}}^{3\times 1} $$ ](img/A448418_1_En_5_Chapter_IEq120.gif)
```
...
图 5-15. Classifier with features learned from autoencoder
...
图 5-16. Combined classification network for prediction of classes
...
`KL` 散度测量两个随机伯努利变量之间的差异或散度。如果两个随机伯努利变量 `X` 和 `Y` 分别有 `ρ_1` 和 `ρ_2` 的均值，那么变量 `X` 和 `Y` 之间的 `KL` 散度由
```
...
从前面的表达式我们可以看出
```
![ $$ {\rho}_1={\rho}_2 $$ ](img/A448418_1_En_5_Chapter_IEq121.gif)
```
时 `KL` 散度为 0；即当两种分布相同时。当
```
![ $$ {\rho}_1\ne {\rho}_2 $$ ](img/A448418_1_En_5_Chapter_IEq122.gif)
```
时，`KL` 散度随着均值的不同而单调增加。如果 `ρ_1` 选择为 0.2，那么 `KL` 散度与 `ρ_2` 的关系曲线如图 5-17 所示。

![A448418_1_En_5_Fig17_HTML.gif](img/A448418_1_En_5_Fig17_HTML.gif)

图 5-17. KL divergence plot for mean
```
![ $$ {\rho}_1=0.2 $$ ](img/A448418_1_En_5_Chapter_IEq123.gif)
```

如我们所见，`KL` 散度在
```
![ $$ {\uprho}_2={\uprho}_1=0.2 $$ ](img/A448418_1_En_5_Chapter_IEq124.gif)
```
处最小，在
```
![ $$ {\uprho}_2=0.2 $$ ](img/A448418_1_En_5_Chapter_IEq125.gif)
```
...
我们之前提到的一件事是，当隐藏层单元的数量大于输入的维数时，在将对应于额外隐藏层的权重设置为零之后，自编码器很可能会学习恒等式变换。事实上，当输入层和隐藏层的数量相同时，连接输入层和隐藏层的权重矩阵的最优解就是单位矩阵。然而，即使当隐藏单元的数量大于输入的维数时，只要提供一些约束，自编码器仍然可以学习数据中有趣的结构。一个这样的约束是将隐藏层输出限制为稀疏的，使得隐藏层单元中的那些激活平均接近于零。我们可以通过在基于 `KL` 散度的成本函数中添加正则化项来实现这种稀疏性。这里，`ρ_1` 将非常接近于零，并且隐藏单元中所有训练样本的平均激活将充当该隐藏单元的 `ρ_2` 。通常，`ρ_1` 被选择为非常小，大约为 0.04，因此如果每个隐藏单元中的平均激活不接近 0.04，那么代价函数将被扣分。

设
```
![ $$ h\in {\mathrm{\mathbb{R}}}^{d\times 1} $$ ](img/A448418_1_En_5_Chapter_IEq126.gif)
```
为输入
```
![ $$ x\in {\mathrm{\mathbb{R}}}^{n\times 1} $$ ](img/A448418_1_En_5_Chapter_IEq127.gif)
```
的隐藏层 `sigmoid` 激活，其中
```
![ $$ d>n $$ ](img/A448418_1_En_5_Chapter_IEq128.gif)
```
。此外，将输入连接到隐藏层的权重由
```
![ $$ W\in {\mathrm{\mathbb{R}}}^{d\times n} $$ ](img/A448418_1_En_5_Chapter_IEq129.gif)
```
给出，将隐藏层连接到输出层的权重由
```
![ $$ {W}^{\hbox{'}}\in {\mathrm{\mathbb{R}}}^{n\times d} $$ ](img/A448418_1_En_5_Chapter_IEq130.gif)
```
...
其中 `h^{(k)}` 和
```
![ $$ {\hat{y}}^{(k)} $$ ](img/A448418_1_En_5_Chapter_IEq131.gif)
```
是隐藏层输出向量和第 `k` 个输入训练数据点的重构输入向量。相对于模型参数(即 `W`，`W'`，`b`，`b'`)要最小化的成本函数由
```
![ $$ C=\sum \limits_{k=1}^m{\hat{y}}^{(k)}-{x^{(k)}}_2²+\lambda \sum \limits_{j=1}^d KL\left(\rho ||{\hat{\rho}}_j\right) $$ ](img/A448418_1_En_5_Chapter_Equci.gif)
```
给出，其中
```
![ $$ {\hat{\rho}}_j $$ ](img/A448418_1_En_5_Chapter_IEq132.gif)
```
是所有训练样本上隐藏层的第 `j` 个单元中的平均激活，可以表示如下。同样， `h_j^{(k)}` 表示第 `k` 个训练样本在单元 `j` 处的隐藏层激活。
```
...
图 5-19. Display of a few encoder/decoder weights as images
...
去噪自编码器像标准自编码器一样工作，在隐藏层中进行非线性激活，唯一的区别是，代替原始输入 `x` 的是 `x` 的噪声版本，比如说 `$$ \tilde{x} $$`，被馈送到网络。在计算重建误差时，将输出层的重建图像与实际输入 `x` 进行比较。其思想是，从噪声数据中学习到的隐藏结构足够丰富，可以重构原始数据。因此，这种形式的自编码器可以用于减少数据中的噪声，因为它从隐藏层学习数据的鲁棒表示。例如，如果图像由于某种失真而变得模糊，那么可以使用去噪自编码器来消除模糊。通过引入随机噪声添加单元，可以将自编码器转换成去噪自编码器。
...
![图 5-21，Reconstructed Images (without Gaussian Noise) generated by the Denoising Auto Encoder](img/A448418_1_En_5_Fig21_HTML.jpg)

![图 5-20，Images with Gaussian noise](img/A448418_1_En_5_Fig20_HTML.jpg)
...
*   从每张图像中单独移除平均像素强度。因此，如果将一张二维图像转换为一个向量，可以从该图像向量中减去其元素的平均值。如果每张图像由向量 ![ $$ {x}^{(i)}\in {\mathrm{\mathbb{R}}}^{nX1} $$ ](img/A448418_1_En_5_Chapter_IEq136.gif) 表示，其中 `i` 代表训练集中的第 `i` 张图像，那么 `x <sup>(i)</sup>` 的均值归一化图像由下式给出：
    ![ $$ {x}^{(i)}={x}^{(i)}-\frac{1}{n}\sum \limits_{j=1}^n{x}_j^{(i)} $$ ](img/A448418_1_En_5_Chapter_Equcl.gif)

*   一旦我们有了均值归一化的图像，就可以如下计算协方差矩阵：
    ![ $$ C=\frac{1}{m}\sum \limits_{i=1}^m{x}^{(i)}{x^{(i)}}^T $$ ](img/A448418_1_En_5_Chapter_Equcm.gif)

*   接下来，我们需要通过奇异值分解（`SVD`）来分解协方差矩阵，如下所示：
    ![ $$ C= UD{U}^T $$ ](img/A448418_1_En_5_Chapter_Equcn.gif)
    通常，`SVD` 分解为 ![ $$ C= UD{V}^T $$ ](img/A448418_1_En_5_Chapter_IEq137.gif)，但由于 `C` 是一个对称矩阵，在这种情况下 ![ $$ U=V $$ ](img/A448418_1_En_5_Chapter_IEq138.gif)。`U` 给出了协方差矩阵的特征向量。特征向量在 `U` 中按列对齐。数据沿特征向量方向的方差由 `D` 对角线上的特征值给出，而 `D` 中的其余条目为零，因为 `D` 是由特征向量给出的不相关方向的协方差矩阵。

*   在 `PCA` 白化中，我们将数据投影到特征向量（或称主成分）上，然后将每个方向上的投影值除以特征值的平方根——即数据投影方向上的标准差。因此，`PCA` 白化变换如下：
    ![ $$ T={D}^{-\frac{1}{2}}{U}^T $$ ](img/A448418_1_En_5_Chapter_Equco.gif)
    一旦将此变换应用于数据，变换后的数据在新的变换分量上具有零相关性和单位方差。原始均值校正图像 `x <sup>(i)</sup>` 的变换数据为：
    ![ $$ {x^{(i)}}_{PW}=T{x}^{(i)} $$ ](img/A448418_1_En_5_Chapter_Equcp.gif)

`PCA` 白化的问题是，尽管它去除了数据的相关性并使新特征的方差一致，但特征不再位于原始空间中，而是在变换后的旋转空间中。这使得诸如图像之类的对象的结构在它们的空间方位方面丢失了大量信息，因为在变换后的特征空间中，每个特征都是所有特征的线性组合。对于利用图像空间结构的算法，如卷积神经网络，这不是一件好事。因此，我们需要某种方法来白化数据，使得数据是去相关的，并且沿着其特征具有单位方差，但是特征仍然在原始特征空间中，而不在某个变换的旋转特征空间中。提供所有这些相关特性的变换称为 `ZCA` 变换。数学上，任何正交矩阵 `R`（其列向量彼此正交）乘以 `PCA` 白化变换 `T` 都会产生另一个白化变换。如果选择了 ![ $$ R=U $$ ](img/A448418_1_En_5_Chapter_IEq139.gif)，则变换
![ $$ Z= UT=U{D}^{-\frac{1}{2}}{U}^T $$ ](img/A448418_1_En_5_Chapter_Equcq.gif)
称为 `ZCA` 变换。`ZCA` 变换的优点是图像数据仍然驻留在相同的像素特征空间中，因此，与 `PCA` 白化不同，原始像素不会因为新特征的创建而变得模糊。同时，数据被白化，即去相关，并且每个特征的单位方差被白化。单位方差和解相关特征可以帮助若干机器学习或深度学习算法实现更快的收敛。同时，由于特征仍然在原始空间中，它们保持它们的拓扑或空间方向，并且诸如利用图像的空间结构的卷积神经网络的算法可以利用该信息。

![A448418_1_En_5_Fig25_HTML.gif](img/A448418_1_En_5_Fig25_HTML.gif)
图 5-25。ZCA whitening illustration

![A448418_1_En_5_Fig24_HTML.gif](img/A448418_1_En_5_Fig24_HTML.gif)
图 5-24。PCA whitening illustration

`PCA` 增白和 `ZCA` 增白的主要区别如图 5-24 和图 5-25 所示。如我们所见，在两种情况下，二维相关数据都被转换成不相关的新数据。然而，有一个主要的区别。在 `PCA` 白化中，基于 `p <sub>1</sub>` 和 `p <sub>2</sub>` 给出的主成分，新轴已经从原始轴改变，而在 `ZCA` 白化中，轴保持与原始轴相同。`p <sub>1</sub>` 和 `p <sub>2</sub>` 是数据的协方差矩阵的特征向量。此外，我们看到标记 `AB` 的方向在 `PCA` 白化的新轴上发生了变化，而在 `ZCA` 白化中保持不变。在这两种情况下，想法都是去除输入变量之间不太有用的双向协方差，以便模型可以专注于学习高阶相关性。
