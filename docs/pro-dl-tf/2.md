...
*   步骤 3–更新权重向量 w，如下所示:
    ...
    *   如果 y <sub>p</sub> <sup>(i)</sup> = 1 且实际类别![ $$ {y}^{(i)}=0 $$ ](img/A448418_1_En_2_Chapter_IEq34.gif)，则更新权重向量为![ $$ w=w-{x}^{(i)}. $$ ](img/A448418_1_En_2_Chapter_IEq35.gif)
...
![A448418_1_En_2_Fig8_HTML.gif](img/A448418_1_En_2_Fig8_HTML.gif)
...
同样，隐藏单元的输出![ $$ {h}_2=1/\left(1+{e}^{-\left({w}_{12}{x}_1+{w}_{22}{x}_2+{b}_2\right)}\right) $$ ](img/A448418_1_En_2_Chapter_IEq82.gif)。
...
*   When the net input z to a sigmoid function is 0 then ![ $$ {e}^{-z} $$ ](img/A448418_1_En_2_Chapter_IEq92.gif) = 1 and so ![ $$ y=\frac{1}{2} $$ ](img/A448418_1_En_2_Chapter_IEq93.gif) .
    ...
    ![A448418_1_En_2_Fig11_HTML.gif](img/A448418_1_En_2_Fig11_HTML.gif)
...
![A448418_1_En_2_Fig13_HTML.gif](img/A448418_1_En_2_Fig13_HTML.gif)
...
参照图 2-7 ，设隐藏单元 h <sub>1</sub> 处的输入输出分别为 i <sub>1</sub> 和 z <sub>1</sub> 。同样，设隐藏单元 h <sub>2</sub> 处的输入输出分别为 i <sub>2</sub> 和 z <sub>2</sub> 。最后，设输出层 p <sub>1</sub> 的输入和输出分别为 i <sub>3</sub> 和 z <sub>3</sub> 。
...
在前面的表达式中，![ $$ {y}_i\in \left\{0,1\right\} $$ ](img/A448418_1_En_2_Chapter_IEq115.gif)，取决于特定于 y <sub>i</sub> 的事件是否活动。
...
现在，![ $$ \frac{\partial C}{\partial {\mathrm{z}}_j^{(2)}} $$ ](img/A448418_1_En_2_Chapter_IEq120.gif)是棘手的计算自 z<sub>j</sub>(2)影响所有三个输出单元:
...
让我们计算成本函数相对于权重 w <sub>ji</sub> <sup>(2)</sup> 的偏导数。现在，权重将影响第 I 个 SoftMax 单元的净输入 s<sub>I</sub>??(3)。然而，与早期网络中的独立二进制激活不同，这里所有三个 SoftMax 输出单元![ $$ {z}_k^{(3)}\forall k\in \left\{1,2,3\right\} $$ ](img/A448418_1_En_2_Chapter_IEq129.gif)将受到 s<sub>I</sub>(3)的影响，因为
...
要在交互模式下运行 TensorFlow 命令，可以调用清单 2-2 中的`Interactive Session()`命令，通过使用`eval()`方法，可以在激活的交互会话下运行 TensorFlow 命令，如清单 2-4 所示。
...
![A448418_1_En_2_Fig20_HTML.gif](img/A448418_1_En_2_Fig20_HTML.gif)
...
Plot of ![ $$ f\left(x,y\right)={x}²-{y}² $$ ](img/A448418_1_En_2_Chapter_IEq156.gif)
...
其中`learning_` `rate`是常数学习率，`cost`是需要通过梯度下降最小化的成本函数。成本函数相对于与成本函数相关联的 TensorFlow 变量被最小化。
...
其中η是学习率，θ <sub>i</sub> <sup>(t)</sup> 和![ $$ {\theta_i}^{\left(t+1\right)} $$ ](img/A448418_1_En_2_Chapter_IEq158.gif)分别是迭代 t 和![ $$ t+1 $$ ](img/A448418_1_En_2_Chapter_IEq159.gif)时第 I 个参数的值。
...
*   对于所有权重，以相同幅度的权重更新开始；即![ $$ {\varDelta_{ij}}^{\left(t=0\right)}={\varDelta_{ij}}^{(0)}=\varDelta $$ ](img/A448418_1_En_2_Chapter_IEq161.gif)。此外，将最大和最小允许重量更新分别设置为δ<sub>max</sub>和δ<sub>min</sub>。
...
*   If the signs of the current and previous gradient components for a dimension are different—i.e., ![ $$ \mathit{\operatorname{sign}}\left(\frac{\partial {C}^{(t)}}{\partial {w}_{ij}}\frac{\partial {C}^{\left(t-1\right)}}{\partial {w}_{ij}}\right)=- ve $$ ](img/A448418_1_En_2_Chapter_IEq164.gif) —then reduce the learning rate by a factor![ $$ {\eta}_{-}=0.5 $$ ](img/A448418_1_En_2_Chapter_IEq165.gif) . The update rule becomes
...
*   If ![ $$ \frac{\partial {C}^{(t)}}{\partial {w}_{ij}}\frac{\partial {C}^{\left(t-1\right)}}{\partial {w}_{ij}}=0 $$ ](img/A448418_1_En_2_Chapter_IEq166.gif), the update rule is as follows:
...
如果我们将![ $$ \sqrt{h_{ij}^{(t)}+\epsilon } $$ ](img/A448418_1_En_2_Chapter_IEq169.gif)表示为 RMS(h <sub> ij </sub> <sup>(t)</sup> ，那么更新规则变成-
...
设每个权重 w <sub>ij</sub> 的梯度均值 m<sub>ij</sub>??【t】和梯度平方均值 v<sub>ij</sub>??【t】的衰减率分别为β <sub>1</sub> 和β <sub>2</sub> 。此外，假设η是常数学习率因子。那么，`Adam`的更新规则如下:
...
梯度的归一化平均值![ $$ \hat{m_{ij}^{(t)}} $$ ](img/A448418_1_En_2_Chapter_IEq170.gif)和平方梯度的平均值![ $$ \hat{v_{ij}^{(t)}} $$ ](img/A448418_1_En_2_Chapter_IEq171.gif)计算如下:
...
其中`learning_rate`是常数学习率η，`cost` C 是需要通过`AdamOptimizer`最小化的代价函数。参数`beta1`和`beta2`分别对应β <sub>1</sub> 和β <sub>2</sub> ，而`epsilon`代表![ $$ \in $$ ](img/A448418_1_En_2_Chapter_IEq172.gif)。
...
其中，α是动量参数，η是学习率。项 v <sub>i</sub> <sup>(t)</sup> 和![ $$ {v}_i^{\left(t+1\right)} $$ ](img/A448418_1_En_2_Chapter_IEq173.gif)分别表示第 I 个参数在迭代 t 和![ $$ \left(t+1\right) $$ ](img/A448418_1_En_2_Chapter_IEq174.gif)时的速度。类似地，w <sub> i </sub> <sup>(t)</sup> 和![ $$ {w_i}^{\left(t+1\right)} $$ ](img/A448418_1_En_2_Chapter_IEq175.gif)分别代表第 I 个参数在迭代 t 和![ $$ t+1 $$ ](img/A448418_1_En_2_Chapter_IEq176.gif)时的权重。

想象在优化成本函数时，优化算法在![ $$ \frac{\partial C}{\partial {w}_i}\left({w_i}^{(t)}\right)\to 0\forall i\in {\mathrm{\mathbb{R}}}^{n\times 1} $$ ](img/A448418_1_En_2_Chapter_IEq177.gif)处达到局部最小值。在不考虑动量的正常梯度下降方法中，参数更新将在局部最小值或鞍点处停止。然而，在基于动量的优化中，考虑到局部极小值具有小的吸引盆，先验速度会驱使算法脱离局部极小值，因为![ $$ {v}_i^{\left(t+1\right)} $$ ](img/A448418_1_En_2_Chapter_IEq178.gif)会因为来自先验梯度的非零速度而非零。此外，如果先前的梯度一致地指向全局最小值或局部最小值，具有良好的推广性和相当大的吸引盆地，则梯度下降的速度或动量将在该方向上。因此，即使存在具有小吸引盆的坏的局部最小值，动量分量也将不仅驱使算法脱离坏的局部最小值，而且将继续朝向全局最小值或好的局部最小值的梯度下降。

如果权重是参数向量θ的一部分，则基于动量的优化器的矢量化更新规则将如下(参见图 2-22 的基于向量的图示):

![ $$ {v}^{\left(t+1\right)}=\alpha {v}^{(t)}-\eta \nabla C\ \left(\theta ={\theta}^{(t)}\right) $$ ](img/A448418_1_En_2_Chapter_Equda.gif)

![ $$ {\theta}^{\left(t+1\right)}={\theta}^{(t)}+{v}^{\left(t+1\right)} $$ ](img/A448418_1_En_2_Chapter_Equdb.gif)

![A448418_1_En_2_Fig22_HTML.gif](img/A448418_1_En_2_Fig22_HTML.gif)

图 2-22。

Parameter vector update in momentum-based gradient-descent optimizer

基于动量的优化器的一个特殊变体是内斯特罗夫加速梯度技术。该方法利用现有的速度 v <sup>(t)</sup> 对参数向量进行更新。因为它是对参数向量的中间更新，所以用![ $$ {\theta}^{\left(t+\frac{1}{2}\right)} $$ ](img/A448418_1_En_2_Chapter_IEq179.gif)来表示很方便。在![ $$ {\theta}^{\left(t+\frac{1}{2}\right)} $$ ](img/A448418_1_En_2_Chapter_IEq180.gif)评估成本函数的梯度，并将其用于更新新的速度。最后，新的参数向量是前一次迭代的参数向量和新的速度之和。

![ $$ {\theta}^{\left(t+\frac{1}{2}\right)}={\theta}^{(t)}+\alpha {v}^{(t)} $$ ](img/A448418_1_En_2_Chapter_Equdc.gif)

![ $$ {v}^{\left(t+1\right)}=\alpha {v}^{(t)}-\eta \nabla C\ \left(\uptheta ={\theta}^{\left(t+\frac{1}{2}\right)}\right) $$ ](img/A448418_1_En_2_Chapter_Equdd.gif)

![ $$ {\theta}^{\left(t+1\right)}={\theta}^{(t)}+{v}^{\left(t+1\right)} $$ ](img/A448418_1_En_2_Chapter_Equde.gif)
...
![A448418_1_En_2_Fig23_HTML.jpg](img/A448418_1_En_2_Fig23_HTML.jpg)
