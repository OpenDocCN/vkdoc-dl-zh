...
![ $$ P\left(r,v\right)=\frac{e^{-\frac{E\left(r,v\right)}{KT}}}{Z} $$ ](A448418_1_En_5_Chapter_Equb.gif)

其中 Z 是由

![ $$ Z=\sum \limits_{i=1}^M{e}^{-\frac{E\left({\left(r,v\right)}_i\right)}{KT}} $$ ](A448418_1_En_5_Chapter_Equc.gif)

给出的配分函数

r 和 v 可以分别有几个值。然而，M 表示 r 和 v 的所有可能的唯一组合，这些组合在前面的等式中已经由(r，v) <sub>i</sub> 表示。如果 r 可以取 n 个不同的坐标值，而 v 可以取 m 个不同的速度值，那么可能配置的总数![ $$ M=n\times m $$ ](A448418_1_En_5_Chapter_IEq3.gif)。在这种情况下，配分函数也可以表示为:

![ $$ Z=\sum \limits_{j=1}^m\sum \limits_{i=1}^n{e}^{-\frac{E\left({r}_i,{v}_j\right)}{KT}} $$ ](A448418_1_En_5_Chapter_Equd.gif)

这里要注意的是，当相关能量低时，任何构型的概率都较高。对于气体分子来说，这也很直观，因为高能态总是与不稳定平衡相联系，因此不太可能长时间保持高能构型。高能组态中的粒子总是在追求占据更稳定的低能态。

如果我们考虑两个组态![ $$ {s}_1=\left({r}_1,{v}_1\right) $$ ](A448418_1_En_5_Chapter_IEq4.gif)和![ $$ {s}_2=\left({r}_2,{v}_2\right) $$ ](A448418_1_En_5_Chapter_IEq5.gif)，并且如果这两个态的气体分子数分别为 N <sub>1</sub> 和 N <sub>2</sub> ，那么这两个态的概率比就是这两个态的能量差的函数:

![ $$ \frac{N_1}{N_2}=\frac{P\left({r}_1,{v}_1\right)}{P\left({r}_2,{v}_2\right)}={e}^{-\frac{E\left({r}_1,{v}_1\right)-\left({r}_2,{v}_2\right)}{KT}} $$ ](A448418_1_En_5_Chapter_Eque.gif)
...
![ $$ Likelihood=P\left( Data/ Model\right) $$ ](A448418_1_En_5_Chapter_Equf.gif)

为了得到由其参数定义的模型，我们最大化所见数据的可能性:

![ $$ Model=\underset{Model}{\underbrace{Arg\ \mathit{\operatorname{Max}}}}P\left( Data/ Model\right) $$ ](A448418_1_En_5_Chapter_Equg.gif)
...
![ $$ P\left( Model/ Data\right)=\frac{P\left( Data/ Model\right)P(Model)}{P(Data)} $$ ](A448418_1_En_5_Chapter_Equh.gif)

p(模型/数据)被称为后验分布，通常信息更丰富，因为它结合了一个人关于数据或模型的先验知识。由于数据的这种概率是独立于模型的，因此后验与似然和先验的乘积成正比:

![ $$ P\left( Model/ Data\right)\propto P\left( Data/ Model\right)P(Model) $$ ](A448418_1_En_5_Chapter_Equi.gif)
...
![ $$ P\left( Data/\theta \right)=P\left({x}_1{x}_2{x}_3{x}_4{x}_5{x}_6/\theta \right) $$ ](A448418_1_En_5_Chapter_Equk.gif)

给出，其中![ $$ {x}_{i\kern0.5em }\forall i\in \left\{1,2,3,4,5,6\right\} $$ ](A448418_1_En_5_Chapter_IEq6.gif)表示正面(H)或反面(T)的事件。

由于硬币的投掷是独立的，可能性可以分解如下:

![ $$ P\left( Data/\theta \right)=P\left({x}_1{x}_2{x}_3{x}_4{x}_5{x}_6/\theta \right)=\prod \limits_{i=1}^6P\left({x}_i/\theta \right) $$ ](A448418_1_En_5_Chapter_Equ1.gif)

(5.1.1)

每次掷骰子遵循伯努利分布，因此正面的概率为θ，反面的概率为![ $$ \left(1-\theta \right) $$ ](A448418_1_En_5_Chapter_IEq7.gif)，通常其概率质量函数由

![ $$ P\left(x=j/\theta \right)={\theta}^j{\left(1-\theta \right)}^{\left(1-j\right)}\forall j\in \left\{0,1\right\} $$ ](A448418_1_En_5_Chapter_Equ2.gif)

(5.1.2)给出，其中![ $$ j=1 $$ ](A448418_1_En_5_Chapter_IEq8.gif)表示正面，![ $$ j=0 $$ ](A448418_1_En_5_Chapter_IEq9.gif)表示反面。

由于组合(1)和(2)有 5 个头和 1 个尾，所以作为θ的函数的似然 L 可以表示如下:

![ $$ L\left(\theta \right)={\theta}^5\left(1-\theta \right) $$ ](A448418_1_En_5_Chapter_Equ3.gif)

(5.1.3)

最大似然法将最小化 L(θ)的![ $$ \hat{\theta} $$ ](A448418_1_En_5_Chapter_IEq10.gif)视为模型参数。于是，

![ $$ \widehat{\theta}=\underset{\theta }{\underbrace{Arg\ \mathit{\operatorname{Max}}}}\ L\left(\theta \right) $$ ](A448418_1_En_5_Chapter_Equl.gif)

如果我们对(5.1.3)中计算的似然性求导并将其设置为零，我们将得到θ:

![ $$ \frac{d L\left(\theta \right)}{d\theta}=5{\theta}^4-6{\theta}^5=0\kern0.5em =>\theta =\frac{5}{6} $$ ](A448418_1_En_5_Chapter_Equm.gif)

的似然估计

一般来说，如果有人问我们对θ的估计，而我们没有做类似的最大似然估计，我们会立刻根据我们在高中学到的概率的基本定义回答概率为![ $$ \frac{5}{6} $$ ](A448418_1_En_5_Chapter_IEq11.gif);即

![ $$ P(event)=\frac{no\ of events}{total number of events in the whole population} $$ ](A448418_1_En_5_Chapter_Equn.gif)

在某种程度上，我们的大脑在思考可能性，并依赖迄今为止看到的数据。

现在，让我们假设我们没有看到数据，有人要求我们确定正面的概率；什么是合理的估计？

这取决于我们对硬币的任何先验信念，即概率会不同。如果我们假设一个公平的硬币，这通常是最明显的假设，因为我们没有关于硬币的信息，![ $$ \theta =\frac{1}{2} $$ ](A448418_1_En_5_Chapter_IEq12.gif)将是一个很好的估计。然而，当我们假设而不是对θ进行先验点估计时，最好在θ上有一个概率分布，最大概率在![ $$ \theta =\frac{1}{2} $$ ](A448418_1_En_5_Chapter_IEq13.gif)。先验概率分布是模型参数θ的分布。

在这种情况下，带有参数![ $$ \alpha =2,\kern0.5em \beta =2 $$ ](A448418_1_En_5_Chapter_IEq14.gif)的贝塔分布将是一个很好的先验分布，因为它在![ $$ \theta =\frac{1}{2} $$ ](A448418_1_En_5_Chapter_IEq15.gif)具有最大概率，并且在它周围是对称的。【T2![ $$ P\left(\theta \right)= Beta\left(\alpha =2,\beta =2\right)=\frac{\theta^{\alpha -1}{\left(1-\theta \right)}^{\beta -1}}{B\left(\alpha, \beta \right)}=\frac{\theta \left(1-\theta \right)}{B\left(\alpha, \beta \right)} $$ ](A448418_1_En_5_Chapter_Equo.gif)

对于α和β的固定值，B(α，β)是常数，并且是该概率分布的归一化或配分函数。可以计算如下:

![ $$ B\left(\alpha, \beta \right)=\frac{\tau \left(\alpha \right)\tau \left(\beta \right)}{\tau \left(\alpha +\beta \right)}=\frac{\tau (2)\tau (2)}{\tau (4)}=\frac{1!1!}{3!}=\frac{1}{6} $$ ](A448418_1_En_5_Chapter_Equp.gif)

即使不记得公式，只要对![ $$ \theta \left(1-\theta \right) $$ ](A448418_1_En_5_Chapter_IEq16.gif)积分，取其倒数作为归一化常数，就可以知道，因为概率分布的积分应该是 1。
...
![ $$ P\left(\theta /D\right)\propto {\theta}^5\left(1-\theta \right)\frac{\theta\ \left(1-\theta \right)}{6}=\kern1em \frac{\theta^6{\left(1-\theta \right)}^2}{6} $$ ](A448418_1_En_5_Chapter_Equq.gif)

由于我们忽略了数据的概率，所以出现了正比例符号。其实我们也可以把 6 拿出来，把后验表示成:

![ $$ P\left(\theta /D\right)\propto {\theta}^6{\left(1-\theta \right)}^2 $$ ](A448418_1_En_5_Chapter_Equr.gif)

现在，![ $$ 0\le \theta \le 1 $$ ](A448418_1_En_5_Chapter_IEq17.gif)既然θ是一个概率。在 0 到 1 的范围内对![ $$ {\theta}^6{\left(1-\theta \right)}^2 $$ ](A448418_1_En_5_Chapter_IEq18.gif)进行积分并取倒数，将得到后验概率的归一化因子，结果为 252。于是，后验可以表述为:

![ $$ P\left(\theta /D\right)=\frac{\theta^6{\left(1-\theta \right)}^2}{252} $$ ](A448418_1_En_5_Chapter_Equ5.gif)

(5.1.5)

现在我们有了后验概率，有两种方法可以估计θ。我们可以最大化后验，得到θ的 MAP 估计如下:

![ $$ {\theta}_{MAP}=\underset{\theta }{\underbrace{Arg\ \mathit{\operatorname{Max}}}}\ P\left(\theta /D\right) $$ ](A448418_1_En_5_Chapter_Equs.gif)

![ $$ \frac{d P\left(\theta /D\right)}{d\theta}=0\kern0.5em =>\theta =\frac{3}{4} $$ ](A448418_1_En_5_Chapter_Equt.gif)

我们看到,![ $$ \frac{3}{4} $$ ](A448418_1_En_5_Chapter_IEq19.gif)的 MAP 估计比![ $$ \frac{5}{6} $$ ](A448418_1_En_5_Chapter_IEq20.gif)的似然估计更保守，因为它考虑了先验，而不是盲目相信数据。
...
![A448418_1_En_5_Fig1c_HTML.gif](A448418_1_En_5_Fig1c_HTML.gif)

图 5-1c。

Posterior probability distribution

![A448418_1_En_5_Fig1b_HTML.gif](A448418_1_En_5_Fig1b_HTML.gif)

图 5-1b。

Prior probability distribution

![A448418_1_En_5_Fig1a_HTML.gif](A448418_1_En_5_Fig1a_HTML.gif)
...
![ $$ \frac{4^{\ast } Area(OAC)}{Area(OABC)}={4}^{\ast}\frac{\left(\frac{1}{4}\right)\pi {r}^2}{r^2}=\pi $$ ](A448418_1_En_5_Chapter_Equw.gif)

![A448418_1_En_5_Fig2_HTML.gif](A448418_1_En_5_Fig2_HTML.gif)
...
![A448418_1_En_5_Fig3_HTML.gif](A448418_1_En_5_Fig3_HTML.gif)
...
![ $$ P(A)\left(B/A\right) $$ ](A448418_1_En_5_Chapter_Equ6.gif)

(5.2.1)给出

同样，从 B 到 A 的概率质量由

![ $$ P(B)P\left(A/B\right) $$ ](A448418_1_En_5_Chapter_Equ7.gif)

(5.2.2)给出

所以，从(5.2.1)和(5.2.2)的稳态中，我们有

![ $$ P(A)\left(B/A\right)=P(B)P\left(A/B\right) $$ ](A448418_1_En_5_Chapter_Equ8.gif)
...
Metropolis 算法是一种马尔可夫链蒙特卡罗方法，它使用当前接受的状态来确定下一个状态。时间![ $$ \left(t+1\right) $$ ](A448418_1_En_5_Chapter_IEq23.gif)处的样本有条件地依赖于时间 t 处的样本。时间![ $$ \left(t+1\right) $$ ](A448418_1_En_5_Chapter_IEq24.gif)处的建议状态来自正态分布，其平均值等于具有指定方差的时间 t 处的当前样本。一旦抽取，检查在时间![ $$ \left(t+1\right) $$ ](A448418_1_En_5_Chapter_IEq25.gif)和时间 t 的样本之间的概率比。如果![ $$ P\Big({x}^{\left(t+1\right)}/P\left({x}^{(t)}\right) $$ ](A448418_1_En_5_Chapter_IEq26.gif)大于或等于 1，则选择概率为 1 的样本![ $$ {x}^{\left(t+1\right)} $$ ](A448418_1_En_5_Chapter_IEq27.gif)；如果小于 1，则随机选择样本。接下来提到的是详细的实现步骤。

*   从任意一个随机采样点 X <sup>(1)</sup> 开始。
*   选择条件依赖于 X <sup>(1)</sup> 的下一个点 X <sup>(2)</sup> 。可以从一个均值为 X <sup>(1)</sup> 和一些有限方差的正态分布中选择 X <sup>(2)</sup> ，假设 S <sup>(2)</sup> 。因此，![ $$ {X}^{(2)}\sim Normal\ \left({X}^{(1)},{S}^2\right). $$ ](A448418_1_En_5_Chapter_IEq28.gif)良好采样的一个关键决定因素是非常明智地选择方差 S <sup>2</sup> 。方差不应该太大，因为在这种情况下，下一个样本 X <sup>(2)</sup> 停留在当前样本 X <sup>(1)</sup> 附近的机会较小，在这种情况下，高概率区域可能不会被探索太多，因为下一个样本在大多数时间被选择为远离当前样本。同时，方差不应该太小。在这种情况下，下一个样本将几乎总是停留在当前点附近，因此探索远离当前区域的不同高概率区域的概率将降低。
*   一旦 X <sup>(2)</sup> 已经从前面的步骤中生成，则使用一些特殊的试探法来确定是否接受它。
    *   如果比率![ $$ P\left({X}^{(2)}\right)/P\left({X}^{(1)}\right)>=1 $$ ](A448418_1_En_5_Chapter_IEq29.gif)则接受 X <sup>(2)</sup> 并将其作为有效样本点。接受的样本成为生成下一个样本的 X <sup>(1)</sup> 。
    *   如果比率大于从 0 和 1 之间的均匀分布中随机产生的数字，则比率![ $$ P\left({X}^{(2)}\right)/P\left({X}^{(1)}\right)<1,{X}^{(2)} $$ ](A448418_1_En_5_Chapter_IEq30.gif)被接受；即 U [0，1]。

如我们所见，如果我们转向高概率样本，那么我们接受新样本，如果我们转向低概率样本，我们有时接受有时拒绝新样本。如果比率 P(X<sup>(2)</sup>/P(X<sup>(1)</sup>)很小，则拒绝的概率增加。比方说 P(X <sup>(2)</sup> )/P(X <sup>(1)</sup> )的比值= 0.1。当我们从一个均匀分布中产生一个介于 0 和 1 之间的随机数 r <sub>u</sub> 时，那么![ $$ {r}_u>0.1 $$ ](A448418_1_En_5_Chapter_IEq31.gif)的概率是 0.9，这又意味着新样本被拒绝的概率是 0.9。一般情况下，

![ $$ P\left({r}_u>r\right)=1-r $$ ](A448418_1_En_5_Chapter_Equx.gif)

其中 r 是新样本和旧样本的概率之比。

让我们试着凭直觉理解为什么这种试探法适用于马尔可夫链蒙特卡罗方法。根据明细余额，

![ $$ P\left({X}^{(1)}\right)P\left({X}^{(2)}/{X}^{(1)}\right)=P\left({X}^{(2)}\right)P\left({X}^{(1)}/{X}^{(2)}\right) $$ ](A448418_1_En_5_Chapter_Equy.gif)
...
![ $$ P\left({X}_1/{X}_2\right)P\left({X}_1\right)=P\left({X}_2/{X}_1\right)P\left({X}_2\right) $$ ](A448418_1_En_5_Chapter_Equz.gif)
...
![ $$ P\left({X}_1/{X}_2\right)P\left({X}_1\right)=P\left({X}_2/{X}_1\right)P\left({X}_2\right) $$ ](A448418_1_En_5_Chapter_Equab.gif)

将理想转移概率替换为假设转移概率和接受概率的乘积，我们得到

![ $$ T\left({X}_2/{X}_1\right)A\left({X}_2/{X}_1\right)P\left({X}_1\right)=T\left({X}_1/{X}_2\right)A\left({X}_1/{X}_2\right)P\left({X}_2\right) $$ ](A448418_1_En_5_Chapter_Equac.gif)

重新排列这个，我们得到的接受概率比是

![ $$ \frac{A\left({X}_2/{X}_1\right)}{A\left({X}_1/{X}_2\right)}=\frac{T\left({X}_1/{X}_2\right)P\left({X}_2\right)}{T\left({X}_2/{X}_1\right)P\left({X}_1\right)} $$ ](A448418_1_En_5_Chapter_Equad.gif)

满足这一点的一个简单建议由 Metropolis 算法给出为

![ $$ A\left({X}_2/{X}_1\right)=\mathit{\min}\left(1,\frac{T\left({X}_1/{X}_2\right)P\left({X}_2\right)}{T\left({X}_2/{X}_1\right)P\left({X}_1\right)}\right) $$ ](A448418_1_En_5_Chapter_Equae.gif)
...
![ $$ A\left({X}_2/{X}_1\right)=\mathit{\min}\left(1,\frac{P\left({X}_2\right)}{P\left({X}_1\right)}\right) $$ ](A448418_1_En_5_Chapter_Equaf.gif)

的接受概率

如果接受概率为 1，那么我们接受概率为 1 的移动，而如果接受概率小于 1，比如说 r，那么我们接受概率为 r 的新样本，拒绝概率为![ $$ \left(1-r\right) $$ ](A448418_1_En_5_Chapter_IEq32.gif)的样本。以概率![ $$ \left(1-r\right) $$ ](A448418_1_En_5_Chapter_IEq33.gif)拒绝样本是通过将比率与从 0 和 1 之间的均匀分布中随机生成的样本 r <sub>u</sub> 进行比较，并在![ $$ {r}_u>r $$ ](A448418_1_En_5_Chapter_IEq34.gif)的情况下拒绝样本来实现的。这是因为对于均匀分布概率![ $$ P\left({r}_u>r\right)=1-r $$ ](A448418_1_En_5_Chapter_IEq35.gif)，这确保了保持期望的拒绝概率。

在清单 [5-2](#Par74) 中，我们展示了通过 Metropolis 算法从二元高斯分布中进行抽样。

![A448418_1_En_5_Fig4_HTML.gif](A448418_1_En_5_Fig4_HTML.gif)
...
![A448418_1_En_5_Fig5_HTML.gif](A448418_1_En_5_Fig5_HTML.gif)

图 5-5。

Restricted Boltzmann machine visible and hidden layers architecture

让 RBM 的可见单元用向量![ $$ v={\left[{v}_1{v}_2\dots {v}_m\right]}^T\in {\mathrm{\mathbb{R}}}^{m\times 1} $$ ](A448418_1_En_5_Chapter_IEq36.gif)表示，隐藏单元用![ $$ h={\left[{h}_1{h}_2\dots {h}_n\right]}^T\in {\mathrm{\mathbb{R}}}^{n\times 1} $$ ](A448418_1_En_5_Chapter_IEq37.gif)表示。同样，让连接第 I 个可见单元和第 j 个隐藏单元的权重用![ $$ {w}_{ij},\forall i\in \left\{1,2,..m\right\},\forall j\in \left\{1,2,..n\right\} $$ ](A448418_1_En_5_Chapter_IEq38.gif)来表示。设包含权重 w <sub>ij</sub> 的矩阵用![ $$ W\in {\mathrm{\mathbb{R}}}^{m\times n} $$ ](A448418_1_En_5_Chapter_IEq39.gif)表示。

具有隐藏状态 h 和可见状态 v 的联合概率分布的能量由

![ $$ P\left(v,h\right)=\frac{e^{-E\left(h,v\right)}}{Z} $$ ](A448418_1_En_5_Chapter_Equ9.gif)

(5.3.1)给出，其中 E(v，h)是联合配置(v，h)的能量，Z 是归一化因子，通常称为配分函数。这个概率基于玻尔兹曼分布，并假设玻尔兹曼常数和热温度为 1。

![ $$ Z=\sum \limits_v\sum \limits_h{e}^{-E\left(v,h\right)} $$ ](A448418_1_En_5_Chapter_Equ10.gif)

(5.3.2)

联合构型(v，h)的能量 E(v，h)由

![ $$ E\left(v,h\right)=-{b}^Tv-{c}^Th-{v}^T Wh $$ ](A448418_1_En_5_Chapter_Equ11.gif)

(5 . 3 . 3)

![ $$ E\left(v,h\right)=-\sum \limits_{i=1}^m{b}_i{v}_i-\sum \limits_{j=1}^n{c}_j{h}_j-\sum \limits_{j=1}^n\sum \limits_{i=1}^m{v}_i{w}_{ij}{h}_j $$ ](A448418_1_En_5_Chapter_Equ12.gif)
...
![ $$ P\left(v,h\right)=\frac{e^{b^Tv+{c}^Th+{v}^T Wh}}{Z} $$ ](A448418_1_En_5_Chapter_Equ13.gif)
...
![ $$ {\displaystyle \begin{array}{l}P\left(h/v\right)=\frac{P\left(v,h\right)}{P(v)}\\ {}\kern3.5em =\frac{P\left(v,h\right)}{\sum_hP\left(v,h\right)}\\ {}\kern3.5em =\frac{\frac{e^{b^Tv+{c}^Th+{v}^T Wh}}{Z}}{\sum_h\frac{e^{b^Tv+{c}^Th+{v}^T Wh}}{Z}}\\ {}\kern3.5em =\frac{e^{b^Tv+{c}^Th+{v}^T Wh}}{\sum_h{e}^{b^Tv+{c}^Th+{v}^T Wh}}\\ {}\kern3.5em =\frac{e^{b^Tv}{e}^{c^Th}{e}^{v^T Wh}}{e^{b^Tv}{\sum}_h{e}^{c^Th}{e}^{v^T Wh}}\\ {}\kern3.5em =\frac{e^{c^Th}{e}^{v^T Wh}}{\sum_h{e}^{c^Th}{e}^{v^T Wh}}\end{array}} $$ ](A448418_1_En_5_Chapter_Equag.gif)
...
现在，让我们看看分母，它看起来类似于分子，只是具有所有可能的隐藏状态 h 的总和。使用(5.3.6)中的表达式![ $$ {e}^{c^Th}{e}^{v^T Wh} $$ ](A448418_1_En_5_Chapter_IEq42.gif)，分母可以表示为
...
矢量和是指其分量的所有组合的和。每个隐藏单元 h <sub>i</sub> 可以有二进制状态 0 或 1，因此有![ $$ {h}_j\in \left\{0,1\right\}\forall j\in \left(1,2,3,..n\right\} $$ ](A448418_1_En_5_Chapter_IEq43.gif)。因此,( 5.3.7)中对向量 h 的求和可以展开成对应于其每个分量的多个求和:
...
以 v 为条件的 h 的元素的联合概率分布已经被分解成以 v 为条件的彼此独立的表达式的乘积。这导致这样的事实:给定 v，h 的分量(即，![ $$ {h}_i\forall i\in \left\{1,2,\dots n\right\} $$ ](A448418_1_En_5_Chapter_IEq45.gif))是彼此有条件独立的。这给出了我们

![ $$ P\left({h}_1{h}_2..{h}_n/v\right)=\kern0.5em P\left({h}_1/v\right)P\left({h}_2/v\right)..P\left({h}_n/v\right) $$ ](A448418_1_En_5_Chapter_Equ19.gif)
...
替换(5.3.12)中的![ $$ {h}_j=1 $$ ](A448418_1_En_5_Chapter_IEq46.gif)和![ $$ {h}_j=0 $$ ](A448418_1_En_5_Chapter_IEq47.gif)，我们得到
...
(5.3.13)和(5.3.14)的表达式说明了隐藏单元![ $$ {h}_i\forall i\in \left\{1,2,\dots n\right\} $$ ](A448418_1_En_5_Chapter_IEq48.gif)是独立的 sigmoid 单元:
...
![ $$ P\left({h}_j=1/v\right)=\sigma \left({c}_j+\sum \limits_{i=1}^m{v}_i{w}_{ij}\right) $$ ](A448418_1_En_5_Chapter_Equ24.gif)

(5.3.16)其中σ(。)表示 sigmoid 函数，使得

![ $$ \sigma (x)=\frac{1}{\left(1+{e}^{-x}\right)} $$ ](A448418_1_En_5_Chapter_Equam.gif)

以类似的方式进行，可以证明

![ $$ P\left({v}_1{v}_2..{v}_m/h\right)=\kern0.5em P\left({v}_1/h\right)P\left({v}_2/h\right)..P\left({v}_m/h\right) $$ ](A448418_1_En_5_Chapter_Equan.gif)

，这意味着给定可见状态，隐藏单元有条件地彼此独立。由于 RBM 是一个对称的无向网络，像可见单元一样，给定隐藏状态的可见单元的概率可以类似地表示为

![ $$ P\left({v}_i=1/h\right)=\sigma \left({b}_i+\sum \limits_{j=1}^n{h}_j{w}_{ij}\right) $$ ](A448418_1_En_5_Chapter_Equ25.gif)
...
![ $$ L\left(\theta \right)=P\left({v}^{(1)}{v}^{(2)}..{v}^{(m)}/\theta \right) $$ ](A448418_1_En_5_Chapter_Equap.gif)

给出

由于输入的数据点是独立给定的模型，

![ $$ L\left(\theta \right)=P\left({v}^{(1)}/\theta \left)P\right({v}^{(2)}/\theta \right)..P\left({v}^{(m)}/\theta \right)=\prod \limits_{t=1}^mP\left({v}^{(t)}/\theta \right) $$ ](A448418_1_En_5_Chapter_Equ26.gif)

(5.3.18)

取两边的对数得到(5.3.18)中函数的对数似然表达式，我们有

![ $$ C=\log L\left(\theta \right)=\sum \limits_{t=1}^m\log P\left({v}^{(t)}/\theta \right) $$ ](A448418_1_En_5_Chapter_Equ27.gif)

(5.3.19)

将(5.3.19)中的概率按其联合概率形式展开，我们得到

![ $$ {\displaystyle \begin{array}{l}C=\sum \limits_{t=1}^m\log P\left({v}^{(t)}/\theta \right)\\ {}\kern1.5em =\sum \limits_{t=1}^m\log \sum \limits_hP\left({v}^{(t)},h/\theta \right)\\ {}\kern1.5em =\sum \limits_{t=1}^m\log \sum \limits_h\frac{e^{-E\left({v}^{(t)},h\right)}}{Z}\\ {}\kern1.5em =\sum \limits_{t=1}^m\log \frac{\sum_h{e}^{-E\left({v}^{(t)},h\right)}}{Z}\\ {}\kern1.5em =\sum \limits_{t=1}^m\log \sum \limits_h{e}^{-E\left({v}^{(t)},h\right)}-\sum \limits_{t=1}^m\log Z\\ {}\kern1.5em =\sum \limits_{t=1}^m\log \sum \limits_h{e}^{-E\left({v}^{(t)},h\right)}-m\log Z\end{array}} $$ ](A448418_1_En_5_Chapter_Equ28.gif)

(5.3.20)

与(5.3.20)中的第一项不同，配分函数 Z 不受可见层输入 v <sup>(t)</sup> 的约束。z 是 v 和 h 的所有可能组合的能量的负指数之和，因此可以表示为

![ $$ Z=\sum \limits_v\sum \limits_h{e}^{-E\left(v,h\right)} $$ ](A448418_1_En_5_Chapter_Equaq.gif)

用(5.3.20)中的这个表达式替换 Z，我们得到

![ $$ C=\sum \limits_{t=1}^m\log \sum \limits_h{e}^{-E\left({v}^{(t)},h\right)}-m\log \sum \limits_v\sum \limits_h{e}^{-E\left(v,h\right)} $$ ](A448418_1_En_5_Chapter_Equ29.gif)

(5.3.21)

现在，让我们取成本函数相对于组合参数θ的梯度。我们可以认为 C 由两部分组成，![ $$ {\rho}^{+} $$ ](A448418_1_En_5_Chapter_IEq49.gif)和![ $$ {\rho}^{-} $$ ](A448418_1_En_5_Chapter_IEq50.gif)，其中

![ $$ {\rho}^{+}=\sum \limits_{t=1}^m\sum \limits_h{e}^{-E\left({v}^{(t)},h\right)} $$ ](A448418_1_En_5_Chapter_Equar.gif)

![ $$ {\rho}^{-}=m\log \sum \limits_v\sum \limits_h{e}^{-E\left(v,h\right)} $$ ](A448418_1_En_5_Chapter_Equas.gif)

取![ $$ {\rho}^{+} $$ ](A448418_1_En_5_Chapter_IEq51.gif)相对于θ的梯度，我们得到

![ $$ {\nabla}_{\theta}\left({\rho}^{+}\right)=\sum \limits_{t=1}^m\frac{\sum_h{e}^{-E\left({v}^{(t)},h\right)}{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)}{\sum_h{e}^{-E\left({v}^{(t)},h\right)}} $$ ](A448418_1_En_5_Chapter_Equ30.gif)

(5.3.22)

现在我们把![ $$ \frac{\sum_h{e}^{-E\left({v}^{(t)},h\right)}{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)}{\sum_h{e}^{-E\left({v}^{(t)},h\right)}} $$ ](A448418_1_En_5_Chapter_IEq52.gif)简化为分子和分母都除以 Z:

![ $$ {\nabla}_{\theta}\left({\rho}^{+}\right)=\sum \limits_{t=1}^m\frac{\sum_h\frac{e^{-E\left({v}^{(t)},h\right)}}{Z}{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)}{\frac{\sum_h{e}^{-E\left({v}^{(t)},h\right)}}{Z}} $$ ](A448418_1_En_5_Chapter_Equ31.gif)

(5 . 3 . 23)![ $$ \frac{e^{-E\left({v}^{(t)},h\right)}}{Z}=P\left({v}^{(t)},h/\theta \right) $$ ](A448418_1_En_5_Chapter_IEq53.gif)![ $$ \frac{\sum_h{e}^{-E\left({v}^{(t)},h\right)}}{Z}=P\left({v}^{(t)}/\theta \right) $$ ](A448418_1_En_5_Chapter_IEq54.gif)。使用(5.3.23)中概率的这些表达式，我们得到

![ $$ {\displaystyle \begin{array}{l}{\nabla}_{\theta}\left({\rho}^{+}\right)=\sum \limits_{t=1}^m\frac{\sum_hP\left({v}^{(t)},h/\theta \right){\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)}{P\left({v}^{(t)}/\theta \right)}\\ {}\kern5.5em =\sum \limits_{t=1}^m\sum \limits_h\frac{P\left({v}^{(t)},h/\theta \right)}{P\left({v}^{(t)}/\theta \right)}{\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)\\ {}\kern5.5em =\sum \limits_{t=1}^m\sum \limits_hP\left(h/{v}^{(t)},\theta \right){\nabla}_{\theta}\left(-E\left({v}^{(t)},h\right)\right)\ \end{array}} $$ ](A448418_1_En_5_Chapter_Equ32.gif)
...
现在，让我们得到![ $$ {\rho}^{-}=m\log \sum \limits_v\sum \limits_h{e}^{-E\left(v,h\right)} $$ ](A448418_1_En_5_Chapter_IEq59.gif)相对于θ :
...
![ $$ {\nabla}_b\left(-E\left(v,h\right)\right)={\nabla}_b\left({b}^Tv+{c}^Th+{v}^T Wh\right)=v $$ ](A448418_1_En_5_Chapter_Equ37.gif)

(5 . 3 . 29)

![ $$ {\nabla}_c\left(-E\left(v,h\right)\right)={\nabla}_c\left({b}^Tv+{c}^Th+{v}^T Wh\right)=h $$ ](A448418_1_En_5_Chapter_Equ38.gif)

(5 . 3 . 30)

![ $$ {\nabla}_W\left(-E\left(v,h\right)\right)={\nabla}_W\left({b}^Tv+{c}^Th+{v}^T Wh\right)=v{h}^T $$ ](A448418_1_En_5_Chapter_Equ39.gif)
...
吉布斯采样根据其他变量的所有当前值生成变量 x <sub>i</sub> 的下一个值。设抽取的第 t 个样本用![ $$ {x}^{(t)}={\left[{x_1}^{(t)}{x_2}^{(t)}..{x_n}^{(t)}\right]}^T $$ ](A448418_1_En_5_Chapter_IEq61.gif)表示。要生成接下来看到的![ $$ \left(t+1\right) $$ ](A448418_1_En_5_Chapter_IEq62.gif)样本，请遵循以下逻辑:

*   Draw the variable ![ $$ {x_j}^{\left(t+1\right)} $$ ](A448418_1_En_5_Chapter_IEq63.gif) by sampling it from a probability distribution conditioned on the rest of the variables. In other words, draw ![ $$ {x_j}^{\left(t+1\right)} $$ ](A448418_1_En_5_Chapter_IEq64.gif) from

    ![ $$ P\left({x_j}^{\left(t+1\right)}/{x_1}^{\left(t+1\right)}{x_2}^{\left(t+1\right)}..{x_{j-1}}^{\left(t+1\right)}{x_{j+1}}^{(t)}..{x_n}^{(t)}\right) $$ ](A448418_1_En_5_Chapter_Equax.gif)

    So basically, for sampling x <sub>j</sub> conditioned on the rest of the variables, for the ![ $$ j-1 $$ ](A448418_1_En_5_Chapter_IEq65.gif) variables before x <sub> j </sub> their values for the ![ $$ \left(t+1\right) $$ ](A448418_1_En_5_Chapter_IEq66.gif) instance are considered since they have already been sampled, while for the rest of the variables their values at instance t are considered since they are yet to be sampled. This step is repeated for all the variables.If each x <sub> j </sub> is discrete and can take, let’s say, two values 0 and 1, then we need to compute the probability p <sub>1</sub> = ![ $$ P\left({x_j}^{\left(t+1\right)}=1/{x_1}^{\left(t+1\right)}{x_2}^{\left(t+1\right)}..{x_{j-1}}^{\left(t+1\right)}{x_{j+1}}^{(t)}..{x_n}^{(t)}\right) $$ ](A448418_1_En_5_Chapter_IEq67.gif). We can then draw a sample u from a uniform probability distribution between 0 and 1 (i.e., U[0, 1]), and if ![ $$ {p}_1\ge u $$ ](A448418_1_En_5_Chapter_IEq68.gif) set ![ $$ {x_j}^{\left(t+1\right)}=1 $$ ](A448418_1_En_5_Chapter_IEq69.gif), else set ![ $$ {x_j}^{\left(t+1\right)}=0 $$ ](A448418_1_En_5_Chapter_IEq70.gif). This kind of random heuristics ensure that the higher the probability p <sub>1</sub>is, the greater the chances are of ![ $$ {x_j}^{\left(t+1\right)} $$ ](A448418_1_En_5_Chapter_IEq71.gif) getting selected as 1\. However, it still leaves room for 0 getting selected with very low probability, even if p <sub>1</sub> is relatively large, thus ensuring that the Markov Chain doesn’t get stuck in a local region and can explore other potential high-density regions as well. There are the same kind of heuristics that we saw for the Metropolis algorithm as well.
...
![ $$ P\left({x}_2/{x}_1\right)=P\left({x}_1,{x}_2\right)/P\left({x}_1\right) $$ ](A448418_1_En_5_Chapter_Equay.gif)

![ $$ P\left({x}_1/{x}_2\right)=P\left({x}_1,{x}_2\right)/P\left({x}_2\right) $$ ](A448418_1_En_5_Chapter_Equaz.gif)

如果将边际分布 P(x <sub>1</sub> )和 P(x <sub>2</sub> )导出为![ $$ \underset{x_2}{\int }P\left({x}_1,{x}_2\right)d{x}_2 $$ ](A448418_1_En_5_Chapter_IEq73.gif)和![ $$ \underset{x_1}{\int }P\left({x}_1,{x}_2\right)d{x}_1 $$ ](A448418_1_En_5_Chapter_IEq74.gif)，则

![ $$ {x}_2/{x}_1\sim Normal\left(\rho {x}_1,1-{p}^2\right) $$ ](A448418_1_En_5_Chapter_Equba.gif)

![ $$ {x}_1/{x}_2\sim Normal\left(\rho {x}_2,1-{p}^2\right) $$ ](A448418_1_En_5_Chapter_Equbb.gif)

### 块状吉布斯采样

吉布斯抽样有几种变体。块状吉布斯采样就是其中之一。在块吉布斯采样中，一个以上的变量被组合在一起，然后在其余变量的条件下对该组变量一起进行采样，这与单独对单个变量进行采样相反。例如，在受限玻尔兹曼机器中，隐藏的单元状态变量![ $$ h={\left[{h}_1\ {h}_2..{h}_n\right]}^T\in {\mathrm{\mathbb{R}}}^{n\times 1} $$ ](A448418_1_En_5_Chapter_IEq75.gif)可以在可见单元状态![ $$ v={\left[{v}_1\ {v}_2..{v}_m\right]}^T\in {\mathrm{\mathbb{R}}}^{m\times 1} $$ ](A448418_1_En_5_Chapter_IEq76.gif)的条件下一起采样，反之亦然。因此，对于从 P(v，h)上的联合概率分布采样，通过块吉布斯采样，可以通过条件分布 P(h/v)采样给定可见单元状态的所有隐藏状态，并且通过条件分布 P(v/h)采样给定隐藏单元状态的所有可见单元状态。吉布斯采样的![ $$ \left(t+1\right) $$ ](A448418_1_En_5_Chapter_IEq77.gif)迭代的样本可以生成为

![ $$ {h}^{\left(t+1\right)}\sim P\left(h/{v}^{(t)}\right) $$ ](A448418_1_En_5_Chapter_Equbc.gif)

![ $$ {v}^{\left(t+1\right)}\sim P\left(v/{h}^{\Big(t+1}\right) $$ ](A448418_1_En_5_Chapter_Equbd.gif)

因此，![ $$ \left({v}^{\left(t+1\right)},{h}^{\left(t+1\right)}\right) $$ ](A448418_1_En_5_Chapter_IEq78.gif)是迭代![ $$ \left(t+1\right) $$ ](A448418_1_En_5_Chapter_IEq79.gif)时的组合样本。
...
![ $$ {h}^{\left(t+1\right)}\sim P\left(h/{v}^{(t)},\theta \right) $$ ](A448418_1_En_5_Chapter_Equbg.gif)

![ $$ {v}^{\left(t+1\right)}\sim P\left(v/{h}^{\left(t+1\right)},\theta \right) $$ ](A448418_1_En_5_Chapter_Equbh.gif)

让采样更容易的是，隐藏单元的 h <sub>j</sub> s 在给定可见单元状态的情况下是独立的，反之亦然:

![ $$ P\left(h/v\right)=P\left({h}_1/v\right)P\left({h}_2/v\right)..P\left({h}_n/v\right)=\prod \limits_{j=1}^nP\left({h}_j/v\right) $$ ](A448418_1_En_5_Chapter_Equbi.gif)
...
![ $$ P\left({h}_j=1/v\right)=\sigma \left({c}_j+\sum \limits_{i=1}^m{v}_i{w}_{ij}\right) $$ ](A448418_1_En_5_Chapter_Equbj.gif)

该概率可以基于当前值![ $$ v={v}^{(t)} $$ ](A448418_1_En_5_Chapter_IEq80.gif)和模型参数![ $$ c,W\in \theta $$ ](A448418_1_En_5_Chapter_IEq81.gif)来计算。将计算的概率![ $$ P\left({h}_j=1/{v}^{(t)}\right) $$ ](A448418_1_En_5_Chapter_IEq82.gif)与从均匀分布 U[0，1]生成的随机样本 U 进行比较。如果![ $$ P\left({h}_j=1/{v}^{(t)}\right)>u $$ ](A448418_1_En_5_Chapter_IEq83.gif)，则采样![ $$ {h}_j=1 $$ ](A448418_1_En_5_Chapter_IEq84.gif)，否则![ $$ {h}_j=0 $$ ](A448418_1_En_5_Chapter_IEq85.gif)。以这种方式采样的所有这种 h<sub>j</sub><sup>形成组合的隐藏单元状态向量![ $ {h}^{\left(t+1\right)} $ ](A448418_1_En_5_Chapter_IEq86.gif)。</sup>

类似地，给定隐藏单元状态，可见单元是独立的:

![ $$ P\left(v/h\right)=P\left({v}_1/h\right)P\left({v}_2/h\right)..P\left({v}_n/h\right)=\prod \limits_{i=1}^mP\left({v}_i/h\right) $$ ](A448418_1_En_5_Chapter_Equbk.gif)

给定![ $$ {h}^{\left(t+1\right)} $$ ](A448418_1_En_5_Chapter_IEq87.gif)，可以对每个可见单元进行独立采样，以与隐藏单元相同的方式获得组合 v <sup>(t+1)</sup> 。在![ $$ \left(t+1\right) $$ ](A448418_1_En_5_Chapter_IEq88.gif)迭代中生成的所需样本由![ $$ \Big({v}^{\left(t+1\right)},{h}^{\left(t+1\right)} $$ ](A448418_1_En_5_Chapter_IEq89.gif)给出。
...
下图 [5-6](#Fig8) 说明了如何对每个输入数据点 v <sup>(t)</sup> 进行吉布斯采样，以通过点估计获得联合概率分布的期望近似值。吉布斯采样从 v <sup>(t)</sup> 开始，基于条件概率分布 P(h/v <sup>(t)</sup> ，获得新的隐藏状态 h’。如前所述，每个隐藏单元 h <sub>j</sub> 可以被独立采样，然后被组合以形成隐藏状态向量 h’。然后基于条件概率分布 P(v/h’)对 v’进行采样。该迭代过程通常运行几次迭代，最终采样的 v 和 h 作为候选样本![ $$ \left(\overline{v},\overline{h}\right) $$ ](A448418_1_En_5_Chapter_IEq93.gif)。

![A448418_1_En_5_Fig6_HTML.gif](A448418_1_En_5_Fig6_HTML.gif)
...
![A448418_1_En_5_Fig8_HTML.jpg](A448418_1_En_5_Fig8_HTML.jpg)

图 5-8。

Simulated images given the hidden states

![A448418_1_En_5_Fig7_HTML.jpg](A448418_1_En_5_Fig7_HTML.jpg)
...
![A448418_1_En_5_Fig11_HTML.gif](A448418_1_En_5_Fig11_HTML.gif)

图 5-11。

Restricted Boltzmann view for User B

![A448418_1_En_5_Fig10_HTML.gif](A448418_1_En_5_Fig10_HTML.gif)
...
![ $$ P\left({v}_i=1/h\right)=\sigma \left({b}_i+\sum \limits_{j=1}^n{h}_j{w}_{ij}\right) $$ ](A448418_1_En_5_Chapter_Equbt.gif)
...
![ $$ P\left({v_i}^{(k)}=1/h\right)=\frac{e^{\left({b_i}^{(k)}+{\sum}_{j=1}^n{h}_j{w_{ij}}^{(k)}\right)}}{\sum_{l=1}^K{e}^{\left({b_i}^{(l)}+{\sum}_{j=1}^n{h}_j{w_{ij}}^{(l)}\right)}} $$ ](A448418_1_En_5_Chapter_Equ46.gif)

(5.4.1)其中 w <sub>ij</sub> <sup>(k)</sup> 是连接可见单元 I 的第 k 个等级指数和第 j 个隐藏单元的权重，b <sub>i</sub> <sup>(k)</sup> 表示可见单元 I 的第 k 个等级的偏差。

一个联合构形的能量 E(v，h)由

![ $$ E\left(v,h\right)=-\sum \limits_{k=1}^K\sum \limits_{i=1}^m{b_i}^{(k)}{v_i}^{(k)}-\sum \limits_{j=1}^n{c}_j{h}_j-\sum \limits_{k=1}^K\sum \limits_{j=1}^n\sum \limits_{i=1}^m{v_i}^{(k)}{w_{ij}}^{(k)}{h}_j $$ ](A448418_1_En_5_Chapter_Equ47.gif)

(5.4.2)给出

所以，

![ $$ P\left(v,h\right)\propto {e}^{-E\left(v,h\right)}={e}^{\sum \limits_{k=1}^K\sum \limits_{i=1}^m{b_i}^{(k)}{v_i}^{(k)}+\sum \limits_{j=1}^n{c}_j{h}_j+\sum \limits_{k=1}^K\sum \limits_{j=1}^n\sum \limits_{i=1}^m{v_i}^{(k)}{w_{ij}}^{(k)}{h}_j} $$ ](A448418_1_En_5_Chapter_Equ48.gif)

(5.4.3)

给定输入 v 的隐藏单元的概率是

![ $$ P\left( hj=1/v\right)=\frac{e^{\left({c}_j+{\sum}_{i=1}^m{\sum}_{k=1}^K{v_i}^{(k)}{w_{ij}}^{(k)}\right)}}{1+{e}^{\left({c}_j+{\sum}_{i=1}^m{\sum}_{k=1}^K{v_i}^{(k)}{w_{ij}}^{(k)}\right)}} $$ ](A448418_1_En_5_Chapter_Equ49.gif)
...
![ $$ P\left({v_q}^{(k)}/V\right)=\sum \limits_hP\left({v_q}^{(k)},h/V\right)=\frac{\sum_hP\left({v_q}^{(k)},h,V\right)}{P(V)} $$ ](A448418_1_En_5_Chapter_Equ50.gif)

(5.4.5)

因为 P(V)对于所有电影分级 k 是固定的，所以从(5.4.5)我们有

![ $$ {\displaystyle \begin{array}{l}P\left({v_q}^{(k)}/V\right)\propto \sum \limits_hP\left({v_q}^{(k)},h,V\right)\\ {}\kern5em \propto \sum \limits_h{e}^{-E\left({v_q}^{(k)},h,V\right)}\end{array}} $$ ](A448418_1_En_5_Chapter_Equ51.gif)

(5.4.6)

这是一种三向能量配置，可以通过在(5.4.2)中添加 v <sub>q</sub> <sup>(k)</sup> 的贡献来轻松计算，如下所示:

![ $$ E\left({v_q}^{(k)},V,h\right)=-\sum \limits_{k=1}^K\sum \limits_{i=1}^m{b_i}^{(k)}{v_i}^{(k)}-\sum \limits_{j=1}^n{c}_j{h}_j-\sum \limits_{k=1}^K\sum \limits_{j=1}^n\sum \limits_{i=1}^m{v_i}^{(k)}{w_{ij}}^{(k)}{h}_j-\sum \limits_{j=1}^n{v_s}^{(k)}{w_{sj}}^{(k)}{h}_j-{v_s}^{(k)}{b}^{(k)} $$ ](A448418_1_En_5_Chapter_Equ52.gif)

(5.4.7)

将![ $$ {v_q}^{(k)}=1 $$ ](A448418_1_En_5_Chapter_IEq95.gif)代入(5.4.7)中，可以发现![ $$ E\left({v_q}^{(k)}=1,V,h\right) $$ ](A448418_1_En_5_Chapter_IEq96.gif)的值与

![ $$ P\left({v_q}^{(k)}=1,V,h\right) $$ ](A448418_1_En_5_Chapter_Equbu.gif)

成正比

对于等级 K 的所有 K 值，需要计算前面的量![ $$ E\left({v_q}^{(k)}=1,V,h\right) $$ ](A448418_1_En_5_Chapter_IEq97.gif),然后归一化以形成概率。然后，可以取概率最大的 k 值，或者从导出的概率中计算 k 的期望值，如下所示:

![ $$ \widehat{k}=\underset{k}{\underbrace{argmax}}P\left({v_q}^{(k)}=1/V\right) $$ ](A448418_1_En_5_Chapter_Equbv.gif)

![ $$ \hat{k}=\sum \limits_{k=1}^5k\times P\left({v_q}^{(k)}=1/V\right) $$ ](A448418_1_En_5_Chapter_Equbw.gif)

与基于最大概率的评级硬分配相比，得出评级的期望方法提供了更好的预测。

此外，利用评级矩阵 V 来导出用户对特定未评级电影 q 的评级 k 的概率的一种简单方法是，在给定可见评级输入 V 的情况下，首先对隐藏状态 h 进行采样；即绘制![ $$ h\sim P\left(h/V\right) $$ ](A448418_1_En_5_Chapter_IEq98.gif)。隐藏单元是所有电影共有的，因此携带了所有电影的模式信息。从采样的隐藏单元中，我们尝试采样 v<sub>q</sub><sup>【k】</sup>的值；即绘制![ $$ {v_q}^{(k)}\sim P\left({v_q}^{(k)}/h\right) $$ ](A448418_1_En_5_Chapter_IEq99.gif)。这种背靠背采样，首先从![ $$ V\to h $$ ](A448418_1_En_5_Chapter_IEq100.gif)开始，然后从![ $$ h\to {v_q}^{(k)} $$ ](A448418_1_En_5_Chapter_IEq101.gif)开始，相当于采样![ $$ {v_q}^{(k)}\sim P\left({v_q}^{(k)}/V\right) $$ ](A448418_1_En_5_Chapter_IEq102.gif)。我希望这有助于提供一个更容易的解释。

### 深度信念网络

深度信念网络基于受限玻尔兹曼机器，但与 RBN 不同，DBN 有多个隐藏层。通过保持先前![ $$ \left(K-1\ \right) $$ ](A448418_1_En_5_Chapter_IEq103.gif)层中的所有权重不变来训练每个隐藏层 K 中的权重。![ $$ \left(K-1\right) $$ ](A448418_1_En_5_Chapter_IEq104.gif)层中隐藏单元的活动作为第 k 层的输入。在训练期间的任何特定时间，两层都参与学习它们之间的重量联系。学习算法与受限玻尔兹曼机器的算法相同。图 [5-12](#Fig14) 所示为深度信念网络的高层示意图。

![A448418_1_En_5_Fig12_HTML.gif](A448418_1_En_5_Fig12_HTML.gif)
...
假设从 x 到 h 的权重由权重矩阵![ $$ W\in {\mathrm{\mathbb{R}}}^{d\times n} $$ ](A448418_1_En_5_Chapter_IEq108.gif)表示，隐藏单元的偏差由![ $$ b={\left[{b}_1{b}_2\ {b}_3..{h}_d\right]}^T\in {\mathrm{\mathbb{R}}}^{d\times 1} $$ ](A448418_1_En_5_Chapter_IEq109.gif)表示。

类似地，假设从 h 到 y 的权重由权重矩阵![ $$ {W}^{\hbox{'}}\in {\mathrm{\mathbb{R}}}^{n\times d} $$ ](A448418_1_En_5_Chapter_IEq110.gif)表示，输出单元的偏差由![ $$ {b}^{\prime }={\left[\ {b}_1{b}_2\ {b}_3..{b}_n\right]}^T\in {\mathrm{\mathbb{R}}}^{n\times 1} $$ ](A448418_1_En_5_Chapter_IEq111.gif)表示。

隐藏单元的输出可以表示为

![ $$ h={f}_1\left( Wx+b\right) $$ ](A448418_1_En_5_Chapter_Equca.gif)

，其中 f <sub>1</sub> 是隐藏层的逐元素激活函数。根据其用途，激活函数可以是线性的、ReLU 的、sigmoid 的等等。

同样，输出层的输出可以表示为

![ $$ \widehat{y}={f}_2\left({W}^{\prime }h+{b}^{\hbox{'}}\right) $$ ](A448418_1_En_5_Chapter_Equcb.gif)

如果输入特征具有连续的性质，可以最小化基于最小平方的成本函数如下，以基于训练数据导出模型权重和偏差:

![ $$ C=\sum \limits_{k=1}^m{\widehat{y}}^{(k)}-{y^{(k)}}_2^2=\sum \limits_{k=1}^m{\widehat{y}}^{(k)}-{x^{(k)}}_2^2 $$ ](A448418_1_En_5_Chapter_Equcc.gif)

其中![ $$ {\hat{y}}^{(k)}-{x^{(k)}}_2^2 $$ ](A448418_1_En_5_Chapter_IEq112.gif)是重构的输出向量和原始输入向量之间的欧几里德或 l <sup>2</sup> 范数距离，m 是模型被训练的数据点的数量。

如果我们用向量![ $$ \theta =\left[W;b;{W}^{\prime };{b}^{\prime}\right] $$ ](A448418_1_En_5_Chapter_IEq113.gif)表示模型的所有参数，那么成本函数 C 可以相对于模型θ的所有参数最小化，以导出模型

![ $$ \widehat{\theta}=\underset{\theta }{\underbrace{Arg\ \mathit{\operatorname{Min}}}}\ C\left(\theta \right)=\underset{\theta }{\underbrace{Arg\ \mathit{\operatorname{Min}}}}\sum \limits_{k=1}^m{\widehat{y}}^{(k)}-{x^{(k)}}_2^2 $$ ](A448418_1_En_5_Chapter_Equcd.gif)

基于梯度下降的模型学习规则为

![ $$ {\theta}^{\left(t+1\right)}={\theta}^{(t)}-\epsilon {\nabla}_{\theta }C\left({\theta}^{(t)}\right) $$ ](A448418_1_En_5_Chapter_Equce.gif)

，其中ϵ为学习速率，t 代表迭代次数，![ $$ {\nabla}_{\theta }C\left({\theta}^{(t)}\right) $$ ](A448418_1_En_5_Chapter_IEq114.gif)为代价函数在![ $$ \theta ={\theta}^{(t)} $$ ](A448418_1_En_5_Chapter_IEq115.gif)处相对于θ的梯度。

现在，让我们考虑如下几种情况:

*   当隐藏层的维度小于输入层的维度时，即![ $$ \left(d<n\right) $$ ](A448418_1_En_5_Chapter_IEq116.gif)，其中 d 是隐藏层，n 是输入层的维度，则自动编码器作为数据压缩网络工作，将数据从高维空间投影到由隐藏层给出的低维空间。这是一种有损数据压缩技术。它还可以用于降低输入信号中的噪声。
*   当![ $$ \left(d<n\right) $$ ](A448418_1_En_5_Chapter_IEq117.gif)和所有的激活函数都是线性的，那么网络学习进行线性 PCA(主成分分析)。
*   当![ $$ \left(d\ge n\right) $$ ](A448418_1_En_5_Chapter_IEq118.gif)和激活函数是线性的时，网络可以学习一个身份函数，这可能没有任何用处。然而，如果成本函数被正则化以产生稀疏隐藏表示，则网络仍然可以学习数据的有趣表示。
*   输入数据的复杂非线性表示可以通过在网络中有更多的隐藏层和使激活函数非线性来学习。这种模型的示意图如图 [5-14](#Fig16) 所示。当采用多个隐藏层时，必须采用非线性激活函数来学习数据的非线性表示，因为几层线性激活等效于单个线性激活层。

### 用于监督学习的通过自动编码器的特征学习

![A448418_1_En_5_Fig14_HTML.gif](A448418_1_En_5_Fig14_HTML.gif)

图 5-14。

Auto-encoder with multiple hidden layers

当我们处理多个隐藏层时，如图 [5-14](#Fig16) 所示，并且在神经单元中具有非线性激活函数，那么隐藏层学习输入数据的变量之间的非线性关系。如果我们正在处理两个类别的分类相关问题，其中输入数据由![ $$ x\in {\mathrm{\mathbb{R}}}^{6\times 1} $$ ](A448418_1_En_5_Chapter_IEq119.gif)表示，我们可以通过如图 [5-14](#Fig16) 所示训练自动编码器，然后使用第二个隐藏层向量![ $$ {h}^{(2)}\in {\mathrm{\mathbb{R}}}^{3\times 1} $$ ](A448418_1_En_5_Chapter_IEq120.gif)的输出，来学习有趣的非线性特征。h <sup>(2)</sup> 给出的这种新的非线性特征表示可以作为分类模型的输入，如图 [5-15](#Fig17) 所示。当我们感兴趣的输出的隐藏层的维数小于输入的维数时，它相当于非线性版本的主成分分析，其中我们只是消耗重要的非线性特征，而将其余的作为噪声丢弃。

![A448418_1_En_5_Fig15_HTML.gif](A448418_1_En_5_Fig15_HTML.gif)
...
![A448418_1_En_5_Fig16_HTML.gif](A448418_1_En_5_Fig16_HTML.gif)
...
![ $$ KL\left({\rho}_1||{\rho}_2\right)={\rho}_1\log \left(\frac{\rho_1}{\rho_2}\right)+\left(1-{\rho}_1\right)\mathit{\log}\left(\frac{1-{\rho}_1}{1-{\rho}_2}\right) $$ ](A448418_1_En_5_Chapter_Equcf.gif)

给出

从前面的表达式我们可以看出![ $$ {\rho}_1={\rho}_2 $$ ](A448418_1_En_5_Chapter_IEq121.gif)时 KL 散度为 0；即当两种分布相同时。当![ $$ {\rho}_1\ne {\rho}_2 $$ ](A448418_1_En_5_Chapter_IEq122.gif)时，KL 散度随着均值的不同而单调增加。如果ρ <sub>1</sub> 选择为 0.2，那么 KL 散度与ρ <sub>2</sub> 的关系曲线如图 [5-17](#Fig19) 所示。

![A448418_1_En_5_Fig17_HTML.gif](A448418_1_En_5_Fig17_HTML.gif)

图 5-17。

KL divergence plot for mean ![ $$ {\rho}_1=0.2 $$ ](A448418_1_En_5_Chapter_IEq123.gif)

如我们所见，KL 散度在![ $$ {\uprho}_2={\uprho}_1=0.2 $$ ](A448418_1_En_5_Chapter_IEq124.gif)处最小，在![ $$ {\uprho}_2=0.2 $$ ](A448418_1_En_5_Chapter_IEq125.gif)两侧单调增加。在下一节中，我们将使用 KL 散度在稀疏自动编码器的隐藏层中引入稀疏性。
...
设![ $$ h\in {\mathrm{\mathbb{R}}}^{d\times 1} $$ ](A448418_1_En_5_Chapter_IEq126.gif)为输入![ $$ x\in {\mathrm{\mathbb{R}}}^{n\times 1} $$ ](A448418_1_En_5_Chapter_IEq127.gif)的隐藏层 sigmoid 激活，其中![ $$ d>n $$ ](A448418_1_En_5_Chapter_IEq128.gif)。此外，将输入连接到隐藏层的权重由![ $$ W\in {\mathrm{\mathbb{R}}}^{d\times n} $$ ](A448418_1_En_5_Chapter_IEq129.gif)给出，将隐藏层连接到输出层的权重由![ $$ {W}^{\hbox{'}}\in {\mathrm{\mathbb{R}}}^{n\times d} $$ ](A448418_1_En_5_Chapter_IEq130.gif)给出。如果隐藏层和输出层的偏置向量分别由 b 和 b’给出，则以下关系成立:

![ $$ {h}^{(k)}=\sigma \left(W{x}^{(k)}+b\right) $$ ](A448418_1_En_5_Chapter_Equcg.gif)

![ $$ {\hat{y}}^{(k)}=f\left({W}^{\prime }{h}^{(k)}+{b}^{\hbox{'}}\right) $$ ](A448418_1_En_5_Chapter_Equch.gif)

其中 h <sup>(k)</sup> 和![ $$ {\hat{y}}^{(k)} $$ ](A448418_1_En_5_Chapter_IEq131.gif)是隐藏层输出向量和第 k 个输入训练数据点的重构输入向量。相对于模型参数(即 W，W′，b，b′)要最小化的成本函数由

![ $$ C=\sum \limits_{k=1}^m{\hat{y}}^{(k)}-{x^{(k)}}_2^2+\lambda \sum \limits_{j=1}^d KL\left(\rho ||{\hat{\rho}}_j\right) $$ ](A448418_1_En_5_Chapter_Equci.gif)

给出，其中![ $$ {\hat{\rho}}_j $$ ](A448418_1_En_5_Chapter_IEq132.gif)是所有训练样本上隐藏层的第 j 个单元中的平均激活，可以表示如下。同样，h <sub> j </sub> <sup>(k)</sup> 表示第 k 个训练样本在单元 j 处的隐藏层激活。

![ $$ {\widehat{\rho}}_j=\frac{1}{m}\sum \limits_{k=1}^m{h}_j^{(k)} $$ ](A448418_1_En_5_Chapter_Equcj.gif)

![ $$ KL\left(\rho ||{\widehat{\rho}}_j\right)=\rho \log \left(\frac{\rho }{{\widehat{\rho}}_j}\right)+\left(1-\rho \right)\mathit{\log}\left(\frac{1-\rho }{1-{\widehat{\rho}}_j}\right) $$ ](A448418_1_En_5_Chapter_Equck.gif)

通常，ρ被选择为 0.04 至 0.05，使得模型学习产生非常接近 0.04 的平均隐藏层单元激活，并且在该过程中，模型学习隐藏层中输入数据的稀疏表示。

稀疏自动编码器在计算机视觉中用于学习低级特征，这些特征表示自然图像中不同位置和方向的不同种类的边缘。隐藏层输出给出了这些低级特征中的每一个的权重，这些权重可以被组合以重建图像。如果![ $$ 10\times 10 $$ ](A448418_1_En_5_Chapter_IEq133.gif)图像被处理为 100 维输入，并且如果有 200 个隐藏单元，那么将输入连接到隐藏单元的权重-即，W 或隐藏单元到输出重建层 W '-将包括 200 个大小为 100 的图像(![ $$ 10\times 10 $$ ](A448418_1_En_5_Chapter_IEq134.gif))。可以显示这些图像来查看它们所代表的特征的性质。稀疏编码在辅以 PCA 白化时效果很好，我们将在本章稍后简要讨论。
...
![A448418_1_En_5_Fig19_HTML.jpg](A448418_1_En_5_Fig19_HTML.jpg)

图 5-19。

Display of a few encoder/decoder weights as images

![A448418_1_En_5_Fig18_HTML.jpg](A448418_1_En_5_Fig18_HTML.jpg)
...
去噪自动编码器像标准自动编码器一样工作，在隐藏层中进行非线性激活，唯一的区别是，代替原始输入 x 的是 x 的噪声版本，比如说![ $$ \tilde{x} $$ ](A448418_1_En_5_Chapter_IEq135.gif)，被馈送到网络。在计算重建误差时，将输出层的重建图像与实际输入 x 进行比较。其思想是，从噪声数据中学习到的隐藏结构足够丰富，可以重构原始数据。因此，这种形式的自动编码器可以用于减少数据中的噪声，因为它从隐藏层学习数据的鲁棒表示。例如，如果图像由于某种失真而变得模糊，那么可以使用去噪自动编码器来消除模糊。通过引入随机噪声添加单元，可以将自动编码器转换成去噪自动编码器。
...
![A448418_1_En_5_Fig21_HTML.jpg](A448418_1_En_5_Fig21_HTML.jpg)

图 5-21。

Reconstructed Images (without Gaussian Noise) generated by the Denoising Auto Encoder

![A448418_1_En_5_Fig20_HTML.jpg](A448418_1_En_5_Fig20_HTML.jpg)
...
![A448418_1_En_5_Fig23_HTML.jpg](A448418_1_En_5_Fig23_HTML.jpg)

图 5-23。

Reconstructed images without the salt and pepper noise generated by the denoising auto-encoder

![A448418_1_En_5_Fig22_HTML.jpg](A448418_1_En_5_Fig22_HTML.jpg)
...
*   Remove the mean pixel intensity from each image separately. So, if a 2D image is converted into a vector, one can subtract the mean of the elements in the image vector from itself. If each image is represented by the vector ![ $$ {x}^{(i)}\in {\mathrm{\mathbb{R}}}^{nX1} $$ ](A448418_1_En_5_Chapter_IEq136.gif), where i represents the ith image in the training set, then the mean normalized image for x <sup>(i)</sup> is given by

    ![ $$ {x}^{(i)}={x}^{(i)}-\frac{1}{n}\sum \limits_{j=1}^n{x}_j^{(i)} $$ ](A448418_1_En_5_Chapter_Equcl.gif)

*   Once we have the mean normalized images we can compute the covariance matrix as follows:

    ![ $$ C=\frac{1}{m}\sum \limits_{i=1}^m{x}^{(i)}{x^{(i)}}^T $$ ](A448418_1_En_5_Chapter_Equcm.gif)

*   Next, we need to decompose the covariance matrix through singular value decomposition (SVD) as follows:

    ![ $$ C= UD{U}^T $$ ](A448418_1_En_5_Chapter_Equcn.gif)

    In general, SVD decomposes as ![ $$ C= UD{V}^T $$ ](A448418_1_En_5_Chapter_IEq137.gif), but since C is a symmetric matrix, ![ $$ U=V $$ ](A448418_1_En_5_Chapter_IEq138.gif) in this case. U gives the Eigen vectors of the covariance matrix. The Eigen vectors are aligned in a column-wise fashion in U. The variances of the data along the direction of the Eigen vectors are given by the Eigen values housed along the diagonals of D, while the rest of the entries in D are zero since D is the covariance matrix for the uncorrelated directions given by the Eigen vectors.
*   In PCA whitening , we project the data along the Eigen vectors, or one may say principal components, and then divide the projection value in each direction by the square root of the Eigen value—i.e., the standard deviation along that direction on which the data is projected. So, the PCA whitening transformation is as follows:
    ...
    ![ $$ T={D}^{-\frac{1}{2}}{U}^T $$ ](A448418_1_En_5_Chapter_Equco.gif)
...
![A448418_1_En_5_Fig25_HTML.gif](A448418_1_En_5_Fig25_HTML.gif)

图 5-25。

ZCA whitening illustration

![A448418_1_En_5_Fig24_HTML.gif](A448418_1_En_5_Fig24_HTML.gif)
