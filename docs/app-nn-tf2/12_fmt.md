...
如前所述，有两个网络形成了生成性对抗网络：生成者网络和鉴别者网络。这两个网络通过一个潜在的空间相互连接，所有的魔法都在这里发生。换句话说，我们使用发生器网络的输出作为鉴别器网络的输入。让我们深入研究一下生成网络和判别网络，以真正理解 GANs 是如何工作的；见图 12-1：
...
生成对抗网络能够“生成”样本。所以，他们天生就有创造力。这就是为什么艺术和时尚是生成性对抗网络最有前途的领域之一。有了训练有素的 GANs，你可以创作绘画、歌曲、服装，甚至诗歌。事实上，Nvidia 的 `StyleGAN` 网络生成的一幅画“Edmond de Belamy，from La Famille de Belamy”在纽约以 43.25 万美元的价格售出。因此，你可以清楚地看到 GAN 如何有潜力被用于艺术世界。
...
这是我们模型中最不寻常的部分：我们正在设置一个定制的训练步骤。在通过装饰器 `tf.function` 定义自定义 `train_step()` 函数后，我们的模型将基于我们定义的自定义 `train_step()` 函数进行训练。

以下带有过多注释的代码是针对训练步骤的。请仔细阅读评论。
...
现在我们已经用 `tf.function` 装饰器定义了我们的定制训练步骤，我们可以为训练循环定义我们的训练函数了。
...
![图 12-5 在 4 x 4 网格布局中 60 个时期后生成的图像](img/501289_1_En_12_Fig5_HTML.jpg)
...
![图 12-6 GAN 模型生成的最新 PNG 文件的显示。注意，它们与图 12-5 所示的样品相同。因为我们从上一个检查点恢复了模型](img/501289_1_En_12_Fig6_HTML.jpg)

使用 `display_image()` 功能，我们可以显示任何我们想要的图像。除此之外，生成一个动画 GIF 图像来显示我们的模型是如何随着时间的推移而演变的，这不是很酷吗？我们可以使用 `glob` 和 `imageio` 库来实现这一点，它们将所有的 PNG 文件堆积起来，创建一个动画 GIF 文件。以下代码行执行此任务：
...
![图 12-7 从不同时代产生的数字例子。了解 GAN 模型如何随着时间的推移学习生成数字](img/501289_1_En_12_Fig7_HTML.jpg)
