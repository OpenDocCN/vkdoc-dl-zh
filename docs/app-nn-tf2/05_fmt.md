...
![图 5-1 用 TensorFlow 构建的深度学习流水线](img/501289_1_En_5_Fig1_HTML.jpg)
...
导入主库后，我们可以使用加载功能导入 TensorFlow 数据集目录页面中列出的常用库之一，该页面可在 [TensorFlow 数据集目录](http://www.tensorflow.org/datasets/catalog/overview) 访问。
...
除了 TensorFlow 数据集目录，Keras 还提供对其目录中列出的有限数量的数据集的访问，可通过 [Keras 数据集](https://keras.io/api/datasets/) 访问。该目录下可访问的数据集有：
...
[TensorFlow I/O GitHub 存储库](https://github.com/tensorflow/io#tensorflow-io)
...
如前几章所述，Keras 是 TensorFlow 的补充库。此外，TensorFlow 2.0 版采用 Keras 作为内置 API 来构建模型和实现附加功能。
...
Keras `Sequential` API 中的示例模型可以使用 Keras `Functional` API 来构建，如下所示：
```py
...
模型子类化的好处是它是完全可定制的，而它的缺点是实现困难。因此，如果您试图构建奇异的神经网络或进行研究级别的研究，那么模型子类化方法是一条可行之路。但是，如果您可以使用 Keras `Sequential` API 或 Keras `Functional` API 来完成您的项目，您就不应该为模型子类化而烦恼。
...
我们可以通过使用自定义类（`CustomModel`）生成一个对象来完成我们的模型构建，如下所示：
```py
model = CustomModel(name='mnist_model')
```
我们在第10和11章中使用模型子类化。
...
我们可以利用各种预制的估算器，也可以用 `Estimator` API 编写自己的模型。`Estimator` API 比 Keras APIs 有一些优势，比如基于参数服务器的训练和完全的 TFX 集成。然而，Keras APIs 将很快具备这些功能，这使得 `Estimator` API 成为可选的。
...
```
将损失函数、指标和优化器作为对象传递比选项1更灵活，因为我们还可以在对象中设置参数。
...
最新列表可在以下网址找到：[www.tensorflow.org/api_docs/python/tf/keras/optimizers](http://www.tensorflow.org/api_docs/python/tf/keras/optimizers)

您可以通过 `tf.keras.optimizers` 模块选择一个优化器。

##### 损失函数

在开始训练之前，必须设置的另一个重要参数是损失函数。`tf.keras.losses` 模块支持许多适用于分类和回归任务的损失函数。完整列表可在以下网址找到：[www.tensorflow.org/api_docs/python/tf/keras/losses](http://www.tensorflow.org/api_docs/python/tf/keras/losses)

#### `model.fit()`

`model.fit()` 为固定数量的 epoch（数据集上的迭代）训练模型。它需要几个参数，比如 `epochs`、`callbacks` 和 `shuffle`，但是它还必须接受另一个参数：我们的数据。根据问题的不同，这些数据可能是 (i) 仅要素或 (ii) 要素和标签。`model.fit()` 功能的使用示例如下：
```py
...
对于定制培训，我们遵循以下步骤：
*   设置优化器、损失函数和指标。
*   运行一个 `for` 循环来计算 epoch 数。
    *   对每个 epoch 的每个 batch 运行嵌套循环：
...
```
如您所见，这要复杂得多，因此，您应该只在绝对必要时使用定制培训。您也可以自定义个人训练步骤、`model.evaluate()` 功能，甚至 `model.predict()` 功能。因此，TensorFlow 几乎总是为研究人员和定制模型开发人员提供足够的灵活性。在本书中，我们利用了第12章中的定制培训。
...
虽然以前旧的 `HDF5` 格式非常流行，但 `SavedModel` 已经成为 TensorFlow 中保存模型的推荐格式。`HDF5` 和 `SavedModel` 的关键区别在于，`HDF5` 使用对象配置来保存模型架构，而 `SavedModel` 保存执行图。这种差异的实际后果是显著的。`SavedModel` 可以保存定制对象，例如用模型子类化构建的模型或没有原始代码的定制层。为了能够以 `HDF5` 格式保存自定义对象，需要额外的步骤，这使得 `HDF5` 不那么吸引人。
