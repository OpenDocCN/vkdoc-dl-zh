...
*   问题回答：开发能够准确给出给定问题答案的系统（例如 Siri）的任务
...
*   词形还原：有两种流行的方法来提取单词的基本形式。
    *   词形还原：通过使用一个实际的词典（例如通过去掉 `-ing` 后缀将 `游泳` 转换为 `游泳`）来去掉单词的无关紧要的词尾，并返回它们的基本词典形式（即词条）。
    *   词干提取：一种将屈折词或派生词还原为其词根形式的方法。虽然词干提取类似于词形还原，但是使用词干提取生成的词根形式不必是真实的单词（例如，单词 `troubl`、`troubl` 和 `troubl` 被词干提取为 `troubl`）。
*   语法归纳：生成描述其语法的全语言形式语法。
*   词素切分：将单词拆分成最小的有意义的单位（即 `语素`），并识别这些单位的类别。
*   词性标注：确定每个单词的词性类型。常见的词类有名词、动词、形容词、副词、代词、介词、连词、感叹词、数词、冠词或限定词。
*   句法分析：确定给定字符串（如句子）的解析树。解析树是一个有序的、有根的树，它代表了字符串的语法结构。
*   分句：寻找句子边界。一些标点符号，如句号或感叹号，对这项任务很有用。
*   分词：将给定文本分割成单独的单词。这个过程通常用于创建一个单词包（`BOW`）和文本向量化。
*   术语抽取：从给定的语料库中提取相关术语。
...
`NLTK` 是一个为 `NLP` 任务设计的重要 Python 库。`NLTK` 支持基本的 `NLP` 任务，例如文本分类、词干提取和词形还原、标记、解析、标记化，甚至推理。
...
`NLP` 项目中最重要的主题之一是文本向量化。在本案例研究中，我们将参考 Andrej Karpathy 的博文《循环神经网络的不合理有效性》<sup>1</sup>，以及 TensorFlow 团队对这篇博文的实现<sup>2</sup>。

研究表明，用于 `NLP` 的最有效的人工神经网络类型之一是循环神经网络（`RNNs`）。`RNNs` 广泛用于 `NLP` 任务，如机器翻译、文本生成和图像字幕。在 `NLP` 任务中，通常，开发者使用 `NLP` 工具和方法来将文本数据处理成向量，然后将它们馈送到选定的人工神经网络，例如 `RNN`、`CNN`，或者甚至前馈神经网络，以完成任务。在我们的案例研究中，我们也遵循这两个标准化的步骤：（I）将文本处理成向量，（ii）用这些向量训练神经网络。
...
你注意到我没有提到 `NLTK` 库吗？原因是 `TensorFlow` 对 `NLP` 任务的支持也有限，在本案例研究中，结合 `NumPy` 操作，我们能够使用 `TensorFlow` 对数据集进行向量化。这主要是因为我们的语料库非常标准化和干净。如果我们需要一个更复杂的 `NLP` 方法，我们将不得不在更大程度上依赖 `NLTK`、`Pandas` 和 `NumPy` 库。
...
我们的整个语料库可以通过一个名为 `text` 的 Python 变量来访问，现在我们可以开始对它进行向量化了。

### 向量化文本

文本向量化是一种基本的 `NLP` 方法，用于将文本数据转换为机器可以理解的有意义的数字向量。文本向量化有多种方法。在本案例研究中，我们是这样一步步进行的：
*   给每个独特的字符一个索引号。
...
图 9-2 具有四维输入和输出层的 RNN 的例子。注意输入和输出字符之间的延迟
...
图 9-3 训练模型的摘要视图。请注意输出形状中的 64，对于训练后的单个预测，它必须为 1
...
为了能够加载我们的权重并保存我们的训练成绩，我们需要使用以下代码设置和配置一个检查点目录：
...
*   存储结果的空变量
*   手动调整预测可变性的温度值
*   对输出进行去矢量化，并且还将输出再次馈送到模型，用于下一次预测
*   将所有生成的字符连接成一个最终字符串
...
1.  循环神经网络的不合理有效性，可用 [`http://karpathy.github.io/2015/05/21/rnn-effectiveness`](http://karpathy.github.io/2015/05/21/rnn-effectiveness)
