...
*   `Pandas`：创建数据帧并执行基本的数据处理任务
*   `NumPy`：生成 `NumPy` 数组，进行数据处理任务
*   来自 `scikit-learn` 的 `train_test_split`：进行训练和测试分割操作
...
创建模型后，我们可以配置它。由于我们致力于预测一部未上映电影的评分，这更像是一项回归任务。因此，使用均方误差(`MSE`)度量`–`而不是交叉熵度量`–`将是更好的选择。此外，我们还选择 `Adam` optimizer 作为我们的优化器。以下代码完成了所有这些工作:
...
通过下面的代码，我们获得了在初始数据处理步骤中提供给用户的新 ID 号，然后使用`np.hstack()`函数创建一个 `NumPy` 数组，并使用`model.predict()`函数生成预测的电影评分:
...
前面的代码给了我们一个 `NumPy` 数组，其中包含所有电影的标准化评分值。但是我们不需要全部。我们只需要评分最高的前 10 部电影。此外，我们需要他们的身份证号码，以便我们可以映射他们，了解他们有哪些头衔。
...
让我们通过过滤用户观看的前 10 部电影来检查用户已经观看并给出高评分的电影:
