...
我们使用最简单的 `TensorFlow API`——`Keras Sequential API`——因为我们不需要太多的灵活性。因此，我们从 `Keras` 导入 `Sequential` 模型对象，并添加 `Conv2D`、`MaxPooling`、`Flatten`、`Dropout` 和 `Dense` 图层。我们已经介绍了 `Conv2D`、`MaxPooling` 和 `Dense` 层。此外，丢弃层通过在训练时忽略一些神经元来对抗过拟合，而展平层在构建完全连接的层之前将二维数组展平为一维数组。
...
虽然图像没有数字 `5`（五）的好笔迹，但是我们的模型能够将其正确分类。
