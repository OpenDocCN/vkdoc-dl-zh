...
云是一个术语，描述的是由称为数据中心的一组计算机组成的庞大集合。这些集群化的机器可以通过仪表板、命令行界面、`REST API` 和客户端库进行交互。数据中心通常分布在多个地理位置。数据中心的大小超过 10 万平方英尺（而且那些还是较小的尺寸！）。云计算解决方案可以广泛分为三类，即 `公共云`、`私有云` 和 `混合云`。让我们简要讨论一下：

-   `公共云`：`公共云` 是传统的云计算模式，云服务提供商将他们的计算基础设施和产品提供给其他企业和个人（见图 1-1）使用。在 `公共云` 中，云服务提供商负责管理硬件配置和提供服务。

    ![图 1-1 公共云](img/463852_1_En_1_Fig1_HTML.jpg)

-   `私有云`：在 `私有云` 中，组织完全负责其计算基础设施的管理和服务。`私有云` 中的机器可以位于本地，或者可以由云服务提供商托管，但通过私有网络进行路由。

-   `混合云`：`混合云` 是 `公共云` 的成本和效率与 `私有云` 的数据主权和内部安全保证之间的折中方案。许多公司和机构通过使用技术解决方案来促进本地和基于云的基础设施之间数据的轻松迁移和共享，选择 `混合云` 和多云。

### 云计算模型

云计算也被分为三种服务交付模型。它们如图 1-2 所示，其中随着我们接近金字塔的顶端，基础设施抽象的层级逐渐增加：

![图 1-2 云计算模型](img/463852_1_En_1_Fig2_HTML.png)
...
`Google Compute` 提供了一系列产品，如图 2-1 所示，以满足广泛的计算需求。计算产品包括 `Compute Engine`（用于自定义处理的虚拟计算实例）、`App Engine`（用于开发网页、移动和物联网应用的云管理平台）、`Kubernetes Engine`（基于 `Kubernetes` 的自定义 `docker` 容器的编排管理器）、容器注册库（私有容器存储）、`Cloud Functions`（连接或扩展云服务的基于云的函数）和 `Cloud Run`（自动扩展您的无状态容器的托管计算平台）。

![图 2-1 云计算服务](img/463852_1_En_2_Fig1_HTML.jpg)

对于我们的机器学习建模目的，我们将关注 `Compute Engine`。正如在第六章节中看到的，`JupyterLab` 将提供所有相关的工具、包和框架，用于数据分析和建模机器学习和深度学习解决方案。
...
图 2-2 中所示的云存储产品包括 `Cloud Storage`（通用存储平台）、`Cloud SQL`（云管理的 `MySQL` 和 `PostgreSQL`）、`Cloud Bigtable`（`NoSQL` 字节存储）、`Cloud Spanner`（可扩展/高可用事务存储）、`Cloud Datastore`（事务型 `NoSQL` 数据库）和持久磁盘（虚拟机的块存储）。

![图 2-2 云存储产品](img/463852_1_En_2_Fig2_HTML.jpg)

### 大数据和数据分析

`Google Cloud Platform` 提供了一系列无服务器大数据和数据分析解决方案，用于数据仓库、流和批量分析、云管理的 `Hadoop` 生态系统、基于云的消息系统以及数据探索。这些服务提供了从大数据中挖掘/生成实时智能的多个视角。

图 2-3 中显示的大数据服务示例包括 `Cloud BigQuery`（无服务器分析/数据仓库平台）、`Cloud Dataproc`（完全管理的 `Hadoop`/`Apache Spark` 基础设施）、`Cloud Dataflow`（批量/流数据转换/处理）、`Cloud Dataprep`（用于分析清洗非结构化/结构化数据的无服务器基础设施）、`Cloud Data Studio`（数据可视化/报告仪表板）、`Cloud Datalab`（用于机器学习/数据分析的托管 `Jupyter` 笔记本）和 `Cloud Pub/Sub`（无服务器消息基础设施）。

![图 2-3 大数据/分析无服务器平台](img/463852_1_En_2_Fig3_HTML.jpg)
...
![图 2-4 云 AI 服务](img/463852_1_En_2_Fig4_HTML.jpg)
...
`Google Cloud SDK` 还为开发者安装了客户端库，以便通过 `API` 以编程方式与 `GCP` 产品和服务交互。截至撰写本文时，`Go`、`Java`、`Node.js`、`Python`、`Ruby`、`PHP` 和 `C#` 语言已覆盖。预计还将添加更多语言。
...
1.  前往 [`https://cloud.google.com/`](https://cloud.google.com/) 打开账户（见图 3-1）。
    ...
    ![图 3-1 Google Cloud Platform 登录页面](img/463852_1_En_3_Fig1_HTML.png)

2.  填写必要的身份信息、地址和信用卡详情。

3.  在平台上创建账户时，请稍等片刻（见图 3-2）。

    ![图 3-2 创建账户](img/463852_1_En_3_Fig2_HTML.jpg)

4.  账户创建后，我们将看到“欢迎来到 `GCP`”页面（见图 3-3）。

    ![图 3-3 欢迎来到 GCP](img/463852_1_En_3_Fig3_HTML.png)

5.  点击页面左上角的三个横杠图标（如图 3-3 中所示，用圆圈标记），然后点击“主页”（如图 3-3 中所示，用矩形标记）以打开 `Google Cloud Platform` 控制台（如图 3-4）。

    ![图 3-4 GCP 控制台](img/463852_1_En_3_Fig4_HTML.png)
...
![图 3-6 选择项目的范围选择器](img/463852_1_En_3_Fig6_HTML.png)

![图 3-5 选择项目](img/463852_1_En_3_Fig5_HTML.png)

### 访问云平台服务

要访问云平台上的资源，点击窗口右上角的三个短横线。分组的服务提供用于组织资源。例如，在图 3-7 中，我们可以看到 `STORAGE` 下的产品：`Bigtable`、`Datastore`、`Storage`、`SQL` 和 `Spanner`。

![图 3-7 Google Cloud Platform 服务](img/463852_1_En_3_Fig7_HTML.png)

### 账户用户和权限

`GCP` 允许您为特定项目中的每个资源定义安全角色和权限。当项目扩展到多个用户时，此功能特别有用。通过 `IAM & 管理` 标签（见图 3-8 和 3-9）为用户创建新的角色和权限。

![图 3-8 打开 IAM & 管理员](img/463852_1_En_3_Fig8_HTML.png)
...
![图 3-11 云 Shell 接口](img/463852_1_En_3_Fig11_HTML.png)

![图 3-10 启动 Cloud Shell](img/463852_1_En_3_Fig10_HTML.png)

![图 3-9 激活 Cloud Shell](img/463852_1_En_3_Fig9_HTML.jpg)
...
1.  前往 [`https://cloud.google.com/sdk/`](https://cloud.google.com/sdk/) 下载并安装适用于您的机器类型的适当 `Cloud SDK`（参见图 3-12）。
    ...
    ![图 3-12 下载 Google Cloud SDK](img/463852_1_En_3_Fig12_HTML.png)

2.  按照操作系统 (`OS`) 类型的说明安装 `Google Cloud SDK`。安装将安装默认的 `Cloud SDK` 组件。

3.  打开您的操作系统终端应用程序并运行命令 `gcloud init` 以开始授权和配置 `Cloud SDK`。
...
4.  选择配置的名称。在此，它被设置为名称 `your-email-id`。
...
1.  选择用于配置的 Google 账户。浏览器将打开以登录所选账户（参见图 3-13、3-14 和 3-15）。然而，如果希望进行纯终端初始化，用户可以运行 `gcloud init --console-only`。
    ...
    ![图 3-15 Cloud SDK 认证确认页面](img/463852_1_En_3_Fig15_HTML.png)
    图 3-15 Cloud SDK 认证确认页面

    ![图 3-14 认证 Cloud SDK 以访问 Google 账户](img/463852_1_En_3_Fig14_HTML.jpg)
    图 3-14 认证 Cloud SDK 以访问 Google 账户

    ![图 3-13 选择授权 Cloud SDK 配置的 Google 账户](img/463852_1_En_3_Fig13_HTML.png)
    图 3-13 选择授权 Cloud SDK 配置的 Google 账户
...
2.  在 Google 账户的基于浏览器的身份验证后，选择要使用的云项目。
...
![图 4-2 创建存储桶](img/463852_1_En_4_Fig2_HTML.png)
图 4-2 创建存储桶

![图 4-1 云存储控制台](img/463852_1_En_4_Fig1_HTML.png)
图 4-1 云存储控制台

1.  如图 4-1 所示，在云存储仪表板上点击“创建存储桶”。

2.  给存储桶起一个独特的名称（见图 4-2）。GCP 中的存储桶必须有一个全局唯一的名称。也就是说，Google Cloud 上的两个存储桶不能有相同的名称。存储桶的常见命名约定是以组织的域名作为前缀。

3.  选择存储类别。多区域存储类别适用于全球各地频繁访问的存储桶，而冷线存储则更多用于存储备份文件。目前，默认选择是合适的。

4.  点击“创建”以在 Google Cloud Storage 上设置存储桶。
...
![图 4-5 上传成功](img/463852_1_En_4_Fig5_HTML.png)
图 4-5 上传成功

![图 4-4 上传对象](img/463852_1_En_4_Fig4_HTML.png)
图 4-4 上传对象

![图 4-3 一个空的存储桶](img/463852_1_En_4_Fig3_HTML.png)
图 4-3 一个空的存储桶

1.  在图 4-3 中的红色高亮处点击“上传文件”。

2.  从文件上传窗口中选择文件，如图 4-4 所示，点击“打开”。

3.  上传完成后，文件作为 GCS 存储桶中的对象上传（见图 4-5）。

### 从存储桶中删除对象

如图 4-6 所示，点击文件旁边的复选框，然后点击“删除”来从存储桶中删除一个对象。

![图 4-6 删除文件](img/463852_1_En_4_Fig6_HTML.png)
图 4-6 删除文件
...
![图 4-8 删除存储桶](img/463852_1_En_4_Fig8_HTML.png)
图 4-8 删除存储桶

![图 4-7 选择要删除的存储桶](img/463852_1_En_4_Fig7_HTML.png)
图 4-7 选择要删除的存储桶
...
![图 5-1 选择虚拟机实例](img/463852_1_En_5_Fig1_HTML.png)
图 5-1 选择虚拟机实例

点击“创建”以开始部署虚拟机实例的过程（见图 5-2）。

![图 5-3 创建实例的选项](img/463852_1_En_5_Fig3_HTML.png)
图 5-3 创建实例的选项

![图 5-2 开始部署虚拟机实例的过程](img/463852_1_En_5_Fig2_HTML.png)
图 5-2 开始部署虚拟机实例的过程
...
2.  选择实例区域和区域。这是您的计算实例所在的地理区域，而区域是区域内的一个位置。

3.  选择机器类型。这允许对虚拟机的核心、内存和 GPU 进行定制（见图 5-4）。

    ![图 5-4 选择机器类型](img/463852_1_En_5_Fig4_HTML.jpg)
    图 5-4 选择机器类型

4.  选择引导磁盘。此选项选择用于引导的磁盘。此磁盘可以是来自操作系统镜像、应用程序镜像、自定义镜像或镜像快照创建的（见图 5-5）。
...
在本节中，我们将使用基于 Web 的控制台和命令行启动 `JupyterLab` 笔记本实例。
...
1.  前往运行时 ➤ 更改运行时类型（见图 7-3）。
    ...
    图 7-3 Python 3 Notebook

2.  在这里，可以选择将 Python 运行时和硬件加速器更改为 `GPU` 或 `TPU`（见图 7-4）。
