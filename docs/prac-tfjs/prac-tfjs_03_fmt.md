...
在这里，你将编写一个网络应用程序，使用任意数据集训练 k-means 模型，并使用可视化库 **Plotly** 可视化其不同的结果。与之前两个问题中你使用 Layers API 实现算法不同，在这个问题中，你将使用一个名为 **ml5.js** 的外部库。这个库是 TensorFlow.js 的高级抽象，提供了一个简单的接口来访问几个预构建和现成的模型。这个练习灵感来源于 ml5.js GitHub 页面上展示的 k-means 示例.
...
<head>
    ...
    <title>K-means Clustering with ml5.js</title>
    <script src="https://cdn.jsdelivr.net/npm/ml5@0.4.3/dist/ml5.min.js"></script>
    <script src="https://cdn.plot.ly/plotly-latest.min.js"></script>
</head>
<body>
    <div>
        Enter K (1 to 4)
        <input type="range" id="k-range" min="1" max="4">
        <p id="k-value">1</p>
    </div>
    <div id="button"></div>
    <div id="plot"></div>
...
这些图是用来区分图上的簇的。例如，来自簇一的数据点以黑色圆圈表示，来自簇二的数据点以绿色正方形表示。接下来，我们将执行函数 `visualizeResult()`，它将是模型在训练结束时执行的回调函数。