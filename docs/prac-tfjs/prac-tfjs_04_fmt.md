...
你将开发的应用程序是一个由在**修改后的国家标准与技术研究院（NIST）**数据集上训练的 CNN 驱动的手写数字检测应用程序。这个 Web 应用程序有一个画布，用户将在其中绘制一个数字，模型将实时识别。
...
```html
<canvas id="draw-canvas" class="digits-canvas"></canvas>
...
这个练习的 MNIST 数据集包含一个大的精灵文件。因为这个格式，解开数据可能会变得混乱。为了简化任务，我们将使用来自官方 TF.js 示例仓库的脚本，该脚本下载、处理并将数据转换为张量。但我们不会只是复制粘贴，而是要理解它做了什么（然后你可以无顾忌地复制粘贴）。所以，作为第一步，创建一个新的文件并将其命名为 `data.js` 。
...
async function drawData() {
  ...
  for (let i = 0; i < sample.xs.shape[0]; i++) {
    digit = sample.xs
...
![img/494424_1_En_4_Chapter/494424_1_En_4_Fig8_HTML.jpg](img/494424_1_En_4_Chapter/494424_1_En_4_Fig8_HTML.jpg)
...
![img/494424_1_En_4_Chapter/494424_1_En_4_Fig9_HTML.jpg](img/494424_1_En_4_Chapter/494424_1_En_4_Fig9_HTML.jpg)
...
1.  修改网络以降低错误率。
2.  使用数据集拟合一个非卷积神经网络。
3.  在文献中实现一个卷积神经网络架构，例如 AlexNet。
4.  步长和内核大小是什么？它们如何影响张量的输出形状？
5.  卷积层和池化层之间有什么区别？
6.  查阅论文“应用于手写邮编识别的反向传播”（Y. LeCun 等，1989），以了解卷积神经网络首次被提及的情况。