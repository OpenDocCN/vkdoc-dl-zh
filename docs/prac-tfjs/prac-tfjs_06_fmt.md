...
`ToxDet` 是在另一个名为 `Universal Sentence Encoder` (Cer et al., 2018) 或 `USE` 的模型之上进行训练的，`USE` 是一个将文本编码或转换为 512 维嵌入的网络。换句话说，`USE` 将文本转换为 512 个浮点数的数组。为了说明这一点，请参见以下使用 TensorFlow.js 实现 `USE` 的示例：
