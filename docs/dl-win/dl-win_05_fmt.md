...
1. 它使特征维度更小，更易于管理。
2. 通过减少网络中所需的参数和计算量，它减少了过拟合的可能性。
3. 它使网络对输入中的小变换、扭曲和平移不变，这意味着输入中的小变化不会显著影响输出。这允许网络更好地泛化。
4. 它使网络对尺度不变，允许在输入图像的任何位置检测到对象。
