...
为图像构建架构的第一步是获取图像的大小，并确定我们希望网络构建的深度，使得最后一层或输出的感受野等于图像的大小。换句话说，它应该已经看到了图像中的内容，以便回答关于图像的任何问题。如果我们有一个大小为 `56x56x3` 的图像，那么使用的卷积核可以是 `3x3x3x16`。这意味着 `3x3` 的卷积核有三组不同的初始化，并与输入或前一层的通道数相匹配。这是因为它必须知道如何混合需要提取并在网络中使用的特征。公式变为：
