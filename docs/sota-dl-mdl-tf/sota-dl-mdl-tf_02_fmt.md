...
```python
model = Sequential([
    Conv2D(filters=32, kernel_size=(5, 5), activation='relu', input_shape=(180, 180, 3)),
    MaxPooling2D(2),
...
我们本可以使用与上一节相同的模型，但我们使用了一个更少的卷积层，并将 softmax 应用于模型输出。
...
我们展示了使用三种不同技术进行数据增强。Keras 技术实现起来相当简单，但不如直接在图像上应用增强那样灵活。`ImageDataGenerator` 技术实现起来最简单，但仅限于较小的项目。
