...
```python
fash_train_img, fash_train_lbl = float_it(fashion_train_img), float_it(train_lbl)
fash_test_img, fash_test_lbl = float_it(fashion_test_img), float_it(test_lbl)
```
...
```python
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import BatchNormalization, Conv2D, MaxPooling2D, Dropout
```
...
'validation files'.format(len(tfr_filenames), len(training_filenames),
...
'batches per validation run.'.format(BATCH_SIZE, steps_per_epoch, validation_steps))
Listing 5-11
