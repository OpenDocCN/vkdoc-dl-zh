...
[https://github.com/paperd/deep-learning-models](https://github.com/paperd/tensorflow)
...
data, info = tfds.load(name='cats_vs_dogs', with_info=True, try_gcs=True)
```
...
'{0:.0%}'.format(train_num), '{0:.0%}'.format(validation_num), '{0:.0%}'.format(test_num)
```
...
shuffle_files=True, as_supervised=True, try_gcs=True)
```
...
```py
len(list(training_set)), len(list(validation_set)), len(list(test_set))
```
...
t = class_labels[labels[i]] + ' (' + str(labels[i].numpy()) + ')'
plt.title(t)
plt.axis('off')
Listing 4-1
Visualize Examples
...
```py
train_batches = training_set.shuffle(SHUFFLE_SIZE).map(format_image).batch(BATCH_SIZE).cache().prefetch(1)
validation_batches = validation_set.map(format_image).batch(BATCH_SIZE).cache().prefetch(1)
test_batches = test_set.map(format_image).batch(BATCH_SIZE).cache().prefetch(1)
```
...
plt.subplot(rows, cols, i + 1)
plt.imshow(images[i], cmap='bone')
t = class_labels[labels[i]] + ' (' + str(labels[i].numpy()) + ') ' + str(images[i].shape)
plt.title(t)
plt.axis('off')
Listing 4-4
...
def build_model():
model = Sequential([
Conv2D(32, (3, 3), activation='relu', input_shape=in_shape, strides=1, kernel_regularizer='l1_l2'),
MaxPooling2D(2, 2),
...
cat_dog_model.compile(optimizer='adam', loss=loss, metrics=['accuracy'])
```
...
```py
epochs = 10
history = cat_dog_model.fit(train_batches, epochs=epochs, verbose=1, validation_data=validation_batches)
```
...
history = cat_dog_layers.fit(
    train_batches, epochs=epochs, verbose=1, validation_data=validation_batches)
```
...
train_batches = train_shuffle.map(partial(preprocess)).batch(BATCH_SIZE).prefetch(1)
validation_batches = validation_set.map(preprocess).batch(BATCH_SIZE).prefetch(1)
test_batches = test_set.map(preprocess).batch(BATCH_SIZE).prefetch(1)
```
...
n_rows, n_cols, cl, p):
    for i in range(num_images):
        ...
        actual = cl[target[i]]
        title_obj = plt.title(actual + ' (' + pred + ') ')
        if pred == actual:
            pass
        else:
            plt.getp(title_obj, 'text').set_color('r')
        plt.tight_layout()
        plt.axis('off')
```
...
```py
rps_info.features['image'].shape, rps_info.features['label'].shape
```
...
def rotate(image: tf.Tensor) -> tf.Tensor:
    return tf.image.rot90(image, tf.random.uniform(
...
```py
def zoom(image: tf.Tensor, min_zoom=0.8, max_zoom=1.0) -> tf.Tensor:
...
finger_model = Sequential([
    Conv2D(64, 3, activation='relu', input_shape=finger_img_shape,
