...
默认情况下，一个 `tf.data.Dataset` 包含一个 `tf.Tensor` 的字典。让我们看看数据集中的一个示例：
...
几张图像整齐地显示出来。在每张图像下方是标签名称作为字符串和括号中的整数。例如，标签为 4 的图像下方有 4(4)，因为其标签是字符串“4”和整数（4）。
...
for i in range(rows * cols):
    plt.subplot(rows, cols, i + 1)
    plt.imshow(images[i], cmap='bone')
    t = classes[labels[i]] + ' (' + str(labels[i].numpy()) + ')'
    plt.title(t)
    plt.axis('off')
Listing 3-2
...
测试集包含 128 个示例，训练集包含 1,034 个示例。验证集包含 133 个示例。当然，如果您愿意，可以手动以不同的方式分割数据集。
