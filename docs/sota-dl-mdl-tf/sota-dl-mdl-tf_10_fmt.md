...
```py
from tensorflow.keras.layers import BatchNormalization, Conv2D, Conv2DTranspose, LeakyReLU, Dropout
```
...
首先向生成器提供小张量（7 × 7 像素），并将其投影到 128 个维度，形成一个 6,272 维的空间。将 7 × 7 × 128 相乘以获得维空间。目标是增加张量图像的大小以匹配 28 × 28 的 Fashion
...
Only a few epochs are needed to see the generated images start to represent rock, paper, and scissors gestures. However, the generators and discriminators we have built are very simple. To generate realistic images, we need to create a more robust model.

## Summary

In this chapter, we built a GAN with a forward network that produces copies of average quality. For Fashion-MNIST, the copies generated by DCGAN are more realistic. For rock, paper, and scissors, the copies are not impressive. We can try training the model for more epochs, but this requires a significant amount of time and may need more memory than your computer can handle.
