...
在 NLP 中，有两种主要的文本摘要方法：抽取式摘要和生成式摘要。
...
模型被训练用于执行特定任务，并且该模型获得的知识被用于在不同数据集上执行另一任务的过程，称为迁移学习。
...
BART 中的 AR 代表自回归，即模型在每一步都将先前生成的输出作为额外的输入。
...
![图 10-6](img/517793_1_En_10_Fig6_HTML.jpg)
