...
+   `padding`：提供一个整数或两个整数的元组。如果提供一个单独的整数，则表示对高度和宽度应用相同的填充；如果提供一个两个整数的元组，则表示对称高度填充和对称宽度填充。

+   `data_format`：输入图像张量的维度，包括高度、宽度和通道数。
...
+   `filters`：输出张量的维度，卷积中的输出滤波器数量

+   `kernel_size`：卷积发生的内核大小；通常由两个整数的列表指定，分别表示内核的高度和宽度

+   `padding`：指的是输入张量的填充；Valid 表示没有填充，而 Same 指定在高度和宽度上等量填充，以保持与输入相同的输出维度

+   `data_format`：指的是数据格式，取决于通道的位置。如果选择 channels_first 类型，则此格式为（batch_size, channels, height, width），如果选择 channels_last，则输入数据格式为（batch_size, height, width, channels）。默认情况下，它是 Keras 的 `image_data_format`，但如果未指定，则默认为 channels_last。

+   `dilation_rate`：这用于指定膨胀卷积的速率（在我们的当前场景中不是必需的）。

+   `groups`：一个整数，指定了沿着通道轴输入被分割的组数。每个组分别与过滤器进行卷积。输出是所有过滤器沿通道的连接。

+   `activation`：要使用的激活函数，在我们的案例中将使用 `Relu`。

+   `use_bias`：指定我们是否想要使用偏置。

+   `bias_initializer`：这指定了偏置的初始化，类似于 dense 网络。

+   `kernel_initializer`：这控制内核的初始化，例如在我们的案例中是 glorot_uniform。

+   `bias_regularizer`：偏置的正则化器

+   `kernel_regularizer`：内核权重张量的正则化器

+   `activity_regularizer`：这是控制卷积层输出的激活函数的正则化器。

+   `kernel_constraint`：应用于内核的约束。

+   `bias_constraint`：应用于偏置的约束。

在这种情况下，使用具有 `filters`、`kernel_size`、`activation` 和 `input data_format` 的 convolution-2D 层：
...
+   `pool_size`：这表示计算最大值时的窗口大小。如果指定了一个单个值，则对高度和宽度应用相同的维度。如果指定了一个两个整数的元组，则这些对应于池化窗口的高度和宽度。

+   `stride`：这可以是一个整数或两个整数的元组，用于指定池化的步长。当未指定时，默认为 pool_size。

+   `padding`：这与之前类似，其中 Valid 表示没有填充，而 Same 表示等量填充。

+   `data_format`：这决定了数据的格式，可以是 channels_first 或 channels_last 格式。
...
其中 `α` 是学习率，`y` 是折扣因子，`S`、`A` 和 `R` 分别代表状态、动作和奖励空间。`Q(S, A)` 代表从状态 `S` 采取动作 `A` 所检索到的值。在这种学习形式中，代理使用 Q 值策略（贪婪策略）从 `S` 中选择动作 `A`。然后它接收 `R`（奖励）并观察到下一个状态 `S**`**`。因此，对旧状态有依赖性，这对离策略算法是有用的。与 Q 学习相比，我们有 SARSA 算法。SARSA 的更新方程如下：
...
如果我们仔细观察，会发现如果我们无限扩展 ∆[θ] V^π()，很容易发现我们可以通过递归展开过程从起始状态 `s` 转换到任何状态，并通过添加所有访问概率，我们将得到 ∆[θ] V^π(s)。这可以通过重复使用 `πθ ∑[S, R] P(s`|s, a) ∆[θ] V^π (s`)` 来简单地证明，以转换到新的状态 `s`，`s`，以及其他。这种形式可以表示为：

∆[θ] V^π(s) = ∑[S]∑[K] p^π(s →x, k)Φ(s),

其中 `p^π(s →x, k)` 表示从状态 `s` 经过 `k` 步转移到 `x` 的转移概率，`Φ(s)` 表示 `∆[θ] πθQ^π(s, a)`。

现在将其代入策略梯度函数进行梯度上升，我们得到：

∆[θ] J(θ) = ∆[θ] V^π(s)

= ∑[S]∑[K] p^π(s[0] →s, k)Φ(s) 从状态 `S[0]` 开始，通过马尔可夫分布和递归

=∑[S] η(s) Φ(s) 令 `η(s)= ∑[K] p^π(s[0] →s, k)`

=(∑[S] η(s)) [ ∑[S] η(s) /(∑[S] η(s))] Φ(s) 归一化 `η(s)`

α [ ∑[S] η(s) /(∑[S] η(s))] Φ(s) `∑[S] η(s)` 是常数

= ∑[S] d^π(s)∑[A] ∆[θ] πθQ^π(s, a) `d^π(s)= ∑[S] η(s) /(∑[S] η(s))` 稳态分布

这就完成了策略梯度定理的证明，我们可以将方程简化如下：

∆[θ] J(θ) α ∑[S] d^π(s)∑[A] ∆[θ] πθQ^π(s, a)

= ∑[S] d^π(s)∑[A] πθQ^π(s, a)[ ( ∆[θ] πθ )/ πθ]

此修改被添加以产生一种新的通用策略梯度算法形式，该形式使用期望值，利用 `ln(x)` (log[e]x) 的导数是 `1/x` 的事实：
...
*   **状态-动作值：** `∑[t] Q^π(s[t], a[t])`

*   **优势函数：** `∑[t] A^π(s[t], a[t])`

*   **时间差分残差：** `r[t +] V^π(s[t+1]) - V^π(s[t])`

优势函数定义为：

`A^π(s[t], a[t]) = Q^π(s[t], a[t]) - V^π(s[t])`

在我们将要仔细研究的 VPG 中，我们在策略梯度方程中使用优势函数，表示为：

∆[θ] J(θ) = E[∑[t]∆[θ] ln(πθ) A^π(s[t], a[t])]

现在，我们已经看到最优策略试图最大化奖励，梯度用于在高维空间中计算最大值。在使用策略方法的深度强化学习中，这是梯度上升步骤，其本质是随机的，并试图最大化参数 `θ`，如下所示：

`Θ[k+1 =] Θ[k] + α ∆[θ] J(θ)`,

其中 `α` 是学习率。策略梯度根据奖励计算优势函数估计，然后通过普通梯度下降（SGD/Adam）使用我们在神经网络中讨论的二次损失函数来最小化值函数中的误差：

`Φ[k+1] α ∑[S]∑[t] (V^π(s[t]) – R[t] )²`,

其中 `α` 表示比例常数，`Φ[k+1]` 是均方损失上的误差梯度。

为了理解这个深度强化学习-同策略算法，我们将研究 VPG 算法的简单实现。打开 `Policy Gradients.ipynb` 笔记本。我们将使用 OpenAI 的 `CartPole` 环境，就像我们之前的大多数示例一样，并开发该算法。然后我们将在 Gym 中其他可训练的深度强化学习环境中使用它，例如 Atari Games (Pong) 和 `MountainCar`。

我们定义 `PGAgent` 类并声明控制探索-利用比率（`gamma`）、学习率、状态大小、动作大小的初始变量，并初始化数组以包含奖励、动作、状态和概率的列表。这由以下几行表示：
...
现在让我们看看这个算法的模型部分，它负责优势的梯度上升以及最小化值估计误差的梯度下降。在我们的案例中，我们将使用密集 MLP 网络。在这种情况下，我们也可以使用卷积网络。前两个密集层使用 64 个单元，激活函数为 "Relu"，内核初始化函数为 `glorot_uniform`。然后我们有一个密集层，激活函数为 `softmax`，输出维度为 `self.action_size`。由于在 `CartPole` 中动作空间包括向左或向右移动，我们也可以使用 `sigmoid` 激活函数。然后我们有一个 `model.compile` 方法，使用 `Adam` 优化器和分类交叉熵损失。由于此代码旨在为大多数 OpenAI Gym 环境提供策略梯度方法；因此，我们使用 `softmax` 激活函数进行多类分类和交叉熵损失。对于 `CartPole`，我们可以通过将损失更改为 `binary_crossentropy` 并在最后一个密集层使用 `sigmoid` 激活函数来进行更改。
...
下一个方法是 `memory`，它填充动作、奖励和状态的数组。在此方法中，每当选择一个动作时，它就在数组中标记为 1。这可以被认为是一种独热编码机制。
...
在 `act` 方法中，我们使用模型根据优势估计（GAE）函数预测代理应采取的动作。`model.predict` 方法用于预测概率。根据 GAE 策略，选择概率最高的动作作为下一个动作。
...
`discount_rewards` 方法使用探索-利用因子 `gamma` 提供折扣奖励。这是一个通用的折扣奖励策略，将在所有算法中使用。
...
在下一个方法 `train` 方法中，我们将训练神经网络。为此，我们归一化奖励数组。然后我们使用 `model.train_on_batch` 方法训练神经网络，并将输入作为状态（`self.states`），输出作为动作（`self.labels`）。
...
在 `load_model` 和 `save_model` 函数中，我们加载和保存模型的权重，如下所示：
...
This uses the actor critic concept mentioned before with the modification that instead of a value update, it updates the advantage estimate (GAE), which is given by the following equation:
...
In the next case, we will study the DQN implementation, which we had a glimpse of in Chapter 1. Open the `CartPole-Rendering.ipynb` Notebook. As usual we have to import all the necessary libraries and modules from `keras` for building the dense model. We will also import the `ipythondisplay` for visualization along with Tensorboard for visualization of the training phase. We will also have a `deque` data structure to store our states, actions, and rewards for each training phase.
...
The `choose_step` method is used to select a particular action if the value estimate of the action is greater than a certain threshold value denoted by the epsilon.
...
The `preprocess_state` method is used to transform (flatten) the states so that it can be fed into the dense network.
...
The `get_epsilon` and `decay_epsilon` are methods to modify the epsilon decay which will be used in the training step for experience replay.
...
在 `PuppoAgent` 类中，我们声明了环境中存在的变量。该类继承自 `agent` 类。我们定义了变换（Transform），例如目标（target）和狗（Puppo），以及用于关节的变量。这些包括 11 个关节，分别构成下腿、上腿、嘴和躯干（身体）。所有这些关节都与一个转向力（torque）和相应的转向速度（angular velocity）相关联。这些在以下代码行中定义。
...
我们还有控制关节驱动控制器脚本的变量，以及控制指向目标方向的向量。在 `Initialize` 方法中，我们为 `dog` 和 `target` 变量赋值。我们实例化一个关节驱动控制器脚本的实例，并将关节变量与该实例关联。
...
接下来是 `CollectObservations` 方法，它使用向量传感器（Vector Sensors）。为此，我们首先通过 `GetCurrentJointForces()` 方法获取相关的关节力。然后，我们分配传感器来收集信息，例如目标与 Puppo 之间的距离。传感器还收集与身体的角速度、法向速度、法向方向以及向上向量方向相关的信息，以控制作用在 Puppo 身体上的力。然后，我们有一个循环，遍历所有接触地面的关节，并借助 `currentXNormalizedRot` 计算沿各自轴的归一化角度旋转。接着，我们添加一个传感器，记录施加在关节上的当前力，并相对于该关节的最大剪切力限制进行归一化。总结来说，此方法控制与关节、角速度、旋转和力以及身体相对于目标的朝向、距离和方向相关的传感器信息。
...
`RotateBody` 方法用于对 Puppo 施加扭矩或角力。它以决策过程特定步骤的相应动作作为输入。它在 0 和 `maxTurnSpeed` 之间进行线性插值（lerp）。然后，它在旋转的归一化方向和前向向量的方向上施加一个力，并乘以速度。这是通过 `AddForceAtPosition` 方法完成的，该方法应用于身体。
...
接下来，我们有 `OnActionReceived` 重写方法，它将环境中的向量观测分配给相应的关节。我们看到，传感器在训练阶段接收了 20 个向量化观测。现在我们为这个模型使用密集神经网络（dense neural networks），并且只使用光线传感器（ray sensor）信息。
...
接下来是 `FixedUpdate` 方法，我们在此控制模拟循环并跟踪决策计数器；如果计数器的值为 3，则通过 Academy 向外部训练环境发送新请求，以采样一组新的决策。这是借助我们在上一章看到的通信器（communicator）模块完成的。在此方法中，有一个 `UpdateDirToTarget` 方法，它计算从关节身体到相应目标代理的距离。还有一个能量守恒步骤，控制 Puppo 的转弯频率。每当 Puppo 快速多次转弯时，这会添加一个负奖励。此外，对于 Puppo 未能到达目标的时间，我们也有一个惩罚。
...
`OnEpisodeBegin` 重写方法用于在每次训练回合结束时重置环境。此方法重置关节及其上的力，并计算从当前回合到目标的方向。
...
Figure 5-34: Puppo agent being trained in PPO using Tensorflow
...
Figure 5-35: Tensorboard visualization of PPO
...
```markdown
# Interfacing with Unity ML Agents
...
Figure 5-36: Real-time Python interfacing with Puppo agent `Unity.exe`
...
from baselines.ppo2.ppo2 import ppo2
import os
...
if (!string.IsNullOrEmpty(debugCommandLineOverride)
...
void OverrideModel()
{
    ...
    if (nnModel == null)
    {
        overrideError = $"Didn't find a model for behaviorName {behaviorName}. Make sure the behaviorName is set correctly in the commandline and that the model file exists";
    }
    else
    {
...
if schedule == ScheduleType.CONSTANT:
    ...
    raise UnityTrainerException(f"The schedule type {schedule} is not implemented.")
```
...
`create_observation_streams` 使用前面的方法，其中包含用于构建观测流的神经网络架构，用于训练。下一个方法 `create_recurrent_encoder` 是另一个重要的方法，它使用循环神经网络或 LSTM 网络进行基于记忆的处理。我们很快会学习 LSTM；然而，在这种情况下，该方法创建一个神经网络，将部分动作/状态/奖励存储在内存中，并将其用于未来参考。TensorFlow（或 Keras）中的 `BasicLSTMCellMethod` 用于此目的。
...
此脚本是最重要的脚本，因为它控制 Unity ML Agents 中不同在策略/离策略算法的网络架构，我们可以修改这些超参数（例如卷积 2D 神经网络中的内核大小/步长或激活函数）并尝试理解训练的结果。在下一节中，我们将简要查看 PPO 脚本，然后深入研究基于 LSTM 的网络。

### Unity ML Agents 中的 PPO

在 Unity ML Agents 中，我们需要导航到 `mlagents/trainers/ppo` 文件夹，在其中可以看到有两个 Python 脚本：`optimizers` 和 `trainer`。

*   **Trainer Python 脚本**：这控制了 ML Agents 中 PPO 算法的实际实现。它从配置文件（`trainer_config.yaml`）中获取超参数，例如批大小、回合数、策略、学习率、gamma 和 epsilon，并检查其有效性。在此上下文中的一个重要方法是 `_process_trajectory` 方法。该方法控制使用 PPO 策略构建的初始轨迹，并获取遵循该特定策略的值估计（观测、奖励、状态）。它计算并将奖励存储在缓冲区中。之后，它计算 GAE，我们在深度 RL 的 PPO 部分提到过。它根据值估计和特定策略的回报计算优势。
...
这在本地和全局都会计算。`_update_policy` 方法用于通过将回报与旧策略的 GAE 进行比较来更新当前策略。它以小批量训练的方式更新策略。`create_policy` 方法用于创建 PPO 策略，并具有多个参数，例如脑参数、种子和 `is training`（布尔值）——所有这些都派生自 `NNPolicy` 类。`add_policy` 方法将当前训练策略添加到训练器。`discount_rewards` 方法与我们在前面章节中编写的方法相同，用于提供奖励的折扣回报。方法 `get_gae` 返回 GAE 优势估计，用于更新策略，由以下方程表示：
...
*   **Optimizer Python 脚本**：此脚本包含我们在深度 RL 部分创建的 PPO 优化器的复杂变体。它控制损失和学习率，并通过更新策略梯度来微调优化。它包含某些超参数，用于控制 epsilon、beta、最大步数、层数和其他属性。它还包含要使用的神经网络架构类型——是使用循环网络、卷积网络还是密集网络。`create_cc_critic` 方法表示创建连续控制的评论家值网络，在循环网络的情况下，它使用我们提到的 `Model.py` 脚本中的 `create_recurrent_encoder` 方法。
...
`create_dc_critic` 表示离散控制的评论家网络，如果策略是 CNN，则它使用 `Model.py` 脚本中的 `create_visual_observation_encoder` 方法。它也可以使用其他视觉编码器变体，例如 Resnet。在离散情况下，我们取缓冲区的最顶层索引进行分析。
...
`_create_losses` 方法包含我们在深度 RL 的 PPO 部分看到的策略实际裁剪。裁剪基于旧策略的 GAE 回报进行，由以下代码行表示：
