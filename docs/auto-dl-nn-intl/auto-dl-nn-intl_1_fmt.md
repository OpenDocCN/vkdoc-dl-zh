...
![图 1-12](img/526245_1_En_1_Fig12_HTML.jpg)

图 1-12 黑盒函数优化的 NNI 最佳试验
...
![图 1-14](img/526245_1_En_1_Fig15_HTML.jpg)

图 1-14 试验编号面板

概览页面右侧面板包含了对顶级试验的总结（图 1-15）。

![图 1-15](img/526245_1_En_1_Fig14_HTML.jpg)
...
![图 1-16](img/526245_1_En_1_Fig16_HTML.jpg)

图 1-16 指标面板

超参数面板包含最有价值的可视化之一。它显示了输入参数与测试指标之间的关系（图 1-17）。

![图 1-17](img/526245_1_En_1_Fig17_HTML.jpg)

图 1-17 超参数面板

此面板允许超参数数据挖掘，帮助你更好地理解所研究的黑盒函数的本质。我们将在此停留一会儿。选择超参数面板（图 1-18）上的前 5% 试验。

![图 1-18](img/526245_1_En_1_Fig18_HTML.jpg)
...
列表 1-6 黑盒函数优化。`ch1/bbf/custom_search.py`
...
![图 1-19](img/526245_1_En_1_Fig19_HTML.jpg)
...
![图 1-20](img/526245_1_En_1_Fig20_HTML.jpg)
...
有关 NNI 命令行工具的更多信息，请参阅 `nnictl --help`。
...
| `searchSpace` | `YAML` 可选 | 用于内联搜索空间定义的字段。例如，在 `ch1/bbf/search_space.json` 中定义的搜索空间可以设置在 `searchSpace` 字段中：<br>`searchSpace:`<br>`  x:`<br>`    _type: quniform`<br>`    _value: [1, 100, 1]`<br>`  y:`<br>`    _type: quniform`<br>`    _value: [1, 10, 1]`<br>`  z:`<br>`    _type: quniform`<br>`    _value: [1, 10000, 0.01]` |
| `trialCommand` | `str` 必需 | 执行试验的命令。在 Linux 和 macOS 上使用 `python3`，在 Windows 上使用 `python` |
| `trialCodeDirectory` | `str` 可选 默认值：`"."` | 试验目录的路径 |
...
| `debug` | `bool` 可选 默认值：`False` | 启用调试模式 |
| `tuner` | `YAML` 可选 | 指定超参数调整器。详细信息请参阅 `第三章` |
| `assessor` | `YAML` 可选 | 指定评估者。详细信息请参阅 `第三章` |
| `advisor` | `YAML` 可选 | 指定顾问。详细信息请参阅 `第三章` |
| `trainingService` | `YAML` 可选 | 指定训练服务。详细信息请参阅 `第七章` |
| `sharedStorage` | `YAML` 可选 | 指定共享存储。详细信息请参阅 `第七章` |
...
列表 1-7 内嵌 NNI。`ch1/bbf/embedded_nni.py`
...
![图 1-21 NNI. 错误徽章](img/526245_1_En_1_Fig21_HTML.jpg)

在这种情况下，请检查 `~/nni-experiments/<experiment_id>/log/nnictl_stderr.log` 中的错误日志文件。

**NNI 正在启动。实验正在运行，但试验状态为失败**，如图 1-22 所示。

![图 1-22 NNI. 失败的试验](img/526245_1_En_1_Fig22_HTML.jpg)

在这种情况下，请检查试验作业面板中的试验日志，如图 1-23 所示。

![图 1-23 NNI. 试验日志](img/526245_1_En_1_Fig23_HTML.jpg)
