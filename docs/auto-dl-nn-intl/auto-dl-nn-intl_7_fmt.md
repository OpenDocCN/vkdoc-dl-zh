...
Listing 7-1 NNI and TensorBoard. `ch7/tb/trial.py`
...
![](img/526245_1_En_7_Fig7_HTML.png)
...
![](img/526245_1_En_7_Fig8_HTML.png)
...
![](img/526245_1_En_7_Fig9_HTML.png)
...
Listing 7-2 Injecting TensorBoard logging in accuracy method

初始化 `SummaryWriter`：

# 第7章：One-shot NAS 的检查点与恢复
...
![图 7-10 共享存储](img/526245_1_En_7_Fig10_HTML.png)

我们可以做的就是在训练子周期之间导出 `DartsTrainer` 的二进制图像，创建检查点，如图 7-11 所示。

![图 7-11 带有检查点的 One-shot NAS](img/526245_1_En_7_Fig11_HTML.png)
...
`Listing 7-3` 演示了如何实现带有检查点导出的单次 NAS 过程。
...
`Listing 7-3` One-shot NAS with checkpoints. `ch7/one_shot_nas/darts_train_with_checkpoint.py`
...
现在，让我们运行我们之前在 `Listing 7-3` 中检查的脚本（`ch7/one_shot_nas/darts_train_with_checkpoint.py`）。我们可以使用 TensorBoard 可视化 One-shot NAS 过程：

```bash
tensorboard --logdir=ch7/one_shot_nas/runs/
```

现在，我们可以使用以下链接监控 Supernet 训练进度：[http://localhost:6006/#scalars](http://localhost:6006/#scalars)。图 7-12 展示了 TensorBoard 网页。

![图 7-12 Supernet 训练进度](img/526245_1_En_7_Fig12_HTML.jpg)
...
![图 7-13 单次 NAS 训练恢复](img/526245_1_En_7_Fig13_HTML.jpg)
