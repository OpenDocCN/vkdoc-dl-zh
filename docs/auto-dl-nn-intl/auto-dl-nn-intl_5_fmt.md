...
`Listing 5-7` TensorFlow 实现。One-shot NAS。`ch5/naive_one_shot_nas/tf/os_search.py`
...
`Listing 5-8` PyTorch 实现。One-shot NAS。`ch5/naive_one_shot_nas/pt/os_search.py`
...
注：表 5-2 中呈现的结果仅用于对各种架构组合进行排序，以选择最佳方案。这些结果并不表征相应组合的准确性。One-shot 模型通常仅用于对模型空间中的架构进行排序。搜索完成后，最佳性能的架构将从头开始重新训练。
...
图 5-9 LeNet One-shot 模型空间

图 5-9 中描述的模型空间的 TensorFlow NNI 实现在列表 5-9 中提供：`LayerChoice` 和 `InputChoice` 方法实现在 `nni.nas.tensorflow.mutables` 包中：

```py
from nni.nas.tensorflow.mutables import InputChoice, LayerChoice
```

`Listing 5-9` TensorFlow 实现。One-shot LeNet NAS。`ch5/model/lenet/tf_lenet.py`
...
图 5-9 中描述的模型空间的 PyTorch NNI 实现在列表 5-10 中提供：`LayerChoice` 和 `InputChoice` 方法实现在 `nni.retiarii.nn.pytorch` 包中：

```py
from nni.retiarii.nn.pytorch import LayerChoice, InputChoice
```

`Listing 5-10` PyTorch 实现。One-shot LeNet NAS。`ch5/model/lenet/pt_lenet.py`
...
为 `conv1` 和 `conv2` 层设置 `LayerChoice`：
...
2.  控制器实现随机策略，这意味着它以概率进行操作。最有希望的子网络有最高的被选中的概率：

```python
for batch in batches(train_ds):
```

3.  子网络网络使用权重共享技术在一个训练批次上训练一次（即，只有一个训练周期）（图 5-11）：

```python
s ← Ctrl.sample() #picks pseudo-random subnet
```

4.  控制器训练循环。在这个循环中，控制器学习如何找到一个子网络，使其获得最高的奖励，即验证数据集上的准确率：

```python
train_once(subnet, batch)
```

5.  控制器选择子网络并在验证数据集批次上计算其准确率：
...
2.  然后，控制器根据 `θ` 权重生成子网络，并在验证数据集上收集准确率。
3.  基于第 2 步中获得的经验，控制器更新 `θ` 值。

图 5-12 展示了我们之前描述的步骤 1–3。

![图 5-12 ENAS 在行动中。初始周期](img/526245_1_En_5_Fig12_HTML.jpg)

接下来，控制器使用权重共享技术选择最有希望的子网络进行训练，并在验证数据集上测试它们的准确率，如图 5-13 所示。

![图 5-13 ENAS 在行动中。中间训练](img/526245_1_En_5_Fig13_HTML.jpg)

在分别训练 Supernet 层并更新 `θ` 经验后，控制器收敛到一些它认为最佳的子网络。图 5-14 说明了这一点。

![图 5-14 ENAS 在行动中。结束训练](img/526245_1_En_5_Fig14_HTML.jpg)
...
`Listing 5-11` 展示了使用 `TensorFlow` `LeNet` 模型空间应用 `ENAS`。
...
`Listing 5-11 ENAS TensorFlow. ch5/enas/enas_tf_search.py`
...
`Listing 5-11` 返回以下最佳模型作为 `ENAS` 算法的结果：
...
`Listing 5-12` 展示了使用 `PyTorch` `LeNet` 模型空间应用 `ENAS`。
...
`Listing 5-12 ENAS PyTorch. ch5/enas/enas_pt_search.py`
...
`Listing 5-12` 返回以下最佳模型作为 `ENAS` 算法的结果：
...
`Listing 5-16` `GeneralSupernet`. PyTorch. `ch5/model/general/pt_general.py`
...
`Listing 5-17` `GeneralSupernet` DARTS. PyTorch. `ch5/cifar10/darts_pt.py`
...
![图 5-21 DARTS GeneralSupernet 最佳架构](img/526245_1_En_5_Fig21_HTML.png)

使用 ENAS (图 5-20) 和 DARTS (图 5-21) 获得的架构相似。它们倾向于使用 `SepConvBranch(5)` 操作并共享 `Cell0` 输出。ENAS 最佳架构和 DARTS 最佳架构分别达到 91.2% 和 92.8% 的准确率。但如果我们增加 `GeneralSupernet` 中的细胞数量 (`num_cells`)，可以进一步提高准确率。这将使搜索时间更长，但将导致更精确的目标架构。美妙的是，我们可以使用相同的 `GeneralSupernet` 和 One-shot 算法来解决任何模式识别问题。这为我们解决典型的深度学习问题提供了一个通用方法。绝对的一步 NAS 是自动深度学习最显著的成就之一。
