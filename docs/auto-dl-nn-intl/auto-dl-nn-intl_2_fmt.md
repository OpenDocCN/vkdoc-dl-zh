...
`Listing 2-1` 中显示的模型有 2 个超参数和 89 个模型参数。超参数通常直接影响模型参数的数量。实际上，在 `Listing 2-1` 中，`l1_dim`和`l2_dim`超参数设置了权重矩阵和偏置向量的维度。`图 2-1` 说明了超参数对 FCNN 模型架构及其参数的影响。
...
我们正在解决经典的二元分类问题，即我们必须确定评论是负面还是正面。然后我们可以应用以下预处理，如 `图 2-3` 所示。
...
样本策略的实现位于 `nni.parameter expressions` 中。您可以手动探索搜索空间采样策略，如 `Listing 2-2` 所示。
...
之后，您可以使用 `quniform` 方法在 `图 2-5` 中观察生成的样本。
...
![图 2-6 随机搜索调优器](img/526245_1_En_2_Fig6_HTML.png)
...
![图 2-7 网格搜索调优器](img/526245_1_En_2_Fig7_HTML.png)
...
```yaml
# Listing 2-4 Experiment configuration. ch2/experiment_pattern/config.yml
trialConcurrency: 1
...
```python
# Listing 2-5 DummyModel class. ch2/experiment_pattern/model.py
...
```python
# Listing 2-6 Trial script pattern. ch2/experiment_pattern/trial.py
import os
import sys
import nni
```
...
![图 2-8 实验文件结构](img/526245_1_En_2_Fig8_HTML.png)
...
![图 2-9 MNIST 数据库](img/526245_1_En_2_Fig9_HTML.jpg)

MNIST 数据集是一组 28×28 的灰度图像。因此，每个数据集对象是一个 (28, 28, 1) 张量。让我们检查这个数据集的几个样本。

```python
# Listing 2-7 MNIST dataset samples. ch2/lenet_hpo/mnist_dataset.py
...
![图 2-10 MNIST 样本](img/526245_1_En_2_Fig10_HTML.jpg)
...
![图 2-11 LeNet 架构](img/526245_1_En_2_Fig11_HTML.jpg)
...
```json
// Listing 2-8 LeNet HPO search space. ch2/lenet_hpo/search_space.json
{
    "filter_size": {
...
```python
# Listing 2-9 LeNet. TensorFlow implementation. ch2/lenet_hpo/tf_lenet_model.py
...
**Listing 2-10** `NNI` trial script with `TensorFlow` `LeNet` implementation. `ch2/lenet_hpo/tf_trial.py`
...
**Listing 2-11** `LeNet`. `PyTorch` implementation. `ch2/lenet_hpo/pt_lenet_model.py`
...
**Listing 2-12** `NNI` trial script with `PyTorch` `LeNet` implementation. `ch2/lenet_hpo/pt_trial.py`
...
**Listing 2-13** `LeNet` `HPO` Experiment configuration. `ch2/lenet_hpo/config.yml`
...
![LeNet HPO 最佳 1%试验的超参数面板](img/526245_1_En_2_Fig12_HTML.jpg)
**图 2-12** LeNet HPO 最佳 1%试验的超参数面板
...
**Listing 2-14** LeNet failed predictions. `ch2/lenet_hpo/display_mnist_failed_predictions.py`
...
![LeNet 预测失败](img/526245_1_En_2_Fig13_HTML.jpg)
**图 2-13** LeNet 预测失败
...
![激活设计超参数](img/526245_1_En_2_Fig14_HTML.png)
**图 2-14** 激活设计超参数
...
![Dropout 设计超参数](img/526245_1_En_2_Fig15_HTML.png)
**图 2-15** Dropout 设计超参数
...
**Listing 2-15** LeNet Upgrade HPO search space. `ch2/lenet_upgrade/search_space.json`
...
**Listing 2-16** LeNet Upgrade. TensorFlow implementation. `ch2/lenet_upgrade/tf_lenet_upgrade_model.py`
...
**Listing 2-17** NNI trial script with TensorFlow LeNetUpgrade implementation. `ch2/lenet_upgrade/tf_trial.py`
...
**Listing 2-18** LeNet Upgrade. PyTorch implementation. `ch2/lenet_upgrade/pt_lenet_upgrade_model.py`
...
![LeNet 升级 HPO 前 1%试验的超参数面板](img/526245_1_En_2_Fig16_HTML.jpg)
...
![升级版 LeNet 模型失败](img/526245_1_En_2_Fig17_HTML.jpg)
...
![人马数据集](img/526245_1_En_2_Fig18_HTML.jpg)
...
![特征提取和决策制定组件](img/526245_1_En_2_Fig19_HTML.png)
...
- `with_pool`: `Conv2D(kernel_size=5, filters=16) → Activation → MaxPool2D(pool_size=3)`
- `none`: 标识符
- `simple`: `Conv2D(kernel_size=3, filters=8) → Activation`

最后，LeNet 进化特征提取组件将呈现如下形式：`Conv2D(kernel_size=5, filters=16) → Activation → MaxPool2D(pool_size=3) → Conv2D(kernel_size=3, filters=8) → Activation`。

我们可以将特征提取序列视为 LeNet 进化模型的构建块，如图 2-20 所示。

![LeNet 进化特征提取组件](img/526245_1_En_2_Fig20_HTML.png)
...
![LeNet 进化决策组件](img/526245_1_En_2_Fig21_HTML.png)
...
| `l2_size` | 第二个线性层的输出大小 | 0, 512, 1024. If 0 value is chosen, then the second linear layer is skipped |
...
- `Conv2D(kernel_size, filters) → Activation → MaxPool2D(pool_size)`
...
`Listing 2-22` 展示了使用 TensorFlow 实现的 `LeNet Evolution` 模型的实现。
...
由于 `LeNetEvolutionModel` 已完成，我们可以使用 `Listing 2-23` 实现 NNI 试验脚本。
...
在本节中，我们将检查使用 PyTorch Lightning 实现的 `LeNet Evolution` 模型。PyTorch Lightning 是一个无缝的 PyTorch 包装器，有助于消除 PyTorch 代码的样板。它更简洁，更适合此类任务。`Listing 2-24` 展示了基于 PyTorch Lightning 的 `LeNet Evolution` 模型。
...
# 第2章 LeNet Evolution 模型与 HPO 实验
...
由于 `LeNetEvolutionModel` 已完成，我们可以使用 `列表 2-15` 实现 NNI 试验脚本。
...
`图 2-22` 显示，本节中我们为人类和马匹分类构建的模型位于原始 `LeNet` 模型和 `AlexNet` 模型之间。我们的模型在测试中表现出 `99.41%` 的显著准确率。但更重要的是，它是完全自动构建的，得益于 `HPO` 技术和 `NNI` 工具！我们没有进行任何复杂的计算或分析。我们仅仅构建了一个灵活的 `LeNet 进化` 模型，其架构依赖于传递的超参数。结果，我们得到了一个完全适应解决特定任务的独特模型。这些结果证实了 `HPO` 方法解决深度学习问题的承诺。
