...
![图 4-1 数据流图](img/526245_1_En_4_Fig1_HTML.png)
...
![图 4-2 数据流图模型空间](img/526245_1_En_4_Fig2_HTML.png)
...
3.  接下来，我们选择一个探索模型空间的搜索策略：

    ```py
    search_strategy = strategy.Random(dedup = True)
    ```

4.  我们使用定义的模型空间、评估器和搜索策略初始化 `Retiarii` 实验：
...
7.  返回最佳结果。在退出之前，我们可以在 `WebUI` 中分析实验的结果：
...
![图 4-3 Trial jobs 面板](img/526245_1_En_4_Fig3_HTML.jpg)
...
![图 4-7 Retiarii 框架](img/526245_1_En_4_Fig7_HTML.png)

让我们逐一详细查看这些组件。

### 基础模型

`基础模型` 是从中做出所有可能的架构修改的起点。例如，`MNIST` 问题 `NAS` 的基模型可以表示为如列表 4-3 所示。
...
`基础模型` 是一个单一模型。为了创建模型空间，我们必须向基础模型添加变异器。每个变异器提供了一种改变基础模型的方法。所有应用于基础模型的可能变异构成了模型空间。`NNI` 提供了以下变异操作：`LayerChoice`、`ValueChoice`、`InputChoice` 和 `Repeat`。
...
![正则化进化策略算法图](img/526245_1_En_4_Fig14_HTML.png)
...
更多细节，请参阅描述正则化进化方法的原始论文：[arXiv:1802.01548 - 正则化进化方法论文](https://arxiv.org/abs/1802.01548)。

### TPE 策略

TPE 策略 (`nni.retiarii.strategy.TPEStrategy`) 是基于树结构帕累托估计器的序列模型优化方法。它与我们第三章节中研究的 TPE 调谐器作用相同。

更多细节，请参阅描述 TPE 方法的原始论文：[NeurIPS 2011 Paper](https://papers.nips.cc/paper/2011/file/86e8f7ab32cfd12577bc2619bc635690-Paper.pdf)。
...
![RL 策略实际应用图](img/526245_1_En_4_Fig15_HTML.png)
...
更多细节，请参阅描述基于强化学习的神经架构搜索的原论文：[arXiv:1611.01578 - 基于强化学习的神经架构搜索论文](https://arxiv.org/pdf/1611.01578.pdf)。
...
![CIFAR-10 数据集样本图](img/526245_1_En_4_Fig16_HTML.jpg)
...
![特征提取块空间图](img/526245_1_En_4_Fig17_HTML.png)
...
![完全连接块空间图](img/526245_1_En_4_Fig18_HTML.png)
...
![LeNet NAS 模型空间图](img/526245_1_En_4_Fig19_HTML.png)
...
`Listing 4-10` `LeNet NAS Experiment. ch4/retiarii/cifar_10_lenet/run_cifar10_lenet_experiment.py`
...
注意：在 Intel Core i7 配备 CUDA (GeForce GTX 1050) 的系统上运行时间约为 20 小时。
...
![LeNet NAS 最佳模型架构](img/526245_1_En_4_Fig20_HTML.jpg)

图 4-20 LeNet NAS 最佳模型架构
...
![瓶颈块空间](img/526245_1_En_4_Fig21_HTML.png)

图 4-21 瓶颈块空间

ResNet 的另一个组件是残差单元。残差单元由一系列瓶颈块组成。残差单元中瓶颈序列的最优长度取决于具体的数据集。图 4-22 展示了残差单元的空间。

![残差单元空间](img/526245_1_En_4_Fig22_HTML.png)
...
![ResNet NAS 空间](img/526245_1_En_4_Fig23_HTML.png)

图 4-23 ResNet NAS 空间

`Listing 4-11` 定义了 ResNet 模型空间中的瓶颈块。
```py
import nni.retiarii.nn.pytorch as nn
class Bottleneck(nn.Module):
```

`Listing 4-11` `ResNet NAS Bottleneck block. ch4/retiarii/cifar_10_resnet/bottle_neck.py`
...
**Listing 4-17** NAS configuration file
...
**注意**：在 Intel Core i7 和 CUDA（GeForce GTX 1050）上持续约 2 小时。
...
![最佳神经网络架构](img/526245_1_En_4_Fig25_HTML.png)
