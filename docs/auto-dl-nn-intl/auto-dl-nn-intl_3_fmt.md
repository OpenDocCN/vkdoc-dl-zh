...
检查 SMBO 方法在实际操作中的表现。假设我们有一些获得的试验结果：`(x[1], f(x[1]))`, `(x[2], f(x[2]))`, `(x[3], f(x[3]))`，如 `图 3-14` 所示。
![图 3-14 试验结果](img/526245_1_En_3_Fig14_HTML.png)

下一步是根据数据创建一个概率函数 `p(y|x)`：`(x[1], f(x[1]))`, `(x[2], f(x[2]))`, `(x[3], f(x[3]))`。`p(y|x)` 被称为目标（或黑盒）函数的“代理”。代理函数确定了目标（或黑盒）函数在搜索空间中任何元素 `x[i]` 的概率分布。这意味着对于任何 `x[i]`，我们可以说以 `p` 的概率，值 `f(x[i]) = y[i]` 落在 `(a, b)` 区间内。这一概念在 `图 3-15` 中得到了演示。
![图 3-15 `x[i]` 和 `x[j]` 的概率分布](img/526245_1_En_3_Fig15_HTML.png)

拥有代理函数 `p(x|y)`，我们可以在整个搜索空间上外推它。`图 3-16` 给出了代理模型的视觉描述：
![图 3-16 三次试验的代理模型：`(x[1], f(x[1]))`, `(x[2], f(x[2]))`, `(x[3], f(x[3]))`](img/526245_1_En_3_Fig16_HTML.png)
...
```
EIy* = ∫_{-∞}^{+∞} max(y* - y, 0) p(y|x) dy
```

如果我们假设 `f(x[2]) = y[2]`，那么 SMBO 将选择 `x[4]` 作为下一个试验参数，如果 `EIy2` 在 `x[4]` 时达到最大值。`图 3-17` 说明了 SMBO 算法选择的下一个试验。
![图 3-17 预期改进](img/526245_1_En_3_Fig17_HTML.png)

在选择 `x[4]` 作为下一个试验值后，我们评估 `f(x[4])` 并根据新的数据重建代理模型：`(x[1], f(x[1]))`, `(x[2], f(x[2]))`, `(x[3], f(x[3]))`, `(x[4], f(x[4]))`，如 `图 3-18` 所示。
![图 3-18 四个试验的代理模型：`(x[1], f(x[1]))`, `(x[2], f(x[2]))`, `(x[3], f(x[3]))`, `(x[4], f(x[4]))`](img/526245_1_En_3_Fig18_HTML.png)
...
2. 接下来，调谐器根据某些分位数 - `γ` 对执行试验进行排序，并将它们分为“良好”组和“不良”组。第一个组，“良好”组，包含给出最佳结果的试验，而“不良”组包含所有其他试验。

`图 3-19` 描述了经过前两个步骤后的 TPE 模型。
![图 3-19 TPE 调谐器。“良好”和“不良”分离](img/526245_1_En_3_Fig19_HTML.png)

3. 使用帕累托估计器（也称为核密度估计器）分别计算“不良”和“良好”组的密度 `l(x)` 和 `g(x)`。帕累托估计器是现有数据点中心核的平均值。
4. 之后，TPE 调优器根据 `g(x)` 密度函数生成 `n` 个随机候选者。这些候选者根据 `g(x)/l(x)` 比率排序，第一个被选为下一个试验。这意味着 TPE 调优器允许在“好”试验更常见的区域选择随机候选者。同时，所有候选者都根据 `g(x)/l(x)` 排序，这意味着具有高密度“好”试验和低密度“坏”试验的候选者将被选中。这种方法在探索和利用之间取得了良好的平衡。

`图 3-20` 说明了选择下一个试验参数的算法。
![图 3-20 TPE 调优器。下一个候选选择](img/526245_1_En_3_Fig20_HTML.png)

5. 重复步骤 2-5，直到达到最大试验次数。
...
- `tpe_args.gamma`:
  `类型`：`float`
  `默认值`：`0.25`
  控制多少个试验被认为是“好”的。代表步骤 2 中的 `γ` 参数。
...
我们可以从 `列表 3-7` 中看到 TPE 调优器是如何优化 `holder_function` 的。
...
# Listing 3-7 TPE Tuner
```

实验完成后，我们可以分析 TPE 调优器的搜索进度。
![图 3-21 TPE 调优器。生成：1](img/526245_1_En_3_Fig21_HTML.jpg)

`图 3-21` 显示，点的分布更像是随机散布，这在调谐器设置中是有意义的，因为我们指定了 `'n_startup_jobs': 20`，这意味着前 20 次试验将是完全随机的。
![图 3-22 TPE Tuner. 上一个生成](img/526245_1_En_3_Fig22_HTML.jpg)

但正如我们在 `图 3-22` 中看到的那样，TPE Tuner 由于概率探索模型，找到了 `holder_function` 的全局最大值。
...
GP Tuner 在并行化问题上遭受了很多困扰。如果你以并发模式（即 `trial_concurrency` > 1）运行实验，多个进程会同时根据相同的历史数据决定它们的下一个试验候选者。因此，不同的进程会在同一时间测试相同的参数。这是所有 SMBO 调谐器的一个大问题。但是，TPE Tuner 可以通过常数谎言技术绕过这个问题，而对于 GP Tuner，这个问题仍然非常严重。在 `图 3-23` 中，我们可以看到 `trial_concurrency = 8` 时的 GP Tuner 的试验指标面板。它显示 GP Tuner 包含相同试验的块，这并不能以任何方式加快探索搜索空间的过程。
![图 3-23 GP Tuner. 并发问题](img/526245_1_En_3_Fig23_HTML.jpg)
...
![图 3-24 GP Tuner. Holder 的黑色盒函数优化](img/526245_1_En_3_Fig24_HTML.jpg)
...
**Listing 3-9** `Custom Tuner. ch3/tuners/custom_tuner/custom_tuner_sample.py`

Tuner 与 Experiment 的交互方式如下：
1. 实验在启动时调用 `update_search_space`。
2. 实验请求通过调用 `generate_parameters` 来搜索空间参数。
3. 实验将 Trial 结果返回给调用 `recieve_trial_result` 的 Tuner。

（步骤 2 和 3 会重复进行，直到达到 `max_trial_number` 或实验停止。）

图 3-25 展示了 Tuner–Experiment 交互作为序列图。
![图 3-25 Tuner–Experiment 序列图](img/526245_1_En_3_Fig25_HTML.jpg)
...
![图 3-26 新进化调谐器](img/526245_1_En_3_Fig26_HTML.png)
...
**Listing 3-10** `Ackley’s function. ch3/bbf/ackley.py`
...
![图 3-27 Ackley 函数](img/526245_1_En_3_Fig27_HTML.jpg)
...
**Listing 3-11** `NewEvolutionTuner. ch3/tuners/custom_tuner/evolution_tuner.py`
...
调谐器有两个参数，`optimize_mode` 和 `population_size`：
...
![图 3-28](img/526245_1_En_3_Fig28_HTML.jpg)

图 3-28 `NewEvolutionTuner` 种群访问过的位置

图 3-29 中的“Trial Metric”面板展示了“爬山法”的实际应用。这种方法的主要问题是它很快就能找到一个局部最大值并停止探索其他搜索空间区域。

![图 3-29](img/526245_1_En_3_Fig29_HTML.jpg)
...
![图 3-30](img/526245_1_En_3_Fig30_HTML.png)
...
![图 3-31](img/526245_1_En_3_Fig31_HTML.png)

图 3-31 HPO 早停与训练早停对比

而图 3-32 展示了相反的情况。训练过程开始恶化，训练早停算法终止了训练过程。相比之下，HPO 早停算法可能会认为当前的试验非常有希望，因为其中间结果与其他试验相比显著优越。

![图 3-32](img/526245_1_En_3_Fig32_HTML.png)

图 3-32 HPO 早停与训练早停对比
...
![图 3-33](img/526245_1_En_3_Fig33_HTML.jpg)
...
![图 3-34](img/526245_1_En_3_Fig34_HTML.jpg)
...
![图 3-35](img/526245_1_En_3_Fig35_HTML.png)
...
![图 3-36](img/526245_1_En_3_Fig36_HTML.png)
...
2.  `fWidth`: 类型：实数。椭圆的副轴（毫米）
3.  `fSize`: 类型：实数。所有像素（光子）内容总和的 10 对数
4.  `fConc`: 类型：实数。两个最高像素总和与 `fSize` 的比值（比率）
5.  `fConc1`: 类型：实数。最高像素与 `fSize` 的比值（比率）
6.  `fAsym`: 类型：实数。最高像素到中心的距离，投影到主轴上（毫米）
7.  `fM3Long`: 类型：实数。沿主轴第三矩的立方根（毫米）
8.  `fM3Trans`: 类型：实数。沿副轴第三矩的立方根（毫米）
9.  `fAlpha`: 类型：实数。主轴与原点向量的角度（度）
10. `fDist`: 类型：实数。原点到椭圆中心的距离（毫米）
11. `class`: 值：`'g'`，`'h'`。伽马（信号），强子（背景）。`g` = 伽马（信号）：12332。`h` = 强子（背景）：6688
...
`Listing 3-19` `Search space. ch3/ml_pipeline/search_space.py`
...
`Listing 3-20` `MlPipelineClassifier. ch3/ml_pipeline/model.py`
...
`Listing 3-21` `EvolutionShrinkTuner. ch3/ml_pipeline/evolution_shrink_tuner.py`
...
这种简单技术引入了搜索空间元素之间的等价概念，从而显著缩小搜索空间。
...
![图 3-41](img/526245_1_En_3_Fig41_HTML.jpg)
