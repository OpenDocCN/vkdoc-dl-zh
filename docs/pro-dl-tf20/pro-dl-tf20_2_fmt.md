...
对于 `x[1] = 1`, `x[2] = 0`, `y = 1`，预测是 `w^Tx = [0 0 0] * [1; 1; 0] = 0`。由于 `w^Tx = 0`，数据点会被分类为 0，这与实际类别 1 不符。因此，根据感知器规则更新的权重向量应该是 `w -> w + x = [0; 0; 0] + [1; 1; 0] = [1; 1; 0]`。

对于 `x[1] = 0`, `x[2] = 1`, `y = 1`，预测是 `w^Tx = [1 1 0] * [1; 0; 1] = 1`。由于 `w^Tx = 1 > 0`，数据点会被正确分类为 1。因此，权重向量不会有更新，并保持在 `[1; 1; 0]`。

对于 `x[1] = 1`, `x[2] = 1`, `y = 0`，预测为 `w^Tx = [1 1 0] * [1; 1; 1] = 2`。由于 `w^Tx = 2`，数据点将被分类为 1，这与实际类别 0 不符。因此，更新的权重向量应该是 `w -> w - x = [1; 1; 0] - [1; 1; 1] = [0; 0; -1]`。

对于 `x[1] = 0`, `x[2] = 0`, `y = 0`，预测为 `w^Tx = [0 0 -1] * [1; 0; 0] = 0`。由于 `w^Tx = 0`，数据点将被正确分类为 0。因此，权重向量 `w` 将不会更新。

因此，在第一次遍历数据点后的权重向量为 `w = [0 0 -1]^T`。基于更新的权重向量 `w`，让我们评估这些点被分类得有多好。

对于数据点 1，`w^Tx = [0 0 -1] * [1; 1; 0] = 0`，因此它被错误地分类为类别 0。

对于数据点 2，`w^Tx = [0 0 -1] * [1; 0; 1] = -1`，因此它被错误地分类为类别 0。

对于数据点 3，`w^Tx = [0 0 -1] * [1; 1; 1] = -1`，因此它被正确地分类为类别 0。

对于数据点 4，`w^Tx = [0 0 -1] * [1; 0; 0] = 0`，因此它被正确地分类为类别 0。

根据前面的分类，我们看到在第一次迭代后，感知机算法成功地将负类正确分类。如果我们再次在数据点上应用感知机学习规则，第二次遍历中权重向量 `w` 的更新如下：

对于数据点 1，`w^Tx = [0 0 -1] * [1; 1; 0] = 0`，因此它被错误地分类为类别 0。因此，根据感知机规则更新的权重为 `w -> w + x = [0; 0; -1] + [1; 1; 0] = [1; 1; -1]`。

对于数据点 2，`w^Tx = [1 1 -1] * [1; 0; 1] = 0`，因此它被错误地分类为类别 0。因此，根据感知机规则更新的权重为 `w -> w + x = [1; 1; -1] + [1; 0; 1] = [2; 1; 0]`。

对于数据点 3，`w^Tx = [2 1 0] * [1; 1; 1] = 3`，因此它被错误地分类为类别 1。因此，根据感知机规则更新的权重为 `w -> w - x = [2; 1; 0] - [1; 1; 1] = [1; 0; -1]`。

对于数据点 4，`w^Tx = [1 0 -1] * [1; 0; 0] = 1`，因此它被错误地分类为类别 1。因此，根据感知机规则更新的权重为 `w -> w - x = [1; 0; -1] - [1; 0; 0] = [0; 0; -1]`。
...
输出单元 `p[1]` 的输出 = `w1 * h1 + w2 * h2 + b[3]`

```
= (w1 * w11 + w2 * w12) * x1 + (w1 * w21 + w2 * w22) * x2 + w1 * b1 + w2 * b2 + b3
```

如前所述，网络的最终输出，即单元 `p[1]` 的输出，是其输入的线性函数，因此网络不能在类别之间产生非线性分离。

如果隐藏层产生的不是线性输出，而是引入一个表示为 `f(x) = 1/(1 + e^(-x))` 的激活函数，那么隐藏单元的输出 `h1 = 1 / (1 + e^(-(w11 * x1 + w21 * x2 + b1)))`。

同样，隐藏单元的输出 `h2 = 1 / (1 + e^(-(w12 * x1 + w22 * x2 + b2)))`。

输出单元的输出 `p1 = w1 / (1 + e^(-(w11 * x1 + w21 * x2 + b1))) + w2 / (1 + e^(-(w12 * x1 + w22 * x2 + b2))) + b3`。
...
在向量表示法中，我们可以表示输出 `y = w^Tx + b`。

如果我们取 `w^Tx + b = z`，那么相对于网络输入 `z` 的输出将如图 2-9 所示。
...
在二进制阈值神经元（见图 2-10）中，如果神经元的净输入超过一个指定的阈值，那么神经元就会被激活；即输出 1，否则输出 0。如果神经元的净线性输入是 `z = w^Tx + b`，而 `k` 是神经元激活的阈值，那么
...
通常，通过调整偏置，将二进制阈值神经元调整到在阈值 0 处激活。当 `w^Tx + b > k` => `w^Tx + (b - k) > 0` 时，神经元被激活。
...
其中 `z = w^Tx + b` 是 sigmoid 激活函数的净输入。
...
+   当 sigmoid 函数的净输入 `z` 为一个正的大数时，`e^(-z)` 趋近于 0，因此 `y` 趋近于 1。
+   当 sigmoid 的净输入 `z` 为一个负的大数时，`e^(-z)` 趋近于无穷大，因此 `y` 趋近于 0。
+   当 sigmoid 函数的净输入 `z` 为 0 时，则 `e^(-z) = 1`，因此 `y = 0.5`。
...
![P(y_i=1/x) = e^(w^(i)T x + b^(i)) / Σ_{j=1}^k e^(w^(j)T x + b^(j))](img/448418_2_En_2_Chapter_TeX_Equp.png)
...
![P(y_1=1/x) = e^(w^(1)T x + b^(1)) / (e^(w^(1)T x + b^(1)) + e^(w^(2)T x + b^(2)))](img/448418_2_En_2_Chapter_TeX_Equq.png)

![= 1 / (1 + e^(-(w^(1) - w^(2))^T x - (b^(1) - b^(2))))](img/448418_2_En_2_Chapter_TeX_Equr.png)
...
SoftMax 层的损失函数，如图 2-12 所示，被称为分类交叉熵，其表达式如下：

![C = Σ_{i=1}^k -y_i log P(y_i=1/x)](img/448418_2_En_2_Chapter_TeX_Equs.png)

![Softmax 激活函数示意图](img/448418_2_En_2_Fig12_HTML.jpg)
...
![y = max(0, w^T x + b)](img/448418_2_En_2_Chapter_TeX_Equt.png)

![矩形线性单元的图像](img/448418_2_En_2_Fig13_HTML.jpg)
...
![PReLU 公式](img/448418_2_En_2_Chapter_TeX_Equu.png)
...
![tanh 公式](img/448418_2_En_2_Chapter_TeX_Equv.png)

![tanh 激活函数的图像](img/448418_2_En_2_Fig14_HTML.jpg)

tanh 激活函数的图像。图中提到了 y 等于 z 的 tanh 的方程。
...
+   当网络输入 `z` 是一个正的大数时，`e^(-z)` 趋近于 0，因此 `y` 趋近于 1。
+   当网络输入 `z` 是一个负的大数时，`e^(z)` 趋近于 0，因此 `y` 趋近于 -1。
...
![SoftPlus 公式](img/448418_2_En_2_Chapter_TeX_Equw.png)

很容易看出，激活函数相对于输入的梯度不过是输入上的 sigmoid 激活，如下所示：

![SoftPlus 梯度公式](img/448418_2_En_2_Chapter_TeX_Equx.png)

SoftPlus 激活函数可以看作是 ReLU 函数的平滑近似，如图 2-15 所示。

![软加激活函数图形](img/448418_2_En_2_Fig15_HTML.jpg)
...
![修改版 SoftPlus 公式](img/448418_2_En_2_Chapter_TeX_Equy.png)

SoftPlus 函数与 ReLU 激活函数类似，提供非负输出。因此，SoftPlus 激活函数常用于神经网络中输出标准差。

### Swish 激活函数

Swish 激活函数在输入 `z` 上的定义是输入与输入上的 sigmoid 激活的乘积（见下文）：

![Swish 公式](img/448418_2_En_2_Chapter_TeX_Equz.png)

Swish 与现有激活函数的不同之处在于，Swish 本质上是非单调的，如图 2-16 所示。

![Swish 激活函数图形](img/448418_2_En_2_Fig16_HTML.jpg)
...
![i_1 = w_{11}x_1 + w_{21}x_2 + b_1](img/448418_2_En_2_Chapter_TeX_Equaa.png)

![i_2 = w_{12}x_1 + w_{22}x_2 + b_2](img/448418_2_En_2_Chapter_TeX_Equab.png)

![z_1 = 1 / (1 + e^{-i_1})](img/448418_2_En_2_Chapter_TeX_Equac.png)

![z_2 = 1 / (1 + e^{-i_2})](img/448418_2_En_2_Chapter_TeX_Equad.png)

![i_3 = w_1 z_1 + w_2 z_2 + b_3](img/448418_2_En_2_Chapter_TeX_Equae.png)

![z_3 = 1 / (1 + e^{-i_3})](img/448418_2_En_2_Chapter_TeX_Equaf.png)

考虑到对数损失成本函数，XOR 问题的总成本函数可以定义为以下：

![C = Σ_{i=1}^4 -y^{(i)} log z_3^{(i)} - (1 - y^{(i)}) log (1 - z_3^{(i)})](img/448418_2_En_2_Chapter_TeX_Equag.png)

如果所有权重和偏差加在一起可以被视为参数向量 `θ`，我们可以通过最小化成本函数 `C(θ)` 来学习模型：

![θ* = Arg Min_θ C(θ)](img/448418_2_En_2_Chapter_TeX_Equah.png)
...
![w_k^{(t+1)} = w_k^{(t)} - η ∂C(w_k^{(t)}) / ∂w_k, ∀w_k ∈ θ](img/448418_2_En_2_Chapter_TeX_Equai.png)
...
![C = -y log z_3 - (1 - y) log (1 - z_3)](img/448418_2_En_2_Chapter_TeX_Equaj.png)

![∂C / ∂w_1 = (dC / dz_3) * (dz_3 / di_3) * (∂i_3 / ∂w_1)](img/448418_2_En_2_Chapter_TeX_Equak.png)

![dC / dz_3 = (z_3 - y) / (z_3 (1 - z_3))](img/448418_2_En_2_Chapter_TeX_Equal.png)
...
![成本函数对 i_1 的偏导数](img/448418_2_En_2_Chapter_TeX_IEq35.png) 可以被认为是关于隐藏层单元 `h[1]` 净输入的误差。它可以通过将 `z1` 因子乘以![对 z_1 的偏导数](img/448418_2_En_2_Chapter_TeX_IEq36.png) 来计算：
...
![预测概率公式](img/448418_2_En_2_Chapter_TeX_IEq37.png) 表示第 `i` 类的预测概率。

让我们计算成本函数关于权重 `w[ji]^(2)` 的偏导数。权重只会影响网络的 `i` 输出单元的输出。
...
前面的内容给出了关于网络最后一层权重和偏置的代价函数偏导数的通用表达式。接下来，让我们计算下层权重和偏置的偏导数。事情变得稍微复杂一些，但仍然遵循一个通用的趋势。让我们通过链式法则计算代价函数关于权重 `w[kj]^(1)` 的偏导数。这个权重会受到所有三个输出单元错误的影响。基本上，隐藏层中 `j` 单元的输出错误会有来自所有输出单元的错误贡献，这些贡献通过连接输出层到 `j` 隐藏单元的权重进行缩放。让我们通过链式法则进行计算，看看它是否符合我们之前宣称的：
...
一旦训练完成并且输出已写入日志，我们可以通过将日志加载到 TensorBoard 来查看计算图。以下是在 Jupyter notebook 中可以使用的代码。
...
![图 2-25：XOR 网络的计算图](img/448418_2_En_2_Fig25_HTML.jpg)
...
![图 2-26：训练过程中的成本（均方误差）与迭代次数对比](img/448418_2_En_2_Fig26_HTML.jpg)
