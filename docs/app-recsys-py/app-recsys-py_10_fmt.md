...
`df1` 的输出截图。它包括股票代码、产品名称、描述、类别、品牌和单价。
...
1.  获取给定客户的相似客户。
2.  获取最近邻居购买的所有商品，并移除目标客户已经购买的商品。
3.  从过滤后的商品集中，统计每个商品在最近邻居的购物车中重复出现的次数，然后根据重复次数对列表进行排序，并按降序返回。
...
![客户 17850 的推荐输出](img/537881_1_En_10_Fig8_HTML.png)
...
![客户 12347 的推荐输出](img/537881_1_En_10_Fig9_HTML.png)
