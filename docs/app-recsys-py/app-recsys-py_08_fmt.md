...
![图 8-1](img/537881_1_En_8_Fig1_HTML.png)

一个框架揭示了构建基于分类模型的步骤。它包括数据收集、数据预处理、特征工程、`EDA`、模型构建、评估和预测。
...
![图 8-2](img/537881_1_En_8_Fig2_HTML.png)

一个数据框揭示了记录数据的头五行（0 到 4）的输出。数据包括 `InvoiceNo`、`StockCode`、`Quantity`、`InvoiceDate`、`ShippingCost` 和 `CustomerID`。

图 8-3 显示了客户数据的头五行（0 到 4）的输出。

![图 8-3](img/537881_1_En_8_Fig3_HTML.jpg)

一个数据框揭示了客户数据的头五行（0 到 4）的输出。数据包括 `CustomerID`、`Gender`、`Age`、`Income`、`Zipcode` 和 `Customer Segment`。

图 8-4 显示了产品数据的头五行（0 到 4）的输出。

![图 8-4](img/537881_1_En_8_Fig4_HTML.png)

一个数据框揭示了产品数据的头五行（0 到 4）的输出。数据包括 `StockCode`、`Product Name`、`Description`、`Category`、`Brand` 和 `Unit Price`。
...
![图 8-5](img/537881_1_En_8_Fig5_HTML.jpg)

数据框显示了分组 `StockCode`（10002）、`CustomerID`（12451、12510、12583、12637 和 12673）以及 `Quantity`（12、24、48、12 和 1）的输出。
...
![图 8-6](img/537881_1_En_8_Fig6_HTML.jpg)

数据框显示了 `StockCode`（48129）和 `CustomerID`（13736、17252、16005、17288 和 14267）从 0 到 4 的输出。
...
![图 8-7](img/537881_1_En_8_Fig7_HTML.jpg)

数据框显示了 `StockCode`（48129）、`CustomerID`（13736、17252、16005、17288 和 14267）以及 `Quantity`（`NaN`、`NaN`、`1.0`、`NaN` 和 `NaN`）从 0 到 4 的输出。

如您所见，`Quantity` 列中存在空值。
...
![图 8-8](img/537881_1_En_8_Fig8_HTML.jpg)
...
![图 8-9](img/537881_1_En_8_Fig9_HTML.jpg)

数据框显示了前 5 行（0 到 4）的输出。数据包括 `StockCode`、`Category`、`Brand` 和 `Unit Price`。

让我们从 `Category` 列中提取第一级层次并连接到 `product_data` 表。
...
![图 8-10](img/537881_1_En_8_Fig10_HTML.jpg)

数据框显示了 `StockCode`、`Brand`、`Unit Price` 和 `Category`（包括 cell、health、video、health 和 home）的输出。
...
![图 8-11](img/537881_1_En_8_Fig11_HTML.png)

数据框显示了合并后前 5 行（0 到 4）的输出。数据包括 `CustomerID`、`Gender`、`Age`、`Income`、`Customer Segment`、`StockCode` 和 `Quantity`。
...
让我们创建一个使用 `Quantity` 列来指示客户是否购买产品的 `flag_buy` 列。

如果 `Quantity` 列为 0，则表示客户没有购买产品。
...
在构建模型之前，让我们对所有分类变量进行编码。同时，存储 `StockCode` 以供进一步使用。
...
+   `ROC`（接收者操作特征）曲线是分类任务的评估指标。`ROC`曲线图的x轴为假阳性率，y轴为真阳性率。它说明了当阈值变化时，类别被区分的强度。`ROC曲线下面积`值越高，预测能力越强。图 8-23 展示了 `ROC` 曲线。

线性和逻辑回归是使用统计作为基础来预测因变量的传统方法。但这些算法有一些缺点。

+   统计建模必须满足之前讨论的`假设`。如果不满足这些`假设`，模型将不可靠，并且无法完全拟合随机预测。

+   这些算法在数据和`非线性目标特征`面前面临挑战。复杂模式难以解码。

+   数据应该是干净的（`缺失值`和`异常值`应该被处理）。

可以使用像`决策树`、`随机森林`、`SVM`和`神经网络`这样的高级机器学习概念来克服这些限制。
...
一张错误接受率与正确接受率的图表揭示了 ROC 曲线的输出。虚线表示 KNN 预测（`AUROC` = 0.852）。趋势从 0.0 增加到 1.0。
...
# 为了构建模型，我们编码了stockcode列，现在我们将解码并推荐。
items = []
