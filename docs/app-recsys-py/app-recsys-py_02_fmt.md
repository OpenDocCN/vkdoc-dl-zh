...
让我们查看来自 Kaggle 电子商务网站的一个开源数据集。从 `www.kaggle.com/carrie1/ecommerce-data?select=data.csv` 下载数据集。
...
一个输出样本解释了 `InvoiceNo`、`StockCode`、`Description`、`Quantity`、`InvoiceDate`、`UnitPrice`、`CustomerID` 和 `Country` 的数据。
...
一个输出样本展示了 `Quantity`、`UnitPrice` 和 `CustomerID` 的计数、平均值、最小值和最大值。

`Quantity` 列有一些负值，这是错误数据的一部分，所以让我们删除这些条目。

以下仅选择 `Quantity` 大于 0 的数据。
...
图 2-4 展示了在 `Quantity` 列中过滤数据后的输出。
...
让我们创建一个新的 `Amount` 特征/列，它是 `Quantity` 和其 `UnitPrice` 之积。

```python
data1['Amount'] = data1['Quantity'] * data1['UnitPrice']
```

现在让我们使用 `groupby` 函数突出显示订单数量最多的客户。
...
来自前 5 名订单数量最多的忠实客户的输出数据样本。数据包括 `CustomerID`、`Country` 和 `InvoiceNo`。
...
让我们再次使用 `groupby` 函数来获取花费金额（发票）最多的客户。
...
最高花费的前 5 名盈利客户的输入和输出截图。输出数据框显示 `CustomerID`、`Country` 和 `Amount`。
...
图 2-12 显示了 `UnitPrice` 箱线图。

![图片](img/537881_1_En_2_Fig12_HTML.png)
`图 2-12` 输出

`UnitPrice` 箱线图。分布从 0 到 4200 以及 8200 单价观察到。

`UnitPrice` = 0 的项目不是异常值。这些是“免费”项目。
...
图 2-13 显示了筛选后的数据输出（`UnitPrice` = 0）。

![图片](img/537881_1_En_2_Fig13_HTML.png)
`图 2-13` 输出

筛选后的输出数据截图包括 `InvoiceNo`、`StockCode`、`year_month`、`month`、`day`、`hour`、`Description`、`Quantity`、`InvoiceDate`、`UnitPrice`、`CustomerID` 和 `Amount`。
...
提供 X 轴刻度标签。由于 2011 年 6 月没有免费项目，因此被排除在外。
...
![不同月份计算出的频率柱状图](img/537881_1_En_2_Fig14_HTML.png)

`图 2-14` 不同月份计算出的频率柱状图。2011 年 11 月的频率范围最高，而 2 月份最低。

`输出`：2011 年 11 月发放了最多的免费商品。最多的订单也是在 2011 年 11 月。
...
![不同月份计算的订单数量柱状图](img/537881_1_En_2_Fig15_HTML.png)

`图 2-15` 不同月份计算的订单数量柱状图。2011 年 11 月的订单数量最多，而 12 月最少。

`输出`：与 5 月份相比，8 月份的销售额有所下降，这表明“免费商品数量”有轻微的影响。
...
![不同月份产生的收入柱状图](img/537881_1_En_2_Fig16_HTML.png)

`图 2-16` 不同月份产生的收入柱状图。2011 年 11 月产生的金额最高。
...
![基于销售数量的前 10 项商品的横向柱状图](img/537881_1_En_2_Fig17_HTML.png)

`图 2-17` 基于销售数量的前 10 项商品的横向柱状图。纸艺和一只小鸟拥有最高的数量。
...
![股票代码和描述的截图](img/537881_1_En_2_Fig18_HTML.png)

`图 2-18` 股票代码和描述的截图。

`输出`：由于 `白色悬挂心形 T-LIGHT 座台` 的数量与长度 856 匹配，因此交叉表对于所有商品看起来都是正确的。
...
在 y 轴上创建描述（或项目）的条形图，在 x 轴上创建唯一客户的总和。仅绘制最常购买的十个项目。
...
![水平条形图显示了最多客户购买的前 10 个项目](img/537881_1_En_2_Fig19_HTML.png)

`图 2-19` 水平条形图显示了最多客户购买的前 10 个项目。Regency Cake stands 3 T I E R 是所有项目中购买最多的。
...
![常订购项目的词云](img/537881_1_En_2_Fig20_HTML.png)
...
![展示了前十个首选选项的截图](img/537881_1_En_2_Fig21_HTML.jpg)

`图 2-21` 展示了前十个首选选项的截图。项目数据和计数被表示出来。
...
![前 10 个首选选项的水平条形图](img/537881_1_En_2_Fig22_HTML.png)

`图 2-22` 前 10 个首选选项的水平条形图。
...
让我们使用分组函数创建一个市场篮子 DataFrame，该 DataFrame 指定特定发票号中所有项目是否存在于所有发票中。以下表示发票号中的数量，必须固定。
...
![按描述和发票汇总的总数量输出](img/537881_1_En_2_Fig23_HTML.png)

`图 2-23` 按描述和发票汇总的总数量输出。

`输出结果`

此输出获取订购的数量（例如，48, 24, 126），但我们只想知道是否购买了项目。因此，让我们将单位编码为 1（如果购买）或 0（未购买）。
...
![按描述和发票汇总的总数量输出](img/537881_1_En_2_Fig24_HTML.png)

`图 2-24` 按描述和发票汇总的总数量输出。
...
![apriori 支持度的示例](img/537881_1_En_2_Fig25_HTML.png)

`图 2-25` apriori 支持度的示例。支持度等于 10 除以 100，即 10%。

`支持度`

让我们看看一个例子。如果 100 个用户中有 10 个购买了牛奶，牛奶的支持度为 10/100 = 10%。计算公式如 `图 2-26` 所示。

![两个用于计算电影推荐和市场篮子优化的公式集](img/537881_1_En_2_Fig26_HTML.png)

`图 2-26` 两个用于计算电影推荐和市场篮子优化的公式集。

`公式`

假设你想要在牛奶和面包之间建立关系。如果 40 个牛奶购买者中有 7 个也购买了面包，那么置信度 = 7/40 = 17.5%。`图 2-27` 解释了置信度。

![一个示例解释了置信度获得的百分比](img/537881_1_En_2_Fig27_HTML.png)

`图 2-27` 一个示例解释了置信度获得的百分比。置信度等于 7 除以 40，即 17.5%。

`置信度`

计算置信度的公式如 `图 2-28` 所示。

![两个用于计算电影推荐和市场篮子优化的公式集](img/537881_1_En_2_Fig28_HTML.png)

`图 2-28` 两个用于计算电影推荐和市场篮子优化的公式集。

`公式`

基本公式是提升度 = 置信度/支持度。因此，这里，提升度 = 17.5/10 = 1.75。`图 2-29` 解释了提升度和公式。

![提升度的示例](img/537881_1_En_2_Fig29_HTML.png)

`图 2-29` 提升度的示例。提升度等于 17.5%除以 10%，即 1.75。两个用于计算电影推荐和市场篮子优化的公式来计算提升度。

`提升度`

### 关联规则

关联规则挖掘在大数据项集中寻找有趣的关联和关系。此规则显示了项集在事务中出现的频率。基于数据集创建的规则进行市场篮子分析。`图 2-30` 解释了关联规则。

![五个事务的示例以及右侧的频繁项集和关联规则](img/537881_1_En_2_Fig30_HTML.png)

`图 2-30` 五个事务的示例以及右侧的频繁项集和关联规则。
...
使用 `apriori` 算法并为样本项创建关联规则。将 `apriori` 算法应用于 `product_wooden_star_df`。
...
![图 2-31 输出](img/537881_1_En_2_Fig31_HTML.png)
...
![图 2-32 输出](img/537881_1_En_2_Fig32_HTML.png)

验证输出。它包括年份、日期、小时、描述、数量、国家以及金额。`bought_together_frequently` 函数和发票中的推荐之间有一些共同的项目，因此推荐器表现良好。
...
![图 2-33 输出](img/537881_1_En_2_Fig33_HTML.jpg)
...
![图 2-34 输出](img/537881_1_En_2_Fig34_HTML.jpg)
...
![图 2-35 输出](img/537881_1_En_2_Fig35_HTML.jpg)

支持度与置信度的散点图。图中最高值超过 0.8。

图 2-36 显示了规则的可视化。

![图 2-36 输出](img/537881_1_En_2_Fig36_HTML.png)
