...
![混合推荐系统架构。它从基于内容的过滤开始，到混合推荐，再到协同过滤。](img/537881_1_En_6_Fig1_HTML.jpg)
图 6-1 混合推荐系统

参考：[《混合论文推荐系统》](https://www.researchgate.net/profile/Xiangjie-Kong-2/publication/330077673/figure/fig5/AS:710433577107459%25401546391972632/A-hybrid-paper-recommendation-system.png)
...
![输出文件描述了前五个订单 DataFrame 的列表。它包括发票号码、库存代码、数量、发票日期、交货日期、折扣百分比、运输方式、运输成本和客户 ID。](img/537881_1_En_6_Fig2_HTML.png)
图 6-2 订单数据
...
![输出文件描述了客户数据框的前五行列表。它包括客户 ID、性别、年龄、收入、邮编和客户细分。](img/537881_1_En_6_Fig3_HTML.jpg)
图 6-3 客户数据
...
![一个输出文件描述了产品数据框的前五行。它包括库存代码、产品名称、描述、类别、品牌和单价。](img/537881_1_En_6_Fig4_HTML.png)
图 6-4 产品数据
...
![一个输出文件描述了合并后的数据框。它包括发票号、库存代码、数量、发票日期、交货日期、折扣百分比和运输方式，随后是单个客户的详细信息。](img/537881_1_En_6_Fig5_HTML.png)
图 6-5 合并数据

## 数据准备

在构建推荐模型之前，所需数据必须以适当的格式存在，以便模型可以接收输入。接下来，获取用户与产品交互矩阵和产品到特征的交互映射。
...
接下来，创建一个函数，从 `DataFrame` 中根据三个特征名称获取唯一值的总列表。它获取三个特征的唯一值列表：客户细分、年龄和性别。
...
![一个输出文件描述了用户与产品关系数据。它包括客户 ID、产品名称和单个客户的数量。](img/537881_1_En_6_Fig6_HTML.jpg)
图 6-6 用户与产品关系数据
...
![一个输出文件描述了产品到特征的关系数据。它包括产品名称、客户细分和单个客户的数量。](img/537881_1_En_6_Fig7_HTML.jpg)
图 6-7 产品到特征关系数据

将用户与产品之间的关系分为训练数据和测试数据。
...
现在数据和 ID 映射已经就绪，为了获取用户与产品以及产品到特征的交互矩阵，首先创建一个返回交互矩阵的函数。
...
然后，使用前面的函数生成训练和测试数据的 `user_item_interaction_matrix`。
