...
![图 7-1](img/537881_1_En_7_Fig1_HTML.jpg)
无监督学习的图展示了聚类结果。沿 x 轴（`x_1`）和 y 轴（`x_2`）表示了三个聚类。
...
![图 7-2](img/537881_1_En_7_Fig2_HTML.png)
...
![图 7-3](img/537881_1_En_7_Fig3_HTML.png)
数据框展示了记录数据的前五行（0 到 4）的输出。数据包括发票号、库存代码、数量、交货日期和运输方式。

图 7-4 展示了客户数据头五行的输出。

![图 7-4](img/537881_1_En_7_Fig4_HTML.jpg)
数据框展示了客户数据的前五行（0 到 4）的输出。数据包括客户 ID、性别、年龄、收入、邮编和客户段。

图 7-5 显示了产品数据前五行的输出。

![图 7-5](img/537881_1_En_7_Fig5_HTML.png)
...
![图 7-6](img/537881_1_En_7_Fig6_HTML.png)
...
![图 7-7](img/537881_1_En_7_Fig7_HTML.png)
...
![图 7-8](img/537881_1_En_7_Fig8_HTML.png)
...
![图 7-9](img/537881_1_En_7_Fig9_HTML.png)
...
![图 7-10](img/537881_1_En_7_Fig10_HTML.jpg)
一个数据框显示了编码值后的输出。年龄、性别、客户段和收入段的数据被表示出来。
...
2. 从整体观察结果或点中选择随机的 *k* 个点作为聚类中心。
3. 计算这些中心与其他数据点之间的距离，并使用以下任何一种距离度量将其分配给最近的中心聚类，该点属于特定的聚类。
    - 欧几里得距离
    - 曼哈顿距离
    - 余弦距离
    - 汉明距离
...
2. 它根据前面讨论的距离度量，确定了两个最接近的观察结果或点。
3. 将这两个最相似的点合并，形成一个聚类。
4. 这一直持续到所有聚类合并形成一个最终的单一聚类。
5. 最后，使用树状图决定理想的聚类数量。
...
让我们为这个用例构建一个 `k-均值` 模型。在构建模型之前，让我们执行肘部方法和树状图方法以找到最优簇数。
...
![图 7-13](img/537881_1_En_7_Fig13_HTML.png)
K 值（x 轴）与 `WCSS`（y 轴）的图表展示了肘部方法输出。图表中的趋势从 1 到 14 K 值逐渐降低。
...
![图 7-14 层次聚类树状图](img/537881_1_En_7_Fig14_HTML.jpg)

*图 7-14* 一个层次聚类树状图展示了节点中的点数（如果没有括号，则为点的索引）。它展示了系统中所有数据点之间的关系。

两种方法的最优或最少簇数都是两个。但让我们考虑在这个用例中使用 15 个簇。

> **注意**：你可以考虑任何数量的簇进行实现，但应该大于 k-均值聚类或树状图的**最优**或**最少**簇数。
...
![图 7-15 创建簇后的 df_cluster 输出](img/537881_1_En_7_Fig15_HTML.png)

*图 7-15* 一个数据框展示了创建簇后的 `df_cluster` 输出。数据包括客户 ID、性别、年龄、收入、邮编和客户细分。
...
![图 7-16 选择特定列后的 df_cluster 输出](img/537881_1_En_7_Fig16_HTML.jpg)

*图 7-16* 一个数据框展示了选择特定列后的 `df_cluster` 输出。数据包括客户 ID、年龄、性别、收入和邮编。
...
![图 7-17 客户细分与簇的对应关系图](img/537881_1_En_7_Fig17_HTML.png)

*图 7-17* 一个三柱状图显示了客户细分（小型企业、企业、中产阶级）与簇的输出。x 轴和 y 轴分别表示簇和客户细分百分比。
...
![图 7-18 收入与簇的对应关系图](img/537881_1_En_7_Fig18_HTML.png)

*图 7-18* 一个三柱状图展示了按百分比收入（高、中、低收入）的簇数据。x 轴和 y 轴分别表示簇和收入百分比。
...
![图 7-19 性别与簇的对应关系图](img/537881_1_En_7_Fig19_HTML.png)

*图 7-19* 双柱状图展示了性别百分比（男性和女性）与集群的关系。男性在集群 9 中观察到最高的百分比，女性在集群 14 中观察到最高的百分比。
...
![图 7-20 每个集群的平均年龄图表](img/537881_1_En_7_Fig20_HTML.jpg)

*图 7-20* 柱状图展示了与集群相关的平均值的输出。x 轴由集群指示。集群 3 的平均值最高（超过 50）。
...
![图 7-21 合并客户数据和订单数据后的输出](img/537881_1_En_7_Fig21_HTML.jpg)

*图 7-21* 数据框暴露了统一客户数据和订单数据后的输出。数据包括股票代码、客户 ID 和聚类表示。
...
![图 7-22 创建 score_df 后的输出](img/537881_1_En_7_Fig22_HTML.jpg)

*图 7-22* 数据框（37032 行和 3 列）展示了 `score_df` 制定后的输出。数据包括集群、股票代码和分数表示。
...
![图 7-23 缺失值输出](img/537881_1_En_7_Fig23_HTML.png)

*图 7-23* 数据框暴露了缺失值的输出。数据包括零值、缺失值、总值的百分比、总零缺失值和数据类型表示。
...
![图 7-24 移除缺失值后的输出](img/537881_1_En_7_Fig24_HTML.png)

*图 7-24* 数据框的表示展示了移除缺失值后的输出。所选数据框有 6 列和 3706 行。没有列有缺失值。
...
![图 7-25 创建产品数据聚类后的输出结果](img/537881_1_En_7_Fig25_HTML.png)

*图 7-25* 数据框显示了创建产品数据聚类后的输出结果。数据包括股票代码、产品名称、描述、类别、品牌、单价和聚类表示。
