...
![偏见识别流程图](img/506619_1_En_8_Fig1_HTML.jpg)
`图 8-1` 偏见识别流程图
...
![显示模型准确率的评估界面](img/506619_1_En_8_Fig10_HTML.png)

**图 8-10** 显示模型准确率的评估界面

# 结论

本章讨论了如何使用 `WIT` 工具解释分类模型结果的推理过程，以及如何识别在区分流失与非流失案例中起关键作用的特征。此外，您还使用了针对单个数据点进行局部解释的方法。
...
![数据点 799 分析](img/506619_1_En_8_Fig4_HTML.png)
...
![特征值微小变化导致的推理分数变化](img/506619_1_En_8_Fig5_HTML.png)
...
![流失预测的推理分数桶视图](img/506619_1_En_8_Fig6_HTML.png)
...
![账户长度的部分依赖图](img/506619_1_En_8_Fig7_HTML.png)

**图 8-7** 账户长度的部分依赖图

随着账户时长的增加，推理得分逐渐降低（见图 8-8）。

![区号变量的部分依赖图](img/506619_1_En_8_Fig8_HTML.png)

**图 8-8** 区号变量的部分依赖图

在评估模型的整体性能时，您可以考虑混淆矩阵，它展示了模型中流失与非流失案例的匹配与不匹配情况。

![性能与公平性界面](img/506619_1_En_8_Fig9_HTML.png)
