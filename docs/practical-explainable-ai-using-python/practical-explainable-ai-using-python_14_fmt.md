...
![图 7-1 涉及 AI 模型的文本解释步骤](img/506619_1_En_7_Fig1_HTML.png)

输入文本以向量的形式表示，并使用文档-词项矩阵来开发分类模型，最后需要使用可解释性库来解释预测的类别，以便了解哪些特征或词语负责预测特定的类别标签。

## 文本分类数据集

在本章中，你将使用来自 [`http://ai.stanford.edu/~amaas/data/sentiment/`](http://ai.stanford.edu/~amaas/data/sentiment/) 的大型电影评论数据集。该数据集包含 50,000 条评论，其中 25,000 条已经用二分类标签（如正面或负面评论）进行了标注。另外还有 25,000 条评论用于测试。你将使用此数据集创建一个文本分类模型来预测电影评论。在预测之后，你将分析预测的原因，并进一步解释该文本分类机器学习模型中特征的重要性及其他可解释性要素。在此任务中，你将评论句子提供给机器学习模型，模型预测情感为正面或负面。因此，这里的 ML 模型被视为一个黑盒模型。这随之引发了一个问题：人们如何能信任来自黑盒模型的结果？
...
```python
eli5.show_weights(clf, feature_names=vec.get_feature_names(), target_names=set(imdb.sentiment)) # 有意义
```

对于基础线性分类器，`ELI5` 模块的结果是没有意义的。它提供了权重和特征。特征名称没有意义。为了让它们有意义，你可以传递特征名称和目标名称。

![图 7-2 从模型中提取的用于正面情感类别的特征](img/506619_1_En_7_Fig2_HTML.jpg)
...
![图 7-3 绘制句子中的重要特征以获得更清晰的展示](img/506619_1_En_7_Fig3_HTML.jpg)
...
![图 7-4 负面类别预测的局部解释](img/506619_1_En_7_Fig4_HTML.jpg)
...
![图 7-5 正面和负面类别的特征高亮显示](img/506619_1_En_7_Fig5_HTML.jpg)
...
![表 7-1](img/506619_1_En_7_Figa_HTML.jpg)
...
![图 7-6 正面类别的高亮特征重要性](img/506619_1_En_7_Fig6_HTML.jpg)
...
![表 7-2](img/506619_1_En_7_Figb_HTML.jpg)

根据表 7-2 和图 7-7，应用 `tf-idf` 变换后，模型准确率没有明显差异。然而，概率值略有变化，分数也略有变化。

![图 7-7 应用 tf-idf 后正面类别的特征高亮](img/506619_1_En_7_Fig7_HTML.jpg)
...
![表 7-3](img/506619_1_En_7_Figc_HTML.jpg)
...
![图 7-8 从更精细模型中高亮显示的重要特征](img/506619_1_En_7_Fig8_HTML.jpg)
...
![图 7-9 模型中二元组和五元组特征的高亮显示](img/506619_1_En_7_Fig9_HTML.jpg)

表 7-4 采用基于 N-gram 的向量化器作为预处理方法，并考虑从二元组到五元组的 n-gram，使用 `TF-IDF` 向量化器创建训练模型，下表显示了分类报告。

![表 7-4 分类报告](img/506619_1_En_7_Figd_HTML.jpg)
...
![图 7-10 非重叠字符 N-gram 作为特征重要性](img/506619_1_En_7_Fig10_HTML.jpg)
...
![图 7-11 非常精细的高亮特征能更好地解释预测结果](img/506619_1_En_7_Fig11_HTML.jpg)
...
![图 7-12 正面类别预测的顶级特征](img/506619_1_En_7_Fig12_HTML.jpg)
...
数据集的来源是 `https://catalog.data.gov/dataset/consumer-complaint-database`。该数据集经过清洗和准备，用于预测投诉类别。读取数据后，创建了 Python 对象 `DF`。以下输出显示了数据集中的前几条记录。
...
![图 7-14 每个类别的重要特征示例](img/506619_1_En_7_Fig14_HTML.jpg)

![图 7-13 多类文本分类模型的分类准确率](img/506619_1_En_7_Fig13_HTML.jpg)
...
![图 7-15 每个类别及其特征名称的最重要特征](img/506619_1_En_7_Fig15_HTML.jpg)
...
![图 7-16 第一条投诉的类别预测解释](img/506619_1_En_7_Fig16_HTML.jpg)
...
![图 7-17 “支票或储蓄账户”类别的特征重要性](img/506619_1_En_7_Fig17_HTML.jpg)

图 7-18 显示，第一条消费者投诉属于“抵押贷款”类别，因为其概率为 84.3% 或 0.843。实际对预测有贡献的词语以绿色显示。

![图 7-18 “抵押贷款”类别预测的特征重要性](img/506619_1_En_7_Fig18_HTML.jpg)
...
表 `7-8` 迭代后，显示“抵押贷款”类别的多分类报告概率再次提升至 `74.2%`
...
图 `7-21` 显示了特征重要性，其中绿色背景突出显示正面特征，红色背景突出显示负面特征。
...
图 `7-22` 显示了作为特征的正面词语（以绿色背景突出显示）和作为特征的负面词语（以红色背景突出显示），针对“抵押贷款”类别。
...
为了处理词汇长度问题，你可以引入哈希向量化器（`HashingVectorizer`），并结合随机梯度提升模型作为分类器。准确率进一步下降至 `85.7%`。你仍然可以检查其解释。
...
图 `7-24` 显示了用于预测“`mortgage`”类别的更精细、高亮效果更好的特征。
...
图 `7-25` 多分类问题中，每个类别对应的正负特征及其权重

![../images/506619_1_En_7_Chapter/506619_1_En_7_Fig26_HTML.jpg](img/506619_1_En_7_Fig26_HTML.jpg)

图 `7-26` 多分类问题中，每个类别对应的正负特征及其权重

![../images/506619_1_En_7_Chapter/506619_1_En_7_Fig27_HTML.jpg](img/506619_1_En_7_Fig27_HTML.jpg)

图 `7-27` 多分类问题中，每个类别对应的正负特征及其权重

### 结论

文本分类是自然语言处理领域最重要的用例。它可用于消费者投诉分类、将原始文本分类为各种实体，以及将情感分类为正面和负面。我们讨论的是二分类和多分类问题。其他用例包括主题建模和摘要，这些内容也包含在本章中，因为这些任务不属于监督式机器学习模型。本章只考虑了监督式机器学习模型，因为当生成预测类别结果时，解释性通常会丢失。用户自然会思考哪些标记或特征被考虑，以及为什么预测结果是如此这般等等。然而，对于基于无监督学习的 NLP 任务，有更简单的方法来理解关系，但无监督学习中没有需要向最终用户解释的预测。在本章中，我们更多地关注了不同的预处理方法、模型及其可解释性。