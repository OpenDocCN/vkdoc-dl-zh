...
公式 (9.7) 表明，要计算关于 `$\boldsymbol {\theta }$` 的梯度，我们只需要找到 `$\log P(\tau ; \boldsymbol {\theta })$` 的导数。然而，需要注意的是，从特定状态 `$s_0$` 开始的概率通常是未知的，必须从数据中估计。这就是为什么我们使用 `$\mu (s_0)$` 来表示初始状态 `$s_0$` 的概率分布。

然后，我们可以将 `$P(\tau ; \boldsymbol {\theta })$` 展开如下：

![$\displaystyle \begin{aligned} P(\tau; \boldsymbol{\theta}) = \mu(s_0) \cdot \prod_{t=0}^{T-1} \pi (a_t|s_t; \boldsymbol{\theta}) \cdot \prod_{t=0}^{T-1} P(s_{t+1}|s_t, a_t) {} \end{aligned} $](img/605748_1_En_9_Chapter_TeX_Equ9.png)

(9.8)

在无模型强化学习中，智能体无法获取初始状态的分布 `$\mu (s_0)$` 或环境动态 `$P(s_{t+1}|s_t, a_t)$`。这就引出了一个问题：我们如何计算关于 `$\boldsymbol {\theta }$` 的梯度？幸运的是，我们可以利用对数运算法则来解决这个问题。

具体来说，我们可以应用对数恒等式，即两个数的对数之和等于它们乘积的对数。通过两次应用这个恒等式，我们可以将公式 (9.8) 拆解为乘积之和，然后得到公式 (9.9)。这使得我们无需访问 `$\mu (s_0)$` 或 `$P(s_{t+1}|s_t, a_t)$` 即可计算梯度。
...
**算法 2：带基线的 REINFORCE 算法 [线性-VFA]**

![](img/605748_1_En_9_Figaab_HTML.gif)

一个 8 行算法接收以下输入，输出近似最优策略`θ*`：折扣率`γ`、策略学习率`α`、基线函数学习率`β`以及回合数`K`。

在实际应用中，我们通常使用神经网络来近似策略和基线函数。这里，我们用`θ`表示策略的参数，用`ψ`表示基线函数的参数。在这种情况下，我们计算样本回报`G_t`与基线网络估计值之间的平方误差损失`(G_t - V̂(s_t; ψ))²`，以及策略网络的负对数似然损失`-log π(a_t|s_t; θ)`。需要注意的是，当使用神经网络时，应在多个时间步上累积梯度，而不是逐步更新参数。然后，我们可以使用神经网络优化器（如随机梯度下降或 Adam）基于累积的梯度来更新参数。

总体而言，使用参数化的策略和值函数，并借助神经网络对其进行近似，可以带来更高效、更有效的强化学习算法。

图 9.3 展示了在车杆经典控制任务上，带基线与不带基线的 REINFORCE 算法的性能表现。性能通过获得的平均奖励来衡量。为了评估智能体的性能，我们在每次训练迭代（包含 20,000 个训练步）结束时，在一个独立的测试环境中运行了 20,000 个评估步。结果取五次独立运行的平均值，并使用窗口大小为 5 的移动平均进行平滑处理。

![](img/605748_1_En_9_Fig3_HTML.png)

一张评估回合回报相对于训练环境步数的折线图。图中绘制了 REINFORCE 和带基线的 REINFORCE 算法呈下凸增长且带有小幅波动的曲线。一条平行于横轴的直线表示最大值，该值从纵轴上的 500 开始。

**图 9.3** 车杆控制任务中带基线与不带基线的 REINFORCE 算法对比。结果取五次独立运行的平均值。我们对这两个智能体使用相同的折扣率`γ = 0.99`、策略神经网络学习率`α = 0.0002`，以及基线网络学习率`β = 0.0005`。

**算法 3：带基线的 REINFORCE 算法 [使用神经网络]**

![](img/605748_1_En_9_Figaac_HTML.gif)

一个标题为“带基线的 REINFORCE 算法”的 10 行算法接收以下输入，输出近似最优策略`θ*`：折扣率`γ`、策略学习率`α`、基线函数学习率`β`以及回合数`K`。
...
评估回合回报与训练环境步数的折线图。图中绘制了带基线的 REINFORCE 和 Actor-Critic 的凹向下递增曲线，伴有小幅波动。一条平行于横轴的线表示最大值，从纵轴上的 500 开始。

**图 9.5** 在车杆控制任务上 Actor-Critic 与带基线的 REINFORCE 的对比。结果取五次独立运行的平均值。我们对这两个智能体使用相同的折扣率 `γ = 0.99`，策略神经网络学习率 `α = 0.0002`，以及基线网络学习率 `β = 0.0005`。对于 Actor-Critic，我们使用序列长度 `N = 64`。

我们对这些智能体使用相同的神经网络架构和学习率，并为 Actor-Critic 智能体使用两个独立的神经网络。对于 Actor-Critic，我们让智能体在更新策略和基线网络参数之前收集 64 个转移。我们使用 Adam 优化器训练神经网络。

我们可以看到，Actor-Critic 智能体的性能几乎与带基线的 REINFORCE 智能体相当。



### 9.6 利用熵促进探索

熵是信息论和统计学中的一个概念，用于衡量概率分布的随机性或不确定性。通常，熵被定义为描述随机变量结果所需的平均信息量。

例如，考虑一次抛硬币，有两种可能结果：正面或反面。如果硬币是均匀的，那么正面的概率为 0.5，反面的概率也为 0.5。该分布的熵为 1 比特，这意味着平均需要 1 比特的信息来描述单次抛硬币的结果。

在基于策略的强化学习算法中，策略通常由一个神经网络表示，该网络根据给定状态输出动作的概率分布。该分布的熵可作为衡量策略不确定性或随机性的指标。

利用熵来鼓励探索的想法最早由 Williams, R. J. [4] 提出。在这种情况下，策略被鼓励进行更多探索并采取更多样化的动作，这有助于学习一个能在多种情况下表现良好的稳健策略。另一方面，低熵意味着策略更具确定性，探索性较弱，这可能导致策略陷入局部最优，表现欠佳。

为了在策略梯度算法中鼓励探索，可以在策略梯度损失中添加一个负熵项。这鼓励策略在仍倾向于选择预期回报较高的动作的同时，采取更多样化的动作。

对于具有离散动作空间的强化学习问题，我们可以使用公式（9.29）来计算某个策略 `π` 的熵 `H`，其中 `N` 是动作空间中的动作数量，`π(a_t|s_t)` 是在遵循策略 `π` 时，在状态 `s_t` 下采取动作 `a_t` 的概率。

```
H = -∑_{k=1}^{N} π(a_k|s_t) log π(a_k|s_t)
```

(9.29)

为了理解为什么使用熵有助于鼓励智能体进行探索，让我们看一个简单的实验。假设对于某个特定状态 `s`，智能体有四个动作可选。我们有三种完全不同的策略：`π_1`、`π_2` 和 `π_3`。策略 `π_1` 是一个最优确定性策略，这意味着在状态 `s` 时，它总是会选择最佳动作（假设是第一个动作），因此在策略 `π_1` 下，不同动作的概率分布类似于 `[1, 0, 0, 0]`。第二个策略 `π_2` 是一个随机策略，在策略 `π_2` 下具有不同的概率分布，其概率分布类似于 `[0.6, 0.4, 0, 0]`。第三个策略 `π_3` 是一个均匀随机策略，这意味着概率均匀分布在所有可用动作上，即 `[0.25, 0.25, 0.25, 0.25]`。然后我们可以使用公式（9.29）来计算这些不同策略在状态 `s` 下的熵。

这些不同策略的熵如下所示：

- 策略 `π_1` 的熵为 `H_{π_1} = -(1.0 * log(1.0)) = 0`
- 策略 `π_2` 的熵为 `H_{π_2} = -(0.6 * log(0.6) + 0.4 * log(0.4)) = 0.67`
- 策略 `π_3` 的熵为 `H_{π_3} = -(0.25 * log(0.25) * 4) = 1.39`

我们可以看到，对于确定性策略 `π_1`，熵为 0，因为它总是会选择最佳动作，所以决策过程中没有随机性。对于随机策略 `π_2`，熵为 0.67。而对于均匀随机策略 `π_3`，熵为 1.39。从这个例子中，我们可以清楚地看到，过程中的随机性越大，熵就越高。



#### 将熵纳入基于策略的方法

为了将熵纳入基于策略的方法算法中，它被作为一项额外的项添加到训练期间优化的目标函数中。该目标函数通常包含用于最大化期望累积奖励的策略梯度项，而熵项则会对低熵策略进行惩罚。这些项的相对权重可以通过系数 `w` 来控制，以平衡探索与利用，如公式 (9.30) 所示，其中 `H(π(s_t; θ))` 是使用公式 (9.29) 针对特定状态 `s_t` 相对于策略 `π_θ` 计算出的熵。
