...
$$
\begin{aligned}
V(s)=R(s) + \gamma \sum_{s' \in \mathcal{S}} P(s'|s) V(s'), \quad \mbox{for all}\; s \in \mathcal{S} 
\end{aligned}
$$
...
```
DP compute state value function for MRP
Input: gamma (discount rate)
Output: V (state value function for MRP)
```
...
$$
\begin{aligned}
\pi' \ge \pi \quad \mbox{if}\; V_{\pi'}(s) \ge V_\pi(s), \quad \mbox{for all}\; s \in \mathcal{S} 
\end{aligned}
$$
...
$$
\begin{aligned}
V_\pi(s) & =\mathbb{E}_{\pi\!\!}\left[R_t + {\gamma} V_\pi(S_{t+1}) \;\Big|\;S_t=s\right] \\
& =\sum_{a \in A} \pi(a|s) \Big[R(s, a) + {\gamma} \sum_{s' \in \mathcal{S}} P(s'|s, a) V_\pi(s') \Big], \quad \mbox{for all}\; s \in \mathcal{S} {} 
\end{aligned}
$$
...
```
策略评估，迭代算法
Input: π (待评估的策略), γ (discount rate), ε (small threshold for stopping criterion)
Output: V_π (estimated state-value function for π)
```
...
策略迭代是在 MDP 中寻找最优策略的一种强大算法。通过迭代地改进策略并更新价值函数，我们可以收敛到最优策略。然而，务必注意，策略迭代算法可能不适用于大型 MDP，因为其计算量可能很大。在这种情况下，其他算法如值迭代（我们将在本章后面介绍）可能更为合适。
