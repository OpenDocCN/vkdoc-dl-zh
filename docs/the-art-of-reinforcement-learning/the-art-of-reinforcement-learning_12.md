...
在强化学习中，对于给定策略 `π`，状态-动作值函数（记为 `Q_π`）用于确定在给定状态下应采取的最佳动作。最佳动作通过选择产生最高状态-动作值的动作来确定，即 `argmax_a Q_π(s, a)`。然而，这并不能提供某个动作相比其他动作好多少的信息。为了解决这个问题，Baird 于 1993 年引入了优势函数。优势函数衡量智能体在给定状态 `s` 下采取特定动作 `a` 所获得的增益或优势。优势函数通过从状态-动作值 `Q_π(s, a)` 中减去状态值 `V_π(s)` 来计算，如公式 (8.10) 所示。

```
A_π(s, a) = Q_π(s, a) - V_π(s)   (8.10)
```

优势函数帮助我们衡量一个动作相比其他动作是好多少还是差多少。这很有用，因为状态值函数 `V_π` 和状态-动作值函数 `Q_π` 可能基于给定状态 `s` 关注不同的特征。由于 `V_π` 对所有动作取平均值，而 `Q_π` 只关注一个特定动作，我们可以使用优势函数重写状态-动作值函数，如公式 (8.11) 所示。

```
Q_π(s, a) = V_π(s) + A_π(s, a)   (8.11)
```

当使用神经网络来近似 `Q_π` 或 `V_π` 时，公式 (8.11) 可以重写为公式 (8.12)，其中 `β, δ` 表示近似函数 `V̂, Â` 的参数。

```
Q̂(s, a; β, δ) = V̂(s; β) + Â(s, a; δ)   (8.12)
```



在实践中，神经网络架构可以设计为让 `\hat{V}` 和 `\hat{A}` 在一定程度上共享相同的权重 `\boldsymbol{\theta}`，同时 `\hat{V}` 和 `\hat{A}` 仍保留各自的权重 `\beta, \delta`。该方程可以写成如下形式：

```
\hat{Q}(s, a; \boldsymbol{\theta}, \beta, \delta) = \hat{V}(s; \boldsymbol{\theta}, \beta) + \hat{A}(s, a; \boldsymbol{\theta}, \delta)
```

(8.13)

然而，当使用 DQN 适配公式 (8.11) 时，无法保证状态值 `\hat{V}` 的准确性，因为它并未反映在 Q 学习的目标函数中。为解决此问题，原始的决斗 DQN 论文提出了一种强制智能体学习状态值函数的解决方案。由此得到的方程如公式 (8.14) 所示：

```
\hat{Q}(s, a; \boldsymbol{\theta}, \beta, \delta) = \hat{V}(s; \boldsymbol{\theta}, \beta) + \Bigl( \hat{A}(s, a; \boldsymbol{\theta}, \delta) - \max_{a \in \cal{A}} \hat{A}(s, a; \boldsymbol{\theta}, \delta) \Bigr)
```

(8.14)

更准确地说，对于最优动作 `a_*`（其中 `a_* = \mathop{\text{argmax}}_{a \in \cal{A}} \hat{A}(s, a; \boldsymbol{\theta}, \delta)`），公式 (8.14) 中的优势项 `\hat{A}(s, a; \boldsymbol{\theta}, \delta) - \max_{a \in \cal{A}} \hat{A}(s, a; \boldsymbol{\theta}, \delta)` 变为零；因此，我们得到 `\hat{Q}(s, a_*; \boldsymbol{\theta}, \beta, \delta) = \hat{V}(s; \boldsymbol{\theta}, \beta)`。

但在实践中，我们采用了一个略有不同的版本，即在优势项中使用平均值而非最大值，如公式 (8.15) 所示，这在训练过程中实现了更好的稳定性。

```
\hat{Q}(s, a; \boldsymbol{\theta}, \beta, \delta) = \hat{V}(s; \boldsymbol{\theta}, \beta) + \Biggl( \hat{A}(s, a; \boldsymbol{\theta}, \delta) - \frac{1}{|\cal{A}|} \sum_{a \in \cal{A}} \hat{A}(s, a; \boldsymbol{\theta}, \delta) \Biggr)
```

(8.15)

Wang 等人 [4] 提出的决斗网络架构是对深度 Q 网络（DQN）的改进，旨在提升其在强化学习问题中估计状态-动作值的能力。在标准 DQN 中，神经网络以游戏画面图像作为输入，并输出所有可能动作的估计状态-动作值。决斗网络架构将该网络拆分为两个独立的头部，一个用于估计状态值，另一个用于估计每个动作的优势值。

图 8.3 展示了决斗网络的架构。该网络以状态 `s`（一个 `84 \times 84 \times 4` 的图像）作为输入。网络的前三层是卷积层，这些层的参数是共享的（对应公式 (8.15) 中的 `\boldsymbol{\theta}`）。来自最后一个隐藏层的特征图随后被输入到两个独立的全连接层：一个用于估计状态值函数 `\hat{V}`，另一个用于估计优势函数 `\hat{A}`。

![](img/605748_1_En_8_Fig3_HTML.png)

DQN 神经网络的示意图如下：输入数据，32 个滤波器，ReLU，64 个滤波器，ReLU，64 个滤波器，ReLU，512 个单元，ReLU，1 个单元的状态值头部和 18 个单元的优势头部。使用 `\hat{V}` 和 `\hat{A}` 计算 `\hat{Q}` 以及状态-动作值。

**图 8.3** DQN 神经网络的决斗架构

状态值函数头部由一个包含一个单元且无激活函数的全连接层组成，用于生成估计的状态值。优势函数头部由一个包含 18 个单元（即可能的动作数量）且无激活函数的全连接层组成，用于生成每个动作的估计优势值。

最终的估计状态-动作值通过一个简单的数学运算计算得出，该运算将估计的状态值和优势值结合起来，如公式 (8.15) 所示。此计算由图 8.3 中的红色方框标出。
...
与前述实验类似，本实验中使用学习率 `\alpha = 0.00025`、折扣率 `\gamma = 0.99`、批次大小 32 以及回放容量 100,000。我们每 5000 次更新更新一次目标网络。
