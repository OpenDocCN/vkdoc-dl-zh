...
这种方法存在两个主要问题。首先，图像中的物体通常跨越多个像素，并且每个像素与其周围像素相关。将图像展平为向量会使识别物体变得更加困难；想象一下，我们正在训练一个强化学习智能体进行自动驾驶，未能成功识别行人可能意味着致命的错误和悲剧。其次，在处理图像时，全连接神经网络中的参数数量可能会变得非常庞大。例如，一张尺寸为 `600 × 400 × 3` 的中等大小图像会生成一个包含 `7.2 × 10⁵` 个元素的向量。当输入到全连接神经网络时，第一个隐藏层的输入数据就有 `7.2 × 10⁵` 个维度。即使第一层只有 10 个隐藏单元，该网络仅这一层就需要 `7.2 × 10⁶` 个参数。当我们有更多隐藏层或隐藏单元时，这个数字会变得更大，从而难以扩展到更大的图像。

为了解决这些挑战，我们通常使用卷积神经网络（CNN）来处理图像。卷积运算是一个数学过程，涉及将滤波器（或卷积核）应用于输入图像或特征图，以提取相关特征。在深度学习中，卷积被用于卷积神经网络（CNN）来分析视觉图像，并且它是许多最先进的计算机视觉模型中的关键组成部分。

![](img/605748_1_En_7_Fig4_HTML.png)

一个流程示意图描绘了一个 6 × 5 × 1 的方格网格。在总共 35 个方格中，有四个方格被着色，然后被转换为线性框。具体来说，第 8、14、15 和 20 个方格被相应地着色。

图 7.4

将灰度图像展平为向量的示例。

卷积的工作原理是：取一个小的数值矩阵（称为滤波器或卷积核），将其在输入图像或特征图上滑动，在每个位置计算滤波器与图像或特征图局部区域之间的点积。此操作的结果是一个新的矩阵，称为输出特征图，它捕获了输入中最重要的特征。

在深度学习中应用卷积有许多优势，包括能够捕获视觉数据中的空间依赖性、减少模型所需的参数数量，以及能够跨图像的不同区域重用学习到的特征。这些特性使卷积成为分析视觉数据的有效工具，并使其广泛应用于各种计算机视觉应用中。

CNN 由一个或多个卷积层组成，这些卷积层直接接收图像数据，而无需将其展平。这保留了原始图像中的感兴趣区域和像素相关性。此外，卷积层具有固定的参数数量，与输入图像大小无关，这使得它易于扩展到更大的图像。

卷积层的核心是一个对输入数据执行卷积运算的滤波器。一个卷积层通常由多个滤波器组成，每个滤波器都有自己的参数集：权重 `W` 和偏置 `b`。权重 `W` 是一个维度为卷积核大小 × 输入通道数的矩阵。卷积核大小是一个可配置的变量，它决定了滤波器对图像进行卷积时工作区域（即感受野）的大小。滤波器以一个称为步长（默认值为 1）的可配置变量在图像上移动，以执行卷积运算。步长控制每次卷积运算时滤波器移动的距离，例如一次移动一个像素，直到到达图像的右边缘。

图 7.5 展示了卷积神经网络中一个卷积层内单个滤波器的运算过程。左侧的输入数据有一个通道（例如，一张灰度图像），中间的单个滤波器的卷积核大小为 `3 × 3`。由于输入数据只有一个通道，该滤波器的权重矩阵 `W` 的维度为 `3 × 3 × 1`。



为了从与滤波器卷积的输入数据的感受野中提取特征，在感受野`X`和权重矩阵`W`之间使用点积符号`X · W`计算点积。感受野是与滤波器卷积的输入数据区域，其宽度和高度与核大小相同，如图 7.5 中的橙色部分所示。

![图 7.6：单个卷积滤波器如何工作的示例（针对最后一个感受野）。数字是随机生成的。](img/605748_1_En_7_Fig6_HTML.png)

图 7.6 展示了单个卷积滤波器的工作原理。它描绘了一个 6x7 表格中的输入数据，其中右下角有一个 3x3 的表格被高亮显示并连接到滤波器。滤波器呈现一个高亮的 3x3 表格。特征图呈现一个在右下角高亮显示的、标记为`X·W`的块。

![图 7.5：单个卷积滤波器如何工作的示例。数字是随机生成的。](img/605748_1_En_7_Fig5_HTML.png)

图 7.5 展示了单个卷积滤波器的工作原理。它描绘了一个 6x7 表格中的输入数据，其中左上角有一个 3x3 的表格被高亮显示并连接到滤波器。滤波器呈现一个高亮的 3x3 表格。特征图呈现一个在左上角高亮显示的、标记为`X·W`的块。

点积结果使用加法运算符与偏置`b`（一个标量参数）相加，得到一个标量值，该值成为输出特征图的一部分。输出特征图使用公式`y = X · W + b`计算。

由于滤波器的核大小通常远小于输入图像或来自先前卷积层的特征图，因此它需要通过滑动整个图像来重复执行相同的过程。每次，滤波器都会重用其参数，包括权重和偏置。图 7.6 展示了滤波器如何针对最后一个感受野工作。

在实践中，单个卷积层会有多个滤波器，每个滤波器负责检测特定的特征。关于卷积层的一个有趣事实是，输出特征图中的通道数完全由该层中的滤波器数量决定，并且与输入数据中的通道数无关。例如，如图 7.7 所示，如果输入数据只有一个通道，但卷积层有四个滤波器（用不同颜色表示），则输出特征图将有四个通道。

![图 7.8：单个卷积滤波器如何工作的示例，其中输入数据有三个通道且我们有一个滤波器。数字是随机生成的。](img/605748_1_En_7_Fig8_HTML.png)

图 7.8 展示了单个卷积滤波器的工作原理。它描绘了 3 个通道的输入数据（6x7 表格），其中左上角有一个 3x3 的部分被高亮显示并连接到一个具有 3 个通道的滤波器。特征图呈现一个在左上角高亮显示的、标记为`X·W`的块。

![图 7.7：多个卷积滤波器如何工作的示例；输出特征图中的通道数仅取决于滤波器的数量。数字是随机生成的。](img/605748_1_En_7_Fig7_HTML.png)

图 7.7 展示了多个卷积滤波器的工作原理。它描绘了一个 6x7 表格中的输入数据，其中左上角有一个 3x3 的表格被高亮显示并连接到多个滤波器。滤波器呈现高亮的 3x3 表格。特征图呈现一个在左上角高亮显示的、标记为`X·W`的块。

前面的示例仅考虑了具有一个通道的输入数据，但在实践中，输入数据通常有多个通道。例如，RGB 图像有三个颜色通道，或者来自先前隐藏层的特征图可能有多个通道。在这些情况下，卷积层中每个滤波器的权重`W`将具有与输入数据相同数量的通道。例如，如图 7.8 所示，如果输入数据有三个通道，则该层中每个滤波器的权重`W`也将有三个通道。输出特征图将只是对应通道之间组合点积的结果：`∑ᵢ Xᵢ · Wᵢ`。然而，由于我们讨论的是单个滤波器，输出特征图将只有一个通道。
...
![](img/605748_1_En_7_Figaaa_HTML.gif) 一种基于神经网络的`Q`学习算法，使用ε-贪婪动作选择、更新`Q`网络，并在`K`步内衰减ε，以逼近最优状态-动作值函数`Q`。
...
**图 7.11** 使用神经网络近似状态-动作值函数 `Q_π` 的概念，其中我们仅将状态 `s` 输入神经网络，神经网络的输出是一个向量，包含给定状态下每个动作的值

这里，`θ` 代表神经网络的参数。

在前面的章节中，我们经常使用表格法和线性方法来计算特定状态-动作对 `(s, a)` 的值。然而，如果我们想找出对于任意状态 `s`，哪个动作能产生最大值 `max_a Q_π(s, a)`，我们必须逐一枚举所有（合法）动作。这意味着我们需要将同一个状态 `s` 多次输入神经网络，如果动作空间很大，这会损害性能。在使用神经网络时，我们希望避免这些顺序操作，并利用并行计算能力，尤其是在使用像 GPU 这样的加速硬件时。

为了解决这个问题，我们可以对使用神经网络计算任意状态的动作值的方式进行一个小改动。我们不再将状态-动作对 `(s, a)` 输入神经网络，而是仅将状态 `s` 作为输入，并一次性计算出每个动作的值，如图 7.11 所示。神经网络的输出是一个向量，其中每个标量分量对应相应动作的估计值。这种方法避免了重复将同一状态输入神经网络（这可能会消耗大量计算资源），也使我们能够轻松地找出给定状态下具有最大值的动作。

我们现在介绍朴素深度 Q 学习算法，它使用（深度）神经网络来近似最优状态-动作值函数 `Q*`。其整体流程与使用线性方法近似 `Q*` 相同。区别在于如何计算梯度以及更新参数。如果我们使用像 PyTorch 这样的深度学习框架软件，我们无需手动计算梯度，然后更新神经网络不同层的权重或偏置。相反，我们依赖这些工具（如使用反向传播和优化器）来自动为我们完成工作。我们只需要使用预测值和 TD（时序差分）目标来计算损失，该损失衡量了预测值与智能体实际获得的奖励之间的差异。值得注意的是，这与 DeepMind 用于玩不同 Atari 游戏的 DQN 智能体并非同一算法。要开发出相同的 DQN 智能体，我们需要对朴素深度 Q 学习算法进行一些改进。

**算法 1：使用 ε-贪婪策略进行探索的朴素深度 Q 学习**

![一个算法，管理一个容量为 N 的回放缓冲区，允许向缓冲区添加和随机采样转换，并在必要时进行替换。](img/605748_1_En_7_Figaab_HTML.gif)

我们在经典的推车杆任务上评估了朴素深度 Q 学习智能体的性能。结果如图 7.12 所示。为了评估智能体的性能，我们在每 20,000 个训练环境步骤后，在一个独立的评估环境中运行了 20,000 个评估环境步骤（约 40 个回合），评估时的探索 epsilon 为 0.05。结果取五次独立运行的平均值，然后使用窗口大小为 5 的移动平均进行平滑处理。

![一个折线图绘制了评估回合回报与训练环境步骤的关系。一条线从 200 开始上升，并在 300 到 400 之间持续波动。](img/605748_1_En_7_Fig12_HTML.png)

**图 7.12** 朴素深度 Q 学习智能体在推车杆任务上的性能。为了评估智能体的性能，我们在每 20,000 个训练环境步骤后，在一个独立的评估环境中运行了 20,000 个评估环境步骤（约 40 个回合），评估时的探索 epsilon 为 0.05。结果取五次独立运行的平均值，然后使用窗口大小为 5 的移动平均进行平滑处理。我们在训练期间使用折扣 `γ = 0.99`，学习率 `α = 0.00025`，并将探索率 `ε` 从 1.0 衰减到 0.1，衰减过程持续 100,000 个环境步骤

结果表明，朴素深度 Q 学习智能体的整体性能比使用线性方法近似值函数更差。此外，该智能体在评估期间未能达到最大回合回报。有人可能会期望，如果我们尝试微调超参数（如学习率和探索率）会得到更好的结果，但朴素深度 Q 学习智能体存在一些潜在问题，我们需要解决这些问题才能使其成为一个强大的智能体。



### 7.5 具有经验回放与目标网络的深度 Q 学习

在机器学习，特别是深度学习中，我们通常依赖独立同分布（IID）的训练数据。然而，在强化学习中，序列中连续的状态和奖励信号往往是相关的。这是因为智能体选择的动作会影响未来的状态和奖励，从而导致数据集非独立同分布。此外，朴素的深度 Q 学习算法仅使用每个转移 `(s, a, r, s')` 一次来更新网络参数，导致数据效率低下。

为了解决这些问题，我们可以使用一种称为经验回放的技术。我们将最近的 *N* 个转移 `(s, a, r, s')` 存储在一个环形缓冲区结构中，并在训练期间从缓冲区中随机采样一个小批量的转移。这使得算法能够多次重复使用相同的转移，从而提高数据效率。此外，通过从大量转移中随机采样，我们打破了连续状态和奖励之间的相关性。

然而，使用经验回放也带来了新的挑战。由于缓冲区中的数据可能来自很久以前，自数据收集以来网络参数可能已经发生了显著变化。这可能导致在由不同行为策略生成的样本上进行训练，这对于像 SARSA 这样的在线策略学习算法来说并不理想。因此，使用像 Q 学习这样的离线策略学习算法与经验回放相结合至关重要。

实现经验回放很简单。我们需要为 *N* 个转移分配空间，并添加添加新转移以及从缓冲区中随机采样小批量的方法。

**算法 2：均匀随机经验回放**

一种算法，通过使用来自回放缓冲区的转移来更新神经网络参数，并定期更新目标网络，从而执行具有经验回放的深度 Q 网络训练。

然而，经验回放并非改进深度 Q 学习的唯一技术。另一个关键技术是使用目标网络。目标网络是 Q 网络的一个独立副本，它会定期更新以匹配 Q 网络的参数。这样做是为了防止算法陷入反馈循环，即 Q 网络的估计值被用来更新自身，从而导致不稳定性。通过使用一个独立的网络来计算目标值，我们可以降低目标估计的方差，并稳定学习过程。

总之，经验回放和目标网络是用于提高深度 Q 学习性能和稳定性的两项关键技术。通过在缓冲区中存储和重用转移，并使用独立的网络计算目标值，我们可以在训练过程中实现更好的数据效率和稳定性。
...
马尔可夫性质（在第 2 章中介绍）指出，后继状态 `s_{t+1}` 仅依赖于当前状态 `s_t` 和动作 `a_t`，而不依赖于所有先前的状态和动作 `(s_0, a_0, s_1, a_1, ..., s_t, a_t)`。然而，在这种情况下（如图 7.16 所示），后继状态 `s_{t+1}` 依赖于过去的信息。例如，如果球正从右角落向下移动，而智能体选择将球拍向右移动，它可能会错过球，这将导致负面后果（失去一条命）。但是，如果球正在向上移动，而智能体同样将球拍向右移动，智能体则可能获得一些奖励，因为球很可能会击中顶部墙壁。对于相同的单帧和动作，智能体可能会得到不同的后继状态和奖励。

解决这个问题的一个方法是将过去的帧叠加起来。图 7.17 展示了一个示例，通过叠加过去的 28 帧来构建一张图像。现在，我们可以判断出球在击中右侧墙壁后正朝向屏幕底部移动。

![](img/605748_1_En_7_Fig16_HTML.png)

一幅插图展示了一个视频游戏，顶部有一组方块，一个小球在方块上随机移动，最终落到底部的一条水平面上。

图 7.16 《打砖块》视频游戏单帧示例

然而，将所有历史帧都叠加起来对空间和计算效率并不友好，因为大多数雅达利游戏可能持续数百万帧。这些游戏大多每秒生成 30 到 60 帧，且游戏中的运动非常简单，因此连续两帧之间的变化非常微小，我们几乎无法分辨它们是否不同。DQN 使用一种称为“动作重复”（有时也称为跳帧）的技术来解决这个问题。通过这种技术，智能体在与环境交互时，会将同一个动作重复执行 `k` 次（默认 `k=4`）。在每个情节开始时，智能体根据网络 `theta` 的 `epsilon`-贪心策略选择某个动作 `a`，然后在与环境交互的过程中将同一个动作 `a` 重复执行 `k` 次（终止状态除外，该状态不需要重复动作）。在第 `k+1` 帧时，智能体选择另一个动作 `a'`，并将 `a'` 重复执行 `k` 次。这个过程不断重复，直到情节终止。结果，智能体只看到来自环境的每第 `k` 帧，从而高效地跳过了 `(k-1)/k` 百分比的帧处理。仅这一项简单的技术就能为智能体节省大量的空间和计算时间。

图 7.18 展示了一个 `k=4` 的动作重复示例，其中灰色显示的帧代表通过重复相同动作而跳过的帧。

最后，为了确保环境状态具有马尔可夫性质，同时节省计算和空间，DQN 使用了最近四帧的历史记录（在应用了动作重复和跳帧之后）。这四帧被叠加在一起，如图 7.19 所示，以构建一张代表环境状态的图像。在输入神经网络之前，图像从 `160 x 210 x 3`（雅达利帧的默认尺寸）缩小到 `84 x 84 x 3`，然后转换为灰度图以节省额外的计算。这种缩小尺寸和灰度转换有助于通过减小输入数据的大小来加速训练过程。最后，输入数据通过除以 255 进行归一化，使得数据范围在 [0, 1] 之间，然后再传递给神经网络进行处理。

![](img/605748_1_En_7_Fig17_HTML.png)

一张带有叠加帧的视频游戏图像。顶部呈现了从底部碎裂的俄罗斯方块。下半部分展示了一个向下移动的小球，朝向底部的一个小水平面。

图 7.17 将过去 28 帧叠加在一起，为《打砖块》视频游戏构建单张图像的示例

总之，DQN 为雅达利游戏使用的预处理步骤包括：动作重复、跳帧、帧叠加、尺寸缩小、灰度转换和归一化。这些步骤有助于减轻处理数百万帧的计算负担，并使神经网络能够从输入数据中高效学习。

图 7.19 展示了在《打砖块》视频游戏中，使用动作重复（跳帧）为 4 后，叠加最后四帧的示例。



我们可以对标准 Atari 游戏环境进行多种修改，以提升 `DQN` 智能体的学习进度。例如，我们可以将失去一条命视为软终止状态，这意味着我们仅将奖励作为 TD 目标，而不通过使用后继状态的折现值进行自举。在软终止状态下，游戏环境不会重置。另一种方法是通过对每个回合的初始状态进行随机化处理，例如执行若干步无操作动作，这也能有效改善智能体的学习进度。此外，通常需要将奖励裁剪到 `[-1, 1]` 范围内，尤其是对于那些奖励信号非常大的游戏。例如，某些游戏的奖励可能高达数百或数千，这会压倒学习过程，使智能体难以收敛到最优策略。

![一组游戏插图，其中 4 个游戏面板处于激活状态，其余面板处于禁用状态。](img/605748_1_En_7_Fig18_HTML.png)

**图 7.18**  Breakout 视频游戏中动作重复 `k=4` 的示例；灰色帧表示被智能体跳过的帧

#### 卷积神经网络

`DQN` 智能体使用一个非常简单的卷积神经网络。它由三个卷积层组成；神经网络的输入数据是一张 `84 × 84 × 4` 的图像。第一个隐藏卷积层有 32 个滤波器，内核大小为 `8 × 8`，步长为 4。这意味着第一个隐藏层的输出特征图将有 32 个通道。第二个隐藏卷积层有 64 个滤波器，内核大小为 `4 × 4`，步长为 2。第三个隐藏卷积层有 64 个滤波器，内核大小为 `3 × 3`，步长为 1。我们在每个卷积层之后应用 `ReLU` 激活函数。然后，最后一个卷积层的特征图被展平并输入到一个包含 512 个单元的全连接层；之后，我们应用 `ReLU` 激活函数。全连接层的输出随后被输入到输出层，该层有 `N` 个单元且没有激活函数。这里，`N` 是特定 Atari 游戏的有效动作数量，根据游戏不同，其值在 4 到 18 之间变化。图 7.20 展示了该网络架构。请注意，我们仅将环境状态 `s` 作为神经网络的输入，即一张 `84 × 84 × 4` 的图像，并输出一个向量，其中包含针对输入状态 `s` 的每个有效动作的状态-动作值。
...
![](img/605748_1_En_7_Fig20_HTML.png)

一个表示神经网络架构的流程图，包含各种层，包括具有不同滤波器大小和 ReLU 激活函数的卷积层，设计用于处理尺寸为 84 × 84 × 4 的输入数据，其后是全连接层。

**图 7.20** DQN 智能体的卷积神经网络架构

注意，对于 Pong 游戏，最大回合回报是 21。我们可以观察到，智能体在 1000 万个训练环境帧（对应于跳帧后智能体实际看到的 250 万帧）后收敛到最优策略。
...
![](img/605748_1_En_7_Fig23_HTML.png)

一条折线图，展示了评估回合回报随训练环境帧数的变化，描绘了回报值在超过 5000 万帧过程中的增长趋势。
