...
def clbk_laser(msg):
    ...
    rospy.loginfo(sectors)

def main():

    rospy.init_node('read_Lidar_data')
    sub=rospy.Subscriber("/ai_rover_remastered/laser_scan/scan", LaserScan, clbk_laser)
...
![图 6-6](img/494112_1_En_6_Fig6_HTML.jpg)
...
![图 6-7](img/494112_1_En_6_Fig7_HTML.jpg)
...
![图 6-8](img/494112_1_En_6_Fig8_HTML.jpg)
...
# 1440/10 = 144 Full rotation of coverage from +/-Ï€ for the LiDAR sensor sweep
# Transform â€˜sectorsâ€™ array into a Python dictionary.

def msg_Lidar_laser(msg):
     sectors = {
     'sectorA': min(min(msg.ranges[0:143]),   10),
     'sectorB': min(min(msg.ranges[144:287]), 10),
     'sectorC': min(min(msg.ranges[288:431]), 10),
     'sectorD': min(min(msg.ranges[432:575]), 10),
     'sectorE': min(min(msg.ranges[576:719]), 10),
     'sectorF': min(min(msg.ranges[720:863]), 10),
     'sectorG': min(min(msg.ranges[864:1007]), 10),
     'sectorH': min(min(msg.ranges[1008:1151]),10),
     'sectorI': min(min(msg.ranges[1152:1295]),10),
     'sectorJ': min(min(msg.ranges[1296:1439]),10),
     }
     rospy.loginfo(sectors);
     sense_and_avoid(sectors);

# Using encoded sensor data to make turning decisions.
# There are five different possible decisions: Forward, turn soft left, turn hard
# left, turn soft right, turn hard right, and turn around 180 degrees aroundâ€¦

def switch_movement_options(argument):
     def forward():
          return 0.5, 0.0;
     ...
     def backward_turn_around():
          return 0.0, 3.14159;

     switch_function_dir = {

          0b0000: forward(),
          0b0001: turn_soft_left(),
          0b0010: turn_soft_left(),
          0b0011: turn_soft_left(),
          0b0100: turn_soft_right(),
          0b0101: turn_hard_left(),
          0b0110: turn_hard_left(),
          0b0111: turn_hard_left(),
          0b1000: turn_soft_right(),
          0b1001: forward(),
          0b1010: turn_hard_right(),
          0b1011: turn_hard_left(),
          0b1100: turn_hard_right(),
          0b1101: turn_hard_right(),
          0b1110: turn_hard_right(),
          0b1111: backward_turn_around()
          }

     return switch_function_dir.get(argument, "Invalid Move Option")

def sense_and_avoid(sectors):
     msg = Twist();
     linear_vel_x = 0;
     spin_angular_vel_z = 0;
     current_state_description = 'Program Start';

     # encode forward looking sensor data
     bitVar = 0b0000;

     # bitVar | 0b1000 We have an obstacle detected in front_center_left
     if sectors['sectorG'] < 1:
          bitVar = bitVar | 8;

     # bitVar | 0b0100 We have an obstacle detected in front_left
     if sectors['sectorF'] < 1:
          bitVar = bitVar | 4;

     # bitVar | 0b0010 We have an obstacle detected in front_right
     if sectors['sectorE'] < 1:
          bitVar = bitVar | 2;

     # bitVar | 0b0001 We have an obstacle detected in front_center_right
     if sectors['sectorD'] < 1:
          bitVar = bitVar | 1;

     # SET ALL VALUES TO BE SENT TO THE DIFFERENTIAL DRIVE VIA TWIST
     linear_vel_x, spin_angular_vel_z = switch_movement_options(bitVar);

     rospy.loginfo(current_state_description);
     msg.linear.x = linear_vel_x;
     msg.angular.z = spin_angular_vel_z;
     # publish the values of linear and angular velocity to diff drive plug-in
     pub.publish(msg);

def main():
     global pub;

     rospy.init_node('sense_avoid_obstacle');
     sub = rospy.Subscriber('/ai_rover_remastered/laser_scan/scan', LaserScan, msg_Lidar_laser);

     pub = rospy.Publisher('ai_rover_remastered/base_controller/cmd_vel', Twist, queue_size=10);
...
2.  如果 AI 探测车与一个具有真实世界质量和惯量的方块相撞，它是否仍然能够将花岗岩方块推到一边？

3.  我们知道我们目前有 16 种 `bitVar` 的组合，以及我们在 `sense_avoid_obstacle.py` 中使用的四个前部区域（D、E、F 和 G）。如果我们添加左侧（区域 H）和右侧（区域 C）的 LiDAR 区域，我们将有多少种 `bitVar` 的组合？请使用区域 C、D、E、F、G 和 H 列出所有 `bitVar` 的组合。

4.  我们可以看到我们只使用了四个前方的激光雷达区域。如果我们把左侧（区域 H）和右侧（区域 C）的激光雷达区域也加入到 `sense_avoid_obstacles.py` 中，我们可能会获得哪些收益？我们该如何重新编程这个特定的 Python ROS 节点程序？

5.  我们目前使用十个区域进行激光雷达传感器阵列扫描？这是我们能使用的激光雷达区域数量的唯一选择吗？如果不是，那么理想的激光雷达传感器扫描区域数量是多少？

6.  我们如何修改模拟激光雷达传感器的源代码，将其范围从 10 米增加到 30 米？