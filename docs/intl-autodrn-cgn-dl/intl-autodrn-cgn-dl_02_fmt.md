...
人工智能漫游车导航系统的类图。`CGS` 的外部用户有地面控制站和路径寻找信息，轮执行器的外部输出设备有轮执行器和 `QGIS` 导航地图，摄像头传感器的外部输入设备有摄像头接口和当前位置。它还包括任务计时器、定位器等。
...
`SLAM` 和认知处理的逻辑图。传感器指向传感器数据、`SLAM`（用于 `LiDAR`）、占用网格到局部地图 `QGIS` 以及更新地图到全局地图 `QGIS` 和认知深度学习引擎路径规划。`SLAM`（用于 `LiDAR`）指向位置到占用网格和认知深度学习引擎路径规划。
