...
![内容图像、风格图像和风格化图像](img/495303_1_En_12_Figa_HTML.gif)

左侧第一幅图像是内容图像，中间的图像是风格图像，右侧的图像是风格化图像。请注意，中间图像的风格是如何应用于左侧图像的内容，从而生成一幅新的风格化图像的。

风格迁移背后的理论并不复杂，我将在本章后面的自定义风格迁移程序中介绍它。那么，让我们开始快速风格迁移吧。
...
![用于选择图像文件的 Colab 界面](img/495303_1_En_12_Fig1_HTML.jpg)

图 12-1 用于选择图像文件的 `Colab` 界面

从单元格右侧的下拉列表中选择所需的目标图像和风格图像。你可以使用以下代码片段，通过 `plt.imshow` 函数显示所选的两张图像：
...
![内容图像和风格图像](img/495303_1_En_12_Fig2_HTML.jpg)
...
该函数通过调用 `tf.io.read_file` 方法读取图像数据。它通过调用 `tf.image.decode_image` 将图像解码为三个 `RGB` 通道。图像被调整为 `256x256`，因为预训练模型使用此特定尺寸进行风格化。最后，图像数据以形状为 `(1, 256, 256, 3)` 的张量形式返回给调用者。我们为图像添加了一个新维度，稍后在处理一批图像时会用到。
...
![风格化图像](img/495303_1_En_12_Fig3_HTML.jpg)

图 12-3 风格化图像

如果你想显示缩小的图像，如图 12-4 所示，只需按如下方式调用 `plt.imshow` 方法：

![缩小的风格化图像](img/495303_1_En_12_Fig4_HTML.jpg)
...
![不同图像的模型推理结果](img/495303_1_En_12_Figb_HTML.gif)
...
![VGG16 架构](img/495303_1_En_12_Fig5_HTML.jpg)
...
![内容和风格图像](img/495303_1_En_12_Fig6_HTML.jpg)
...
![模型摘要](img/495303_1_En_12_Fig7_HTML.jpg)
...
![$$ {L}_{GM}\left(S,G,l\right)=\frac{1}{4{N}_l²{M}_l²}\sum \limits_{ij}{\left( GM\left[l\right]{(S)}_{ij}- GM\left[l\right]{(G)}_{ij}\right)}² $$](img/495303_1_En_12_Chapter_TeX_Equb.png)
...
![内容图像、风格图像和风格化图像](img/495303_1_En_12_Fig8_HTML.jpg)
...
列表 12-6 `CustomStyleTransfer` 完整源代码
