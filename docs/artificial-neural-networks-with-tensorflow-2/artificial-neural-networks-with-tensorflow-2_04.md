...
![../images/495303_1_En_2_Chapter/495303_1_En_2_Fig10_HTML.jpg](img/495303_1_En_2_Fig10_HTML.jpg)
...
![../images/495303_1_En_2_Chapter/495303_1_En_2_Fig11_HTML.jpg](img/495303_1_En_2_Fig11_HTML.jpg)
...
请注意，你需要为 CSV 文件设置正确的路径。

如果你决定使用本书 GitHub 仓库中的数据，请使用以下代码替换前面的代码段：
...
现场收集的数据可能会按照数据收集者的便利性和舒适度以特定顺序排列。为了获得更好的机器学习效果，你应该对数据进行随机化处理，这样学习过程就不会遵循数据中不期望的模式。因此，我们使用以下语句打乱数据：
...
你可以通过打印 `data` 数据帧的内容来验证数据是否正确加载。我没有通过调用 `data.head()` 来打印前几行，而是打印了整个数据集，以便你了解其中的记录数和列数。如图 2-12 所示。

![../images/495303_1_En_2_Chapter/495303_1_En_2_Fig12_HTML.jpg](img/495303_1_En_2_Fig12_HTML.jpg)
...
现在，既然你已经将数据加载到内存中，下一步任务是在将其输入到我们的网络之前对其进行清洗。我们称之为数据预处理，接下来将讨论这一点。

### 数据预处理

原始的实时数据可能并不总是满足神经网络（ANN）的训练要求。具体来说，你需要检查数据并针对以下项目进行处理：

*   数据可能包含空值。
*   数据库中的所有字段可能并非都对学习有用。
*   数值字段的值可能存在较大差异，因此必须将其缩放到同一水平。
*   某些字段可能包含分类值，例如男性和女性；这些需要编码为 0 和 1。
*   最后，你需要决定哪些字段用作特征，以及标签是什么。
...
对空值计算总和会生成错误。很明显，我们的数据集不包含任何空值。因此，不存在从数据集中删除行（包含空字段的行）的问题。
...
至此，你已经准备好了特征（`X`）和标签（`y`）张量。

#### 编码分类列

你必须检查所选列中是否有任何列包含分类值。为此，我们将使用以下语句检查所有选定列的数据类型：
...
![../images/495303_1_En_2_Chapter/495303_1_En_2_Fig13_HTML.jpg](img/495303_1_En_2_Fig13_HTML.jpg)
...
![../images/495303_1_En_2_Chapter/495303_1_En_2_Fig14_HTML.jpg](img/495303_1_En_2_Fig14_HTML.jpg)
...
![../images/495303_1_En_2_Chapter/495303_1_En_2_Fig15_HTML.jpg](img/495303_1_En_2_Fig15_HTML.jpg)

图 2-15

TensorBoard 中的准确率和损失指标
...
![../images/495303_1_En_2_Chapter/495303_1_En_2_Fig17_HTML.jpg](img/495303_1_En_2_Fig17_HTML.jpg)

图 2-17

混淆矩阵

![../images/495303_1_En_2_Chapter/495303_1_En_2_Fig16_HTML.jpg](img/495303_1_En_2_Fig16_HTML.jpg)
...
![../images/495303_1_En_2_Chapter/495303_1_En_2_Fig18_HTML.jpg](img/495303_1_En_2_Fig18_HTML.jpg)
...
在本章中，您使用 TensorFlow 2.x 搭建了深度学习环境，并利用 Colab 开发 Python 笔记本。通过一个简单的应用，您熟悉了开发环境。随后，我们通过一个更详细的真实示例，学习了如何加载外部数据库、预处理数据使其适用于机器学习、定义深度神经网络、编译模型、训练模型、使用 TensorBoard 和 `matplotlib` 图表评估模型性能，以及最终如何使用训练好的模型对未见过的数据进行预测。尽管我们评估了模型性能，但并未讨论如何改进它。在下一章中，您将学习一些提升模型性能的技巧。下一章将更深入地探讨 TensorFlow Keras 的集成，并讨论图像分类问题。
