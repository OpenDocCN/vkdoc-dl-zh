...
为了修正一个例子，让我们考虑图 5-4 所示的情况。我们将首先执行一个称为“正向传播”的步骤，这是一个为给定的一组要素计算预测值的过程。从输入层开始，第一个操作将特征 *X* <sub>0</sub> 乘以权重*w*T8】0。然后我们应用一个激活函数， *f* ()，它产生下一层节点，*X*T14】1。我们再次乘以下一组权重， *w* <sub>1</sub> ，并应用另一个激活函数，产生输出， *Y* 。这显示在等式 5-1 中。
