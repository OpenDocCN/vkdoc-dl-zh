...
*方程式 3-14。一个具有两种状态的门限自回归(TAR)模型。*
...
我们来看一个带有截距项和单变量的线性回归的单个 SGD 步骤，其中`θ_t = [α_t, β_t]`。我们将从迭代 0 开始，并假设我们已经为该批数据计算出梯度`g_0 = [0.25, 0.33]`。此外，我们将学习率`lr`设置为 0.01。这对`θ_1`意味着什么？利用等式 3-24，我们可以看到`θ_1 = [α_0 + 0.025, β_0 - 0.033]`。也就是说，我们将`α_0`减少 0.025，将`β_0`增加 0.033。
