...
在这种情况下，模型的深层参数——通常描述技术、约束和偏好——在模型之外进行校准或估计，因此在实施解决方法之前就已经知道了。TensorFlow在这种情况下的作用是实现微分方程系统的求解。
...
TensorFlow 不仅为我们提供了一种训练深度学习模型的方法，还提供了一套可用于解决任意数学模型的工具。这包括经济学和金融学中常用的模型。在这一章中，我们使用一个玩具模型(吃蛋糕模型)和计算文学中的一个通用基准:新古典商业周期模型来检验如何做到这一点。使用经济学中的常规方法求解这两个模型都是微不足道的，但提供了一种简单的方法来演示如何使用TensorFlow来求解与经济学家相关的理论模型。
