...
![$$ \sum \limits_{i=0}^{S^2}{\mathbbm{I}}_i^{obj}\sum \limits_{c\in classes\ }{\left({p}_i(c)-{\hat{p}}_i(c)\right)}^2 $$](img/470317_1_En_7_Chapter_TeX_Equd.png)
...
![$$ {\lambda}_{coord}\sum \limits_{i=0}^{S^2}\sum \limits_{j=0}^B{\mathbbm{I}}_i^{obj}\left[{\left({x}_i-{\hat{x}}_i\right)}^2+{\left({y}_i-{\hat{y}}_i\right)}^2\right]+{\lambda}_{coord}\sum \limits_{i=0}^{S^2}\sum \limits_{j=0}^B{\mathbbm{I}}_i^{obj}\left[{\left(\sqrt{w_i}-\sqrt{{\hat{w}}_i}\right)}^2+{\left(\sqrt{h_i}-\sqrt{{\hat{h}}_i}\right)}^2\right] $$](img/470317_1_En_7_Chapter_TeX_Eque.png)

#### 信心丧失

置信度损失度量了当决定一个对象是否在盒子中时的误差。

![$$ \sum \limits_{i=0}^{S^2}\sum \limits_{j=0}^B{\mathbbm{I}}_{ij}^{obj}{\left({\mathrm{C}}_{\mathrm{i}}-{\hat{\mathrm{C}}}_{\mathrm{i}}\right)}^2 $$](img/470317_1_En_7_Chapter_TeX_Equf.png)
...
![$$ {\lambda}_{noobj}\sum \limits_{i=0}^{S^2}\sum \limits_{j=0}^B{\mathbbm{I}}_{ij}^{noobj}{\left({\mathrm{C}}_{\mathrm{i}}-{\hat{\mathrm{C}}}_{\mathrm{i}}\right)}^2 $$](img/470317_1_En_7_Chapter_TeX_Equg.png)
...
![$$ L=\sum \limits_{i=0}^{S^2}{\mathbbm{I}}_i^{obj}\sum \limits_{c\in classes\ }{\left({p}_i(c)-{\hat{p}}_i(c)\right)}^2+{\lambda}_{coord}\sum \limits_{i=0}^{S^2}\sum \limits_{j=0}^B{\mathbbm{I}}_i^{obj}\left[{\left({x}_i-{\hat{x}}_i\right)}^2+{\left({y}_i-{\hat{y}}_i\right)}^2\right]+{\lambda}_{coord}\sum \limits_{i=0}^{S^2}\sum \limits_{j=0}^B{\mathbbm{I}}_i^{obj}\left[{\left(\sqrt{w_i}-\sqrt{{\hat{w}}_i}\right)}^2+{\left(\sqrt{h_i}-\sqrt{{\hat{h}}_i}\right)}^2\right]+\sum \limits_{i=0}^{S^2}\sum \limits_{j=0}^B{\mathbbm{I}}_{ij}^{obj}{\left({\mathrm{C}}_{\mathrm{i}}-{\hat{\mathrm{C}}}_{\mathrm{i}}\right)}^2+{\lambda}_{noobj}\sum \limits_{i=0}^{S^2}\sum \limits_{j=0}^B{\mathbbm{I}}_{ij}^{noobj}{\left({\mathrm{C}}_{\mathrm{i}}-{\hat{\mathrm{C}}}_{\mathrm{i}}\right)}^2 $$](img/470317_1_En_7_Chapter_TeX_Equh.png)
...
在这一点上，你可以简单地执行你的对象检测与此: <sup>[5](#Fn5)</sup>
